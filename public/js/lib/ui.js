mapp.utils.merge(mapp.dictionaries,{en:{layer_zoom_to_extent:"Zoom to filtered layer extent",layer_visibility:"Toggle visibility"},de:{layer_zoom_to_extent:"Zoom zum Ausmaß des gefilterten Datensatzes",layer_visibility:"Umschalten der Ansicht"},cn:{layer_zoom_to_extent:"缩放至相应筛选范围",layer_visibility:"切换可见性"},pl:{layer_zoom_to_extent:"Pokaż zasięg warstwy",layer_visibility:"Widoczność"},ko:{layer_zoom_to_extent:"필터된 레이어크기에 줌(zoom)",layer_visibility:"토글 가시성"},fr:{layer_zoom_to_extent:"Zoom sur l'étendue de la couche",layer_visibility:"Changer la visiblité"},ja:{layer_zoom_to_extent:"フィルターされたレイヤー範囲をズームに",layer_visibility:"表示切替"}});var w=e=>{let t=mapp.utils.html`
  ...
  <div class="layer-key">
    <span>
      ${e.location.layer.name}`}var oe={view:B,listview:H,infoj:U,entries:le};var ne=async e=>{e.Tabulator=await mapp.ui.utils.Tabulator(e.target,e.table),e.table.selectable&&e.Tabulator.on("rowClick",(t,a)=>{if(typeof e.rowSelect=="function"){e.rowSelect(t,a);return}let i=a.getData();!e.layer||!i[e.layer.qID]||(mapp.location.get({layer:e.layer,id:i[e.layer.qID]}),a.deselect())}),e.setData=t=>{!t&&e.data||(e.noDataMask&&!t?(e.target.style.display="none",e.mask=!e.mask&&e.target.parentElement?.appendChild(mapp.utils.html.node`
        ...
        <div class="dataviewMask">No Data`)):(e.mask&&e.mask.remove(),delete e.mask,e.target.style.display="block"),a||(a={datasets:[{data:[]}]}),a.datasets||(a={datasets:[{data:a}]}),e.data=a,e.chart.datasets?.length&&a.datasets.forEach((i,l)=>Object.assign(i,e.chart.datasets[l])),a.labels=a.labels||e.chart.labels,e.ChartJS.data=a,e.ChartJS.update()}};var se=async e=>{if(e.target=typeof e.target=="string"&&document.getElementById(e.target)||e.target,e.target=e.target instanceof HTMLElement&&e.target||mapp.utils.html.node`
      <div
        class="dataview-target"
        style="position: absolute; width: 100%; height: 100%">`,e.update=async()=>{if(!e.query)return;let t=e.viewport&&e.layer.mapview.getBounds();e.viewport&&(e.queryparams=Object.assign(e.queryparams||{},{layer:!0}));let a=e.queryparams?.center&&ol.proj.transform(e.layer.mapview.Map.getView().getCenter(),`EPSG:${_xyz.mapview.srid}`,"EPSG:4326"),i=mapp.utils.paramString(Object.assign({},e.queryparams||{},{template:encodeURIComponent(e.query),filter:e.queryparams?.filter&&e.layer?.filter?.current,layer:e.queryparams?.layer&&e.layer.key,locale:e.queryparams?.layer&&e.layer.mapview.locale.key,id:e.queryparams?.id&&e.location&&e.location.id,lat:a&&a[1],lng:a&&a[0],z:e.queryparams?.z&&e.layer.mapview.Map.getView().getZoom(),viewport:t&&[t.west,t.south,t.east,t.north,e.layer.mapview.srid]})),l=await mapp.utils.xhr(`${e.host||e.location.layer.mapview.host}/api/query?`+i);if(!(l instanceof Error)){if(typeof e.responseFunction=="function")return e.responseFunction(l);typeof e.setData=="function"&&e.setData(l)}},e.toolbar){e.target=mapp.utils.html.node`
      <div class="dataview-target">`;let t=Object.keys(e.toolbar).map(a=>mapp.ui.elements.toolbar_el[a](e));e.panel=mapp.utils.html.node`
      <div class="grid">
        <div class="btn-row">${t}</div>
        ${e.target}`}return e.chart&&await re(e),typeof e.columns!="undefined"&&(console.warn("Table dataviews should be configured inside a tables object"),e.table={columns:e.columns}),e.table&&await ne(e),e.mapChange&&e.layer&&e.layer.mapview.Map.getTargetElement().addEventListener("changeEnd",()=>{e.layer&&!e.layer.display||e.tab&&!e.tab.classList.contains("active")||typeof e.mapChange=="function"&&e.mapChange()||e.update()}),e};var de=e=>{!e.node||(e.tabs=e.node.appendChild(mapp.utils.html.node`<div class="tabs">`),e.panel=e.node.appendChild(mapp.utils.html.node`<div class="panel">`),e.id&&e.node.setAttribute("data-id",e.id),e.addTab=Ze,e.node.addEventListener("addTab",t=>e.addTab(t.detail)))};function Ze(e){let t=this;e.tab=mapp.utils.html.node`
    <div class="tab">
      <div
        class="header"
        style="${e.tab_style||""}"
  ...
  <label 
    ...
    <li
      ...
      ${mapp.dictionary.cancel}`),this.popup({coords:this.interaction.vertices[this.interaction.vertices.length-1],content:mapp.utils.html.node`<ul>${t}`})}function Xe(e){if(this.interaction.vertices.length===0)return;e&&e.preventDefault();let t=this.interaction.Layer.getSource().getFeatures(),a=[];t.length&&a.push(mapp.utils.html`
  <li
    onclick=${()=>this.interaction.finish(this.interaction.getFeature())}>
  ...
  <button 
    ...
    <div 
      ...
      <div style="display: grid; grid-row-gap: 5px;">
        ${t}
        ${a}`})};var be=new XMLSerializer,_e=e=>{if(e.svg||e.type||e.layers){let t=e.layers&&Array.isArray(e.layers)&&e.layers.map(i=>`url(${i.svg})`).reverse().join(",")||`url(${e.svg||e.url||mapp.utils.svgSymbols[e.type](e)})`,a=`
      background-position: center;
      background-repeat: no-repeat;
      background-size: contain;
    ...
    <div
      ...
      <input id="b" type="range"
        ...
        step=${e.step||1}
        value=${e.val_b}
        oninput=${a=>{a.target.parentNode.style.setProperty(`--${a.target.id}`,a.target.value),a.target.parentNode.querySelectorAll("input").forEach(i=>{i.id==a.target.id&&i!=a.target&&(i.value=a.target.value)}),e.callback_b&&e.callback_b(a)}}/>`;var ke={viewport:nt,download_json:rt,download_csv:st};function nt(e){return mapp.utils.html`
    <button
      class=${`flat ${e.viewport&&"active"||""}`}
      onclick=${t=>{t.target.classList.toggle("active"),e.viewport=!e.viewport,e.update()}}>Viewport`}function rt(e){return mapp.utils.html`
    <button
      class="flat"
      onclick=${()=>{e.Tabulator.download("json",`${e.title||"table"}.json`)}}>JSON`}function st(e){return mapp.utils.html`
    <button
      class="flat"
      onclick=${()=>{e.Tabulator.download("csv",`${e.title||"table"}.csv`)}}>CSV`}var xe={card:ce,chkbox:pe,contextMenu:me,drawer:ue,drawing:he,dropdown:ge,dropdown_multi:fe,isoline_params_here:ve,isoline_params_mapbox:we,legendIcon:_e,slider:ye,slider_ab:$e,toolbar_el:ke};var _=null;async function dt(){if(window.Chart){_=window.Chart;return}let e=await import("https://cdn.skypack.dev/chart.js@3.7.0");e.Chart.register(...e.registerables);let t=await import("https://cdn.skypack.dev/chartjs-plugin-datalabels@2.0.0");e.Chart.register(t.default);let a=await import("https://cdn.skypack.dev/chartjs-plugin-annotation@1.3.0");e.Chart.register(a.default),_=e.Chart}async function Le(e,t){return _||await dt(),new _(e,t)}var y=null;async function ct(){if(window.Tabulator){y=window.Tabulator;return}y=(await import("https://cdn.skypack.dev/pin/tabulator-tables@v5.0.10-xDu7yyLVhsxCKJbN5vfH/mode=imports/optimized/tabulator-tables.js")).TabulatorFull}async function je(){return y||await ct(),new y(...arguments)}var u={idle:600},Se=e=>{Object.assign(u,e),u.idle!==0&&(window.onload=f,window.onmousemove=f,window.onmousedown=f,window.ontouchstart=f,window.onclick=f,window.onkeypress=f,f(),S())};function f(){u.locked||(u.timeout&&clearTimeout(u.timeout),u.timeout=setTimeout(Ce,u.idle*1e3))}function Ce(){u.locked=!0,u.renew&&clearTimeout(u.renew);let e=new XMLHttpRequest;e.open("GET",`${u.host}/api/user/cookie?destroy=true`),e.send();let t=mapp.utils.html.node`
    <form
      class="login"
