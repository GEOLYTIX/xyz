mapp.utils.merge(mapp.dictionaries,{en:{layer_zoom_to_extent:"Zoom to filtered layer extent",layer_visibility:"Toggle visibility"},de:{layer_zoom_to_extent:"Zoom zum Ausma\xDF des gefilterten Datensatzes",layer_visibility:"Umschalten der Ansicht"},cn:{layer_zoom_to_extent:"\u7F29\u653E\u81F3\u76F8\u5E94\u7B5B\u9009\u8303\u56F4",layer_visibility:"\u5207\u6362\u53EF\u89C1\u6027"},pl:{layer_zoom_to_extent:"Poka\u017C zasi\u0119g warstwy",layer_visibility:"Widoczno\u015B\u0107"},ko:{layer_zoom_to_extent:"\uD544\uD130\uB41C \uB808\uC774\uC5B4\uD06C\uAE30\uC5D0 \uC90C(zoom)",layer_visibility:"\uD1A0\uAE00 \uAC00\uC2DC\uC131"},fr:{layer_zoom_to_extent:"Zoom sur l'\xE9tendue de la couche",layer_visibility:"Changer la visiblit\xE9"},ja:{layer_zoom_to_extent:"\u30D5\u30A3\u30EB\u30BF\u30FC\u3055\u308C\u305F\u30EC\u30A4\u30E4\u30FC\u7BC4\u56F2\u3092\u30BA\u30FC\u30E0\u306B",layer_visibility:"\u8868\u793A\u5207\u66FF"}});var _=e=>{let t=mapp.utils.html`
  ...
  <div class="dropdown active">
    ...
    <div class="legend">
      ...
      <div class="mask-icon expander"></div>`,content:t}),e.style.drawer};var R=e=>{let t=e.style.theme,i=[];if(i.push(mapp.utils.html`
    <div
      class="switch-all"
    ...
    <div 
      ...
      <div 
        data-id=${a.value}
        class="contents">
        ${o}${r}`)}),e.style.legend=mapp.utils.html.node`<div class="grid">${i}`,e.style.legend};var V=e=>{let t=[];t.push(mapp.ui.elements.dropdown({entries:Object.entries(e.grid_fields).map(o=>({title:o[0],options:o[1]})),callback:(o,r)=>{e.grid_size=r.options,e.reload()}}));let i=e.style.range.length,a=new XMLSerializer,l=mapp.utils.html`
  <div
    class="grid"
  ...
  <div
    ...
    <button 
      ...
      <div
        class="${`location ${e.class}`}">`,e.target=t,e.display&&mapp.ui.Dataview(e).then(()=>e.update()));let l=e.label&&mapp.ui.elements.chkbox({label:e.label,checked:!!e.display,onchange:r=>{if(e.display=r,t){t.style.display=e.display?"block":"none",e.display&&typeof e.update=="function"&&e.update(),e.display&&typeof e.update!="function"&&mapp.ui.Dataview(e).then(()=>e.update());return}e.display?a():e.remove()}});return mapp.utils.html.node`${l}${t||""}`};var k=e=>{let t;return e.edit?t=mapp.utils.html.node`
      <input
    ...
    <div
      class="images-grid">${t}`;return a;async function l(r){e.location.view?.classList.add("disabled");let n=new FileReader,s=r.target.files[0];!s||(n.onload=c=>{let d=new Image;d.onload=async()=>{let m=mapp.utils.html.node`<canvas>`,u=1024,h=d.width,g=d.height;h>g&&h>u?(g*=u/h,h=u):g>u&&(h*=u/g,g=u),m.width=h,m.height=g,m.getContext("2d").drawImage(d,0,0,h,g);let Ie=m.toDataURL("image/jpeg",.5),b=await mapp.utils.xhr({method:"POST",requestHeader:{"Content-Type":"application/octet-stream"},url:`${e.location.layer.mapview.host}/api/provider/cloudinary?${mapp.utils.paramString({public_id:s.name.replace(/.*\//,"").replace(/\.([\w-]{3})/,""),resource_type:"image"})}`,body:mapp.utils.dataURLtoBlob(Ie)});await mapp.utils.xhr(`${e.location.layer.mapview.host}/api/query?`+mapp.utils.paramString({template:"set_field_array",locale:e.location.layer.mapview.locale.key,layer:e.location.layer.key,table:e.location.table,qID:e.location.layer.qID,id:e.location.id,action:"append",field:e.field,value:b.secure_url}));let Pe=mapp.utils.html.node`
          <div>
            <img
              src=${b.secure_url}
              onclick=${mapp.ui.utils.imagePreview}>
    ...
    <div
      class="val"
      style="${`${e.css_val||""}`}">${t}`};var L=e=>{let t=!isNaN(e.value)&&e.type==="integer"?parseInt(e.value).toLocaleString("en-GB",{maximumFractionDigits:0}):parseFloat(e.value).toLocaleString("en-GB",{maximumFractionDigits:2});return e.edit&&(e.edit.range?t=mapp.ui.elements.slider({min:e.edit.range.min,max:e.edit.range.max,val:e.value,callback:a=>{t.value=parseFloat(a.target.value),e.newValue=t.value,e.location.view?.dispatchEvent(new CustomEvent("valChange",{detail:e}))}}):t=mapp.utils.html.node`
      <input
        type="number"
        value="${e.value||""}"
  ...
  <div
    ...
    <div
      class="val"
      style="${`${e.css_val||""}`}">
      ${t}`};var se={key:Xe,boolean:Z,dataview:K,date:k,datetime:k,defaults:X,documents:Q,geometry:Y,html:S,images:ee,integer:L,isoline_here:x,isoline_mapbox:x,json:te,numeric:L,pin:ae,report:ie,streetview:le,tab:oe,text:ne,textarea:S,time:re};function Xe(e){return mapp.utils.html.node`
  <div class="layer-key">
    <span>
      ${e.location.layer.name}`}var de={view:H,listview:W,infoj:J,entries:se};var ce=async e=>{if(e.target=typeof e.target=="string"&&document.getElementById(e.target)||e.target,e.target=e.target instanceof HTMLElement&&e.target||mapp.utils.html.node`<div class="dataview-target">`,e.update=async()=>{if(!e.query)return;let t=mapp.utils.queryParams(e),i=mapp.utils.paramString(t),a=await mapp.utils.xhr(`${e.host||e.location.layer.mapview.host}/api/query?${i}`);if(!(a instanceof Error)){if(typeof e.responseFunction=="function")return e.responseFunction(a);typeof e.setData=="function"&&e.setData(a)}},e.toolbar){e.target=mapp.utils.html.node`<div class="dataview-target">`;let t=Object.keys(e.toolbar).map(i=>mapp.ui.elements.toolbar_el[i](e));e.panel=mapp.utils.html.node`
      <div class="flex-col">
  ...
  <input
    ...
    <input
      ...
      type="number"
      placeholder="Max"
      oninput=${r=>{let n=t.getColumn().getField();r.target.value.length?e.layer.filter.current[n]=Object.assign(e.layer.filter.current[n]||{},{lte:Number(r.target.value)}):delete e.layer.filter.current[n],e.layer.reload(),e.update()}}></input>`}var pe=e=>{!e.node||(e.tabs=e.node.appendChild(mapp.utils.html.node`<div class="tabs">`),e.panel=e.node.appendChild(mapp.utils.html.node`<div class="panel">`),e.id&&e.node.setAttribute("data-id",e.id),e.addTab=it,e.node.addEventListener("addTab",t=>e.addTab(t.detail)))};function it(e){let t=this;e.tab=mapp.utils.html.node`
    <div class="tab">
      <div
        class="header"
        style="${e.tab_style||""}"
  ...
  <div 
    ...
    ${e.content}`;var ue=e=>mapp.utils.html`
  <label 
  ...
  <div 
    ...
    <button
      class="flat wide bold primary-colour"
      onclick=${r=>t(r,i)}>
      Create Isoline`),o}var ve=e=>mapp.utils.html`
  <button 
    ...
    <div 
      ...
      <div style="display: grid; grid-row-gap: 5px;">
        ${t}
        ${i}`})};var ye=new XMLSerializer,$e=e=>{if(e.svg||e.type||e.layers){let t=e.layers&&Array.isArray(e.layers)&&e.layers.map(a=>`url(${a.svg})`).reverse().join(",")||`url(${e.svg||e.url||mapp.utils.svgSymbols[e.type](e)})`,i=`
      background-position: center;
      background-repeat: no-repeat;
      background-size: contain;
      ...
      <svg height=${e.height} width=${e.width}>
        <path d=${t}
          ...
          stroke-width=${e.strokeWidth||1}/>`,a=`data:image/svg+xml,${encodeURIComponent(ye.serializeToString(i))}`,l=`
      background-position: center;
      background-repeat: no-repeat;
      background-size: contain;
      width: ${e.width}px;
      ...
      <svg height=${e.height} width=${e.width}>
        <rect
          ...
          stroke=${e.strokeColor}
          stroke-width=${e.strokeWidth||1}>`,i=`data:image/svg+xml,${encodeURIComponent(ye.serializeToString(t))}`,a=`
      background-position: center;
      background-repeat: no-repeat;
      background-size: contain;
      width: ${e.width}px;
    ...
    <div
      ...
      <input id="a"
        ...
        value=${e.val}
        step=${e.step||1}
        oninput=${t}>`;function t(i){i.target.closest(".input-range").style.setProperty(`--${i.target.id}`,i.target.value),i.target.closest(".input-range").querySelectorAll("input").forEach(a=>{a.id==i.target.id&&a!=i.target&&(a.value=i.target.value)}),e.callback&&e.callback(i)}};var xe=e=>{let t=mapp.utils.html.node`
    <div
      role="group"
      ...
      <input id="b" type="range"
        ...
        step=${e.step||1}
        value=${e.val_b}
        oninput=${i}/>`;function i(a){a.target.value=a.target.value>e.max?e.max:a.target.value,a.target.value=a.target.value<e.min?e.min:a.target.value,t.style.setProperty(`--${a.target.id}`,a.target.value),t.querySelectorAll("input").forEach(l=>{l.id==a.target.id&&l!=a.target&&(l.value=a.target.value)}),a.target.id==="a"&&typeof e.callback_a=="function"&&e.callback_a(a),a.target.id==="b"&&typeof e.callback_b=="function"&&e.callback_b(a)}return t};var Le={viewport:gt,download_json:ft,download_csv:vt};function gt(e){return mapp.utils.html`
    <button
    ...
    <button
      class="flat"
      onclick=${()=>{e.Tabulator.download("csv",`${e.title||"table"}.csv`)}}>Download as CSV`}var Se={card:me,chkbox:ue,contextMenu:he,drawer:ge,drawing:fe,dropdown:ve,dropdown_multi:we,isoline_params_here:be,isoline_params_mapbox:_e,legendIcon:$e,slider:ke,slider_ab:xe,toolbar_el:Le};var E,v=null;async function Ee(e,t){return v?new v(e,t):(E||(E=new Promise(async i=>{if(window.Chart){v=window.Chart,i();return}Promise.all([import("https://cdn.skypack.dev/chart.js@3.7.1"),import("https://cdn.skypack.dev/chartjs-plugin-datalabels@2.0.0"),import("https://cdn.skypack.dev/chartjs-plugin-annotation@1.4.0")]).then(a=>{a[0].Chart.register(...a[0].registerables),a[0].Chart.register(a[1].default),a[0].Chart.register(a[2].default),v=a[0].Chart,i()}).catch(a=>{console.error(a.message),alert("Failed to load Chart.js library. Please reload the browser.")})})),await E,new v(e,t))}var C,w=null;async function Ce(){return w?new w(...arguments):(C||(C=new Promise(async e=>{if(window.Tabulator){w=window.Tabulator,e();return}Promise.all([import("https://cdn.skypack.dev/-/tabulator-tables@v5.2.7-uFL2B6RLQMMWIcn9QymG/dist=es2019,mode=imports/optimized/tabulator-tables.js")]).then(t=>{w=t[0].TabulatorFull,e()}).catch(t=>{console.error(t.message),alert("Failed to load Chart.js library. Please reload the browser.")})})),await C,new w(...arguments))}var p={idle:600},Oe=e=>{Object.assign(p,e),p.idle!==0&&(window.onload=f,window.onmousemove=f,window.onmousedown=f,window.ontouchstart=f,window.onclick=f,window.onkeypress=f,f(),ze())};function f(){p.locked||(p.timeout&&clearTimeout(p.timeout),p.timeout=setTimeout(je,p.idle*1e3))}function je(){p.locked=!0,p.renew&&clearTimeout(p.renew);let e=new XMLHttpRequest;e.open("GET",`${p.host}/api/user/cookie?destroy=true`),e.onload=t=>location.reload(),e.send()}function ze(){p.renew=setTimeout(e,(p.idle-20)*1e3);function e(){let t=new XMLHttpRequest;t.open("GET",`${p.host}/api/user/cookie?renew=true`),t.onload=i=>{if(i.target.status===401)return je();ze()},t.send()}}var Te=e=>{document.body.append(mapp.utils.html.node`
    <div class="interface-mask">
      <div class="bg-image" style=${`background-image:url(${e.target.src})`}>
      <button class="btn-close mask-icon close"
        onclick=${t=>t.target.parentElement.parentElement.remove()}>`)};var De=e=>{e.target.addEventListener("mousedown",i=>{i.preventDefault(),document.body.style.cursor="grabbing",window.addEventListener("mousemove",e.resizeEvent),window.addEventListener("mouseup",t)}),e.target.addEventListener("touchstart",i=>{i.preventDefault(),window.addEventListener("touchmove",e.resizeEvent),window.addEventListener("touchend",t)},{passive:!0});function t(){document.body.style.cursor="auto",window.removeEventListener("mousemove",e.resizeEvent),window.removeEventListener("touchmove",e.resizeEvent),window.removeEventListener("mouseup",t),window.removeEventListener("touchend",t)}};var qe={Chart:Ee,Tabulator:Ce,idleLogout:Oe,imagePreview:Te,resizeHandler:De};var Me=e=>{let t=Object.assign({input:e.target.querySelector("input"),result:e.target.querySelector("ul"),sources:{glx:a,mapbox:o,google:l}},e);t.input.placeholder=t.placeholder||"",t.input.oninput=n=>{t.xhr&&t.xhr.abort(),t.result.innerHTML="",n.target.value.length>0&&i(n.target.value)},t.input.addEventListener("keydown",n=>{(n.keyCode||n.charCode)===13&&t.result.querySelector("li").click()});function i(n){let s=n.split(",").map(parseFloat);if(s.length===2&&s.every(c=>!isNaN(c))){t.result.appendChild(mapp.utils.html.node`
      <li 
