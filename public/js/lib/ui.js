mapp.utils.merge(mapp.dictionaries,{en:{layer_zoom_to_extent:"Zoom to filtered layer extent",layer_visibility:"Toggle visibility"},de:{layer_zoom_to_extent:"Zoom zum Ausma\xDF des gefilterten Datensatzes",layer_visibility:"Umschalten der Ansicht"},cn:{layer_zoom_to_extent:"\u7F29\u653E\u81F3\u76F8\u5E94\u7B5B\u9009\u8303\u56F4",layer_visibility:"\u5207\u6362\u53EF\u89C1\u6027"},pl:{layer_zoom_to_extent:"Poka\u017C zasi\u0119g warstwy",layer_visibility:"Widoczno\u015B\u0107"},ko:{layer_zoom_to_extent:"\uD544\uD130\uB41C \uB808\uC774\uC5B4\uD06C\uAE30\uC5D0 \uC90C(zoom)",layer_visibility:"\uD1A0\uAE00 \uAC00\uC2DC\uC131"},fr:{layer_zoom_to_extent:"Zoom sur l'\xE9tendue de la couche",layer_visibility:"Changer la visiblit\xE9"},ja:{layer_zoom_to_extent:"\u30D5\u30A3\u30EB\u30BF\u30FC\u3055\u308C\u305F\u30EC\u30A4\u30E4\u30FC\u7BC4\u56F2\u3092\u30BA\u30FC\u30E0\u306B",layer_visibility:"\u8868\u793A\u5207\u66FF"}});var _=e=>{let t=e.filter.zoomToExtent&&mapp.utils.html`
    <button
      ...
      onclick=${async o=>{let r=await e.zoomToExtent();o.target.disabled=!r}}>`||"",i=mapp.utils.html.node`
    <button
    ...
    ${t}
    ${i}
    <div class="mask-icon expander"></div>`,l=Object.keys(e).map(o=>mapp.ui.layers.panels[o]&&mapp.ui.layers.panels[o](e)).filter(o=>typeof o<"u");if(e.meta){let o=mapp.utils.html.node`<p class="meta">`;o.innerHTML=e.meta,l.unshift(o)}e.view=mapp.ui.elements.drawer({data_id:"layer-drawer",class:"layer-view raised",header:a,content:l}),e.showCallbacks.push(()=>{i.classList.add("on")}),e.hideCallbacks.push(()=>{i.classList.remove("on")}),e.mapview.Map.getTargetElement().addEventListener("changeEnd",()=>{if(!!e.tables){if(e.tableCurrent()===null)return e.view.classList.add("disabled");e.view.classList.remove("disabled")}}),e.view.children.length<=1&&e.view.classList.add("empty")};mapp.utils.merge(mapp.dictionaries,{en:{layer_group_hide_layers:"Hide all layers in group"},de:{layer_group_hide_layers:"Ausschalten aller Ebenen in Gruppe"},cn:{layer_group_hide_layers:"\u9690\u85CF\u56FE\u5C42"},pl:{layer_group_hide_layers:"Ukryj warstwy z tej grupy"},ko:{layer_group_hide_layers:"\uADF8\uB8F9\uC5D0\uC11C \uB808\uC774\uC5B4 \uC228\uAE30\uAE30"},fr:{layer_group_hide_layers:"Cacher les couches du groupe"},ja:{layer_group_hide_layers:"\u30B0\u30EB\u30FC\u30D7\u304B\u3089\u30EC\u30A4\u30E4\u30FC\u3092\u96A0\u3059"}});function j(e){if(!e.mapview||!e.target)return;let t={node:e.target,groups:{}};Object.values(e.mapview.layers).forEach(l=>i(l));function i(l){if(!l.hidden){if(_(l),!l.group){t.node.appendChild(l.view),t.node.dispatchEvent(new CustomEvent("addLayerView",{detail:l}));return}t.groups[l.group]||a(l),t.groups[l.group].addLayer(l),t.node.dispatchEvent(new CustomEvent("addLayerView",{detail:l}))}}function a(l){let o={list:[]};t.groups[l.group]=o;let r=mapp.utils.html.node`
      <button
        class="mask-icon on visibility-off"
        title=${mapp.dictionary.layer_group_hide_layers}
        onclick=${n=>{n.target.style.visibility="hidden",o.list.filter(s=>s.display).forEach(s=>s.hide())}}>`;o.meta=mapp.utils.html.node`<div class="meta">`,o.drawer=mapp.ui.elements.drawer({data_id:"layer-drawer",class:"layer-group",header:mapp.utils.html`
        <h2>${l.group}</h2>
        ${r}
        <div class="mask-icon expander"></div>`,content:o.meta}),t.node.appendChild(o.drawer),o.chkVisibleLayer=()=>{r.style.visibility=o.list.some(n=>n.display)?"visible":"hidden"},o.addLayer=n=>{if(n.group=o,n.groupmeta){let s=o.meta.appendChild(mapp.utils.html.node`<div>`);s.innerHTML=n.groupmeta}o.list.push(n),o.drawer.appendChild(n.view),o.chkVisibleLayer(),n.showCallbacks.push(()=>o.chkVisibleLayer()),n.hideCallbacks.push(()=>o.chkVisibleLayer())}}}var D={like:z,match:z,numeric:T,integer:T,in:He,date:O,datetime:O,boolean:Ne,null:Be},$;function y(e,t){clearTimeout($);let i=e.view.querySelector("[data-id=zoomToExtent]");i&&(i.disabled=!1),$=setTimeout(()=>{$=null,e.reload(),e.mapview.Map.getTargetElement().dispatchEvent(new Event("changeEnd"))},500)}function z(e,t){return mapp.utils.html.node`
  <input
    type="text"
    onkeyup=${i=>{i.target.value.length?e.filter.current[t.filter.field]={[t.filter.type]:encodeURIComponent(i.target.value)}:delete e.filter.current[t.filter.field],y(e)}}>`}function Ne(e,t){function i(a){e.filter.current[t.filter.field]={boolean:a},e.reload(),e.mapview.Map.getTargetElement().dispatchEvent(new Event("changeEnd"))}return i(!1),mapp.ui.elements.chkbox({label:t.label||t.title||"chkbox",onchange:i})}function Be(e,t){function i(a){e.filter.current[t.filter.field]={null:a},e.reload(),e.mapview.Map.getTargetElement().dispatchEvent(new Event("changeEnd"))}return i(!1),mapp.ui.elements.chkbox({label:t.label||t.title||"chkbox",onchange:i})}async function T(e,t){if(!t.filter.max){let i=await mapp.utils.xhr(`${e.mapview.host}/api/query?${mapp.utils.paramString({template:"field_max",locale:e.mapview.locale.key,layer:e.key,table:e.tableCurrent(),field:t.field})}`);t.filter.max=i.max}if(!t.filter.min){let i=await mapp.utils.xhr(`${e.mapview.host}/api/query?${mapp.utils.paramString({template:"field_min",locale:e.mapview.locale.key,layer:e.key,table:e.tableCurrent(),field:t.field})}`);t.filter.min=i.min}return t.filter.step||(t.filter.step=t.filter.type==="integer"?1:.01),e.filter.current[t.field]=Object.assign({gte:Number(t.filter.min),lte:Number(t.filter.max)},e.filter.current[t.field]),y(e),mapp.ui.elements.slider_ab({min:Number(t.filter.min),max:Number(t.filter.max),step:t.filter.step,label_a:"Greater than",val_a:Number(t.filter.min),callback_a:i=>{e.filter.current[t.field].gte=Number(i.target.value),y(e)},label_b:"Lesser than",val_b:Number(t.filter.max),callback_b:i=>{e.filter.current[t.field].lte=Number(i.target.value),y(e)}})}async function He(e,t){if(t.filter.distinct){let a=await mapp.utils.xhr(`${e.mapview.host}/api/query?`+mapp.utils.paramString({template:"distinct_values",dbs:e.dbs,table:e.tableCurrent(),field:t.field}));t.filter.in=a.map(l=>l[t.field]).filter(l=>l!==null)}let i=new Set(e.filter?.current[t.filter.field]?.in||[]);return t.filter.dropdown?mapp.ui.elements.dropdown_multi({placeholder:"Select Multiple",entries:t.filter.in.map(a=>({title:a,option:a})),callback:async(a,l)=>{Object.assign(e.filter.current,{[t.filter.field]:{in:l}}),e.reload(),e.mapview.Map.getTargetElement().dispatchEvent(new Event("changeEnd"))}}):t.filter.in.map(a=>mapp.ui.elements.chkbox({val:a,label:a,checked:i.has(a),onchange:(l,o)=>{if(l)e.filter.current[t.filter.field]||(e.filter.current[t.filter.field]={}),e.filter.current[t.filter.field].in||(e.filter.current[t.filter.field].in=[]),e.filter.current[t.filter.field].in.push(encodeURIComponent(o));else{let r=e.filter.current[t.filter.field].in.indexOf(encodeURIComponent(o));e.filter.current[t.filter.field].in.splice(r,1),e.filter.current[t.filter.field].in.length||delete e.filter.current[t.filter.field].in}e.reload(),e.mapview.Map.getTargetElement().dispatchEvent(new Event("changeEnd"))}}))}function O(e,t){let i=mapp.utils.html.node`
    <input
      data-id="inputAfter"
      onchange=${l}
    ...
    <button
      ...
      <div class="mask-icon expander"></div>`,content:mapp.utils.html`
      ${t}
      ${i}`}),e.filter.view};var M=e=>{let t=mapp.utils.html.node`
  <div class="dropdown active">
    <input id="gazetteerInput" type="text" placeholder=${e.gazetteer.placeholder||"Search"}>
    <ul></ul>`;return mapp.ui.Gazetteer(Object.assign({mapview:e.mapview,target:t,layer:e.key},e.gazetteer)),t};mapp.utils.merge(mapp.dictionaries,{en:{layer_dataview_header:"Data Views"},de:{layer_dataview_header:"Datenansichten"},cn:{layer_dataview_header:"\u6570\u636E\u68C0\u89C6"},pl:{layer_dataview_header:"Widoki danych"},ko:{layer_dataview_header:"\uB370\uC774\uD130 \uBCF4\uAE30"},fr:{layer_dataview_header:"Vues des donn\xE9es"},ja:{layer_dataview_header:"\u30C7\u30FC\u30BF\u30D3\u30E5\u30FC"}});var I=e=>{let t=Object.entries(e.dataviews).map(a=>{let l=Object.assign(a[1],{key:a[0],layer:e,host:e.mapview.host}),o=document.querySelector(`[data-id=${l.target}]`);if(!o)return;l.target=mapp.utils.html.node`<div class="dataview-target">`,e.display&&l.display&&r();function r(){if(l.show)return l.show();mapp.ui.Dataview(l).then(()=>l.show()),o.dispatchEvent(new CustomEvent("addTab",{detail:l}))}return e.showCallbacks.push(()=>{l.display&&r()}),mapp.ui.elements.chkbox({label:l.title||l.key,checked:!!l.display,onchange:n=>{l.display=n,l.display?r():l.remove()}})}),i=mapp.ui.elements.drawer({data_id:"dataviews-drawer",class:"raised",header:mapp.utils.html`
      <h3>${mapp.dictionary.layer_dataview_header}</h3>
      <div class="mask-icon expander"></div>`,content:mapp.utils.html`${t}`});if(!e.hideDataviewPanel)return i};var P=e=>{let t=Object.keys(e.reports).map(a=>{let l=e.reports[a];l.key=a,l.host=e.mapview.host;let o=`${l.host}/view?${mapp.utils.paramString({template:l.template,lat:mapp.hooks.current?.lat,lng:mapp.hooks.current?.lng,z:mapp.hooks.current?.z})}`;return mapp.utils.html`
      <a
        class="link-with-img"
        target="_blank"
        href="${o}">
    ...
    <div class="legend">
      ...
      <div class="mask-icon expander"></div>`,content:t}),!e.hideStylePanel))return e.style.drawer};var R=e=>{let t=e.style.theme,i=[];if(e.filter&&i.push(mapp.utils.html`
    <div
      class="switch-all"
    ...
    <div 
      ...
      <div 
        data-id="cluster"
        class="contents">
        ${a}${l}`)}return e.style.legend=mapp.utils.html.node`<div class="grid">${i}`,e.style.legend};var V=e=>{let t=e.style.theme,i=[];return t.cat_arr.forEach(a=>{let l=Object.assign({},e.style.default,a.style&&a.style.icon||a.style||a),o=mapp.utils.html`
      <div
        style="height: 24px; width: 24px; grid-column: 1;">
        ${mapp.ui.elements.legendIcon(Object.assign({width:24,height:24},l))}`,r=mapp.utils.html`
      ...
      <div 
        data-id=${a.value}
        class="contents">
        ${o}${r}`)}),e.style.legend=mapp.utils.html.node`<div class="grid">${i}`,e.style.legend};var G=e=>{let t=[];t.push(mapp.ui.elements.dropdown({entries:Object.entries(e.grid_fields).map(o=>({title:o[0],options:o[1]})),callback:(o,r)=>{e.grid_size=r.options,e.reload()}}));let i=e.style.range.length,a=new XMLSerializer,l=mapp.utils.html`
  <div
    class="grid"
    ...
    <div data-id="size-icons" class="contents">
      ${e.style.range.map((o,r)=>{let n=mapp.utils.svg.node`<svg height=50 width=50>
          <circle
            fill='#777'
            cx=27
            cy=27
          ...
          <circle
            ...
            cx=25
            cy=25
            r='${23/i*(r+1)}'/>`,s=`data:image/svg+xml,${encodeURIComponent(a.serializeToString(n))}`,c=`
          grid-row:2;
          grid-column:${r+1};
    ...
    <div data-id="colour-icons" class="contents">
      ${e.style.range.map((o,r)=>mapp.utils.html`
        <div style=${`grid-row:3;grid-column:${r+1};background-color:${o};width:100%;height:100%;`}>
      `)}
    </div>
  ...
  <div
    ...
    <button
      ...
      <button
        ...
        onclick = ${o=>{o.target.classList.contains("on")?(o.target.classList.remove("on"),a()):(o.target.classList.add("on"),e.infoj.forEach(r=>{!r._edit||(r.edit=r._edit,delete r._edit)})),i.remove(),i=mapp.ui.locations.infoj(e),e.view.appendChild(i)}}>`;t.push(l)}t.push(mapp.utils.html`
    <button
    ...
    <button
      ...
      onclick = ${a=>{e.remove()}}>`),e.view=mapp.ui.elements.drawer({class:"location-view raised expanded",header:t});let i=e.view.appendChild(mapp.ui.locations.infoj(e));e.view.querySelector(".header").style.borderBottom=`3px solid ${e.record.colour}`,e.view.addEventListener("valChange",a=>{a.detail.value!=a.detail.newValue?a.detail.node.classList.add("val-changed"):(delete a.detail.newValue,a.detail.node.classList.remove("val-changed")),e.view.querySelector(".cloud-upload").style.display=e.infoj.some(l=>typeof l.newValue<"u")&&"inline-block"||"none"}),e.view.addEventListener("updateInfo",()=>{i.remove(),e.view.querySelector(".cloud-upload").style.display="none",e.view.classList.remove("disabled"),i=mapp.ui.locations.infoj(e),e.view.appendChild(i)})};mapp.utils.merge(mapp.dictionaries,{en:{location_clear_all:"Clear locations"},de:{location_clear_all:"Entferne Auswahl"},cn:{location_clear_all:"\uBAA8\uB4E0 \uC704\uCE58 \uC81C\uAC70"},pl:{location_clear_all:"Wyczy\u015B\u0107 selekcje"},ko:{location_clear_all:"\u6E05\u9664\u6240\u6709\u5730\u70B9"},fr:{location_clear_all:"Des\xE9lectionner tous les lieux."},ja:{location_clear_all:"\u5168\u30ED\u30B1\u30FC\u30B7\u30E7\u30F3\u3092\u30AF\u30EA\u30A2"}});var U=[{symbol:"A",colour:"#2E6F9E"},{symbol:"B",colour:"#EC602D"},{symbol:"C",colour:"#5B8C5A"},{symbol:"D",colour:"#B84444"},{symbol:"E",colour:"#514E7E"},{symbol:"F",colour:"#E7C547"},{symbol:"G",colour:"#368F8B"},{symbol:"H",colour:"#841C47"},{symbol:"I",colour:"#61A2D1"},{symbol:"J",colour:"#37327F"}],W=e=>{if(!e.mapview||!e.target)return;let t={node:e.target,mapview:e.mapview},i=t.node.appendChild(mapp.utils.html.node`
    <button 
      ...
      ${mapp.dictionary.location_clear_all}`);return e.mapview.locations=new Proxy(e.mapview.locations,{set:function(a,l,o){let r=U.find(n=>!n.hook);return r.hook=o.hook,o.record=r,o.style={strokeColor:r.colour,fillColor:r.colour,fillOpacity:.2},o.Style=mapp.utils.style([{strokeColor:"#000",strokeOpacity:.1,strokeWidth:8},{strokeColor:"#000",strokeOpacity:.1,strokeWidth:6},{strokeColor:"#000",strokeOpacity:.1,strokeWidth:4},{strokeColor:o.style.strokeColor||"#000",strokeWidth:2,fillColor:o.style.fillColor||o.style.strokeColor||"#fff",fillOpacity:o.style.fillOpacity||.2}]),o.pinStyle=mapp.utils.style({icon:{type:"markerLetter",letter:r.symbol,color:o.style.strokeColor,scale:3,anchor:[.5,1]}}),Reflect.set(...arguments),mapp.ui.locations.view(o),Object.values(t.node.children).forEach(n=>n.classList.remove("expanded")),t.node.insertBefore(o.view,i.nextSibling),o.view.dispatchEvent(new Event("addLocationView")),i.style.display="block",document.querySelector("[data-id=locations]").click(),document.querySelector("[data-id=locations]").style.display="block",!0},deleteProperty:function(a,l){Reflect.deleteProperty(...arguments);let o=U.find(r=>r.hook===l);return o&&delete o.hook,setTimeout(()=>{if(!document.querySelectorAll("#locations > .location-view").length){document.querySelector("[data-id=layers]").click(),i.style.display="none";let r=document.querySelector("#locations #gazetteerInput");r?r.value="":document.querySelector("[data-id=locations]").style.display="none"}},300),!0}}),t};var J=(e,t)=>{if(!e.infoj)return;let i=mapp.utils.html.node`<div class="location-view-grid">`,a={};for(let l of t||e.infoj){if(e.view&&e.view.classList.contains("disabled"))break;if(l.listview=i,l.type=l.type||"text",l.skipEntry||l.skipFalsyValue&&!l.value&&!l.edit||l.skipUndefinedValue&&typeof l.value>"u"&&!l.edit||l.skipNullValue&&l.value===null&&!l.edit)continue;if(l.nullValue&&l.value===null&&!l.defaults&&!l.edit&&(l.value=l.nullValue),l.objectAssignFromField){let r=l.location.infoj.find(n=>n.field===l.objectAssignFromField);r&&Object.assign(l,r.value)}if(l.objectMergeFromField){let r=l.location.infoj.find(n=>n.field===l.objectMergeFromField);r&&mapp.utils.merge(l,r.value)}l.group&&(a[l.group]||(a[l.group]=l.listview.appendChild(mapp.ui.elements.drawer({class:`group ${l.expanded&&"expanded"||""}`,header:mapp.utils.html`
              <h3>${l.group}</h3>
      ...
      <div
        ...
        <div
          class="label"
          style="${`${l.css_title||""}`}"
          title="${l.tooltip||null}">${l.title}`),(l.value===null||typeof l.value>"u")&&(l.value=l.default||l.value);let o=mapp.ui.locations.entries[l.type]&&mapp.ui.locations.entries[l.type](l);if(o==="break")break;o&&l.node.append(o)}return i};var Z=e=>{let t=mapp.ui.elements.chkbox({label:e.label||e.title,checked:e.value,disabled:!e.edit,onchange:a=>{e.newValue=a,e.location.view?.dispatchEvent(new CustomEvent("valChange",{detail:e}))}});return mapp.utils.html.node`${t}`};var X=e=>{if(e.layer=e.location.layer,e.host=e.host||e.location.layer.mapview.host,e.dependents&&e.dependents.some(t=>e.location.infoj.some(i=>!i.value&&i.field===t))&&delete e.display,e.update)return e.display&&e.update(),mapp.utils.html.node`
      ${e.chkbox||""}
    ...
    ${e.chkbox||""}
    ${e.locationViewTarget||""}`};function K(e){if(e.show)return e.show();mapp.ui.Dataview(e).then(()=>{e.tab_style=`border-bottom: 3px solid ${e.location.style.strokeColor}`,e.tabview.dispatchEvent(new CustomEvent("addTab",{detail:e})),e.display&&e.show()})}var k=e=>{let t;return e.edit?t=mapp.utils.html.node`
      <input
    ...
    <div
      class="val"
      style="${`${e.css_val||""}`}">
      ${e.prefix}${t}${e.suffix}`};var Y=e=>{let t=e.value.map(r=>mapp.utils.html`
      ...
      ${e.edit&&mapp.utils.html.node`
        ...
        <a target="_blank"
          href=${r}>${r.replace(/.*\//,"").replace(/\.([\w-]{3})/,"")}`),i=mapp.utils.html.node`
    <div class="mask-icon cloud-upload">
      <input
        ...
        <div class="link-with-img">
          ...
          <a target="_blank"
            href=${p}>${u}`;a.insertBefore(m,i),e.location.view?.classList.remove("disabled")},n.readAsDataURL(s),r.target.value="")}async function o(r){if(!confirm("Remove document link?"))return;let n=r.target;mapp.utils.xhr(`${e.location.layer.mapview.host}/api/provider/cloudinary?${mapp.utils.paramString({destroy:!0,public_id:n.dataset.name})}`),await mapp.utils.xhr(`${e.location.layer.mapview.host}/api/query?`+mapp.utils.paramString({template:"set_field_array",locale:e.location.layer.mapview.locale.key,layer:e.location.layer.key,table:e.location.table,qID:e.location.layer.qID,id:e.location.id,action:"remove",field:e.field,value:n.dataset.href})),n.parentNode.remove()}};var ee=e=>{if(e.value=typeof e.value=="string"&&JSON.parse(e.value)||e.value,e.display=e.display&&e.value,!e.value&&!e.edit)return;e.Style=e.Style||typeof e.style=="object"&&mapp.utils.style(Object.assign({},e.location?.style||{},e.style))||e.location.Style,e.show=We;let t=mapp.ui.elements.chkbox({label:e.label||"Geometry",data_id:`${e.field}-chkbox`,checked:!!e.display,disabled:!e.value,onchange:o=>{o?e.show():(e.display=!1,e.L&&e.location.layer.mapview.Map.removeLayer(e.L))}});e.display&&e.show();let i={entry:e,layer:e.location.layer,srid:e.srid||e.location.layer.srid,edit:e.edit,mapview:e.location.layer.mapview},a=typeof e.edit=="object"&&Object.keys(e.edit).map(o=>mapp.ui.elements.drawing[o]&&mapp.ui.elements.drawing[o](i,Je)).filter(o=>!!o);if(e.edit?.geometry){let o=mapp.utils.html.node`
      <button
    ...
    ${mapp.ui.elements.legendIcon(Object.assign({width:24,height:24},e.style))}`;return a?mapp.ui.elements.drawer({data_id:"draw-drawer",header:mapp.utils.html`
        ...
        ${a}`}):mapp.utils.html.node`<div class="flex-spacer">${t}${l}`};function We(){this.display=!0;let e=this.location.view?.querySelector(`[data-id=${this.field}-chkbox] input`);e&&(e.checked=!0),this.L&&(this.location.layer.mapview.Map.removeLayer(this.L),delete this.L),this.L=this.location.layer.mapview.geoJSON({zIndex:this.zIndex||99,geometry:this.value,Style:this.Style,dataProjection:this.srid||this.location?.layer?.srid}),this.location.Layers.push(this.L)}function Je(e,t){let i=e.target;if(i.classList.contains("active")){i.classList.remove("active"),t.mapview.interactions.highlight();return}i.classList.add("active"),!t.entry.display&&t.entry.show(),t.entry.L&&t.entry.location.layer.mapview.Map.removeLayer(t.entry.L),t.mapview.interactions.draw({type:t.type,geometryFunction:t.geometryFunction,tooltip:t.tooltip,srid:t.srid,callback:a=>{if(i.classList.remove("active"),t.mapview.interactions.highlight(),a){t.entry.newValue=a.geometry,t.entry.location.update();return}t.entry.L&&t.entry.location.layer.mapview.Map.addLayer(t.entry.L)}})}function Ze(e,t){let i=e.target;if(i.classList.contains("active")){i.classList.remove("active"),t.mapview.interactions.highlight();return}i.classList.add("active"),!t.entry.display&&t.entry.show(),t.entry.location.layer.mapview.Map.removeLayer(t.entry.L);let a=t.entry.L.getSource().getFeatures()[0];t.mapview.interactions.modify({Feature:a.clone(),snapLayer:t.entry.location.layer,srid:t.entry.srid||t.entry.location.layer.srid,callback:l=>{if(i.classList.remove("active"),t.mapview.interactions.highlight(),l){t.entry.newValue=l.geometry,t.entry.location.update();return}t.entry.location.layer.mapview.Map.addLayer(t.entry.L)}})}var te=e=>{let t=e.value.map(r=>mapp.utils.html`
    <div>
      <img src=${r}
        ...
        ${e.edit&&mapp.utils.html.node`
          <button
            ...
            data-name=${r.replace(/.*\//,"").replace(/\.([\w-]{3})/,"")}
            data-src=${r}
            onclick=${n=>o(n)}>`}`),i=mapp.utils.html.node`
    ...
    <div
      class="images-grid">${t}`;return a;async function l(r){e.location.view?.classList.add("disabled");let n=new FileReader,s=r.target.files[0];!s||(n.onload=c=>{let d=new Image;d.onload=async()=>{let p=mapp.utils.html.node`<canvas>`,u=1024,m=d.width,f=d.height;m>f&&m>u?(f*=u/m,m=u):f>u&&(m*=u/f,f=u),p.width=m,p.height=f,p.getContext("2d").drawImage(d,0,0,m,f);let Re=p.toDataURL("image/jpeg",.5),w=await mapp.utils.xhr({method:"POST",requestHeader:{"Content-Type":"application/octet-stream"},url:`${e.location.layer.mapview.host}/api/provider/cloudinary?${mapp.utils.paramString({public_id:s.name.replace(/.*\//,"").replace(/\.([\w-]{3})/,""),resource_type:"image"})}`,body:mapp.utils.dataURLtoBlob(Re)});await mapp.utils.xhr(`${e.location.layer.mapview.host}/api/query?`+mapp.utils.paramString({template:"set_field_array",locale:e.location.layer.mapview.locale.key,layer:e.location.layer.key,table:e.location.table,qID:e.location.layer.qID,id:e.location.id,action:"append",field:e.field,value:w.secure_url}));let Ve=mapp.utils.html.node`
          <div>
            <img
              src=${w.secure_url}
              onclick=${mapp.ui.utils.imagePreview}>
    ...
    <div
      class="val"
      style="${`${e.css_val||""}`}">${t}`};var ie=e=>{let t=e.location.layer.mapview;e.Layer=t.layers[e.layer];let i=mapp.ui.elements.chkbox({label:e.label||"MVT Clone",data_id:`${e.field}-chkbox`,checked:!!e.display,onchange:r=>{r?o():(e.display=!1,t.Map.removeLayer(e.L),e.Layer.clones.delete(e.L),e.legend?.remove())}});e.display&&o();let a=mapp.utils.html.node`<div>${i}`;function l(){if(!!e.Layer.tables){if(e.Layer.tableCurrent()===null)return a.querySelector("input").disabled=!0;a.querySelector("input").disabled=!1}}return l(),t.Map.getTargetElement().addEventListener("changeEnd",l),e.location.removeCallbacks.push(()=>{t.Map.getTargetElement().removeEventListener("changeEnd",l)}),a;async function o(){if(e.L){t.Map.addLayer(e.L),e.Layer.clones.add(e.L);return}let r=mapp.utils.queryParams(e),n=mapp.utils.paramString(r),s=await mapp.utils.xhr(`${e.host||e.location.layer.mapview.host}/api/query?${n}`);if(!e.location.remove)return;let c={style:{default:e.Layer.style.default},featureLookup:s};e.theme&&(c.style.theme=e.theme,e.legend=e.node.appendChild(mapp.utils.html.node`
        <div class="legend">
          ${e.theme.title||""}
          ${mapp.ui.layers.styles[e.theme.type](c)}`)),e.L=new ol.layer.VectorTile({source:e.Layer.L.getSource(),renderBuffer:200,style:mapp.layer.Style(c)}),e.Layer.clones.add(e.L),t.Map.addLayer(e.L),e.location.removeCallbacks.push(()=>{e.location.layer.mapview.Map.removeLayer(e.L),e.Layer.clones.delete(e.L)})}};var L=e=>{let t=!isNaN(e.value)&&e.type==="integer"?parseInt(e.value).toLocaleString("en-GB",{maximumFractionDigits:0}):parseFloat(e.value).toLocaleString("en-GB",{maximumFractionDigits:2});return e.edit&&(e.edit.range?t=mapp.ui.elements.slider({min:e.edit.range.min,max:e.edit.range.max,val:e.value,callback:a=>{t.value=parseFloat(a.target.value),e.newValue=t.value,e.location.view?.dispatchEvent(new CustomEvent("valChange",{detail:e}))}}):t=mapp.utils.html.node`
      <input
        type="number"
        value="${e.value||""}"
    ...
    <div
      class="val"
      style="${`${e.css_val||""}`}">
      ${e.prefix}${i}${e.suffix}`,t};function et(e){let t=e.edit.options.find(l=>typeof l=="object"&&Object.values(l)[0]===e.value||l===e.value)||e.value;e.value=t&&typeof t=="object"&&Object.keys(t)[0]||t||e.value||" ";let i=e.edit.options.map(l=>({title:typeof l=="string"&&l||Object.keys(l)[0],option:typeof l=="string"&&l||Object.values(l)[0]}));return mapp.ui.elements.dropdown({span:e.value,entries:i,callback:(l,o)=>{e.newValue=o.option,e.location.view?.dispatchEvent(new CustomEvent("valChange",{detail:e}))}})}var E=e=>{e.query&&mapp.utils.xhr(`${e.location.layer.mapview.host}/api/query/${e.query}`).then(a=>{e.node.querySelector(".val").innerHTML=a[e.params.field]});let t=e.type!=="html"?e.value:"";e.edit&&(t=mapp.utils.html`
    <textarea
      style="auto; min-height: 50px;"
      onfocus=${a=>{a.target.style.height=a.target.scrollHeight+"px"}}
      onfocusout=${a=>{a.target.style.height="auto"}}
  ...
  <div
    class="val"
    style="${`${e.css_val||""}`}">${t}`;return!e.edit&&e.type==="html"&&(i.innerHTML=e.value||""),i};var de=e=>{let t,i=e.value&&e.value.toString().replace(".",":");return i=i&&i.length<3&&`${i}:00`||i,e.edit?t=mapp.utils.html.node`
      <input
        type="time"
        value=${i}
    ...
    <div
      class="val"
      style="${`${e.css_val||""}`}">
      ${t}`};var ce={key:tt,boolean:Z,dataview:X,date:k,datetime:k,defaults:Q,documents:Y,geometry:ee,html:E,images:te,integer:L,isoline_here:x,isoline_mapbox:x,json:ae,mvt_clone:ie,numeric:L,pin:le,report:oe,streetview:re,tab:ne,text:se,textarea:E,time:de};function tt(e){return mapp.utils.html.node`
  <div class="layer-key">
    <span>
      ${e.location.layer.name}`}var pe={view:H,listview:W,infoj:J,entries:ce};var ue=async e=>{if(typeof e.target=="string"&&(e.target=document.getElementById(e.target)),!e.target){console.warn("Dataview creation requires a target key-value"),console.log(e);return}if(e.update=async()=>{if(!e.query)return;let t=mapp.utils.queryParams(e),i=mapp.utils.paramString(t),a=await mapp.utils.xhr(`${e.host||e.location.layer.mapview.host}/api/query?${i}`);if(!(a instanceof Error)){if(typeof e.responseFunction=="function")return e.responseFunction(a);typeof e.setData=="function"&&e.setData(a)}},e.toolbar&&e.target instanceof HTMLElement){let t=mapp.utils.html.node`<div class="dataview-target">`,i=Object.keys(e.toolbar).map(a=>mapp.ui.elements.toolbar_el[a](e));e.panel=e.target.appendChild(mapp.utils.html.node`
        <div class="flex-col">
          <div class="btn-row">${i}</div>
          ${t}`),e.target=t}return e.chart&&await at(e),typeof e.columns<"u"&&(console.warn("Table dataviews should be configured inside a tables object"),e.table={columns:e.columns}),e.table&&await it(e),e.mapChange&&e.layer&&e.layer.mapview.Map.getTargetElement().addEventListener("changeEnd",()=>{e.layer&&!e.layer.display||e.tab&&!e.tab.classList.contains("active")||typeof e.mapChange=="function"&&e.mapChange()||e.update()}),e};async function at(e){let t=e.target.appendChild(mapp.utils.html.node`<canvas>`);e.ChartJS=await mapp.ui.utils.Chart(t,mapp.utils.merge({type:"bar",options:{plugins:{legend:{display:!1},datalabels:{display:!1}}}},e.chart)),e.setData=i=>{e.noDataMask&&!i?(e.target.style.display="none",e.mask=!e.mask&&e.target.parentElement?.appendChild(mapp.utils.html.node`
        <div class="dataview-mask">No Data`)):(e.mask&&e.mask.remove(),delete e.mask,e.target.style.display="block"),i||(i={datasets:[{data:[]}]}),i.datasets||(i={datasets:[{data:i}]}),e.data=i,e.chart.datasets?.length&&i.datasets.forEach((a,l)=>Object.assign(a,e.chart.datasets[l])),i.labels=i.labels||e.chart.labels,e.ChartJS.data=i,e.ChartJS.update()}}async function it(e){e.table.columns.forEach(t=>{typeof t.headerFilter=="string"&&mapp.ui.utils.tabulator.headerFilter[t.headerFilter]&&(t.headerFilter=mapp.ui.utils.tabulator.headerFilter[t.headerFilter](e))}),e.Tabulator=await mapp.ui.utils.Tabulator(e.target,Object.assign({renderHorizontal:"virtual"},e.table)),e.table.autoResize===!1&&(e.resizeObserver=new ResizeObserver(mapp.utils.debounce(()=>{e.target.offsetHeight>9&&e.Tabulator.redraw()},800)),e.resizeObserver.observe(e.target)),e.table.selectable&&e.Tabulator.on("rowClick",(t,i)=>{if(typeof e.rowSelect=="function"){e.rowSelect(t,i);return}let a=i.getData();!e.layer||!a[e.layer.qID]||(mapp.location.get({layer:e.layer,id:a[e.layer.qID]}),i.deselect())}),typeof e.events=="object"&&Object.entries(e.events).forEach(t=>{typeof t[1]=="function"&&e.Tabulator.on(t[0],t[1])}),e.setData=t=>{!t&&e.data||(e.noDataMask&&!t?(e.target.style.display="none",e.mask=!e.mask&&e.target.parentElement?.appendChild(mapp.utils.html.node`
        <div class="dataview-mask">No Data`)):(e.mask&&e.mask.remove(),delete e.mask,e.target.style.display="block"),t=!t&&[]||t.length&&t||[t],e.Tabulator.setData(t),e.data=t,typeof e.setDataCallback=="function"&&e.setDataCallback(e))}}var me=e=>{!e.node||(e.tabs=e.node.appendChild(mapp.utils.html.node`<div class="tabs">`),e.panel=e.node.appendChild(mapp.utils.html.node`<div class="panel">`),e.id&&e.node.setAttribute("data-id",e.id),e.addTab=lt,e.node.addEventListener("addTab",t=>e.addTab(t.detail)))};function lt(e){let t=this;e.layer&&(e.layer.showCallbacks.push(()=>{e.display&&e.show()}),e.layer.hideCallbacks.push(()=>{e.remove()})),e.tab=mapp.utils.html.node`
    <div class="tab">
      <div
        class="header"
        style="${e.tab_style||""}"
  ...
  <div 
    ...
    ${e.content}`;var fe=e=>mapp.utils.html`
  <label 
    ...
    <div></div>
    <span>${e.label}`;var ge={modify:ot,draw:rt};mapp.utils.merge(mapp.dictionaries,{en:{remove_last_vertex:"Remove last vertex"},de:{remove_last_vertex:"Remove last vertex"},cn:{remove_last_vertex:"\u5220\u9664\u6700\u540E\u4E00\u4E2A\u9876\u70B9"},pl:{remove_last_vertex:"Usu\u0144 ostatni wierzcho\u0142ek"},ko:{remove_last_vertex:"\uB9C8\uC9C0\uB9C9 \uC815\uC810(\uAF2D\uC9C0\uC810) \uC81C\uAC70"},fr:{remove_last_vertex:"Effacer le dernier point"},ja:{remove_last_vertex:"\u6700\u5F8C\u306E\u30D0\u30FC\u30C6\u30C3\u30AF\u30B9\u3092\u524A\u9664"}});function ot(e){e&&e.preventDefault();let t=[];t.push(mapp.utils.html`
    <li
    ...
    <li
      ...
      ${mapp.dictionary.cancel}`),this.popup({coords:this.interaction.vertices[this.interaction.vertices.length-1],content:mapp.utils.html.node`<ul>${t}`})}function rt(e){if(this.interaction.vertices.length===0)return;e&&e.preventDefault();let t=this.interaction.Layer.getSource().getFeatures(),i=[];t.length&&i.push(mapp.utils.html`
  <li
    onclick=${()=>this.interaction.finish(this.interaction.getFeature())}>
    ...
    <li
      onclick=${()=>this.interaction.finish()}>
      ${mapp.dictionary.cancel}`),this.popup({coords:this.interaction.vertices[this.interaction.vertices.length-1],content:mapp.utils.html.node`<ul>${i}`,autoPan:!0})}var ve=e=>mapp.utils.html.node`
  <div 
    ...
    <button
      ...
      ${mapp.dictionary.draw_position}`}function dt(e,t){let i=Object.assign({},e,{type:"LineString"});return mapp.utils.html.node`
    <button
      class="flat wide bold primary-colour"
      onclick=${a=>t(a,i)}>
      ${mapp.dictionary.draw_line}`}function ct(e,t){let i=Object.assign({},e,{type:"LineString",freehand:!0});return mapp.utils.html.node`
    <button
      class="flat wide bold primary-colour"
      onclick=${a=>t(a,i)}>
      ${mapp.dictionary.draw_freehand}`}function pt(e,t){let i=Object.assign({},e,{type:"Polygon"});return mapp.utils.html.node`
    <button
      class="flat wide bold primary-colour"
      onclick=${a=>t(a,i)}>
      ${mapp.dictionary.draw_polygon}`}function ut(e,t){let i=Object.assign({},e,{type:"Circle",geometryFunction:ol.interaction.Draw.createRegularPolygon(33),tooltip:e.edit.circle.tooltip});return mapp.utils.html.node`
    <button
      class="flat wide bold primary-colour"
      onclick=${a=>t(a,i)}>
      ${mapp.dictionary.draw_circle}`}function mt(e,t){let i=Object.assign({},e,{type:"Circle",geometryFunction:ol.interaction.Draw.createBox(),tooltip:e.edit.rectangle.tooltip});return mapp.utils.html.node`
    <button
      class="flat wide bold primary-colour"
      onclick=${a=>t(a,i)}>
      ${mapp.dictionary.draw_rectangle}`}function ht(e,t){let i=Object.assign({},e,{type:"Point",geometryFunction:r=>mapp.utils.here.geometryFunction(r,e.layer,l)});typeof i.edit.isoline_here!="object"&&(i.edit.isoline_here={});let a={"range[type]":"time",range:10,rangeMin:5,rangeMax:60,transportMode:"car",optimizeFor:"balanced"},l=Object.assign(i.edit.isoline_here,a),o=mapp.ui.elements.isoline_params_here(l);return o.append(mapp.utils.html.node`
    <button
      class="flat wide bold primary-colour"
      onclick=${r=>t(r,i)}>
      ${mapp.dictionary.create}`),o}function ft(e,t){let i=Object.assign({},e,{type:"Point",geometryFunction:r=>mapp.utils.mapboxGeometryFunction(r,e.layer,l)});typeof i.edit.isoline_mapbox!="object"&&(i.edit.isoline_mapbox={});let a={profile:"driving",minutes:10,minutesMin:5,minutesMax:60},l=Object.assign(i.edit.isoline_mapbox,a),o=mapp.ui.elements.isoline_params_mapbox(l);return o.append(mapp.utils.html.node`
    <button
      class="flat wide bold primary-colour"
      onclick=${r=>t(r,i)}>
      ${mapp.dictionary.create}`),o}var we=e=>mapp.utils.html`
  <button 
    ...
    <div 
      ...
      <div style="display: grid; grid-row-gap: 5px;">
        ${t}
        ${i}`})};var ke=new XMLSerializer,xe=e=>{if(e.svg||e.type||e.layers){let t=e.layers&&Array.isArray(e.layers)&&e.layers.map(a=>`url(${a.svg})`).reverse().join(",")||`url(${e.svg||e.url||mapp.utils.svgSymbols[e.type](e)})`,i=`
      background-position: center;
      background-repeat: no-repeat;
      background-size: contain;
      ...
      <svg height=${e.height} width=${e.width}>
        <path d=${t}
          ...
          stroke-width=${e.strokeWidth||1}/>`,a=`data:image/svg+xml,${encodeURIComponent(ke.serializeToString(i))}`,l=`
      background-position: center;
      background-repeat: no-repeat;
      background-size: contain;
      width: ${e.width}px;
      ...
      <svg height=${e.height} width=${e.width}>
        <rect
          ...
          stroke=${e.strokeColor}
          stroke-width=${e.strokeWidth||1}>`,i=`data:image/svg+xml,${encodeURIComponent(ke.serializeToString(t))}`,a=`
      background-position: center;
      background-repeat: no-repeat;
      background-size: contain;
      width: ${e.width}px;
      height: ${e.height}px;
      background-image: url(${i});`;return mapp.utils.html`<div style=${a}>`}};var Le=e=>{return mapp.utils.html.node`
    <div
      role="group"
      ...
      <input id="a"
        ...
        value=${e.val}
        step=${e.step||1}
        oninput=${t}>`;function t(i){i.target.closest(".input-range").style.setProperty(`--${i.target.id}`,i.target.value),i.target.closest(".input-range").querySelectorAll("input").forEach(a=>{a.id==i.target.id&&a!=i.target&&(a.value=i.target.value)}),e.callback&&e.callback(i)}};var Ee=e=>{let t=mapp.utils.html.node`
    <div
      role="group"
      ...
      <input id="b" type="range"
        ...
        step=${e.step||1}
        value=${e.val_b}
        oninput=${i}/>`;function i(a){a.target.value=a.target.value>e.max?e.max:a.target.value,t.style.setProperty(`--${a.target.id}`,a.target.value),t.querySelectorAll("input").forEach(l=>{l.id==a.target.id&&l!=a.target&&(l.value=a.target.value)}),a.target.id==="a"&&typeof e.callback_a=="function"&&e.callback_a(a),a.target.id==="b"&&typeof e.callback_b=="function"&&e.callback_b(a)}return t};var Se={viewport:gt,download_json:vt,download_csv:bt};function gt(e){return mapp.utils.html`
    <button
      class=${`flat ${e.viewport&&"active"||""}`}
      onclick=${t=>{t.target.classList.toggle("active"),e.viewport=!e.viewport,e.update()}}>Viewport`}function vt(e){return mapp.utils.html`
    <button
      class="flat"
      onclick=${()=>{e.Tabulator.download("json",`${e.title||"table"}.json`)}}>Export as JSON`}function bt(e){return mapp.utils.html`
    <button
      class="flat"
      onclick=${()=>{e.Tabulator.download("csv",`${e.title||"table"}.csv`)}}>Download as CSV`}var Ce={card:he,chkbox:fe,contextMenu:ge,drawer:ve,drawing:be,dropdown:we,dropdown_multi:_e,isoline_params_here:ye,isoline_params_mapbox:$e,legendIcon:xe,slider:Le,slider_ab:Ee,toolbar_el:Se};var S,v=null;async function je(e,t){return v?new v(e,t):(S||(S=new Promise(async i=>{if(window.Chart){v=window.Chart,i();return}Promise.all([import("https://cdn.skypack.dev/chart.js@3.7.1"),import("https://cdn.skypack.dev/chartjs-plugin-datalabels@2.0.0"),import("https://cdn.skypack.dev/chartjs-plugin-annotation@1.4.0")]).then(a=>{a[0].Chart.register(...a[0].registerables),a[0].Chart.register(a[1].default),a[0].Chart.register(a[2].default),v=a[0].Chart,i()}).catch(a=>{console.error(a.message),alert("Failed to load Chart.js library. Please reload the browser.")})})),await S,new v(e,t))}var C,b=null;async function ze(){return b?new b(...arguments):(C||(C=new Promise(async e=>{if(window.Tabulator){b=window.Tabulator,e();return}Promise.all([import("https://cdn.skypack.dev/pin/tabulator-tables@v5.3.0-3P9dSwNRL29tgc0Vmk59/mode=imports,min/optimized/tabulator-tables.js")]).then(t=>{b=t[0].TabulatorFull,e()}).catch(t=>{console.error(t.message),alert("Failed to load Tabulator library. Please reload the browser.")})})),await C,new b(...arguments))}var Te={headerFilter:{like:wt,numeric:_t}};function wt(e){return(t,i,a,l,o)=>{let r=t.getColumn().getField();function n(s){let c=e.Tabulator.getFilters().find(d=>d.field===r&&d.type==">=");c&&(e.Tabulator.removeFilter(...Object.values(c)),o.layer&&delete e.layer.filter.current[r]),s.target.value.length&&(e.Tabulator.setFilter([{field:r,type:"like",value:s.target.value}]),o.layer&&(e.layer.filter.current[r]={[o.type]:encodeURIComponent(s.target.value)},e.layer.reload(),e.update()))}return mapp.utils.html.node`<span>
      <input
        type="text"
        placeholder="Filter"
        oninput=${n}
        onblur=${n}>`}}function _t(e){return(t,i,a,l,o)=>{let r=t.getColumn().getField(),n=mapp.utils.html`
      <input 
        type="number" 
        placeholder="Min" 
        onchange=${s}
        onblur=${s}>`;function s(p){let u=e.Tabulator.getFilters().find(m=>m.field===r&&m.type==">=");u&&(e.Tabulator.removeFilter(...Object.values(u)),o.layer&&delete e.layer.filter.current[r]),Number(p.target.value)&&(e.Tabulator.addFilter(r,">=",Number(p.target.value)),o.layer&&(e.layer.filter.current[r]=Object.assign(e.layer.filter.current[r]||{},{gte:Number(p.target.value)}),e.layer.reload(),e.update()))}let c=mapp.utils.html`
      <input 
        type="number" 
        placeholder="Max" 
        onchange=${d}
        onblur=${d}>`;function d(p){let u=e.Tabulator.getFilters().find(m=>m.field===r&&m.type=="<=");u&&(e.Tabulator.removeFilter(...Object.values(u)),o.layer&&delete e.layer.filter.current[r]),Number(p.target.value)&&(e.Tabulator.addFilter(r,"<=",Number(p.target.value)),o.layer&&(e.layer.filter.current[r]=Object.assign(e.layer.filter.current[r]||{},{lte:Number(p.target.value)}),e.layer.reload(),e.update()))}return mapp.utils.html.node`
      <div><div style="display: flex;">${n}${c}`}}var h={idle:600},qe=e=>{Object.assign(h,e),h.idle!==0&&(window.onload=g,window.onmousemove=g,window.onmousedown=g,window.ontouchstart=g,window.onclick=g,window.onkeypress=g,g(),De())};function g(){h.locked||(h.timeout&&clearTimeout(h.timeout),h.timeout=setTimeout(Oe,h.idle*1e3))}function Oe(){h.locked=!0,h.renew&&clearTimeout(h.renew);let e=new XMLHttpRequest;e.open("GET",`${h.host}/api/user/cookie?destroy=true`),e.onload=t=>location.reload(),e.send()}function De(){h.renew=setTimeout(e,(h.idle-20)*1e3);function e(){let t=new XMLHttpRequest;t.open("GET",`${h.host}/api/user/cookie?renew=true`),t.onload=i=>{if(i.target.status===401)return Oe();De()},t.send()}}var Fe=e=>{document.body.append(mapp.utils.html.node`
    <div class="interface-mask">
      <div class="bg-image" style=${`background-image:url(${e.target.src})`}>
      <button class="btn-close mask-icon close"
        onclick=${t=>t.target.parentElement.parentElement.remove()}>`)};var Me=e=>{e.target.addEventListener("mousedown",i=>{i.preventDefault(),document.body.style.cursor="grabbing",window.addEventListener("mousemove",e.resizeEvent),window.addEventListener("mouseup",t)}),e.target.addEventListener("touchstart",i=>{i.preventDefault(),window.addEventListener("touchmove",e.resizeEvent),window.addEventListener("touchend",t)},{passive:!0});function t(){document.body.style.cursor="auto",window.removeEventListener("mousemove",e.resizeEvent),window.removeEventListener("touchmove",e.resizeEvent),window.removeEventListener("mouseup",t),window.removeEventListener("touchend",t)}};var Ie={Chart:je,Tabulator:ze,tabulator:Te,idleLogout:qe,imagePreview:Fe,resizeHandler:Me};var Pe=e=>{let t=Object.assign({input:e.target.querySelector("input"),result:e.target.querySelector("ul"),sources:{glx:a,mapbox:o,google:l}},e);t.input.placeholder=t.placeholder||"",t.input.oninput=n=>{t.xhr&&t.xhr.abort(),t.result.innerHTML="",n.target.value.length>0&&i(n.target.value)},t.input.addEventListener("keydown",n=>{(n.keyCode||n.charCode)===13&&t.result.querySelector("li").click()});function i(n){let s=n.split(",").map(parseFloat);if(s.length===2&&s.every(c=>!isNaN(c))){t.result.appendChild(mapp.utils.html.node`
      <li 
        onclick=${c=>{t.result.innerHTML="",r({label:`Latitude:${s[0]}, Longitude:${s[1]}`,source:"Coordinates",lng:s[1],lat:s[0]})}}><span>Latitutde:${s[0]}, Longitude:${s[1]}`);return}t.xhr=new XMLHttpRequest,t.xhr.open("GET",t.mapview.host+"/api/gazetteer?"+mapp.utils.paramString({locale:t.mapview.locale.key,layer:t.layer,q:encodeURIComponent(n)})),t.xhr.setRequestHeader("Content-Type","application/json"),t.xhr.responseType="json",t.xhr.onload=c=>{if(c.target.status===200){if(c.target.response.length===0)return t.result.appendChild(mapp.utils.html.node`
          ...
          <li
            ...
            <span>${d.label}</span>`)})}},t.xhr.send()}function a(n){mapp.location.get({layer:t.mapview.layers[n.layer],table:n.table,id:n.id}).then(s=>s&&s.flyTo())}function l(n){mapp.utils.xhr(`${t.mapview.host}/api/proxy?url=${encodeURIComponent(`https://maps.googleapis.com/maps/api/place/details/json?placeid=${n.id}&{GOOGLE}`)}`).then(s=>{r({label:n.label,source:n.source,lng:s.result.geometry.location.lng,lat:s.result.geometry.location.lat})})}function o(n){r({label:n.label,source:n.source,lng:n.marker[0],lat:n.marker[1]})}function r(n){Object.assign(n,{layer:{mapview:t.mapview},Layers:[],hook:n.label});let s=[{title:n.label,value:n.source},{type:"pin",value:[n.lng,n.lat],srid:"4326",class:"display-none",location:n}];mapp.location.decorate(Object.assign(n,{infoj:s})),t.mapview.locations[n.hook]=n,n.flyTo()}};var Ae={layers:B,locations:pe,elements:Ce,utils:Ie,Gazetteer:Pe,Dataview:ue,Tabview:me};typeof window.mapp=="object"&&(window.mapp.ui=Ae);var Mi=Ae;export{Mi as default};
