var Nt=Object.defineProperty;var Mt=(e,t)=>{for(var i in t)Nt(e,i,{get:t[i],enumerable:!0})};async function F(e){if(!(!e.dynamic&&e.create instanceof Function)){if(e.create instanceof Function){e.create();return}if(typeof e.target=="string"&&(e.target=document.getElementById(e.target)),!(e.target instanceof HTMLElement)){console.warn("Dataviews require a HTMLHtmlElement target");return}return Ht(e)instanceof Error?e.err:(typeof e.label=="string"&&(e.chkbox=mapp.ui.elements.chkbox({checked:!!e.display,data_id:e.key,disabled:e.disabled,label:e.label,onchange:t=>{e.display=t,e.display?e.show():e.hide()}})),e.update??=Rt,e.show??=Vt,e.hide??=Pt,mapp.ui.utils.dataview.Toolbar(e),mapp.ui.utils.dataview.mapChange(e),e)}}async function Vt(){this.display=!0,this.create===void 0?(this.create=()=>{mapp.ui.utils[this.dataview].create(this)},this.create(),typeof this.update=="function"&&this.update()):this.dynamic?(typeof this.create=="function"&&await this.create(),typeof this.update=="function"&&this.update()):this.reload&&typeof this.update=="function"&&this.update(),this.btnRow?.style.setProperty("display","flex"),this.target.style.display="block"}function Pt(){this.display=!1,this.target.style.display="none",this.btnRow?.style.setProperty("display","none")}function Ht(e){if(e.key??=e.query||e.title||e.label,e.dataview||(e.chart&&(e.dataview="chartjs"),typeof e.columns<"u"&&(console.warn("Table dataviews should be configured inside a tables object"),e.table={columns:e.columns},e.dataview="tabulator"),e.table&&(e.dataview="tabulator")),!Object.hasOwn(mapp.ui.utils,e.dataview))return e.err=new Error(`mapp.ui.utils.${e.dataview} doesnt exist`),console.error(e.err),e.update=()=>console.warn(`Unable to update ${e.key} dataview.`),e.err;if(typeof mapp.ui.utils[e.dataview].create!="function")return e.err=new Error(`mapp.ui.utils.${e.dataview}.create() method doesn't exist`),console.error(e.err),e.err}async function Rt(){if(this.create===void 0&&(this.create=function(){mapp.ui.utils[this.dataview].create(this)},this.create()),!this.query)return;let e=mapp.utils.queryParams(this),t=mapp.utils.paramString(e);this.host??=this.layer?.mapview?.host;let i=await mapp.utils.xhr(`${this.host||mapp.host}/api/query?${t}`);if(!(i instanceof Error)){if(typeof this.responseFunction=="function"){this.responseFunction(i);return}typeof this.setData=="function"&&this.setData(i)}}function I(e){typeof e=="string"&&(e={text:e}),e.title??=mapp.dictionary.information,e.icon??="info",e.data_id??="alert",e.class??="alert-confirm box-shadow no-select";let t=e.icon===""?"":mapp.utils.html`<span class="material-symbols-outlined notranslate">${e.icon}</span>`;return e.header=mapp.utils.html`${t}${e.title}`,e.innerContent??=mapp.utils.html`<p>${e.text}</p>`,e.content=mapp.utils.html`
  ...
  <div
    ...
    <button
      ...
      ${e.draw.point.label}`,e.draw.point.btn}function ei(e){let t={content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
    ...
    <button
      ...
      ${e.draw.line.label}`,e.draw.line.btn}function ti(e){let t={content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
    ...
    <button
      ...
      ${e.draw.polygon.label}`,e.draw.polygon.btn}function ii(e){let t={content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
  ...
  <button
    ...
    ${e.draw.rectangle.label}`,e.draw.rectangle.btn}function oi(e){let t={content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
  ...
  <button
    ...
    ${e.draw.circle_2pt.label}`,e.draw.circle_2pt.btn}function li(e){let i={helpDialog:{content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
    ...
    <h3>${mapp.dictionary.circle_config}</h3>`,content:o,popout:i.popout,dialog:i.dialog}),i.drawer)}function Ke(e,t){let i=e.unitsConfig[t];e.units=i.option,e.radius??=i.min,e.radius=e.radius>i.max?i.max:e.radius,mapp.utils.render(e.rangeSlider,mapp.ui.elements.slider({callback:o=>{e.radius=parseFloat(o)},max:i.max,min:i.min,val:e.radius}))}function ni(e){return e.draw.locator={label:mapp.dictionary.draw_position,layer:e,type:"Point",...e.draw.locator},e.draw.locator.btn=mapp.utils.html.node`
    <button
      class="action wide"
      ...
      ${e.draw.locator.label}`,e.draw.locator.btn}function V(e){e.selectedTitles=new Set,e.selectedOptions=new Set,e.placeholder??=e.span||"",e.entries=e.entries?.filter?.(o=>o.option!==""),si(e),di(e),e.onChange??=it,e.options=ai(e);let t=e.selectedTitles.size?Array.from(e.selectedTitles).join(", "):e.placeholder,i=e.keepPlaceholder?e.placeholder:t;return e.placeHolderOption=mapp.utils.html.node`
    ...
    ${e.pills?.container}
    ${e.search||e.select}`,e.node}function tt(e){e.target.selectedIndex=0}function it(e,t){let i=e.target.selectedIndex;e.target.selectedIndex=0;let o=t.entries[i-1];if(o.selected=!o.selected,t.multi){if(t.options[i].classList.toggle("selected")?(t.selectedTitles.add(o.title),t.selectedOptions.add(o.option),t.pills?.add(o.title)):(t.selectedTitles.delete(o.title),t.selectedOptions.delete(o.option),t.pills?.remove(o.title)),!t.pills&&!t.keepPlaceholder){let n=t.selectedTitles?.size&&Array.from(t.selectedTitles).join(", ");t.placeHolderOption.textContent=n||t.placeholder}t.callback?.(e,[...t.selectedOptions],o);return}t.options.forEach(l=>{l.classList.remove("selected"),l.style.backgroundColor="var(--color-base-secondary)"}),t.options[i].classList.add("selected"),t.options[i].style.removeProperty("background-color"),t.keepPlaceholder||(t.placeHolderOption.textContent=o.title),t.callback?.(e,o)}function ai(e){return e.entries.map(i=>(i.li=mapp.utils.html.node`<option
        value=${i.option}>
        ${i.title||i.label||i.field}`,i.selected&&(i.li.classList.add("selected"),e.selectedTitles.add(i.title),e.selectedOptions.add(i.option),e.pills?.add(i.title)),i.li))}function si(e){e.pills&&(e.pills=mapp.ui.elements.pills({addCallback:(t,i)=>{e.callback?.(null,[...i])},pills:[...e.selectedTitles],removeCallback:(t,i)=>{let o=e.entries.findIndex(n=>n.option===t),l=e.entries.findIndex(n=>n.option===t)+1;e.entries.find(n=>n.option===t).selected=!1,e.options[l].classList.remove("selected"),e.selectedTitles.delete(o.title),e.selectedOptions.delete(o.option),e.callback?.(null,[...i])}}))}function di(e){if(!e.search)return;let t=`${e.field}-search-options`,i=e.placeholder||"Enter search term...",o=mapp.utils.html.node`<input
    placeholder=${i}
    type="search" list=${t}
    onInput=${n=>ci(n,e)}
    onfocus="this.placeholder=''"
    onblur=${n=>n.target.placeholder=i}>`;e.searchOptions=[];for(let n of e.entries){let a=mapp.utils.html.node`
      <option value=${n.option} data-title=${n.title}>${n.title}`;e.searchOptions.push(a)}let l=mapp.utils.html.node`<datalist id=${t}>${e.searchOptions}`;return e.search=mapp.utils.html.node`${o}${l}`,e.search}function ci(e,t){t.entries.forEach(i=>{i.title===e.target.value&&(i.selected=!i.selected,i.selected?(t.selectedTitles.add(i.title),t.selectedOptions.add(i.option),t.pills?.add(i.title)):(t.selectedTitles.delete(i.title),t.selectedOptions.delete(i.option),t.pills?.remove(i.title)),e.target.value="",e.target.dispatchEvent(new Event("blur")),t.pills||t.callback?.(e,[...t.selectedOptions]))})}var P;function H(e){P?.node.remove(),e&&(mapp.user?.hideHelp||(P={closeBtn:!0,contained:!0,css_style:"padding: 0.5em;",headerDrag:!0,height:"auto",left:"6em",minimizeBtn:!0,target:document.getElementById("Map"),top:"3em",...e},mapp.ui.elements.dialog(P)))}async function R(e){let t=await mapp.utils.esmImport("vanilla-jsoneditor@3.3.1");return e.content??={json:{}},typeof e.data=="object"&&(e.content.json=e.data),t.createJSONEditor({props:{content:e.content,...e.props},target:e.target})}var U={hover:ui,hovers:pi,icon_scaling:vi,label:mi,labels:fi,opacitySlider:hi,panel:ri,theme:gi,themes:bi},ot=U;function ri(e){if(!e.style)return;e.style.elements??=Object.keys(e.style);let t=[];for(let i of e.style.elements){if(!Object.hasOwn(e.style,i)||!Object.hasOwn(U,i))continue;let o=U[i](e);o&&t.push(o)}if(t.length)return e.style.view=mapp.utils.html.node`<div>${t}`,e.style.view}function ui(e){if(e.style.hover&&!e.style.hover.hidden)return mapp.ui.elements.chkbox({checked:!!e.style.hover.display,data_id:`hoverCheckbox-${e.key}`,label:e.style.hovers&&mapp.dictionary.layer_style_display_hover||e.style.hover.title||mapp.dictionary.layer_style_display_hover,onchange:t=>{e.style.hover.display=t}})}function pi(e){if(!e.style.hover||e.style.hover.hidden||!e.style.hovers||Object.keys(e.style.hovers).length<2)return;let t=Object.keys(e.style.hovers).filter(i=>!e.style.hovers[i].hidden).map(i=>({option:i,title:e.style.hovers[i].title||i}));return mapp.ui.elements.dropdown({callback:(i,o)=>{let l=e.style.hover.display;e.style.hover=e.style.hovers[o.option],e.style.hover.method??=mapp.layer.featureHover,e.style.hover.display=l},data_id:`hoversDropdown-${e.key}`,entries:t,placeholder:e.style.hover.title})}function mi(e){if(!e.style.label||e.style.label.hidden)return;if(e.style.labelCheckbox)return e.style.labelCheckbox;let t=e.style.labels&&mapp.dictionary.layer_style_display_labels;return t??=e.style.label.title||mapp.dictionary.layer_style_display_labels,e.style.labelCheckbox=mapp.ui.elements.chkbox({checked:!!e.style.label.display,data_id:`labelCheckbox-${e.key}`,label:t,onchange:i=>{e.style.label.display=i,e.reload()}}),e.changeEndCallbacks.push(lt),e.style.labelCheckbox}function lt(e){if(e.style.label.minZoom||e.style.label.maxZoom){let t=e.mapview.Map.getView().getZoom();t<=e.style.label.minZoom||t>=e.style.label.maxZoom?e.style.labelCheckbox?.classList.add("disabled"):e.style.labelCheckbox?.classList.remove("disabled")}}function fi(e){if(!e.style.label||e.style.label.hidden||!e.style.labels||Object.keys(e.style.labels).length<2)return;let t=Object.keys(e.style.labels).filter(o=>!e.style.labels[o].hidden).map(o=>({option:o,title:e.style.labels[o].title||o}));return mapp.utils.html`
  <div>${mapp.dictionary.layer_style_select_label}</div>
  ${mapp.ui.elements.dropdown({callback:i,data_id:"labelsDropdown",entries:t,placeholder:e.style.label.title})}`;function i(o,l){let n=e.style.label.display;e.style.label=e.style.labels[l.option],e.style.label.display=n,e.reload(),lt(e)}}function hi(e){return mapp.ui.elements.slider({callback:t=>{e.L.setOpacity(parseFloat(t/100))},data_id:"opacitySlider",label:`${mapp.dictionary.layer_style_opacity}`,max:100,min:0,val:parseInt(e.L.getOpacity()*100)})}function gi(e){if(!e.style.theme)return;e.style.labels&&Object.hasOwn(e.style.labels,e.style.theme?.setLabel)&&(e.style.label=e.style.labels[e.style.theme.setLabel]),e.style.hovers&&Object.hasOwn(e.style.hovers,e.style.theme?.setHover)&&(e.style.hover=e.style.hovers[e.style.theme.setHover]);let t=[];return e.style.theme?.meta&&(e.style.theme.meta_node=mapp.utils.html.node`<p>${e.style.theme.meta}`,t.push(e.style.theme.meta_node)),Object.hasOwn(mapp.ui.layers.legends,e.style.theme?.type)&&(mapp.ui.layers.legends[e.style.theme.type](e),e.style.legend&&t.push(e.style.legend)),mapp.utils.html.node`<div data-id="layerTheme">${t}`}function bi(e){if(!e.style.themes||Object.keys(e.style.themes).length<2)return;let t=Object.keys(e.style.themes).map(l=>({option:l,title:e.style.themes[l].title||l}));function i(l,n){e.style.theme=e.style.themes[n.option],e.style.theme.setLabel&&e.style.labels&&(e.style.label=e.style.labels[e.style.theme.setLabel]),e.style.theme.setHover&&e.style.hovers&&(e.style.hover=e.style.hovers[e.style.theme.setHover],e.style.hover.method??=mapp.layer.featureHover);let a=mapp.ui.layers.panels.style(e);e.style.panel&&e.style.panel.replaceChildren(...a.children),e.view?.querySelector("[data-id=style-drawer]").replaceChildren(...a.children),e.reload()}return mapp.utils.html`<div>
  ...
  class="notranslate material-symbols-outlined active"
    ...
    <div class="link-with-img">
      ...
      <span>${e.label}`}var yt={documents:eo,image:Qi,images:Ki},C=e=>yt[e.type](e);function Qi(e){if(e.value){let t=e.edit&&mapp.utils.html`<button 
