import{t as e}from"./chunk-DK3Fl9T5.js";async function t(e){if(!(!e.dynamic&&typeof e.create==`function`)){if(typeof e.create==`function`){e.create();return}if(typeof e.target==`string`&&(e.target=document.getElementById(e.target)),!(e.target instanceof HTMLElement)){console.warn(`Dataviews require a HTMLHtmlElement target`);return}return i(e)instanceof Error?e.err:(typeof e.label==`string`&&(e.chkbox=mapp.ui.elements.chkbox({checked:!!e.display,data_id:e.key,disabled:e.disabled,label:e.label,onchange:t=>{e.display=t,e.display?e.show():e.hide()}})),e.update??=a,e.show??=n,e.hide??=r,mapp.ui.utils.dataview.Toolbar(e),mapp.ui.utils.dataview.mapChange(e),e)}}async function n(){this.display=!0,this.create===void 0?(this.create=()=>{mapp.ui.utils[this.dataview].create(this)},this.create(),typeof this.update==`function`&&this.update()):this.dynamic?(typeof this.create==`function`&&await this.create(),typeof this.update==`function`&&this.update()):this.reload&&typeof this.update==`function`&&this.update(),this.btnRow?.style.setProperty(`display`,`flex`),this.target.style.display=`block`}function r(){this.display=!1,this.target.style.display=`none`,this.btnRow?.style.setProperty(`display`,`none`)}function i(e){if(e.key??=e.query||e.title||e.label,e.dataview||(e.chart&&(e.dataview=`chartjs`),e.columns!==void 0&&(console.warn(`Table dataviews should be configured inside a tables object`),e.table={columns:e.columns},e.dataview=`tabulator`),e.table&&(e.dataview=`tabulator`)),!Object.hasOwn(mapp.ui.utils,e.dataview))return e.err=Error(`mapp.ui.utils.${e.dataview} doesnt exist`),console.error(e.err),e.update=()=>console.warn(`Unable to update ${e.key} dataview.`),e.err;if(typeof mapp.ui.utils[e.dataview].create!=`function`)return e.err=Error(`mapp.ui.utils.${e.dataview}.create() method doesn't exist`),console.error(e.err),e.err}async function a(){if(this.create===void 0&&(this.create=function(){mapp.ui.utils[this.dataview].create(this)},this.create()),!this.query)return;let e=mapp.utils.queryParams(this),t=mapp.utils.paramString(e);this.host??=this.layer?.mapview?.host;let n=await mapp.utils.xhr(`${this.host||mapp.host}/api/query?${t}`);if(!(n instanceof Error)){if(typeof this.responseFunction==`function`){this.responseFunction(n);return}typeof this.setData==`function`&&this.setData(n)}}function o(e){typeof e==`string`&&(e={text:e}),e.title??=mapp.dictionary.information,e.icon??=`info`,e.data_id??=`alert`,e.class??=`alert-confirm box-shadow no-select`;let t=e.icon===``?``:mapp.utils.html`<span class="material-symbols-outlined notranslate">${e.icon}</span>`;return e.header=mapp.utils.html`${t}${e.title}`,e.innerContent??=mapp.utils.html`<p>${e.text}</p>`,e.content=mapp.utils.html`
  ...
  class="notranslate material-symbols-outlined active"
    ...
    <h3>${mapp.dictionary.layer_style_header}</h3>`;return e.style.dialog&&(e.style.dialog={btn_label:`Styling`,title:`Styling`,icon:`brush`,...e.style.dialog}),e.style.drawer=mapp.ui.elements.drawer({drawer:e.style.drawer,data_id:`style-drawer`,class:e.style.classList,header:n,content:t,popout:e.style.popout,dialog:e.style.dialog,layer:e}),e.style.drawer}var qt={dataviews:Nt,draw:Ft,filter:It,gazetteer:Vt,jsonEditor:Ht,meta:Wt,reports:Gt,style:Kt};function Jt(e){if(e.view===null)return;Yt(e);let t=mapp.utils.html`
    <h2>${e.name}</h2>
    ...
    <div class="link-with-img">
      ...
      <input
        type="file"
        accept="image/*;capture=camera" onchange=${t=>{I(t,e)}}>`}function sn(e){let t=[];if(e.value?.map(n=>{let r=e.edit&&mapp.utils.html`<button title="${mapp.dictionary.delete}"
      class="notranslate material-symbols-outlined color-danger delete"
      ...
      <input
        type="file"
        accept="image/*;capture=camera" onchange=${t=>{I(t,e)}}>`;t.push(n)}if(t.length)return mapp.utils.html.node`<div class="images-grid">${t}`}function cn(e){let t=[];if(e.value?.map(n=>{let r=e.edit&&mapp.utils.html`<button
      title="${mapp.dictionary.delete}"
      class="notranslate material-symbols-outlined color-danger delete"
      data-name=${n.replace(/^.*\//,``).replace(/\.([\w-]{3})/,``)}
      data-href=${n}
  ...
  <div
    class="val"
    style="${e.css_val}">${t}`;return!e.edit&&e.type===`html`&&(n.innerHTML=e.value||``),n}function Gn(e){let t,n=e.value?.toString().replace(`.`,`:`);return n=n&&n.length<3&&`${n}:00`||n,t=e.edit?mapp.utils.html.node`
      <input
        type="time"
        value=${n}
    ...
    <div
      class="val"
      style="${e.css_val}">
      ${t}`}function Kn(e){return mapp.utils.html.node`
        <div
          class="label"
          style=${e.css_title}
          title=${e.tooltip}>${e.title}`}function qn(){console.warn(`The type:defaults entry method has been deprecated.`)}function q(e){return console.warn(`The type:${e.type} entry method has been deprecated in favour of the type:layer entry method.`),e.type=`layer`,e.qID??=`id`,e.name??=e.label,U(e)}var Jn={boolean:an,dataview:dn,date:V,datetime:V,defaults:qn,documents:F,geometry:mn,html:K,image:F,images:F,integer:G,json:Sn,key:Cn,layer:U,link:W,mvt_clone:q,numeric:G,pills:jn,pin:Mn,ping:In,query_button:Ln,report:W,tab:zn,text:Bn,textarea:K,time:Gn,title:Kn,vector_layer:q},J;function Yn(e,t){if(!e.infoj)return;let n=mapp.utils.html.node`<div class="location-view-grid">`;J={},t??=e?.layer?.infoj_order;let r=Array.isArray(t)?t.map(i).filter(e=>e!==void 0):e.infoj;function i(t){if(typeof t==`string`){let n=e.infoj.find(e=>new Set([e.key,e.field,e.query]).has(t));return n||console.warn(`infoj_order field: "${t}" not found in location.infoj. Please add entry.key, entry.field, or entry.query to the entry.`),n}else if(typeof t==`object`)return t.location=e,t}let a=0;for(let t of r){if(t.key??=t.field||a++,e.view?.classList.contains(`disabled`))break;if(t.listview=n,t.type??=`text`,t.edit===!0&&(t.edit={}),Xn(t),Zn(t),Qn(t)||($n(t),er(t),tr(t),nr(t),rr(t),ir(t)))continue;if(!Object.hasOwn(mapp.ui.locations.entries,t.type)){console.error(`entry.type:${t.type} method not found.`);continue}let r=mapp.ui.locations.entries[t.type]?.(t);r&&t.node.append(r),or(t)}return n}function Xn(e){e.jsonb_field&&e.jsonb_key&&e.value!==null&&typeof e.value==`object`&&e.value.jsonb&&(e.value=e.value.jsonb[e.jsonb_field][e.jsonb_key])}function Zn(e){if(e.objectAssignFromField&&e.objectMergeFromField){console.warn(`${e.key}: objectAssignFromField and objectMergeFromField cannot both be used on the same entry.`);return}let t=e.objectAssignFromField||e.objectMergeFromField||e.json_field;if(!t)return;let n=e.location.infoj.find(e=>e.field===t);if(n&&typeof n.value==`object`){if(e.json_field){if(!e.json_key){console.warn(`json_field requires entry.json_key to be specified`);return}if(!n.value)return;e.value=n.value[e.json_key]}e.objectAssignFromField&&Object.assign(e,n.value),e.objectMergeFromField&&mapp.utils.merge(e,n.value)}}function Qn(e){if(e.skipEntry||e.skipFalsyValue&&!e.value&&!e.edit||e.skipUndefinedValue&&e.value===void 0&&!e.edit||e.skipNullValue&&e.value===null&&!e.edit)return!0}function $n(e){e.nullValue!==void 0&&(e.edit||(e.value??=e.nullValue))}function er(e){e.default!==void 0&&e.edit&&(e.value||(e.newValue=e.default,e.location.view?.dispatchEvent(new CustomEvent(`valChange`,{detail:e}))))}function tr(e){e.group&&(J[e.group]||(J[e.group]=e.listview.appendChild(mapp.ui.elements.drawer({data_id:`location-infoj-group`,class:`group ${e.groupClassList||``}`,header:mapp.utils.html`
          <h3>${e.group}</h3>`}))),e.listview=J[e.group])}function nr(e){let t=`contents ${e.type} ${e.class||``} ${e.inline&&`inline`||``}`;e.node=e.listview.appendChild(mapp.utils.html.node`
  <div
    data-type=${e.type}
    class=${t}>`)}function rr(e){e.title&&e.node.append(mapp.ui.locations.entries.title(e))}function ir(e){if(e.query)if(e.queryCheck??=e.run,e.queryCallback??=ar,e.runOnce||e.queryCheck){delete e.runOnce,e.host??=e.location.layer?.mapview?.host||mapp.host;let t=mapp.utils.queryParams(e),n=mapp.utils.paramString(t);return e.url=`${e.host}/api/query?${n}`,e.blocking&&e.location.view?.classList.add(`loading`),mapp.utils.xhr(e).then(t=>e.queryCallback(t,e)),!0}else e.field&&!e.xhr&&console.warn(`field:"${e.field}" has a query:"${e.query}" which is not set to run, please add the run or runOnce flag to the entry.`)}function ar(e,t){if(t.blocking&&t.location.view?.classList.remove(`loading`),e?t.value=t.field?e[t.field]:e:t.value=t.nullValue||null,Qn(t)){t.node.remove();return}let n=mapp.ui.locations.entries[t.type]?.(t);n&&t.node.appendChild(n)}function or(e){if(!e.tooltip)return;let t=e.node.querySelector(`label`)||e.node.querySelector(`div.label`);t&&(e.tooltipElement||(e.tooltipElement=mapp.ui.elements.tooltip({content:e.tooltip}),t.append(e.tooltipElement)))}var sr=[{symbol:`A`,colour:`#2E6F9E`},{symbol:`B`,colour:`#EC602D`},{symbol:`C`,colour:`#5B8C5A`},{symbol:`D`,colour:`#B84444`},{symbol:`E`,colour:`#514E7E`},{symbol:`F`,colour:`#E7C547`},{symbol:`G`,colour:`#368F8B`},{symbol:`H`,colour:`#841C47`},{symbol:`I`,colour:`#61A2D1`},{symbol:`J`,colour:`#37327F`},{symbol:`K`,colour:`#FFA500`},{symbol:`L`,colour:`#7FFF00`},{symbol:`M`,colour:`#FFD700`},{symbol:`N`,colour:`#00CED1`},{symbol:`O`,colour:`#9932CC`},{symbol:`P`,colour:`#FF69B4`},{symbol:`Q`,colour:`#008B8B`},{symbol:`R`,colour:`#8A2BE2`},{symbol:`S`,colour:`#FF4500`},{symbol:`T`,colour:`#00FF00`},{symbol:`U`,colour:`#FF00FF`},{symbol:`V`,colour:`#708090`},{symbol:`W`,colour:`#F4A460`},{symbol:`X`,colour:`#000080`},{symbol:`Y`,colour:`#D2691E`},{symbol:`Z`,colour:`#8FBC8F`}];function cr(e){e.mapview||console.warn(`A mapview is required in the locations listview params argument.`),e.target||console.warn(`A target element is required in the locations listview params argument.`);let t=e.mapview.locale;t.locations??={},t.locations.pinStyle??={anchor:[.5,1],scale:3,type:`markerLetter`},t.locations.style??={fillColor:`#fff`,fillOpacity:.1,strokeColor:`#fff`},t.locations.records??=t.listview_records||structuredClone(sr),t.locations.clearAll=e.target.appendChild(mapp.utils.html.node`
    <button
    ...
    `);async function i(){e.infoj.some(e=>e.newValue!==void 0)&&!await mapp.ui.elements.confirm({text:mapp.dictionary.location_close_without_save})||e.remove()}e.view=mapp.ui.elements.drawer({data_id:`location-${e.layer.key}-${e.id}`,class:`location-view raised expanded`,header:r}),e.viewEntries=e.view.appendChild(mapp.ui.locations.infoj(e)),e.view.querySelector(`.header`).style.borderBottom=`3px solid ${e.record.colour}`,e.view.addEventListener(`valChange`,fr),e.renderLocationView=pr,e.view.addEventListener(`render`,()=>e.renderLocationView()),e.view.addEventListener(`updateInfo`,()=>{e.editToggle&&(e.editToggle.classList.remove(`toggle-on`),e.removeEdits()),e.layer?.dataviews&&Object.values(e.layer.dataviews).forEach(e=>{e.display===!0&&e.update()}),e.renderLocationView()})}function dr(e){if(e.layer?.toggleLocationViewEdits===!1)return e.removeEdits(),!1;if(!e.layer?.toggleLocationViewEdits||!e.infoj.some(e=>e.edit))return!1;!e.new&&e.removeEdits();let t=`notranslate material-symbols-outlined ${e.new?`toggle-on`:``}`;return e.editToggle=mapp.utils.html.node`<button
    title="Enable edits"
    class=${t}
    onclick=${n}>edit`,!0;async function n(t){await e.confirmUpdate()===void 0&&(t.target.classList.toggle(`toggle-on`)?e.restoreEdits():e.removeEdits(),e.renderLocationView())}}function fr(e){let t=e.detail,n=t.location;if(typeof t.valChangeMethod==`function`){t.valChangeMethod(t);return}if(t.value==t.newValue?(delete t.newValue,t.node?.classList.remove(`val-changed`)):t.node?.classList.add(`val-changed`),n.infoj.some(e=>e.invalid)){n.view.querySelector(`.btn-save`).style.display=`none`;return}n.view.querySelector(`.btn-save`).style.display=n.infoj.some(e=>e.newValue!==void 0)?`inline-block`:`none`}function pr(){let e=this;e.view.querySelector(`.btn-save`).style.display=`none`,e.viewEntries.remove(),e.view.classList.remove(`disabled`),e.viewEntries=e.view.appendChild(mapp.ui.locations.infoj(e))}var mr={entries:Jn,infoj:Yn,listview:cr,view:ur};function hr(e){if(e.node)return e.tabs=e.node.appendChild(mapp.utils.html.node`<div class="tabs">`),e.panel=e.node.appendChild(mapp.utils.html.node`<div class="panel">`),e.id&&(e.node.dataset.id=e.id),e.addTab=gr,e.node.addEventListener(`addTab`,t=>e.addTab(t.detail)),e}function gr(e){if(e.tab&&!e.dynamic)return;let t=this;e.layer?.dataviews?.[e.key]&&(e.disableTabClose??=e.layer.dataviews.hide),e.activate??=a,e.location?e.location.removeCallbacks.push(()=>e.remove()):e.layer&&(e.layer.showCallbacks.push(()=>{e.display&&e.show()}),e.layer.hideCallbacks.push(()=>{e.remove()})),e.label??=e.title||e.key||`Tab`,e.tab_btn=mapp.utils.html.node`<button
    ...
    overflow: hidden !important`;Object.assign(e.dataview_dialog,{header:mapp.utils.html.node`<h1> ${e.label}`,data_id:`${e.key}-dataviews-dialog`,target:document.getElementById(`Map`),height:`auto`,left:`5%`,top:`0.5em`,class:`box-shadow tabview`,css_style:t,containedCentre:!0,contained:!0,headerDrag:!0,closeBtn:!0,onClose:()=>{e.chkbox&&(e.chkbox.querySelector(`input`).checked=!1)}}),mapp.ui.elements.dialog(e.dataview_dialog);let n=e.dataview_dialog.node.querySelector(`.content`);n.classList.add(`panel`),e.target=n,e.dataview_dialog.dialog.close(),e.show=()=>{e.update(),e.toolbar&&e.dataview_dialog.node.querySelector(`.btn-row`).style.removeProperty(`display`),e.chart&&(e.dataview_dialog.node.appendChild(e.target.querySelector(`canvas`)),e.dataview_dialog.node.removeChild(e.target)),e.dataview_dialog.dialog.show()},e.hide=()=>{e.dataview_dialog.dialog.close()},e.location?.removeCallbacks?.push?.(()=>e.dataview_dialog.dialog.close())}var Dr={create:Or};async function Or(e){e.query??=`histogram`,e.options??={};let t=`column-gap: ${e.options.columnGap||10}px`;e.node=mapp.utils.html.node`<div class="histogram" style=${t}>`,e.options.width&&(e.node.style.width=`${e.options.width}px`),e.options.height&&(e.node.style.height=`${e.options.height}px`),e.options.title&&e.target.append(mapp.utils.html.node`<h3 class="histogram-title">${e.options.title}`),e.setData=kr,e.target.append(e.node),e.options.xlabel&&(e.xlabel=mapp.utils.html.node`<div class="histogram-xlabel">${e.options.xlabel}`,e.options.width&&(e.xlabel.style.width=`${e.options.width}px`),e.target.append(e.xlabel)),e.options.caption&&(e.caption=mapp.utils.html.node`<span class="histogram-caption">${e.options.caption}`,e.target.append(e.caption))}function kr(e){this.data=e,Ar(this)}function Ar(e){if(!Array.isArray(e.data)){mapp.utils.render(e.node,mapp.utils.html`<div>`);return}let t=Math.max(...e.data.map(e=>e.count));e.options.height??=100;let n=e.data.map(n=>{let r=`height: ${Math.round(e.options.height*n.count/t)}px`,i=``;return e.options.tooltip&&(i=mapp.utils.html`<div class="bucket-info">
      ...
      style=${r}>
      ${i}`}),r=e.options.ylabel?mapp.utils.html`<div class="ylabel">${e.options.ylabel}`:``;mapp.utils.render(e.node,mapp.utils.html`${r}${n}`)}var X={idle:600};function jr(e){Object.assign(X,e),X.idle!==0&&(window.onload=Z,window.onmousemove=Z,window.onmousedown=Z,window.ontouchstart=Z,window.onclick=Z,window.onkeypress=Z,Z(),Nr())}function Z(){X.locked||(X.timeout&&clearTimeout(X.timeout),X.timeout=setTimeout(Mr,X.idle*1e3))}function Mr(){X.locked=!0,X.renew&&clearTimeout(X.renew);let e={url:`${X.host}/api/user/cookie?destroy=true`};e.onLoad=e=>location.reload(),mapp.utils.xhr(e)}function Nr(){X.renew=setTimeout(Pr,(X.idle-20)*1e3)}function Pr(){let e={url:`${X.host}/api/user/cookie`};e.onLoad=e=>{if(e.target.status===401)return Mr();Nr()},mapp.utils.xhr(e)}function Fr(e){let t=mapp.utils.html.node`
  <div class="interface-mask">
    ...
    <button 
      ...
      onclick=${e=>e.target.closest(`.interface-mask`).remove()}>close</button>`;document.body.append(t)}var Ir={create:Lr,toolbar:{copyToClipboard:Br,csvupload:Vr,jsonfile:zr,sqlinsert:Hr}};async function Lr(e){e.props??={mainMenuBar:!1,mode:`text`,readOnly:!0},e.jsoneditor=await mapp.ui.elements.jsoneditor(e),e.setData=Rr,e.data&&e.setData(e.data)}function Rr(e){this.data=e;let t={json:e};this.jsoneditor.set(t)}function zr(e){return e.title??=e.label||`Unknown`,mapp.utils.html.node`<button onclick=${()=>{let t={filename:`${e.title}.json`,text:JSON.stringify(e.data,null,2),type:`application/json`};mapp.utils.textFile(t)}}>Download</button>`}function Br(e){return mapp.utils.html.node`<button onclick=${()=>{let t=JSON.stringify(e.data,null,2);mapp.utils.copyToClipboard(t)}}>Copy to Clipboard</button>`}function Vr(e){return e.toolbar.csvupload.label??=`CSV Upload`,e.toolbar.csvupload.input=mapp.utils.html.node`<input 
...
`)}`});return}e.setData(n)}}>`,e.toolbar.csvupload.input}function Hr(e){return e.props={mainMenuBar:!1,mode:`text`},mapp.utils.html.node`<button onclick=${()=>{let t=e.jsoneditor.get();if(t.text===``)return;let n=mapp.utils.paramString(e.toolbar.sqlinsert),r=`${mapp.host}/api/query?${n}`;return mapp.utils.xhr({body:t.text,method:`POST`,url:r})}}>SQL Insert</button>`}async function Ur(e,t,n){return await mapp.ui.elements.jsoneditor({props:{onRenderMenu:e=>Wr(e,n),content:t,mode:`text`},target:e})}function Wr(e,t){return e.push(t),e.filter(e=>e.text!==`text`).filter(e=>e.text!==`tree`).filter(e=>e.text!==`table`).filter(e=>e.type!==`separator`).filter(e=>e.className!==`jse-undo`).filter(e=>e.className!==`jse-redo`).filter(e=>e.className!==`jse-search`).filter(e=>e.className!==`jse-contextmenu`).filter(e=>e.className!==`jse-sort`).filter(e=>e.className!==`jse-transform`)}async function Gr(e){if(Array.isArray(e.Features)&&!e.filter?.viewport)return e.Features.length;let t={layer:e,queryparams:{}};t.viewport=e.filter?.viewport||e.queryparams?.viewport;let n=mapp.utils.queryParams(t),r=mapp.utils.paramString({...n,filter:e.filter?.current,layer:e.key,table:e.tableCurrent(),geom:e.geomCurrent(),qID:e.qID,template:`location_count`}),i=await mapp.utils.xhr(`${e.mapview.host}/api/query?${r}`);if(i instanceof Error)if(e.filter?.viewport){console.warn(`locationCount failed on viewport.`);return}else return e.filter.viewport=!0,await Gr(e);return i}var Q={},$={X:{min:75,lockSize:200,grid:`gridTemplateColumns`},Y:{min:0,lockSize:50,grid:`gridTemplateRows`},gridTemplate:{X:`auto`,Z:`10px`,Y:`auto`}};function Kr(e){e.resizeEvent??=qr.bind(e),e.stopResize=Yr.bind(e),e.axis??=`X`,e.doubleClickClose&&(e.target.ondblclick=t=>Jr(t,e)),e.target.addEventListener(`mousedown`,t=>{t.preventDefault(),document.body.style.cursor=`grabbing`,globalThis.addEventListener(`mousemove`,e.resizeEvent),globalThis.addEventListener(`mouseup`,e.stopResize)}),e.target.addEventListener(`touchstart`,t=>{t.preventDefault(),globalThis.addEventListener(`touchmove`,e.resizeEvent),globalThis.addEventListener(`touchend`,e.stopResize)},{passive:!0}),Q.gridTemplateColumns=getComputedStyle(document.body).gridTemplateColumns,Q.gridTemplateRows=getComputedStyle(document.body).gridTemplateRows}function qr(e){let t=this.axis;$.X.size=e.touches?.[0]?.pageX||e.pageX,$.Y.size=globalThis.innerHeight-(e.touches?.[0]?.pageY||e.pageY);let n=$[t];if(n.size<n.min){$.gridTemplate[t]=`0px`,document.body.style[n.grid]=Object.values($.gridTemplate).join(` `);return}n.size<n.lockSize||(t===`X`&&$.X.size>globalThis.innerWidth/2&&($.X.size=globalThis.innerWidth/2),t===`Y`&&($.Y.size>globalThis.innerHeight-10&&($.Y.size=globalThis.innerHeight),OL.style.marginTop=`-${$.Y.size/2}px`),$.gridTemplate[t]=`${n.size}px`,document.body.style[n.grid]=Object.values($.gridTemplate).join(` `),$.gridTemplate={X:`auto`,Z:`10px`,Y:`auto`})}function Jr(e,t){if($.X.size=e.touches?.[0]?.pageX||e.pageX,$.Y.size=globalThis.innerHeight-(e.touches?.[0]?.pageY||e.pageY),$[t.axis].size<$[t.axis].min){document.body.style[$[t.axis].grid]=Q[$[t.axis].grid];return}$.gridTemplate[t.axis]=`0px`,document.body.style[$[t.axis].grid]=Object.values($.gridTemplate).join(` `),$.gridTemplate={X:`auto`,Z:`10px`,Y:`auto`}}function Yr(){document.body.style.cursor=`auto`,globalThis.removeEventListener(`mousemove`,this.resizeEvent),globalThis.removeEventListener(`touchmove`,this.resizeEvent),globalThis.removeEventListener(`mouseup`,this.stopResize),globalThis.removeEventListener(`touchend`,this.stopResize)}var Xr={Dataview:t,elements:lt,Gazetteer:ut,layers:rn,locations:mr,Tabview:hr,utils:{cssColour:_r,dataview:br,dataviewDialog:Er,histogram:Dr,idleLogout:jr,imagePreview:Fr,Json:Ir,layerJSE:Ur,locationCount:Gr,resizeHandler:Kr}};globalThis.ui=Xr,globalThis.mapp??={},mapp.ui=Xr;
