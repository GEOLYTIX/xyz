mapp.utils.merge(mapp.dictionaries,{en:{layer_zoom_to_extent:"Zoom to filtered layer extent",layer_visibility:"Toggle visibility"},de:{layer_zoom_to_extent:"Zoom zum Ausma\xDF des gefilterten Datensatzes",layer_visibility:"Umschalten der Ansicht"},cn:{layer_zoom_to_extent:"\u7F29\u653E\u81F3\u76F8\u5E94\u7B5B\u9009\u8303\u56F4",layer_visibility:"\u5207\u6362\u53EF\u89C1\u6027"},pl:{layer_zoom_to_extent:"Poka\u017C zasi\u0119g warstwy",layer_visibility:"Widoczno\u015B\u0107"},ko:{layer_zoom_to_extent:"\uD544\uD130\uB41C \uB808\uC774\uC5B4\uD06C\uAE30\uC5D0 \uC90C(zoom)",layer_visibility:"\uD1A0\uAE00 \uAC00\uC2DC\uC131"},fr:{layer_zoom_to_extent:"Zoom sur l'\xE9tendue de la couche",layer_visibility:"Changer la visiblit\xE9"},ja:{layer_zoom_to_extent:"\u30D5\u30A3\u30EB\u30BF\u30FC\u3055\u308C\u305F\u30EC\u30A4\u30E4\u30FC\u7BC4\u56F2\u3092\u30BA\u30FC\u30E0\u306B",layer_visibility:"\u8868\u793A\u5207\u66FF"}});var _=e=>{let t=e.filter.zoomToExtent&&mapp.utils.html`
  ...
  <div class="dropdown active">
    ...
    <ul></ul>`;return mapp.ui.Gazetteer(Object.assign({mapview:e.mapview,target:t,layer:e.key},e.gazetteer)),t};mapp.utils.merge(mapp.dictionaries,{en:{layer_dataview_header:"Data Views"},de:{layer_dataview_header:"Datenansichten"},cn:{layer_dataview_header:"\u6570\u636E\u68C0\u89C6"},pl:{layer_dataview_header:"Widoki danych"},ko:{layer_dataview_header:"\uB370\uC774\uD130 \uBCF4\uAE30"},fr:{layer_dataview_header:"Vues des donn\xE9es"},ja:{layer_dataview_header:"\u30C7\u30FC\u30BF\u30D3\u30E5\u30FC"}});var I=e=>{let t=Object.entries(e.dataviews).map(a=>{let l=Object.assign(a[1],{key:a[0],layer:e,host:e.mapview.host}),o=document.querySelector(`[data-id=${l.target}]`);if(!o)return;l.target=mapp.utils.html.node`<div class="dataview-target">`,e.display&&l.display&&r();function r(){if(l.show)return l.show();mapp.ui.Dataview(l).then(()=>l.show()),o.dispatchEvent(new CustomEvent("addTab",{detail:l}))}return e.showCallbacks.push(()=>{l.display&&r()}),mapp.ui.elements.chkbox({label:l.title||l.key,checked:!!l.display,onchange:n=>{l.display=n,l.display?r():l.remove()}})});return mapp.ui.elements.drawer({data_id:"dataviews-drawer",class:"raised",header:mapp.utils.html`
      <h3>${mapp.dictionary.layer_dataview_header}</h3>
      <div class="mask-icon expander"></div>`,content:mapp.utils.html`${t}`})};var P=e=>{let t=Object.keys(e.reports).map(a=>{let l=e.reports[a];l.key=a,l.host=e.mapview.host;let o=`${l.host}/view?${mapp.utils.paramString({template:l.template,lat:mapp.hooks.current?.lat,lng:mapp.hooks.current?.lng,z:mapp.hooks.current?.z})}`;return mapp.utils.html`
      <a
        class="link-with-img"
        target="_blank"
        href="${o}">
      ...
      <div>${mapp.dictionary.layer_style_select_theme}</div>
        ${mapp.ui.elements.dropdown({entries:Object.keys(e.style.themes).map(i=>({title:e.style.themes[i].title||i,option:i})),callback:(i,a)=>{e.style.theme=e.style.themes[a.option],mapp.utils.render(e.style.drawer.querySelector(".drawer > .legend"),mapp.ui.layers.styles[e.style.theme.type](e)),e.reload()}})}`),e.style.theme?.title&&!e.style.themes&&t.push(mapp.utils.html`
    ...
    <div class="legend">
      ...
      <div class="mask-icon expander"></div>`,content:t}),e.style.drawer};var R=e=>{let t=e.style.theme,i=[];if(e.filter&&i.push(mapp.utils.html`
    <div
      class="switch-all"
  ...
  <div
    ...
    <button
      ...
      onclick = ${a=>{e.remove()}}>`),e.view=mapp.ui.elements.drawer({class:"location-view raised expanded",header:t});let i=e.view.appendChild(mapp.ui.locations.infoj(e));e.view.querySelector(".header").style.borderBottom=`3px solid ${e.record.colour}`,e.view.addEventListener("valChange",a=>{a.detail.value!=a.detail.newValue?a.detail.node.classList.add("val-changed"):(delete a.detail.newValue,a.detail.node.classList.remove("val-changed")),e.view.querySelector(".cloud-upload").style.display=e.infoj.some(l=>typeof l.newValue<"u")&&"inline-block"||"none"}),e.view.addEventListener("updateInfo",()=>{i.remove(),e.view.querySelector(".cloud-upload").style.display="none",e.view.classList.remove("disabled"),i=mapp.ui.locations.infoj(e),e.view.appendChild(i)})};mapp.utils.merge(mapp.dictionaries,{en:{location_clear_all:"Clear locations"},de:{location_clear_all:"Entferne Auswahl"},cn:{location_clear_all:"\uBAA8\uB4E0 \uC704\uCE58 \uC81C\uAC70"},pl:{location_clear_all:"Wyczy\u015B\u0107 selekcje"},ko:{location_clear_all:"\u6E05\u9664\u6240\u6709\u5730\u70B9"},fr:{location_clear_all:"Des\xE9lectionner tous les lieux."},ja:{location_clear_all:"\u5168\u30ED\u30B1\u30FC\u30B7\u30E7\u30F3\u3092\u30AF\u30EA\u30A2"}});var U=[{symbol:"A",colour:"#2E6F9E"},{symbol:"B",colour:"#EC602D"},{symbol:"C",colour:"#5B8C5A"},{symbol:"D",colour:"#B84444"},{symbol:"E",colour:"#514E7E"},{symbol:"F",colour:"#E7C547"},{symbol:"G",colour:"#368F8B"},{symbol:"H",colour:"#841C47"},{symbol:"I",colour:"#61A2D1"},{symbol:"J",colour:"#37327F"}],J=e=>{if(!e.mapview||!e.target)return;let t={node:e.target,mapview:e.mapview},i=t.node.appendChild(mapp.utils.html.node`
    <button 
  ...
  <button 
    ...
    <button
      ...
      <div><div style="display: flex;">${n}${c}`}}var h={idle:600},De=e=>{Object.assign(h,e),h.idle!==0&&(window.onload=f,window.onmousemove=f,window.onmousedown=f,window.ontouchstart=f,window.onclick=f,window.onkeypress=f,f(),qe())};function f(){h.locked||(h.timeout&&clearTimeout(h.timeout),h.timeout=setTimeout(Oe,h.idle*1e3))}function Oe(){h.locked=!0,h.renew&&clearTimeout(h.renew);let e=new XMLHttpRequest;e.open("GET",`${h.host}/api/user/cookie?destroy=true`),e.onload=t=>location.reload(),e.send()}function qe(){h.renew=setTimeout(e,(h.idle-20)*1e3);function e(){let t=new XMLHttpRequest;t.open("GET",`${h.host}/api/user/cookie?renew=true`),t.onload=i=>{if(i.target.status===401)return Oe();qe()},t.send()}}var Me=e=>{document.body.append(mapp.utils.html.node`
    <div class="interface-mask">
      <div class="bg-image" style=${`background-image:url(${e.target.src})`}>
      <button class="btn-close mask-icon close"
        onclick=${t=>t.target.parentElement.parentElement.remove()}>`)};var Fe=e=>{e.target.addEventListener("mousedown",i=>{i.preventDefault(),document.body.style.cursor="grabbing",window.addEventListener("mousemove",e.resizeEvent),window.addEventListener("mouseup",t)}),e.target.addEventListener("touchstart",i=>{i.preventDefault(),window.addEventListener("touchmove",e.resizeEvent),window.addEventListener("touchend",t)},{passive:!0});function t(){document.body.style.cursor="auto",window.removeEventListener("mousemove",e.resizeEvent),window.removeEventListener("touchmove",e.resizeEvent),window.removeEventListener("mouseup",t),window.removeEventListener("touchend",t)}};var Ie={Chart:je,Tabulator:ze,tabulator:Te,idleLogout:De,imagePreview:Me,resizeHandler:Fe};var Pe=e=>{let t=Object.assign({input:e.target.querySelector("input"),result:e.target.querySelector("ul"),sources:{glx:a,mapbox:o,google:l}},e);t.input.placeholder=t.placeholder||"",t.input.oninput=n=>{t.xhr&&t.xhr.abort(),t.result.innerHTML="",n.target.value.length>0&&i(n.target.value)},t.input.addEventListener("keydown",n=>{(n.keyCode||n.charCode)===13&&t.result.querySelector("li").click()});function i(n){let s=n.split(",").map(parseFloat);if(s.length===2&&s.every(c=>!isNaN(c))){t.result.appendChild(mapp.utils.html.node`
      <li 
        onclick=${c=>{t.result.innerHTML="",r({label:`Latitude:${s[0]}, Longitude:${s[1]}`,source:"Coordinates",lng:s[1],lat:s[0]})}}><span>Latitutde:${s[0]}, Longitude:${s[1]}`);return}t.xhr=new XMLHttpRequest,t.xhr.open("GET",t.mapview.host+"/api/gazetteer?"+mapp.utils.paramString({locale:t.mapview.locale.key,layer:t.layer,q:encodeURIComponent(n)})),t.xhr.setRequestHeader("Content-Type","application/json"),t.xhr.responseType="json",t.xhr.onload=c=>{if(c.target.status===200){if(c.target.response.length===0)return t.result.appendChild(mapp.utils.html.node`
          ...
          <li
            ...
            <span class="label">${t.mapview.layers[d.layer]&&t.mapview.layers[d.layer].name||d.layer||d.source}</span>
