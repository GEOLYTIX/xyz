mapp.utils.merge(mapp.dictionaries,{en:{layer_zoom_to_extent:"Zoom to filtered layer extent",layer_visibility:"Toggle visibility"},de:{layer_zoom_to_extent:"Zoom zum Ausma\xDF des gefilterten Datensatzes",layer_visibility:"Umschalten der Ansicht"},cn:{layer_zoom_to_extent:"\u7F29\u653E\u81F3\u76F8\u5E94\u7B5B\u9009\u8303\u56F4",layer_visibility:"\u5207\u6362\u53EF\u89C1\u6027"},pl:{layer_zoom_to_extent:"Poka\u017C zasi\u0119g warstwy",layer_visibility:"Widoczno\u015B\u0107"},ko:{layer_zoom_to_extent:"\uD544\uD130\uB41C \uB808\uC774\uC5B4\uD06C\uAE30\uC5D0 \uC90C(zoom)",layer_visibility:"\uD1A0\uAE00 \uAC00\uC2DC\uC131"},fr:{layer_zoom_to_extent:"Zoom sur l'\xE9tendue de la couche",layer_visibility:"Changer la visiblit\xE9"},ja:{layer_zoom_to_extent:"\u30D5\u30A3\u30EB\u30BF\u30FC\u3055\u308C\u305F\u30EC\u30A4\u30E4\u30FC\u7BC4\u56F2\u3092\u30BA\u30FC\u30E0\u306B",layer_visibility:"\u8868\u793A\u5207\u66FF"}});var _=e=>{let t=mapp.utils.html`
  ...
  <div class="dropdown active">
    ...
    <div 
      ...
      <div 
        data-id=${a.value}
        class="contents">
        ${o}${r}`)}),e.style.legend=mapp.utils.html.node`<div class="grid">${i}`,e.style.legend};var N=e=>{let t=[];t.push(mapp.ui.elements.dropdown({entries:Object.entries(e.grid_fields).map(o=>({title:o[0],options:o[1]})),callback:(o,r)=>{e.grid_size=r.options,e.reload()}}));let i=e.style.range.length,a=new XMLSerializer,l=mapp.utils.html`
  <div
    class="grid"
  ...
  <div
    ...
    <button 
      ...
      <div
        class="${`location ${e.class}`}">`,e.target=t,e.display&&mapp.ui.Dataview(e).then(()=>e.update())),e.chkbox=e.label&&mapp.ui.elements.chkbox({label:e.label,checked:!!e.display,onchange:l=>{if(e.display=l,t){t.style.display=e.display?"block":"none",e.display&&typeof e.update=="function"&&e.update(),e.display&&typeof e.update!="function"&&mapp.ui.Dataview(e).then(()=>e.update());return}e.display?i():e.remove()}}),mapp.utils.html.node`${e.chkbox}${t||""}`};var x=e=>{let t;return e.edit?t=mapp.utils.html.node`
      <input
    ...
    <div
      class="val"
      style="${`${e.css_val||""}`}">${t}`};var ae=e=>{let t=mapp.ui.elements.chkbox({label:e.label||"MVT Clone",data_id:`${e.field}-chkbox`,checked:!!e.display,onchange:a=>{a?i():(e.display=!1,e.location.layer.mapview.Map.removeLayer(e.L),e.Layer.clones.delete(e.L),e.legend?.remove())}});return e.display&&i(),mapp.utils.html.node`<div>${t}`;async function i(){let a=e.location.layer.mapview;if(e.L){a.Map.addLayer(e.L),e.Layer.clones.add(e.L);return}e.Layer=a.layers[e.layer];let l=mapp.utils.queryParams(e),o=mapp.utils.paramString(l),r=await mapp.utils.xhr(`${e.host||e.location.layer.mapview.host}/api/query?${o}`);if(!e.location.remove)return;let n={style:{default:e.Layer.style.default},featureLookup:r};e.theme&&(n.style.theme=e.theme,e.legend=e.node.appendChild(mapp.utils.html.node`
        <div class="legend">
          ${e.theme.title||""}
          ${mapp.ui.layers.styles[e.theme.type](n)}`)),e.L=new ol.layer.VectorTile({source:e.Layer.L.getSource(),renderBuffer:200,style:mapp.layer.Style(n)}),e.Layer.clones.add(e.L),a.Map.addLayer(e.L),e.location.removeCallbacks.push(()=>{e.location.layer.mapview.Map.removeLayer(e.L),e.Layer.clones.delete(e.L)})}};var L=e=>{let t=!isNaN(e.value)&&e.type==="integer"?parseInt(e.value).toLocaleString("en-GB",{maximumFractionDigits:0}):parseFloat(e.value).toLocaleString("en-GB",{maximumFractionDigits:2});return e.edit&&(e.edit.range?t=mapp.ui.elements.slider({min:e.edit.range.min,max:e.edit.range.max,val:e.value,callback:a=>{t.value=parseFloat(a.target.value),e.newValue=t.value,e.location.view?.dispatchEvent(new CustomEvent("valChange",{detail:e}))}}):t=mapp.utils.html.node`
      <input
        type="number"
        value="${e.value||""}"
  ...
  <div
    ...
    <div
      class="val"
      style="${`${e.css_val||""}`}">
      ${t}`};var de={key:Ye,boolean:Z,dataview:K,date:x,datetime:x,defaults:X,documents:Q,geometry:Y,html:S,images:ee,integer:L,isoline_here:k,isoline_mapbox:k,json:te,mvt_clone:ae,numeric:L,pin:ie,report:le,streetview:oe,tab:re,text:ne,textarea:S,time:se};function Ye(e){return mapp.utils.html.node`
  <div class="layer-key">
    <span>
      ${e.location.layer.name}`}var ce={view:H,listview:W,infoj:J,entries:de};var pe=async e=>{if(e.target=typeof e.target=="string"&&document.getElementById(e.target)||e.target,e.target=e.target instanceof HTMLElement&&e.target||mapp.utils.html.node`<div class="dataview-target">`,e.update=async()=>{if(!e.query)return;let t=mapp.utils.queryParams(e),i=mapp.utils.paramString(t),a=await mapp.utils.xhr(`${e.host||e.location.layer.mapview.host}/api/query?${i}`);if(!(a instanceof Error)){if(typeof e.responseFunction=="function")return e.responseFunction(a);typeof e.setData=="function"&&e.setData(a)}},e.toolbar){e.target=mapp.utils.html.node`<div class="dataview-target">`;let t=Object.keys(e.toolbar).map(i=>mapp.ui.elements.toolbar_el[i](e));e.panel=mapp.utils.html.node`
      <div class="flex-col">
        <div class="btn-row">${t}</div>
        ${e.target}`}return e.chart&&await et(e),typeof e.columns<"u"&&(console.warn("Table dataviews should be configured inside a tables object"),e.table={columns:e.columns}),e.table&&await tt(e),e.mapChange&&e.layer&&e.layer.mapview.Map.getTargetElement().addEventListener("changeEnd",()=>{e.layer&&!e.layer.display||e.tab&&!e.tab.classList.contains("active")||typeof e.mapChange=="function"&&e.mapChange()||e.update()}),e};async function et(e){let t=e.target.appendChild(mapp.utils.html.node`<canvas>`);e.ChartJS=await mapp.ui.utils.Chart(t,mapp.utils.merge({type:"bar",options:{plugins:{legend:{display:!1},datalabels:{display:!1}}}},e.chart)),e.setData=i=>{e.noDataMask&&!i?(e.target.style.display="none",e.mask=!e.mask&&e.target.parentElement?.appendChild(mapp.utils.html.node`
  ...
  <button 
    ...
    <ul>${e.entries.map(t=>mapp.utils.html.node`
      <li onclick=${i=>{let a=i.target.closest("button.dropdown");a.classList.toggle("active"),a.querySelector("[data-id=header-span]").textContent=t.title,e.callback&&e.callback(i,t)}}>${t.title}`)}`;var be=e=>{let t=new Set,i=new Set;return mapp.utils.html`
    <button 
      data-id=${e.data_id||"dropdown"}
      class="dropdown">
      <div class="head"
    ...
    <div 
      ...
      <div style="display: grid; grid-row-gap: 5px;">
        ${t}
        ${i}`})};var $e=new XMLSerializer,xe=e=>{if(e.svg||e.type||e.layers){let t=e.layers&&Array.isArray(e.layers)&&e.layers.map(a=>`url(${a.svg})`).reverse().join(",")||`url(${e.svg||e.url||mapp.utils.svgSymbols[e.type](e)})`,i=`
      background-position: center;
      background-repeat: no-repeat;
      background-size: contain;
      ...
      width: ${e.width}px;
      height: ${e.height}px;
      background-image: url(${i});`;return mapp.utils.html`<div style=${a}>`}};var ke=e=>{return mapp.utils.html.node`
    <div
      role="group"
    ...
    <button
      class="flat"
      onclick=${()=>{e.Tabulator.download("json",`${e.title||"table"}.json`)}}>Export as JSON`}function ft(e){return mapp.utils.html`
    <button
      class="flat"
      onclick=${()=>{e.Tabulator.download("csv",`${e.title||"table"}.csv`)}}>Download as CSV`}var Ee={card:me,chkbox:he,contextMenu:ge,drawer:fe,drawing:ve,dropdown:we,dropdown_multi:be,isoline_params_here:_e,isoline_params_mapbox:ye,legendIcon:xe,slider:ke,slider_ab:Le,toolbar_el:Se};var E,v=null;async function Ce(e,t){return v?new v(e,t):(E||(E=new Promise(async i=>{if(window.Chart){v=window.Chart,i();return}Promise.all([import("https://cdn.skypack.dev/chart.js@3.7.1"),import("https://cdn.skypack.dev/chartjs-plugin-datalabels@2.0.0"),import("https://cdn.skypack.dev/chartjs-plugin-annotation@1.4.0")]).then(a=>{a[0].Chart.register(...a[0].registerables),a[0].Chart.register(a[1].default),a[0].Chart.register(a[2].default),v=a[0].Chart,i()}).catch(a=>{console.error(a.message),alert("Failed to load Chart.js library. Please reload the browser.")})})),await E,new v(e,t))}var C,w=null;async function je(){return w?new w(...arguments):(C||(C=new Promise(async e=>{if(window.Tabulator){w=window.Tabulator,e();return}Promise.all([import("https://cdn.skypack.dev/pin/tabulator-tables@v5.3.0-3P9dSwNRL29tgc0Vmk59/mode=imports,min/optimized/tabulator-tables.js")]).then(t=>{w=t[0].TabulatorFull,e()}).catch(t=>{console.error(t.message),alert("Failed to load Chart.js library. Please reload the browser.")})})),await C,new w(...arguments))}var ze={headerFilter:{like:vt,numeric:wt}};function vt(e){return(t,i,a,l,o)=>{let r=t.getColumn().getField();function n(s){let c=e.Tabulator.getFilters().find(d=>d.field===r&&d.type==">=");c&&(e.Tabulator.removeFilter(...Object.values(c)),o.layer&&delete e.layer.filter.current[r]),s.target.value.length&&(e.Tabulator.setFilter([{field:r,type:"like",value:s.target.value}]),o.layer&&(e.layer.filter.current[r]={[o.type]:encodeURIComponent(s.target.value)},e.layer.reload(),e.update()))}return mapp.utils.html.node`<span>
      <input
        type="text"
        placeholder="Filter"
        oninput=${n}
      ...
      <div><div style="display: flex;">${n}${c}`}}var h={idle:600},De=e=>{Object.assign(h,e),h.idle!==0&&(window.onload=f,window.onmousemove=f,window.onmousedown=f,window.ontouchstart=f,window.onclick=f,window.onkeypress=f,f(),Te())};function f(){h.locked||(h.timeout&&clearTimeout(h.timeout),h.timeout=setTimeout(Oe,h.idle*1e3))}function Oe(){h.locked=!0,h.renew&&clearTimeout(h.renew);let e=new XMLHttpRequest;e.open("GET",`${h.host}/api/user/cookie?destroy=true`),e.onload=t=>location.reload(),e.send()}function Te(){h.renew=setTimeout(e,(h.idle-20)*1e3);function e(){let t=new XMLHttpRequest;t.open("GET",`${h.host}/api/user/cookie?renew=true`),t.onload=i=>{if(i.target.status===401)return Oe();Te()},t.send()}}var qe=e=>{document.body.append(mapp.utils.html.node`
