mapp.utils.merge(mapp.dictionaries,{en:{layer_zoom_to_extent:"Zoom to filtered layer extent",layer_visibility:"Toggle visibility"},de:{layer_zoom_to_extent:"Zoom zum Ausmaß des gefilterten Datensatzes",layer_visibility:"Umschalten der Ansicht"},cn:{layer_zoom_to_extent:"缩放至相应筛选范围",layer_visibility:"切换可见性"},pl:{layer_zoom_to_extent:"Pokaż zasięg warstwy",layer_visibility:"Widoczność"},ko:{layer_zoom_to_extent:"필터된 레이어크기에 줌(zoom)",layer_visibility:"토글 가시성"},fr:{layer_zoom_to_extent:"Zoom sur l'étendue de la couche",layer_visibility:"Changer la visiblité"},ja:{layer_zoom_to_extent:"フィルターされたレイヤー範囲をズームに",layer_visibility:"表示切替"}});var w=e=>{let t=mapp.utils.html`
  ...
  <input
    ...
    <div class="legend">
      ...
      <div class="mask-icon expander"></div>`,content:t}),e.style.drawer};var F=e=>{let t=e.style.theme,a=[];if(a.push(mapp.utils.html`
    <div
      class="switch-all"
    ...
    <div 
      ...
      <div 
        data-id="cluster"
        class="contents">
        ${i}${l}`)}return e.style.legend=mapp.utils.html.node`<div class="grid">${a}`,e.style.legend};var P=e=>{let t=e.style.theme,a=[];return t.cat_arr.forEach(i=>{let l=Object.assign({},e.style.default,i.style&&i.style.icon||i.style||i),o=mapp.utils.html`
      <div
        style="height: 24px; width: 24px;">
        ${mapp.ui.elements.legendIcon(Object.assign({width:24,height:24},l))}`,n=mapp.utils.html`
      ...
      <div 
        data-id=${i.value}
        class="contents">
        ${o}${n}`)}),e.style.legend=mapp.utils.html.node`<div class="grid">${a}`,e.style.legend};var R=e=>{let t=[];t.push(mapp.ui.elements.dropdown({entries:Object.entries(e.grid_fields).map(o=>({title:o[0],options:o[1]})),callback:(o,n)=>{e.grid_size=n.options,e.reload()}}));let a=e.style.range.length,i=new XMLSerializer,l=mapp.utils.html`
  <div
    class="grid"
  ...
  <div
    ...
    <div
      class="val"
      style="${`${e.css_val||""}`}">
      ${t}`};var J=e=>{let t=e.value.map(n=>mapp.utils.html`
  ...
  <div class="layer-key">
    ...
    <button
      class="flat"
      onclick=${()=>{e.Tabulator.download("csv",`${e.title||"table"}.csv`)}}>CSV`}var se=async e=>(e.target=typeof e.target=="string"&&document.getElementById(e.target)||e.target,e.target=e.target instanceof HTMLElement&&e.target||mapp.utils.html.node`
      <div
        class="dataview-target"
        style="position: absolute; width: 100%; height: 100%">`,e.update=async()=>{if(!e.query)return;let t=e.viewport&&e.layer.mapview.getBounds();e.viewport&&(e.queryparams=Object.assign(e.queryparams||{},{layer:!0}));let a=e.queryparams?.center&&ol.proj.transform(e.layer.mapview.Map.getView().getCenter(),`EPSG:${_xyz.mapview.srid}`,"EPSG:4326"),i=mapp.utils.paramString(Object.assign({},e.queryparams||{},{template:encodeURIComponent(e.query),filter:e.queryparams?.filter&&e.layer?.filter?.current,layer:e.queryparams?.layer&&e.layer.key,locale:e.queryparams?.layer&&e.layer.mapview.locale.key,id:e.queryparams?.id&&e.location&&e.location.id,lat:a&&a[1],lng:a&&a[0],z:e.queryparams?.z&&e.layer.mapview.Map.getView().getZoom(),viewport:t&&[t.west,t.south,t.east,t.north,e.layer.mapview.srid]})),l=await mapp.utils.xhr(`${e.host||e.location.layer.mapview.host}/api/query?`+i);if(!(l instanceof Error)){if(typeof e.responseFunction=="function")return e.responseFunction(l);typeof e.setData=="function"&&e.setData(l)}},e.toolbar&&re(e),e.chart&&await ne(e),typeof e.columns!="undefined"&&(console.warn("Table dataviews should be configured inside a tables object"),e.table=Object.assign(e.table||{},{columns:e.columns})),e.table&&await oe(e),e.mapChange&&e.layer&&e.layer.mapview.Map.getTargetElement().addEventListener("changeEnd",()=>{e.layer&&!e.layer.display||e.tab&&!e.tab.classList.contains("active")||typeof e.mapChange=="function"&&e.mapChange()||e.update()}),e);var de=e=>{!e.node||(e.tabs=e.node.appendChild(mapp.utils.html.node`<div class="tabs">`),e.panel=e.node.appendChild(mapp.utils.html.node`<div class="panel">`),e.id&&e.node.setAttribute("data-id",e.id),e.addTab=Qe,e.node.addEventListener("addTab",t=>e.addTab(t.detail)))};function Qe(e){let t=this;e.tab=mapp.utils.html.node`
    <div class="tab">
      <div
        class="header"
        style="${e.tab_style||""}"
  ...
  <li
    ...
    <li
      onclick=${()=>this.interaction.finish()}>
      ${mapp.dictionary.cancel}`),this.popup({coords:this.interaction.vertices[this.interaction.vertices.length-1],content:mapp.utils.html.node`<ul>${a}`,autoPan:!0})}var ue=e=>mapp.utils.html.node`
  <div 
  ...
  <button 
    ...
    <ul>${e.entries.map(t=>mapp.utils.html.node`
      <li onclick=${a=>{let i=a.target.closest("button.dropdown");i.classList.toggle("active"),i.querySelector("[data-id=header-span]").textContent=t.title,e.callback&&e.callback(a,t)}}>${t.title}`)}`;var fe=e=>{let t=new Set,a=new Set;return mapp.utils.html`
    <button 
      data-id=${e.data_id||"dropdown"}
      class="dropdown">
      <div class="head"
    ...
    <div 
      ...
      <div style="display: grid; grid-row-gap: 5px;">
        ${t}
        ${a}`})};var _e=new XMLSerializer,be=e=>{if(e.svg||e.type||e.layers){let t=e.layers&&Array.isArray(e.layers)&&e.layers.map(i=>`url(${i.svg})`).reverse().join(",")||`url(${e.svg||e.url||mapp.utils.svgSymbols[e.type](e)})`,a=`
      background-position: center;
      background-repeat: no-repeat;
      background-size: contain;
      ...
      <svg height=${e.height} width=${e.width}>
        <path d=${t}
          ...
          stroke-width=${e.strokeWidth||1}/>`,i=`data:image/svg+xml,${encodeURIComponent(_e.serializeToString(a))}`,l=`
      background-position: center;
      background-repeat: no-repeat;
      background-size: contain;
      width: ${e.width}px;
      ...
      <svg height=${e.height} width=${e.width}>
        <rect
          ...
          stroke=${e.strokeColor}
          stroke-width=${e.strokeWidth||1}>`,a=`data:image/svg+xml,${encodeURIComponent(_e.serializeToString(t))}`,i=`
      background-position: center;
      background-repeat: no-repeat;
      background-size: contain;
      width: ${e.width}px;
      height: ${e.height}px;
      background-image: url(${a});`;return mapp.utils.html`<div style=${i}>`}};var ye=e=>{return mapp.utils.html.node`
    <div
      role="group"
      ...
      <input id="a"
        ...
        value=${e.val}
        step=${e.step||1}
        oninput=${t}>`;function t(a){a.target.closest(".input-range").style.setProperty(`--${a.target.id}`,a.target.value),a.target.closest(".input-range").querySelectorAll("input").forEach(i=>{i.id==a.target.id&&i!=a.target&&(i.value=a.target.value)}),e.callback&&e.callback(a)}};var $e=e=>mapp.utils.html.node`
    <div
      role="group"
      ...
      <input id="b" type="range"
        ...
        step=${e.step||1}
        value=${e.val_b}
        oninput=${a=>{a.target.parentNode.style.setProperty(`--${a.target.id}`,a.target.value),a.target.parentNode.querySelectorAll("input").forEach(i=>{i.id==a.target.id&&i!=a.target&&(i.value=a.target.value)}),e.callback_b&&e.callback_b(a)}}/>`;var ke={card:ce,chkbox:pe,contextMenu:me,drawer:ue,drawing:he,dropdown:ge,dropdown_multi:fe,isoline_params_here:ve,isoline_params_mapbox:we,legendIcon:be,slider:ye,slider_ab:$e};var b=null;async function ct(){if(window.Chart){b=window.Chart;return}let e=await import("https://cdn.skypack.dev/chart.js@3.7.0");e.Chart.register(...e.registerables);let t=await import("https://cdn.skypack.dev/chartjs-plugin-datalabels@2.0.0");e.Chart.register(t.default);let a=await import("https://cdn.skypack.dev/chartjs-plugin-annotation@1.3.0");e.Chart.register(a.default),b=e.Chart}async function xe(e,t){return b||await ct(),new b(e,t)}var y=null;async function pt(){if(window.Tabulator){y=window.Tabulator;return}y=(await import("https://cdn.skypack.dev/pin/tabulator-tables@v5.0.10-xDu7yyLVhsxCKJbN5vfH/mode=imports/optimized/tabulator-tables.js")).TabulatorFull}async function Le(){return y||await pt(),new y(...arguments)}var u={idle:600},je=e=>{Object.assign(u,e),u.idle!==0&&(window.onload=f,window.onmousemove=f,window.onmousedown=f,window.ontouchstart=f,window.onclick=f,window.onkeypress=f,f(),S())};function f(){u.locked||(u.timeout&&clearTimeout(u.timeout),u.timeout=setTimeout(Se,u.idle*1e3))}function Se(){u.locked=!0,u.renew&&clearTimeout(u.renew);let e=new XMLHttpRequest;e.open("GET",`${u.host}/api/user/cookie?destroy=true`),e.send();let t=mapp.utils.html.node`
    <form
      class="login"
        ...
        ${t.label?mapp.utils.html.node`
          <span
            style="padding: 0 2px; cursor: help; border-radius: 2px;">
              ...
              <span>${c.label}</span>`),t.target.classList.add("active")})}},t.xhr.send()}function i(r){r=typeof r=="string"&&JSON.parse(r)||r,t.layer&&t.mapview.Map.removeLayer(t.layer),t.layer=t.mapview.geoJSON({geometry:r,dataProjection:"4326",Style:mapp.utils.style({icon:t.icon})});let d=t.mapview.Map.getView();d.animate({zoom:t.zoom||d.getMaxZoom()},{center:ol.proj.fromLonLat(r.coordinates)})}function l(r){t.clear&&t.clear(),t.input&&(t.input.value=r.label),t.sources[r.source](r)}function o(r){if(t.callback)return t.callback(r);r.callback&&r.callback()}function n(r){t.createFeature({type:"Point",coordinates:r.marker}),r.callback&&r.callback()}function s(r){mapp.utils.xhr(`${t.mapview.host}/api/proxy?url=${encodeURIComponent(`https://maps.googleapis.com/maps/api/place/details/json?placeid=${r.id}&{GOOGLE}`)}`).then(d=>{let p={type:"Point",coordinates:[d.result.geometry.location.lng,d.result.geometry.location.lat]};if(t.createFeature(p),t.callback)return t.callback(p);r.callback&&r.callback(p)})}function m(r){t.createFeature({type:"Point",coordinates:r.marker}),r.callback&&r.callback()}};var Oe={layers:V,locations:le,elements:ke,utils:ze,gazetteer:De,Dataview:se,Tabview:de};typeof window.mapp=="object"&&(window.mapp.ui=Oe);var mt=Oe;export{mt as default};
