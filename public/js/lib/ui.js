mapp.utils.merge(mapp.dictionaries,{en:{layer_zoom_to_extent:"Zoom to filtered layer extent",layer_visibility:"Toggle visibility"},de:{layer_zoom_to_extent:"Zoom zum Ausma\xDF des gefilterten Datensatzes",layer_visibility:"Umschalten der Ansicht"},cn:{layer_zoom_to_extent:"\u7F29\u653E\u81F3\u76F8\u5E94\u7B5B\u9009\u8303\u56F4",layer_visibility:"\u5207\u6362\u53EF\u89C1\u6027"},pl:{layer_zoom_to_extent:"Poka\u017C zasi\u0119g warstwy",layer_visibility:"Widoczno\u015B\u0107"},ko:{layer_zoom_to_extent:"\uD544\uD130\uB41C \uB808\uC774\uC5B4\uD06C\uAE30\uC5D0 \uC90C(zoom)",layer_visibility:"\uD1A0\uAE00 \uAC00\uC2DC\uC131"},fr:{layer_zoom_to_extent:"Zoom sur l'\xE9tendue de la couche",layer_visibility:"Changer la visiblit\xE9"},ja:{layer_zoom_to_extent:"\u30D5\u30A3\u30EB\u30BF\u30FC\u3055\u308C\u305F\u30EC\u30A4\u30E4\u30FC\u7BC4\u56F2\u3092\u30BA\u30FC\u30E0\u306B",layer_visibility:"\u8868\u793A\u5207\u66FF"}});var w=e=>{let t=mapp.utils.html`
  ...
  <input
    ...
    <button
      ...
      <div class="mask-icon expander"></div>`,content:mapp.utils.html`${t}`})};var I=e=>{let t=Object.keys(e.reports).map(a=>{let l=e.reports[a];l.key=a,l.host=e.mapview.host;let o=`${l.host}/view?${mapp.utils.paramString({template:l.template,lat:mapp.hooks.current?.lat,lng:mapp.hooks.current?.lng,z:mapp.hooks.current?.z})}`;return mapp.utils.html`
      <a
        class="link-with-img"
        target="_blank"
        href="${o}">
    ...
    <div
      ...
      <div
        ...
        onclick=${s=>{s.target.classList.toggle("disabled"),s.target.classList.contains("disabled")?(e.filter.current[t.field]||(e.filter.current[t.field]={}),e.filter.current[t.field].ni||(e.filter.current[t.field].ni=[]),e.filter.current[t.field].ni.push(a[1].keys||a[0]),e.filter.current[t.field].ni=e.filter.current[t.field].ni.flat()):(Array.isArray(a[1].keys)?a[1].keys.forEach(m=>{e.filter.current[t.field].ni.splice(e.filter.current[t.field].ni.indexOf(m),1)}):e.filter.current[t.field].ni.splice(e.filter.current[t.field].ni.indexOf(a[0]),1),e.filter.current[t.field].ni.length||delete e.filter.current[t.field]),e.reload()}}>${a[1].label||a[0]}`;i.push(mapp.utils.html`
    <div 
      data-id=${a[0]}
      class="contents">
    ...
    <div data-id="size-icons" class="contents">
      ${e.style.range.map((o,n)=>{let s=mapp.utils.svg.node`<svg height=50 width=50>
          ...
          <circle
            ...
            cx=25
            cy=25
            r='${23/i*(n+1)}'/>`,m=`data:image/svg+xml,${encodeURIComponent(a.serializeToString(s))}`,r=`
          grid-row:2;
          grid-column:${n+1};
  ...
  <div
    ...
    <button
      ...
      onclick = ${a=>{e.remove()}}>`),e.view=mapp.ui.elements.drawer({class:"location-view expanded",header:t});let i=e.view.appendChild(mapp.ui.locations.infoj(e));e.view.querySelector(".header").style.borderBottom=`3px solid ${e.record.colour}`,e.view.addEventListener("valChange",a=>{a.detail.value!=a.detail.newValue?a.detail.node.classList.add("val-changed"):(delete a.detail.newValue,a.detail.node.classList.remove("val-changed")),e.view.querySelector(".cloud-upload").style.display=e.infoj.some(l=>typeof l.newValue<"u")&&"inline-block"||"none"}),e.view.addEventListener("updateInfo",()=>{i.remove(),e.view.querySelector(".cloud-upload").style.display="none",e.view.classList.remove("disabled"),i=mapp.ui.locations.infoj(e),e.view.appendChild(i)})};mapp.utils.merge(mapp.dictionaries,{en:{location_clear_all:"Clear locations"},de:{location_clear_all:"Entferne Auswahl"},cn:{location_clear_all:"\uBAA8\uB4E0 \uC704\uCE58 \uC81C\uAC70"},pl:{location_clear_all:"Wyczy\u015B\u0107 selekcje"},ko:{location_clear_all:"\u6E05\u9664\u6240\u6709\u5730\u70B9"},fr:{location_clear_all:"Des\xE9lectionner tous les lieux."},ja:{location_clear_all:"\u5168\u30ED\u30B1\u30FC\u30B7\u30E7\u30F3\u3092\u30AF\u30EA\u30A2"}});var Ae=[{symbol:"A",colour:"#2E6F9E"},{symbol:"B",colour:"#EC602D"},{symbol:"C",colour:"#5B8C5A"},{symbol:"D",colour:"#B84444"},{symbol:"E",colour:"#514E7E"},{symbol:"F",colour:"#E7C547"},{symbol:"G",colour:"#368F8B"},{symbol:"H",colour:"#841C47"},{symbol:"I",colour:"#61A2D1"},{symbol:"J",colour:"#37327F"}],H=e=>{if(!e.mapview||!e.target)return;let t={node:e.target,mapview:e.mapview},i=t.node.appendChild(mapp.utils.html.node`
    <button 
      ...
      ${mapp.dictionary.location_clear_all}`);return e.mapview.locations=new Proxy(e.mapview.locations,{set:function(a,l,o){let n=Ae.find(s=>!s.hook);return n.hook=o.hook,o.record=n,o.style={strokeColor:n.colour,fillColor:n.colour,fillOpacity:.2},o.Style=mapp.utils.style([{strokeColor:"#000",strokeOpacity:.1,strokeWidth:8},{strokeColor:"#000",strokeOpacity:.1,strokeWidth:6},{strokeColor:"#000",strokeOpacity:.1,strokeWidth:4},{strokeColor:o.style.strokeColor||"#000",strokeWidth:2,fillColor:o.style.fillColor||o.style.strokeColor||"#fff",fillOpacity:o.style.fillOpacity||.2}]),o.pinStyle=mapp.utils.style({icon:{type:"markerLetter",letter:n.symbol,color:o.style.strokeColor,scale:3,anchor:[.5,1]}}),Reflect.set(...arguments),mapp.ui.locations.view(o),Object.values(t.node.children).forEach(s=>s.classList.remove("expanded")),t.node.insertBefore(o.view,i.nextSibling),o.view.dispatchEvent(new Event("addLocationView")),document.querySelector("[data-id=locations]").style.display="block",i.style.display="block",document.querySelector("[data-id=locations]").click(),!0},deleteProperty:function(a,l){return Reflect.deleteProperty(...arguments),setTimeout(()=>{document.querySelectorAll("#locations > .location-view").length||(i.style.display="none",document.querySelector("#locations #gazetteerInput")||(document.querySelector("[data-id=locations]").style.display="none",document.querySelector("[data-id=layers]").click()))},300),!0}}),t};var U=(e,t)=>{if(!e.infoj)return;let i=mapp.utils.html.node`<div class="location-view-grid">`,a={};for(let l of t||e.infoj){if(e.view&&e.view.classList.contains("disabled"))break;if(l.listview=i,l.type=l.type||"text",l.skipEntry||l.skipFalsyValue&&!l.value&&!l.edit||l.skipUndefinedValue&&typeof l.value>"u"&&!l.edit||l.skipNullValue&&l.value===null&&!l.edit)continue;l.nullValue&&l.value===null&&!l.defaults&&!l.edit&&(l.value=l.nullValue),l.group&&(a[l.group]||(a[l.group]=l.listview.appendChild(mapp.ui.elements.drawer({class:`lv-1 group ${l.expanded&&"expanded"||""}`,header:mapp.utils.html`
              <h3>${l.group}</h3>
    ...
    <div
      class="images-grid">${t}`;return a;async function l(n){e.location.view?.classList.add("disabled");let s=new FileReader,m=n.target.files[0];!m||(s.onload=r=>{let d=new Image;d.onload=async()=>{let p=mapp.utils.html.node`<canvas>`,c=1024,h=d.width,g=d.height;h>g&&h>c?(g*=c/h,h=c):g>c&&(h*=c/g,g=c),p.width=h,p.height=g,p.getContext("2d").drawImage(d,0,0,h,g);let De=p.toDataURL("image/jpeg",.5),v=await mapp.utils.xhr({method:"POST",requestHeader:{"Content-Type":"application/octet-stream"},url:`${e.location.layer.mapview.host}/api/provider/cloudinary?${mapp.utils.paramString({public_id:m.name.replace(/.*\//,"").replace(/\.([\w-]{3})/,""),resource_type:"image"})}`,body:mapp.utils.dataURLtoBlob(De)});await mapp.utils.xhr(`${e.location.layer.mapview.host}/api/query?`+mapp.utils.paramString({template:"set_field_array",locale:e.location.layer.mapview.locale.key,layer:e.location.layer.key,table:e.location.table,qID:e.location.layer.qID,id:e.location.id,action:"append",field:e.field,value:v.secure_url}));let Te=mapp.utils.html.node`
          <div>
            <img
              src=${v.secure_url}
              onclick=${mapp.ui.utils.imagePreview}>
    ...
    <div
      class="val"
      style="${`${e.css_val||""}`}">
      ${e.prefix}${t}${e.suffix}`};function Je(e){let t=e.edit.options.find(l=>typeof l=="object"&&Object.values(l)[0]===e.value||l===e.value)||e.value;e.value=t&&typeof t=="object"&&Object.keys(t)[0]||t||e.value||" ";let i=e.edit.options.map(l=>({title:typeof l=="string"&&l||Object.keys(l)[0],option:typeof l=="string"&&l||Object.values(l)[0]}));return mapp.ui.elements.dropdown({span:e.value,entries:i,callback:(l,o)=>{e.newValue=o.option,e.location.view?.dispatchEvent(new CustomEvent("valChange",{detail:e}))}})}var E=e=>{e.query&&mapp.utils.xhr(`${e.location.layer.mapview.host}/api/query/${e.query}`).then(a=>{e.node.querySelector(".val").innerHTML=a[e.params.field]});let t=e.type!=="html"?e.value:"";e.edit&&(t=mapp.utils.html`
    <textarea
      style="auto; min-height: 50px;"
      onfocus=${a=>{a.target.style.height=a.target.scrollHeight+"px"}}
      onfocusout=${a=>{a.target.style.height="auto"}}
  ...
  <div
    class="val"
    style="${`${e.css_val||""}`}">${t}`;return!e.edit&&e.type==="html"&&(i.innerHTML=e.value||""),i};var oe={key:We,boolean:J,dataview:W,date:x,datetime:x,defaults:Z,documents:X,geometry:K,html:E,images:Q,integer:S,isoline_here:L,isoline_mapbox:L,json:Y,numeric:S,pin:ee,report:te,streetview:ae,tab:ie,text:le,textarea:E};function We(e){return mapp.utils.html.node`
  <div class="layer-key">
    <span>
      ${e.location.layer.name}`}var ne={view:N,listview:H,infoj:U,entries:oe};var re=async e=>{if(e.target=typeof e.target=="string"&&document.getElementById(e.target)||e.target,e.target=e.target instanceof HTMLElement&&e.target||mapp.utils.html.node`
      <div
      ...
      <div class="grid">
        ...
        <div class="dataviewMask">No Data`)):(e.mask&&e.mask.remove(),delete e.mask,e.target.style.display="block"),t=!t&&[]||t.length&&t||[t],e.Tabulator.setData(t),e.data=t,typeof e.setDataCallback=="function"&&e.setDataCallback(e))}}var se=e=>{!e.node||(e.tabs=e.node.appendChild(mapp.utils.html.node`<div class="tabs">`),e.panel=e.node.appendChild(mapp.utils.html.node`<div class="panel">`),e.id&&e.node.setAttribute("data-id",e.id),e.addTab=Ke,e.node.addEventListener("addTab",t=>e.addTab(t.detail)))};function Ke(e){let t=this;e.tab=mapp.utils.html.node`
    <div class="tab">
      <div
        class="header"
        style="${e.tab_style||""}"
  ...
  <div 
    ...
    ${e.content}`;var ce=e=>mapp.utils.html`
  <label 
  ...
  <li
    ...
    <li
      onclick=${()=>this.interaction.finish()}>
      ${mapp.dictionary.cancel}`),this.popup({coords:this.interaction.vertices[this.interaction.vertices.length-1],content:mapp.utils.html.node`<ul>${i}`,autoPan:!0})}var me=e=>mapp.utils.html.node`
  <div 
    ...
    <button
      class="flat wide primary-colour"
      onclick=${n=>t(n,i)}>
      Create Isoline`),o}var he=e=>mapp.utils.html`
  <button 
    ...
    <ul>${e.entries.map(t=>mapp.utils.html.node`
      <li onclick=${i=>{let a=i.target.closest("button.dropdown");a.classList.toggle("active"),a.querySelector("[data-id=header-span]").textContent=t.title,e.callback&&e.callback(i,t)}}>${t.title}`)}`;var ge=e=>{let t=new Set,i=new Set;return mapp.utils.html`
    <button 
      data-id=${e.data_id||"dropdown"}
      class="dropdown">
      <div class="head"
    ...
    <div style="display: grid; grid-template-columns: 100px 1fr; align-items: center;">
      ...
      <div style="grid-column: 2;">
        ${mapp.ui.elements.dropdown({entries:[{title:[mapp.dictionary.here_mode_driving],option:"car"},{title:[mapp.dictionary.here_mode_walking],option:"pedestrian"}],callback:(r,d)=>{e.transportMode=d.option}})}`,i=mapp.utils.html.node`
    <div style="display: grid; grid-template-columns: 100px 1fr; align-items: center;">
    ...
    <div>
      ${a}
      ${l}`,n=mapp.ui.elements.slider({label:mapp.dictionary.here_range_minutes,min:e.rangeMin,max:e.rangeMax,val:10,callback:r=>{e.range=parseInt(r.target.value)}}),s=mapp.ui.elements.chkbox({label:"Reverse Direction Isoline",checked:!!e.reverseDirection,onchange:r=>{a.textContent=r&&mapp.dictionary.here_datetime_arrive||mapp.dictionary.here_datetime_depart,e.reverseDirection=r}});return mapp.ui.elements.drawer({header:mapp.utils.html`
      <h4>Here Isoline</h4>
    ...
    <div 
      ...
      <div style="display: grid; grid-row-gap: 5px;">
        ${t}
        ${i}`})};var we=new XMLSerializer,be=e=>{if(e.svg||e.type||e.layers){let t=e.layers&&Array.isArray(e.layers)&&e.layers.map(a=>`url(${a.svg})`).reverse().join(",")||`url(${e.svg||e.url||mapp.utils.svgSymbols[e.type](e)})`,i=`
      background-position: center;
      background-repeat: no-repeat;
      background-size: contain;
      ...
      <svg height=${e.height} width=${e.width}>
        <path d=${t}
          ...
          stroke-width=${e.strokeWidth||1}/>`,a=`data:image/svg+xml,${encodeURIComponent(we.serializeToString(i))}`,l=`
      background-position: center;
      background-repeat: no-repeat;
      background-size: contain;
      width: ${e.width}px;
      ...
      <svg height=${e.height} width=${e.width}>
        <rect
          ...
          stroke=${e.strokeColor}
          stroke-width=${e.strokeWidth||1}>`,i=`data:image/svg+xml,${encodeURIComponent(we.serializeToString(t))}`,a=`
      background-position: center;
      background-repeat: no-repeat;
      background-size: contain;
      width: ${e.width}px;
      height: ${e.height}px;
      background-image: url(${i});`;return mapp.utils.html`<div style=${a}>`}};var _e=e=>{return mapp.utils.html.node`
    <div
      role="group"
      ...
      <input id="a"
        ...
        value=${e.val}
        step=${e.step||1}
        oninput=${t}>`;function t(i){i.target.closest(".input-range").style.setProperty(`--${i.target.id}`,i.target.value),i.target.closest(".input-range").querySelectorAll("input").forEach(a=>{a.id==i.target.id&&a!=i.target&&(a.value=i.target.value)}),e.callback&&e.callback(i)}};var ye=e=>{let t=mapp.utils.html.node`
    <div
      role="group"
      ...
      <input id="b" type="range"
        ...
        step=${e.step||1}
        value=${e.val_b}
        oninput=${i}/>`;function i(a){t.style.setProperty(`--${a.target.id}`,a.target.value),t.querySelectorAll("input").forEach(l=>{l.id==a.target.id&&l!=a.target&&(l.value=a.target.value)}),a.target.id==="a"&&typeof e.callback_a=="function"&&e.callback_a(a),a.target.id==="b"&&typeof e.callback_b=="function"&&e.callback_a(b)}return t};var $e={viewport:dt,download_json:ct,download_csv:pt};function dt(e){return mapp.utils.html`
    <button
    ...
    <button
      class="flat"
      onclick=${()=>{e.Tabulator.download("csv",`${e.title||"table"}.csv`)}}>Download as CSV`}var ke={card:de,chkbox:ce,contextMenu:pe,drawer:me,drawing:ue,dropdown:he,dropdown_multi:ge,isoline_params_here:fe,isoline_params_mapbox:ve,legendIcon:be,slider:_e,slider_ab:ye,toolbar_el:$e};var y=null;async function mt(){if(window.Chart){y=window.Chart;return}let e=await import("https://cdn.skypack.dev/chart.js@3.7.0");e.Chart.register(...e.registerables);let t=await import("https://cdn.skypack.dev/chartjs-plugin-datalabels@2.0.0");e.Chart.register(t.default);let i=await import("https://cdn.skypack.dev/chartjs-plugin-annotation@1.3.0");e.Chart.register(i.default),y=e.Chart}async function xe(e,t){return y||await mt(),new y(e,t)}var $=null;async function ut(){if(window.Tabulator){$=window.Tabulator;return}$=(await import("https://cdn.skypack.dev/pin/tabulator-tables@v5.0.10-xDu7yyLVhsxCKJbN5vfH/mode=imports/optimized/tabulator-tables.js")).TabulatorFull}async function Le(){return $||await ut(),new $(...arguments)}var u={idle:600},Ee=e=>{Object.assign(u,e),u.idle!==0&&(window.onload=f,window.onmousemove=f,window.onmousedown=f,window.ontouchstart=f,window.onclick=f,window.onkeypress=f,f(),C())};function f(){u.locked||(u.timeout&&clearTimeout(u.timeout),u.timeout=setTimeout(Se,u.idle*1e3))}function Se(){u.locked=!0,u.renew&&clearTimeout(u.renew);let e=new XMLHttpRequest;e.open("GET",`${u.host}/api/user/cookie?destroy=true`),e.send();let t=mapp.utils.html.node`
    <form
      class="login"
