mapp.utils.merge(mapp.dictionaries,{en:{layer_zoom_to_extent:"Zoom to filtered layer extent",layer_visibility:"Toggle visibility"},de:{layer_zoom_to_extent:"Zoom zum Ausma\xDF des gefilterten Datensatzes",layer_visibility:"Umschalten der Ansicht"},cn:{layer_zoom_to_extent:"\u7F29\u653E\u81F3\u76F8\u5E94\u7B5B\u9009\u8303\u56F4",layer_visibility:"\u5207\u6362\u53EF\u89C1\u6027"},pl:{layer_zoom_to_extent:"Poka\u017C zasi\u0119g warstwy",layer_visibility:"Widoczno\u015B\u0107"},ko:{layer_zoom_to_extent:"\uD544\uD130\uB41C \uB808\uC774\uC5B4\uD06C\uAE30\uC5D0 \uC90C(zoom)",layer_visibility:"\uD1A0\uAE00 \uAC00\uC2DC\uC131"},fr:{layer_zoom_to_extent:"Zoom sur l'\xE9tendue de la couche",layer_visibility:"Changer la visiblit\xE9"},ja:{layer_zoom_to_extent:"\u30D5\u30A3\u30EB\u30BF\u30FC\u3055\u308C\u305F\u30EC\u30A4\u30E4\u30FC\u7BC4\u56F2\u3092\u30BA\u30FC\u30E0\u306B",layer_visibility:"\u8868\u793A\u5207\u66FF"}});var _=e=>{let t=e.filter.zoomToExtent&&mapp.utils.html`
  ...
  <div class="dropdown active">
    ...
    <div 
      ...
      <div 
        data-id="cluster"
        class="contents">
        ${a}${l}`)}return e.style.legend=mapp.utils.html.node`<div class="grid">${i}`,e.style.legend};var x=e=>{let t=e.style.theme,i=[];return t.cat_arr.forEach(a=>{let l=Object.assign({},e.style.default,a.style&&a.style.icon||a.style||a),o=mapp.utils.html`
      <div
        style="height: 24px; width: 24px; grid-column: 1;">
        ${mapp.ui.elements.legendIcon(Object.assign({width:24,height:24},l))}`,r=mapp.utils.html`
      ...
      <div 
        data-id=${a.value}
        class="contents">
        ${o}${r}`)}),e.style.legend=mapp.utils.html.node`<div class="grid">${i}`,e.style.legend};var L=e=>{let t=[];t.push(mapp.ui.elements.dropdown({entries:Object.entries(e.grid_fields).map(o=>({title:o[0],options:o[1]})),callback:(o,r)=>{e.grid_size=r.options,e.reload()}}));let i=e.style.range.length,a=new XMLSerializer,l=mapp.utils.html`
  <div
    class="grid"
  ...
  <div
    ...
    <div
      class="val"
      style="${`${e.css_val||""}`}">
      ${t}`};var ce={key:tt,boolean:Z,dataview:X,date:S,datetime:S,defaults:Q,documents:Y,geometry:ee,html:z,images:te,integer:j,isoline_here:C,isoline_mapbox:C,json:ae,mvt_clone:ie,numeric:j,pin:le,report:oe,streetview:re,tab:ne,text:se,textarea:z,time:de};function tt(e){return mapp.utils.html.node`
  <div class="layer-key">
    <span>
  ...
  <button 
    ...
    <button
      class="flat"
      onclick=${()=>{e.Tabulator.download("csv",`${e.title||"table"}.csv`)}}>Download as CSV`}var Ce={card:he,chkbox:fe,contextMenu:ge,drawer:ve,drawing:be,dropdown:we,dropdown_multi:_e,isoline_params_here:ye,isoline_params_mapbox:$e,legendIcon:xe,slider:Le,slider_ab:Ee,toolbar_el:Se};var T,v=null;async function je(e,t){return v?new v(e,t):(T||(T=new Promise(async i=>{if(window.Chart){v=window.Chart,i();return}Promise.all([import("https://cdn.skypack.dev/chart.js@3.7.1"),import("https://cdn.skypack.dev/chartjs-plugin-datalabels@2.0.0"),import("https://cdn.skypack.dev/chartjs-plugin-annotation@1.4.0")]).then(a=>{a[0].Chart.register(...a[0].registerables),a[0].Chart.register(a[1].default),a[0].Chart.register(a[2].default),v=a[0].Chart,i()}).catch(a=>{console.error(a.message),alert("Failed to load Chart.js library. Please reload the browser.")})})),await T,new v(e,t))}var O,b=null;async function ze(){return b?new b(...arguments):(O||(O=new Promise(async e=>{if(window.Tabulator){b=window.Tabulator,e();return}Promise.all([import("https://cdn.skypack.dev/pin/tabulator-tables@v5.3.0-3P9dSwNRL29tgc0Vmk59/mode=imports,min/optimized/tabulator-tables.js")]).then(t=>{b=t[0].TabulatorFull,e()}).catch(t=>{console.error(t.message),alert("Failed to load Tabulator library. Please reload the browser.")})})),await O,new b(...arguments))}var Te={headerFilter:{like:wt,numeric:_t}};function wt(e){return(t,i,a,l,o)=>{let r=t.getColumn().getField();function n(s){let c=e.Tabulator.getFilters().find(d=>d.field===r&&d.type==">=");c&&(e.Tabulator.removeFilter(...Object.values(c)),o.layer&&delete e.layer.filter.current[r]),s.target.value.length&&(e.Tabulator.setFilter([{field:r,type:"like",value:s.target.value}]),o.layer&&(e.layer.filter.current[r]={[o.type]:encodeURIComponent(s.target.value)},e.layer.reload(),e.update()))}return mapp.utils.html.node`<span>
      <input
        type="text"
        placeholder="Filter"
        oninput=${n}
