mapp.utils.merge(mapp.dictionaries,{en:{layer_zoom_to_extent:"Zoom to filtered layer extent",layer_visibility:"Toggle visibility"},de:{layer_zoom_to_extent:"Zoom zum Ausma\xDF des gefilterten Datensatzes",layer_visibility:"Umschalten der Ansicht"},cn:{layer_zoom_to_extent:"\u7F29\u653E\u81F3\u76F8\u5E94\u7B5B\u9009\u8303\u56F4",layer_visibility:"\u5207\u6362\u53EF\u89C1\u6027"},pl:{layer_zoom_to_extent:"Poka\u017C zasi\u0119g warstwy",layer_visibility:"Widoczno\u015B\u0107"},ko:{layer_zoom_to_extent:"\uD544\uD130\uB41C \uB808\uC774\uC5B4\uD06C\uAE30\uC5D0 \uC90C(zoom)",layer_visibility:"\uD1A0\uAE00 \uAC00\uC2DC\uC131"},fr:{layer_zoom_to_extent:"Zoom sur l'\xE9tendue de la couche",layer_visibility:"Changer la visiblit\xE9"},ja:{layer_zoom_to_extent:"\u30D5\u30A3\u30EB\u30BF\u30FC\u3055\u308C\u305F\u30EC\u30A4\u30E4\u30FC\u7BC4\u56F2\u3092\u30BA\u30FC\u30E0\u306B",layer_visibility:"\u8868\u793A\u5207\u66FF"}});var w=e=>{let t=mapp.utils.html`
  ...
  <input
    ...
    <button
      ...
      <div class="mask-icon expander"></div>`,content:mapp.utils.html`${t}`})};var P=e=>{let t=Object.keys(e.reports).map(a=>{let i=e.reports[a];i.key=a,i.host=e.mapview.host;let o=`${i.host}/view?${mapp.utils.paramString({template:i.template,lat:mapp.hooks.current?.lat,lng:mapp.hooks.current?.lng,z:mapp.hooks.current?.z})}`;return mapp.utils.html`
      <a
        class="link-with-img"
        target="_blank"
        href="${o}">
    ...
    <div
      ...
      <div
        ...
        onclick=${s=>{s.target.classList.toggle("disabled"),s.target.classList.contains("disabled")?(e.filter.current[t.field]||(e.filter.current[t.field]={}),e.filter.current[t.field].ni||(e.filter.current[t.field].ni=[]),e.filter.current[t.field].ni.push(a[1].keys||a[0]),e.filter.current[t.field].ni=e.filter.current[t.field].ni.flat()):(Array.isArray(a[1].keys)?a[1].keys.forEach(m=>{e.filter.current[t.field].ni.splice(e.filter.current[t.field].ni.indexOf(m),1)}):e.filter.current[t.field].ni.splice(e.filter.current[t.field].ni.indexOf(a[0]),1),e.filter.current[t.field].ni.length||delete e.filter.current[t.field]),e.reload()}}>${a[1].label||a[0]}`;l.push(mapp.utils.html`
    <div 
      data-id=${a[0]}
      class="contents">
      ...
      <div 
        data-id="cluster"
        class="contents">
        ${a}${i}`)}return e.style.legend=mapp.utils.html.node`<div class="grid">${l}`,e.style.legend};var G=e=>{let t=e.style.theme,l=[];return t.cat_arr.forEach(a=>{let i=Object.assign({},e.style.default,a.style&&a.style.icon||a.style||a),o=mapp.utils.html`
      <div
        style="height: 24px; width: 24px;">
        ${mapp.ui.elements.legendIcon(Object.assign({width:24,height:24},i))}`,n=mapp.utils.html`
  ...
  <div
    ...
    <button
      ...
      onclick = ${a=>{e.remove()}}>`),e.view=mapp.ui.elements.drawer({class:"location-view expanded",header:t});let l=e.view.appendChild(mapp.ui.locations.infoj(e));e.view.querySelector(".header").style.borderBottom=`3px solid ${e.record.colour}`,e.view.addEventListener("valChange",a=>{a.detail.value!=a.detail.newValue?a.detail.node.classList.add("val-changed"):(delete a.detail.newValue,a.detail.node.classList.remove("val-changed")),e.view.querySelector(".cloud-upload").style.display=e.infoj.some(i=>typeof i.newValue<"u")&&"inline-block"||"none"}),e.view.addEventListener("updateInfo",()=>{l.remove(),e.view.querySelector(".cloud-upload").style.display="none",e.view.classList.remove("disabled"),l=mapp.ui.locations.infoj(e),e.view.appendChild(l)})};mapp.utils.merge(mapp.dictionaries,{en:{location_clear_all:"Clear locations"},de:{location_clear_all:"Entferne Auswahl"},cn:{location_clear_all:"\uBAA8\uB4E0 \uC704\uCE58 \uC81C\uAC70"},pl:{location_clear_all:"Wyczy\u015B\u0107 selekcje"},ko:{location_clear_all:"\u6E05\u9664\u6240\u6709\u5730\u70B9"},fr:{location_clear_all:"Des\xE9lectionner tous les lieux."},ja:{location_clear_all:"\u5168\u30ED\u30B1\u30FC\u30B7\u30E7\u30F3\u3092\u30AF\u30EA\u30A2"}});var x=[{symbol:"A",colour:"#2E6F9E"},{symbol:"B",colour:"#EC602D"},{symbol:"C",colour:"#5B8C5A"},{symbol:"D",colour:"#B84444"},{symbol:"E",colour:"#514E7E"},{symbol:"F",colour:"#E7C547"},{symbol:"G",colour:"#368F8B"},{symbol:"H",colour:"#841C47"},{symbol:"I",colour:"#61A2D1"},{symbol:"J",colour:"#37327F"}],U=e=>{if(!e.mapview||!e.target)return;let t={node:e.target,mapview:e.mapview},l=t.node.appendChild(mapp.utils.html.node`
    <button 
      style="display: none;"
      class="tab-display bold primary-colour text-shadow"
      onclick=${a=>{Object.values(t.mapview.locations).forEach(i=>i.remove())}}>
      ${mapp.dictionary.location_clear_all}`);return e.mapview.locations=new Proxy(e.mapview.locations,{set:function(a,i,o){let n=x.find(s=>!s.hook);return n.hook=o.hook,o.record=n,o.style={strokeColor:n.colour,fillColor:n.colour,fillOpacity:.2},o.Style=mapp.utils.style([{strokeColor:"#000",strokeOpacity:.1,strokeWidth:8},{strokeColor:"#000",strokeOpacity:.1,strokeWidth:6},{strokeColor:"#000",strokeOpacity:.1,strokeWidth:4},{strokeColor:o.style.strokeColor||"#000",strokeWidth:2,fillColor:o.style.fillColor||o.style.strokeColor||"#fff",fillOpacity:o.style.fillOpacity||.2}]),o.pinStyle=mapp.utils.style({icon:{type:"markerLetter",letter:n.symbol,color:o.style.strokeColor,scale:3,anchor:[.5,1]}}),Reflect.set(...arguments),mapp.ui.locations.view(o),Object.values(t.node.children).forEach(s=>s.classList.remove("expanded")),t.node.insertBefore(o.view,t.node.firstChild.nextSibling.nextSibling),o.view.dispatchEvent(new Event("addLocationView")),document.querySelector("[data-id=locations]").style.display="block",l.style.display="block",document.querySelector("[data-id=locations]").click(),!0},deleteProperty:function(a,i){Reflect.deleteProperty(...arguments);let o=x.find(n=>n.hook===i);return delete o.hook,setTimeout(()=>{x.filter(n=>n.hook).length||(document.querySelector("[data-id=layers]").click(),l.style.display="none")},300),!0}}),t};var J=(e,t)=>{if(!e.infoj)return;let l=mapp.utils.html.node`<div class="location-view-grid">`,a={};for(let i of t||e.infoj){if(e.view&&e.view.classList.contains("disabled"))break;if(i.listview=l,i.type=i.type||"text",i.skipEntry||i.skipFalsyValue&&!i.value&&!i.edit||i.skipUndefinedValue&&typeof i.value>"u"&&!i.edit||i.skipNullValue&&i.value===null&&!i.edit)continue;i.nullValue&&i.value===null&&!i.defaults&&!i.edit&&(i.value=i.nullValue),i.group&&(a[i.group]||(a[i.group]=i.listview.appendChild(mapp.ui.elements.drawer({class:`lv-1 group ${i.expanded&&"expanded"||""}`,header:mapp.utils.html`
              <h3>${i.group}</h3>
  ...
  <div
    class="val"
    style="${`${e.css_val||""}`}">${t}`;return!e.edit&&e.type==="html"&&(l.innerHTML=e.value||""),l};var ne={key:We,boolean:W,dataview:Z,date:L,datetime:L,defaults:X,documents:K,geometry:Q,html:C,images:Y,integer:E,isoline_here:S,isoline_mapbox:S,json:ee,numeric:E,pin:te,report:ae,streetview:ie,tab:le,text:oe,textarea:C};function We(e){return mapp.utils.html.node`
  <div class="layer-key">
    <span>
      ${e.location.layer.name}`}var re={view:H,listview:U,infoj:J,entries:ne};var se=async e=>{if(e.target=typeof e.target=="string"&&document.getElementById(e.target)||e.target,e.target=e.target instanceof HTMLElement&&e.target||mapp.utils.html.node`
      <div
