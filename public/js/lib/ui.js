mapp.utils.merge(mapp.dictionaries,{en:{layer_zoom_to_extent:"Zoom to filtered layer extent",layer_visibility:"Toggle visibility"},de:{layer_zoom_to_extent:"Zoom zum Ausma\xDF des gefilterten Datensatzes",layer_visibility:"Umschalten der Ansicht"},cn:{layer_zoom_to_extent:"\u7F29\u653E\u81F3\u76F8\u5E94\u7B5B\u9009\u8303\u56F4",layer_visibility:"\u5207\u6362\u53EF\u89C1\u6027"},pl:{layer_zoom_to_extent:"Poka\u017C zasi\u0119g warstwy",layer_visibility:"Widoczno\u015B\u0107"},ko:{layer_zoom_to_extent:"\uD544\uD130\uB41C \uB808\uC774\uC5B4\uD06C\uAE30\uC5D0 \uC90C(zoom)",layer_visibility:"\uD1A0\uAE00 \uAC00\uC2DC\uC131"},fr:{layer_zoom_to_extent:"Zoom sur l'\xE9tendue de la couche",layer_visibility:"Changer la visiblit\xE9"},ja:{layer_zoom_to_extent:"\u30D5\u30A3\u30EB\u30BF\u30FC\u3055\u308C\u305F\u30EC\u30A4\u30E4\u30FC\u7BC4\u56F2\u3092\u30BA\u30FC\u30E0\u306B",layer_visibility:"\u8868\u793A\u5207\u66FF"}});var _=e=>{let t=mapp.utils.html`
    ...
    ${i}`,l=Object.keys(e).map(o=>mapp.ui.layers.panels[o]&&mapp.ui.layers.panels[o](e)).filter(o=>typeof o<"u");if(e.meta){let o=mapp.utils.html.node`<p class="meta">`;o.innerHTML=e.meta,l.unshift(o)}e.view=mapp.ui.elements.drawer({data_id:"layer-drawer",class:"layer-view raised",header:a,content:l}),e.showCallbacks.push(()=>{i.classList.add("on"),e.tabs?.forEach(o=>o.display&&o.show&&o.show())}),e.hideCallbacks.push(()=>{i.classList.remove("on"),e.tabs?.forEach(o=>o.remove())}),e.mapview.Map.getTargetElement().addEventListener("changeEnd",()=>{if(!!e.tables){if(e.tableCurrent()===null)return e.view.classList.add("disabled");e.view.classList.remove("disabled")}}),e.view.children.length<=1&&e.view.classList.add("empty")};mapp.utils.merge(mapp.dictionaries,{en:{layer_group_hide_layers:"Toggle all layers in group"},de:{layer_group_hide_layers:"Umschalten aller Ebenen in Gruppe"},cn:{layer_group_hide_layers:"\u9690\u85CF\u56FE\u5C42"},pl:{layer_group_hide_layers:"Ukryj warstwy z tej grupy"},ko:{layer_group_hide_layers:"\uADF8\uB8F9\uC5D0\uC11C \uB808\uC774\uC5B4 \uC228\uAE30\uAE30"},fr:{layer_group_hide_layers:"Cacher les couches du groupe"},ja:{layer_group_hide_layers:"\u30B0\u30EB\u30FC\u30D7\u304B\u3089\u30EC\u30A4\u30E4\u30FC\u3092\u96A0\u3059"}});function j(e){if(!e.mapview||!e.target)return;let t={node:e.target,groups:{}};Object.values(e.mapview.layers).forEach(l=>i(l));function i(l){if(!l.hidden){if(_(l),!l.group){t.node.appendChild(l.view),t.node.dispatchEvent(new CustomEvent("addLayerView",{detail:l}));return}t.groups[l.group]||a(l),t.groups[l.group].addLayer(l),t.node.dispatchEvent(new CustomEvent("addLayerView",{detail:l}))}}function a(l){let o={list:[]};t.groups[l.group]=o,o.meta=mapp.utils.html.node`<div class="meta">`,o.drawer=mapp.ui.elements.drawer({data_id:"layer-drawer",class:"layer-group",header:mapp.utils.html`
        <h2>${l.group}</h2>
        <div class="mask-icon expander"></div>`,content:o.meta}),t.node.appendChild(o.drawer),o.addLayer=n=>{if(n.group=o,n.groupmeta){let r=o.meta.appendChild(mapp.utils.html.node`<div>`);r.innerHTML=n.groupmeta}o.list.push(n),o.drawer.appendChild(n.view)}}}var D={like:z,match:z,numeric:O,integer:O,in:Ve,date:T,datetime:T,boolean:Ge},$;function y(e,t){clearTimeout($),e.view.querySelector("[data-id=zoomToExtent]").disabled=!1,$=setTimeout(()=>{$=null,e.reload(),e.mapview.Map.getTargetElement().dispatchEvent(new Event("changeEnd"))},500)}function z(e,t){return mapp.utils.html.node`
  <input
    type="text"
  ...
  <div class="dropdown active">
    ...
    <div 
      ...
      <div 
        data-id=${a.value}
        class="contents">
        ${o}${n}`)}),e.style.legend=mapp.utils.html.node`<div class="grid">${i}`,e.style.legend};var V=e=>{let t=[];t.push(mapp.ui.elements.dropdown({entries:Object.entries(e.grid_fields).map(o=>({title:o[0],options:o[1]})),callback:(o,n)=>{e.grid_size=n.options,e.reload()}}));let i=e.style.range.length,a=new XMLSerializer,l=mapp.utils.html`
  <div
    class="grid"
  ...
  <div
    ...
    <div
      class="val"
      style="${`${e.css_val||""}`}">
      ${e.prefix}${t}${e.suffix}`};var Q=e=>{let t=e.value.map(n=>mapp.utils.html`
    ...
    <div
      class="images-grid">${t}`;return a;async function l(n){e.location.view?.classList.add("disabled");let r=new FileReader,s=n.target.files[0];!s||(r.onload=c=>{let d=new Image;d.onload=async()=>{let m=mapp.utils.html.node`<canvas>`,u=1024,h=d.width,g=d.height;h>g&&h>u?(g*=u/h,h=u):g>u&&(h*=u/g,g=u),m.width=h,m.height=g,m.getContext("2d").drawImage(d,0,0,h,g);let Pe=m.toDataURL("image/jpeg",.5),b=await mapp.utils.xhr({method:"POST",requestHeader:{"Content-Type":"application/octet-stream"},url:`${e.location.layer.mapview.host}/api/provider/cloudinary?${mapp.utils.paramString({public_id:s.name.replace(/.*\//,"").replace(/\.([\w-]{3})/,""),resource_type:"image"})}`,body:mapp.utils.dataURLtoBlob(Pe)});await mapp.utils.xhr(`${e.location.layer.mapview.host}/api/query?`+mapp.utils.paramString({template:"set_field_array",locale:e.location.layer.mapview.locale.key,layer:e.location.layer.key,table:e.location.table,qID:e.location.layer.qID,id:e.location.id,action:"append",field:e.field,value:b.secure_url}));let Ae=mapp.utils.html.node`
          <div>
            <img
              src=${b.secure_url}
              onclick=${mapp.ui.utils.imagePreview}>
  ...
  <div
    ...
    <div
      class="val"
      style="${`${e.css_val||""}`}">
      ${t}`};var de={key:Qe,boolean:Z,dataview:K,date:x,datetime:x,defaults:X,documents:Q,geometry:Y,html:S,images:ee,integer:L,isoline_here:k,isoline_mapbox:k,json:te,mvt_clone:ae,numeric:L,pin:ie,report:le,streetview:oe,tab:ne,text:re,textarea:S,time:se};function Qe(e){return mapp.utils.html.node`
  <div class="layer-key">
    <span>
  ...
  <input
    type="text"
    placeholder="Filter"
    oninput=${n=>{let r=t.getColumn().getField();if(!o.layer){e.Tabulator.setFilter([{field:r,type:"like",value:n.target.value}]);return}n.target.value.length?e.layer.filter.current[r]={[o.type]:encodeURIComponent(n.target.value)}:delete e.layer.filter.current[r],e.layer.reload(),e.update()}}>`}function it(e,t,i,a,l,o){return mapp.utils.html.node`<span>
    <input
      style="width: 50%"
      type="number"
      placeholder="Min"
      oninput=${n=>{let r=t.getColumn().getField();n.target.value.length?e.layer.filter.current[r]=Object.assign(e.layer.filter.current[r]||{},{gte:Number(n.target.value)}):delete e.layer.filter.current[r],e.layer.reload(),e.update()}}></input>
    <input
      style="width: 50%"
      type="number"
      placeholder="Max"
      oninput=${n=>{let r=t.getColumn().getField();n.target.value.length?e.layer.filter.current[r]=Object.assign(e.layer.filter.current[r]||{},{lte:Number(n.target.value)}):delete e.layer.filter.current[r],e.layer.reload(),e.update()}}></input>`}var me=e=>{!e.node||(e.tabs=e.node.appendChild(mapp.utils.html.node`<div class="tabs">`),e.panel=e.node.appendChild(mapp.utils.html.node`<div class="panel">`),e.id&&e.node.setAttribute("data-id",e.id),e.addTab=lt,e.node.addEventListener("addTab",t=>e.addTab(t.detail)))};function lt(e){let t=this;e.tab=mapp.utils.html.node`
    <div class="tab">
      <div
        class="header"
        style="${e.tab_style||""}"
  ...
  <button 
    ...
    <div
      ...
      <input id="b" type="range"
        ...
        step=${e.step||1}
        value=${e.val_b}
        oninput=${i}/>`;function i(a){a.target.value=a.target.value>e.max?e.max:a.target.value,a.target.value=a.target.value<e.min?e.min:a.target.value,t.style.setProperty(`--${a.target.id}`,a.target.value),t.querySelectorAll("input").forEach(l=>{l.id==a.target.id&&l!=a.target&&(l.value=a.target.value)}),a.target.id==="a"&&typeof e.callback_a=="function"&&e.callback_a(a),a.target.id==="b"&&typeof e.callback_b=="function"&&e.callback_b(a)}return t};var Se={viewport:ft,download_json:vt,download_csv:wt};function ft(e){return mapp.utils.html`
    <button
    ...
    <button
      class="flat"
      onclick=${()=>{e.Tabulator.download("csv",`${e.title||"table"}.csv`)}}>Download as CSV`}var Ee={card:ue,chkbox:he,contextMenu:ge,drawer:fe,drawing:ve,dropdown:we,dropdown_multi:be,isoline_params_here:_e,isoline_params_mapbox:ye,legendIcon:xe,slider:ke,slider_ab:Le,toolbar_el:Se};var E,v=null;async function Ce(e,t){return v?new v(e,t):(E||(E=new Promise(async i=>{if(window.Chart){v=window.Chart,i();return}Promise.all([import("https://cdn.skypack.dev/chart.js@3.7.1"),import("https://cdn.skypack.dev/chartjs-plugin-datalabels@2.0.0"),import("https://cdn.skypack.dev/chartjs-plugin-annotation@1.4.0")]).then(a=>{a[0].Chart.register(...a[0].registerables),a[0].Chart.register(a[1].default),a[0].Chart.register(a[2].default),v=a[0].Chart,i()}).catch(a=>{console.error(a.message),alert("Failed to load Chart.js library. Please reload the browser.")})})),await E,new v(e,t))}var C,w=null;async function je(){return w?new w(...arguments):(C||(C=new Promise(async e=>{if(window.Tabulator){w=window.Tabulator,e();return}Promise.all([import("https://cdn.skypack.dev/pin/tabulator-tables@v5.3.0-3P9dSwNRL29tgc0Vmk59/mode=imports,min/optimized/tabulator-tables.js")]).then(t=>{w=t[0].TabulatorFull,e()}).catch(t=>{console.error(t.message),alert("Failed to load Chart.js library. Please reload the browser.")})})),await C,new w(...arguments))}var p={idle:600},Te=e=>{Object.assign(p,e),p.idle!==0&&(window.onload=f,window.onmousemove=f,window.onmousedown=f,window.ontouchstart=f,window.onclick=f,window.onkeypress=f,f(),Oe())};function f(){p.locked||(p.timeout&&clearTimeout(p.timeout),p.timeout=setTimeout(ze,p.idle*1e3))}function ze(){p.locked=!0,p.renew&&clearTimeout(p.renew);let e=new XMLHttpRequest;e.open("GET",`${p.host}/api/user/cookie?destroy=true`),e.onload=t=>location.reload(),e.send()}function Oe(){p.renew=setTimeout(e,(p.idle-20)*1e3);function e(){let t=new XMLHttpRequest;t.open("GET",`${p.host}/api/user/cookie?renew=true`),t.onload=i=>{if(i.target.status===401)return ze();Oe()},t.send()}}var De=e=>{document.body.append(mapp.utils.html.node`
    <div class="interface-mask">
      <div class="bg-image" style=${`background-image:url(${e.target.src})`}>
      <button class="btn-close mask-icon close"
        onclick=${t=>t.target.parentElement.parentElement.remove()}>`)};var qe=e=>{e.target.addEventListener("mousedown",i=>{i.preventDefault(),document.body.style.cursor="grabbing",window.addEventListener("mousemove",e.resizeEvent),window.addEventListener("mouseup",t)}),e.target.addEventListener("touchstart",i=>{i.preventDefault(),window.addEventListener("touchmove",e.resizeEvent),window.addEventListener("touchend",t)},{passive:!0});function t(){document.body.style.cursor="auto",window.removeEventListener("mousemove",e.resizeEvent),window.removeEventListener("touchmove",e.resizeEvent),window.removeEventListener("mouseup",t),window.removeEventListener("touchend",t)}};var Me={Chart:Ce,Tabulator:je,idleLogout:Te,imagePreview:De,resizeHandler:qe};var Fe=e=>{let t=Object.assign({input:e.target.querySelector("input"),result:e.target.querySelector("ul"),sources:{glx:a,mapbox:o,google:l}},e);t.input.placeholder=t.placeholder||"",t.input.oninput=r=>{t.xhr&&t.xhr.abort(),t.result.innerHTML="",r.target.value.length>0&&i(r.target.value)},t.input.addEventListener("keydown",r=>{(r.keyCode||r.charCode)===13&&t.result.querySelector("li").click()});function i(r){let s=r.split(",").map(parseFloat);if(s.length===2&&s.every(c=>!isNaN(c))){t.result.appendChild(mapp.utils.html.node`
      <li 
