mapp.utils.merge(mapp.dictionaries,{en:{layer_zoom_to_extent:"Zoom to filtered layer extent",layer_visibility:"Toggle visibility"},de:{layer_zoom_to_extent:"Zoom zum Ausma\xDF des gefilterten Datensatzes",layer_visibility:"Umschalten der Ansicht"},cn:{layer_zoom_to_extent:"\u7F29\u653E\u81F3\u76F8\u5E94\u7B5B\u9009\u8303\u56F4",layer_visibility:"\u5207\u6362\u53EF\u89C1\u6027"},pl:{layer_zoom_to_extent:"Poka\u017C zasi\u0119g warstwy",layer_visibility:"Widoczno\u015B\u0107"},ko:{layer_zoom_to_extent:"\uD544\uD130\uB41C \uB808\uC774\uC5B4\uD06C\uAE30\uC5D0 \uC90C(zoom)",layer_visibility:"\uD1A0\uAE00 \uAC00\uC2DC\uC131"},fr:{layer_zoom_to_extent:"Zoom sur l'\xE9tendue de la couche",layer_visibility:"Changer la visiblit\xE9"},ja:{layer_zoom_to_extent:"\u30D5\u30A3\u30EB\u30BF\u30FC\u3055\u308C\u305F\u30EC\u30A4\u30E4\u30FC\u7BC4\u56F2\u3092\u30BA\u30FC\u30E0\u306B",layer_visibility:"\u8868\u793A\u5207\u66FF"}});var _=e=>{let t=mapp.utils.html`
  ...
  <input
    ...
    <div
      ...
      <div
        ...
        onclick=${s=>{s.target.classList.toggle("disabled"),s.target.classList.contains("disabled")?(e.filter.current[t.field]||(e.filter.current[t.field]={}),e.filter.current[t.field].ni||(e.filter.current[t.field].ni=[]),e.filter.current[t.field].ni.push(a[1].keys||a[0]),e.filter.current[t.field].ni=e.filter.current[t.field].ni.flat()):(Array.isArray(a[1].keys)?a[1].keys.forEach(r=>{e.filter.current[t.field].ni.splice(e.filter.current[t.field].ni.indexOf(r),1)}):e.filter.current[t.field].ni.splice(e.filter.current[t.field].ni.indexOf(a[0]),1),e.filter.current[t.field].ni.length||delete e.filter.current[t.field]),e.reload()}}>${a[1].label||a[0]}`;i.push(mapp.utils.html`
    <div 
      data-id=${a[0]}
      class="contents">
    ...
    <div data-id="size-icons" class="contents">
      ${e.style.range.map((o,n)=>{let s=mapp.utils.svg.node`<svg height=50 width=50>
          ...
          <circle
            ...
            cx=25
            cy=25
            r='${23/i*(n+1)}'/>`,r=`data:image/svg+xml,${encodeURIComponent(a.serializeToString(s))}`,d=`
          grid-row:2;
          grid-column:${n+1};
          ...
          width: 100%;
          height: 100%;
          background-image: url(${r});`;return mapp.utils.html`<div style=${d}>`})}
    </div>
  ...
  <div
    ...
    <button 
      ...
      ${mapp.dictionary.location_clear_all}`);return e.mapview.locations=new Proxy(e.mapview.locations,{set:function(a,l,o){let n=U.find(s=>!s.hook);return n.hook=o.hook,o.record=n,o.style={strokeColor:n.colour,fillColor:n.colour,fillOpacity:.2},o.Style=mapp.utils.style([{strokeColor:"#000",strokeOpacity:.1,strokeWidth:8},{strokeColor:"#000",strokeOpacity:.1,strokeWidth:6},{strokeColor:"#000",strokeOpacity:.1,strokeWidth:4},{strokeColor:o.style.strokeColor||"#000",strokeWidth:2,fillColor:o.style.fillColor||o.style.strokeColor||"#fff",fillOpacity:o.style.fillOpacity||.2}]),o.pinStyle=mapp.utils.style({icon:{type:"markerLetter",letter:n.symbol,color:o.style.strokeColor,scale:3,anchor:[.5,1]}}),Reflect.set(...arguments),mapp.ui.locations.view(o),Object.values(t.node.children).forEach(s=>s.classList.remove("expanded")),t.node.insertBefore(o.view,i.nextSibling),o.view.dispatchEvent(new Event("addLocationView")),document.querySelector("[data-id=locations]").style.display="block",i.style.display="block",document.querySelector("[data-id=locations]").click(),!0},deleteProperty:function(a,l){Reflect.deleteProperty(...arguments);let o=U.find(n=>n.hook===l);return o&&delete o.hook,setTimeout(()=>{if(!document.querySelectorAll("#locations > .location-view").length){document.querySelector("[data-id=layers]").click(),i.style.display="none";let n=document.querySelector("#locations #gazetteerInput");n?n.value="":document.querySelector("[data-id=locations]").style.display="none"}},300),!0}}),t};var J=(e,t)=>{if(!e.infoj)return;let i=mapp.utils.html.node`<div class="location-view-grid">`,a={};for(let l of t||e.infoj){if(e.view&&e.view.classList.contains("disabled"))break;if(l.listview=i,l.type=l.type||"text",l.skipEntry||l.skipFalsyValue&&!l.value&&!l.edit||l.skipUndefinedValue&&typeof l.value>"u"&&!l.edit||l.skipNullValue&&l.value===null&&!l.edit)continue;l.nullValue&&l.value===null&&!l.defaults&&!l.edit&&(l.value=l.nullValue),l.group&&(a[l.group]||(a[l.group]=l.listview.appendChild(mapp.ui.elements.drawer({class:`lv-1 group ${l.expanded&&"expanded"||""}`,header:mapp.utils.html`
              <h3>${l.group}</h3>
    ...
    <div class="mask-icon cloud-upload">
      <input
        ...
        <div class="link-with-img">
          ...
          <a target="_blank"
            href=${c}>${u}`;a.insertBefore(h,i),e.location.view?.classList.remove("disabled")},s.readAsDataURL(r),n.target.value="")}async function o(n){if(!confirm("Remove document link?"))return;let s=n.target;mapp.utils.xhr(`${e.location.layer.mapview.host}/api/provider/cloudinary?${mapp.utils.paramString({destroy:!0,public_id:s.dataset.name})}`),await mapp.utils.xhr(`${e.location.layer.mapview.host}/api/query?`+mapp.utils.paramString({template:"set_field_array",locale:e.location.layer.mapview.locale.key,layer:e.location.layer.key,table:e.location.table,qID:e.location.layer.qID,id:e.location.id,action:"remove",field:e.field,value:s.dataset.href})),s.parentNode.remove()}};var Y=e=>{if(e.value=typeof e.value=="string"&&JSON.parse(e.value)||e.value,e.display=e.display&&e.value,!e.value&&!e.edit)return;e.Style=e.Style||typeof e.style=="object"&&mapp.utils.style(Object.assign({},e.location?.style||{},e.style))||e.location.Style,e.show=Ve;let t=mapp.ui.elements.chkbox({label:e.label||"Geometry",data_id:`${e.field}-chkbox`,checked:!!e.display,disabled:!e.value,onchange:o=>{o?e.show():(e.display=!1,e.L&&e.location.layer.mapview.Map.removeLayer(e.L))}});e.display&&e.show();let i={entry:e,layer:e.location.layer,srid:e.srid||e.location.srid,edit:e.edit,mapview:e.location.layer.mapview},a=typeof e.edit=="object"&&Object.keys(e.edit).map(o=>mapp.ui.elements.drawing[o]&&mapp.ui.elements.drawing[o](i,Be)).filter(o=>!!o);e.edit?.geometry&&a.push(mapp.utils.html`
    <button
      ...
      <img src=${n}
        ...
        ${e.edit&&mapp.utils.html.node`
          <button
            ...
            data-name=${n.replace(/.*\//,"").replace(/\.([\w-]{3})/,"")}
            data-src=${n}
            onclick=${s=>o(s)}>`}`),i=mapp.utils.html.node`
    ...
    <div
      class="images-grid">${t}`;return a;async function l(n){e.location.view?.classList.add("disabled");let s=new FileReader,r=n.target.files[0];!r||(s.onload=d=>{let p=new Image;p.onload=async()=>{let c=mapp.utils.html.node`<canvas>`,u=1024,h=p.width,g=p.height;h>g&&h>u?(g*=u/h,h=u):g>u&&(h*=u/g,g=u),c.width=h,c.height=g,c.getContext("2d").drawImage(p,0,0,h,g);let Ie=c.toDataURL("image/jpeg",.5),w=await mapp.utils.xhr({method:"POST",requestHeader:{"Content-Type":"application/octet-stream"},url:`${e.location.layer.mapview.host}/api/provider/cloudinary?${mapp.utils.paramString({public_id:r.name.replace(/.*\//,"").replace(/\.([\w-]{3})/,""),resource_type:"image"})}`,body:mapp.utils.dataURLtoBlob(Ie)});await mapp.utils.xhr(`${e.location.layer.mapview.host}/api/query?`+mapp.utils.paramString({template:"set_field_array",locale:e.location.layer.mapview.locale.key,layer:e.location.layer.key,table:e.location.table,qID:e.location.layer.qID,id:e.location.id,action:"append",field:e.field,value:w.secure_url}));let Fe=mapp.utils.html.node`
          <div>
            <img
              src=${w.secure_url}
              onclick=${mapp.ui.utils.imagePreview}>
              <button
                ...
                onclick=${Pe=>o(Pe)}>`;a.insertBefore(Fe,i),e.location.view?.classList.remove("disabled")},p.src=d.target.result},s.readAsDataURL(r),n.target.value="")}async function o(n){if(!confirm("Remove image?"))return;let s=n.target;mapp.utils.xhr(`${e.location.layer.mapview.host}/api/provider/cloudinary?${mapp.utils.paramString({destroy:!0,public_id:s.dataset.name})}`),await mapp.utils.xhr(`${e.location.layer.mapview.host}/api/query?`+mapp.utils.paramString({template:"set_field_array",locale:e.location.layer.mapview.locale.key,layer:e.location.layer.key,table:e.location.table,qID:e.location.layer.qID,id:e.location.id,action:"remove",field:e.field,value:s.dataset.src})),s.parentNode.remove()}};var Ne={isoline_here:Je,isoline_mapbox:We},L=e=>{e.value=typeof e.value=="string"&&JSON.parse(e.value)||e.value,e.Style=e.Style||typeof e.style=="object"&&mapp.utils.style(e.style)||e.location.Style;let t=mapp.ui.elements.chkbox({label:e.label||"Isoline",checked:!!e.display,onchange:a=>{a?e.show():(e.display=!1,e.L&&e.location.layer.mapview.Map.removeLayer(e.L))}});if(e.show=Ue,e.display&&e.show())return"break";let i=e.style&&mapp.utils.html`
    ${mapp.ui.elements.legendIcon(Object.assign({width:24,height:24},e.style))}`;return mapp.utils.html.node`<div class="flex-spacer">${t}${i}`};function Ue(){if(this.display=!0,this.L){this.location.layer.mapview.Map.removeLayer(this.L),this.location.layer.mapview.Map.addLayer(this.L);return}if(this.value){this.L=this.location.layer.mapview.geoJSON({zIndex:this.zIndex,geometry:this.value,Style:this.Style,dataProjection:"4326"}),this.location.Layers.push(this.L);return}let e=this.location.infoj.find(t=>t.type==="pin");return this.params.latlng=ol.proj.transform(e.value,`EPSG:${e.srid||"3857"}`,"EPSG:4326"),this.location.view?.classList.add("disabled"),Ne[this.type](this),!0}function We(e){let t=Object.assign({profile:"driving",minutes:10},e.params);mapp.utils.xhr(`https://api.mapbox.com/isochrone/v1/mapbox/${t.profile}/${t.latlng[0]},${t.latlng[1]}?${mapp.utils.paramString({contours_minutes:t.minutes,polygons:!0,access_token:t.access_token})}`).then(async i=>{!e.location.remove||!i.features||(e.newValue=i.features[0].geometry,e.location.update())})}function Je(e){let t=Object.assign({"range[type]":"time",minutes:10,reverseDirection:!1,transportMode:"car",optimizeFor:"balanced"},e.params);t["range[values]"]=t.minutes*60,delete t.minutes,t.origin=`${t.latlng[1]},${t.latlng[0]}`,delete t.latlng,mapp.utils.xhr(`${e.location.layer.mapview.host}/api/proxy?url=${encodeURIComponent(`https://isoline.router.hereapi.com/v8/isolines?${mapp.utils.paramString(t)}&{HERE}`)}`).then(i=>{if(!e.location.remove)return;if(!i.isolines)return console.log(i),alert("Failed to process request");let a=mapp.utils.here.decodeIsoline(i.isolines[0].polygons[0].outer);a.polyline.forEach(l=>l.reverse()),a.polyline.push(a.polyline[0]),e.newValue={type:"Polygon",coordinates:[a.polyline]},e.location.update()})}var te=e=>{let t=mapp.utils.html`
    ...
    <div
      class="val"
      style="${`${e.css_val||""}`}">
      ${e.prefix}${i}${e.suffix}`,t};function Ze(e){let t=e.edit.options.find(l=>typeof l=="object"&&Object.values(l)[0]===e.value||l===e.value)||e.value;e.value=t&&typeof t=="object"&&Object.keys(t)[0]||t||e.value||" ";let i=e.edit.options.map(l=>({title:typeof l=="string"&&l||Object.keys(l)[0],option:typeof l=="string"&&l||Object.values(l)[0]}));return mapp.ui.elements.dropdown({span:e.value,entries:i,callback:(l,o)=>{e.newValue=o.option,e.location.view?.dispatchEvent(new CustomEvent("valChange",{detail:e}))}})}var E=e=>{e.query&&mapp.utils.xhr(`${e.location.layer.mapview.host}/api/query/${e.query}`).then(a=>{e.node.querySelector(".val").innerHTML=a[e.params.field]});let t=e.type!=="html"?e.value:"";e.edit&&(t=mapp.utils.html`
    <textarea
      style="auto; min-height: 50px;"
      onfocus=${a=>{a.target.style.height=a.target.scrollHeight+"px"}}
      onfocusout=${a=>{a.target.style.height="auto"}}
  ...
  <div class="layer-key">
    <span>
      ...
      <div class="grid">
        ...
        ${e.target}`}return e.chart&&await Ke(e),typeof e.columns<"u"&&(console.warn("Table dataviews should be configured inside a tables object"),e.table={columns:e.columns}),e.table&&await Qe(e),e.mapChange&&e.layer&&e.layer.mapview.Map.getTargetElement().addEventListener("changeEnd",()=>{e.layer&&!e.layer.display||e.tab&&!e.tab.classList.contains("active")||typeof e.mapChange=="function"&&e.mapChange()||e.update()}),e};async function Ke(e){let t=e.target.appendChild(mapp.utils.html.node`<canvas>`);e.ChartJS=await mapp.ui.utils.Chart(t,mapp.utils.merge({type:"bar",options:{plugins:{legend:{display:!1},datalabels:{display:!1}}}},e.chart)),e.setData=i=>{e.noDataMask&&!i?(e.target.style.display="none",e.mask=!e.mask&&e.target.parentElement?.appendChild(mapp.utils.html.node`
        <div class="dataviewMask">No Data`)):(e.mask&&e.mask.remove(),delete e.mask,e.target.style.display="block"),i||(i={datasets:[{data:[]}]}),i.datasets||(i={datasets:[{data:i}]}),e.data=i,e.chart.datasets?.length&&i.datasets.forEach((a,l)=>Object.assign(a,e.chart.datasets[l])),i.labels=i.labels||e.chart.labels,e.ChartJS.data=i,e.ChartJS.update()}}async function Qe(e){e.Tabulator=await mapp.ui.utils.Tabulator(e.target,e.table),e.table.selectable&&e.Tabulator.on("rowClick",(t,i)=>{if(typeof e.rowSelect=="function"){e.rowSelect(t,i);return}let a=i.getData();!e.layer||!a[e.layer.qID]||(mapp.location.get({layer:e.layer,id:a[e.layer.qID]}),i.deselect())}),e.setData=t=>{!t&&e.data||(e.noDataMask&&!t?(e.target.style.display="none",e.mask=!e.mask&&e.target.parentElement?.appendChild(mapp.utils.html.node`
        <div class="dataviewMask">No Data`)):(e.mask&&e.mask.remove(),delete e.mask,e.target.style.display="block"),t=!t&&[]||t.length&&t||[t],e.Tabulator.setData(t),e.data=t,typeof e.setDataCallback=="function"&&e.setDataCallback(e))}}var pe=e=>{!e.node||(e.tabs=e.node.appendChild(mapp.utils.html.node`<div class="tabs">`),e.panel=e.node.appendChild(mapp.utils.html.node`<div class="panel">`),e.id&&e.node.setAttribute("data-id",e.id),e.addTab=Ye,e.node.addEventListener("addTab",t=>e.addTab(t.detail)))};function Ye(e){let t=this;e.tab=mapp.utils.html.node`
    <div class="tab">
      <div
        class="header"
        style="${e.tab_style||""}"
  ...
  <label 
    ...
    <li
      ...
      ${mapp.dictionary.cancel}`),this.popup({coords:this.interaction.vertices[this.interaction.vertices.length-1],content:mapp.utils.html.node`<ul>${t}`})}function tt(e){if(this.interaction.vertices.length===0)return;e&&e.preventDefault();let t=this.interaction.Layer.getSource().getFeatures(),i=[];t.length&&i.push(mapp.utils.html`
  <li
    onclick=${()=>this.interaction.finish(this.interaction.getFeature())}>
  ...
  <div 
    ...
    <button
      ...
      ${mapp.dictionary.draw_position}`}function lt(e,t){let i=Object.assign({},e,{type:"LineString"});return mapp.utils.html.node`
    <button
      class="flat wide primary-colour"
      onclick=${a=>t(a,i)}>
      ${mapp.dictionary.draw_line}`}function ot(e,t){let i=Object.assign({},e,{type:"LineString",freehand:!0});return mapp.utils.html.node`
    <button
      class="flat wide primary-colour"
      onclick=${a=>t(a,i)}>
      ${mapp.dictionary.draw_freehand}`}function nt(e,t){let i=Object.assign({},e,{type:"Polygon"});return mapp.utils.html.node`
    <button
      class="flat wide primary-colour"
      onclick=${a=>t(a,i)}>
  ...
  <button 
    ...
    <div>
      ${a}
      ${l}`,n=mapp.ui.elements.slider({label:mapp.dictionary.here_range_minutes,min:e.rangeMin,max:e.rangeMax,val:10,callback:d=>{e.range=parseInt(d.target.value)}}),s=mapp.ui.elements.chkbox({label:"Reverse Direction Isoline",checked:!!e.reverseDirection,onchange:d=>{a.textContent=d&&mapp.dictionary.here_datetime_arrive||mapp.dictionary.here_datetime_depart,e.reverseDirection=d}});return mapp.ui.elements.drawer({header:mapp.utils.html`
      <h4>Here Isoline</h4>
    ...
    <div
      ...
      <input id="b" type="range"
        ...
        step=${e.step||1}
        value=${e.val_b}
        oninput=${i}/>`;function i(a){t.style.setProperty(`--${a.target.id}`,a.target.value),t.querySelectorAll("input").forEach(l=>{l.id==a.target.id&&l!=a.target&&(l.value=a.target.value)}),a.target.id==="a"&&typeof e.callback_a=="function"&&e.callback_a(a),a.target.id==="b"&&typeof e.callback_b=="function"&&e.callback_a(b)}return t};var Le={viewport:pt,download_json:mt,download_csv:ut};function pt(e){return mapp.utils.html`
    <button
    ...
    <button
      class="flat"
      onclick=${()=>{e.Tabulator.download("json",`${e.title||"table"}.json`)}}>Export as JSON`}function ut(e){return mapp.utils.html`
    <button
      class="flat"
      onclick=${()=>{e.Tabulator.download("csv",`${e.title||"table"}.csv`)}}>Download as CSV`}var Se={card:me,chkbox:ue,contextMenu:he,drawer:ge,drawing:fe,dropdown:ve,dropdown_multi:we,isoline_params_here:_e,isoline_params_mapbox:be,legendIcon:$e,slider:ke,slider_ab:xe,toolbar_el:Le};var C,v=null;async function Ee(e,t){return v?new v(e,t):(C||(C=new Promise(async i=>{if(window.Chart){v=window.Chart,i();return}Promise.all([import("https://cdn.skypack.dev/chart.js@3.8.0"),import("https://cdn.skypack.dev/chartjs-plugin-datalabels@2.0.0"),import("https://cdn.skypack.dev/chartjs-plugin-annotation@1.4.0")]).then(a=>{a[0].Chart.register(...a[0].registerables),a[0].Chart.register(a[1].default),a[0].Chart.register(a[2].default),v=a[0].Chart,i()}).catch(a=>{console.error(a.message),alert("Failed to load Chart.js library. Please reload the browser.")})})),await C,new v(e,t))}var $=null;async function ht(){if(window.Tabulator){$=window.Tabulator;return}$=(await import("https://cdn.skypack.dev/-/tabulator-tables@v5.2.7-uFL2B6RLQMMWIcn9QymG/dist=es2019,mode=imports/optimized/tabulator-tables.js")).TabulatorFull}async function Ce(){return $||await ht(),new $(...arguments)}var m={idle:600},ze=e=>{Object.assign(m,e),m.idle!==0&&(window.onload=f,window.onmousemove=f,window.onmousedown=f,window.ontouchstart=f,window.onclick=f,window.onkeypress=f,f(),j())};function f(){m.locked||(m.timeout&&clearTimeout(m.timeout),m.timeout=setTimeout(je,m.idle*1e3))}function je(){m.locked=!0,m.renew&&clearTimeout(m.renew);let e=new XMLHttpRequest;e.open("GET",`${m.host}/api/user/cookie?destroy=true`),e.send();let t=mapp.utils.html.node`
    <form
      class="login"
    ...
    </form>`;t.onsubmit=i=>{i.preventDefault();let a=new XMLHttpRequest;a.open("POST",`${m.host}/api/user/login`),a.setRequestHeader("Content-Type","application/x-www-form-urlencoded"),a.onload=l=>{l.target.status===401&&(t.querySelector(".msg").textContent=l.target.response),l.target.status===200&&(delete m.locked,m.mask.remove(),f(),j())},a.send(`login=true&email=${t.querySelector("#auth_user_email").value}&password=${t.querySelector("#auth_user_password").value}`)},m.mask=document.body.appendChild(mapp.utils.html.node`
    <div class="interfaceMask">${t}`)}function j(){m.renew=setTimeout(e,(m.idle-20)*1e3);function e(){let t=new XMLHttpRequest;t.open("GET",`${m.host}/api/user/cookie?renew=true`),t.onload=i=>{if(i.target.status===401)return je();j()},t.send()}}var Oe=e=>{document.body.append(mapp.utils.html.node`
    <div class="interfaceMask">
      <div class="bg-image" style=${`background-image:url(${e.target.src})`}>
      <button class="btn-close mask-icon close"
        onclick=${t=>t.target.parentElement.parentElement.remove()}>`)};var Te=e=>{e.target.addEventListener("mousedown",i=>{i.preventDefault(),document.body.style.cursor="grabbing",window.addEventListener("mousemove",e.resizeEvent),window.addEventListener("mouseup",t)}),e.target.addEventListener("touchstart",i=>{i.preventDefault(),window.addEventListener("touchmove",e.resizeEvent),window.addEventListener("touchend",t)},{passive:!0});function t(){document.body.style.cursor="auto",window.removeEventListener("mousemove",e.resizeEvent),window.removeEventListener("touchmove",e.resizeEvent),window.removeEventListener("mouseup",t),window.removeEventListener("touchend",t)}};var De={Chart:Ee,Tabulator:Ce,idleMask:ze,imagePreview:Oe,resizeHandler:Te};var qe=e=>{let t=Object.assign({input:e.target.querySelector("input"),result:e.target.querySelector("ul"),sources:{glx:a,mapbox:n,google:l,opencage:o}},e);t.input.placeholder=t.placeholder||"",t.input.oninput=r=>{t.xhr&&t.xhr.abort(),t.result.innerHTML="",r.target.value.length>0&&i(r.target.value)},t.input.addEventListener("keydown",r=>{(r.keyCode||r.charCode)===13&&t.result.querySelector("li").click()});function i(r){let d=r.split(",").map(parseFloat);if(d.length===2&&d.every(p=>!isNaN(p))){t.result.appendChild(mapp.utils.html.node`
      <li 
        onclick=${p=>{t.result.innerHTML="",s({label:`Latitude:${d[0]}, Longitude:${d[1]}`,source:"Coordinates",lng:d[1],lat:d[0]})}}><span>Latitutde:${d[0]}, Longitude:${d[1]}`);return}t.xhr=new XMLHttpRequest,t.xhr.open("GET",t.mapview.host+"/api/gazetteer?"+mapp.utils.paramString({locale:t.mapview.locale.key,q:encodeURIComponent(r),source:e.source})),t.xhr.setRequestHeader("Content-Type","application/json"),t.xhr.responseType="json",t.xhr.onload=p=>{if(p.target.status===200){if(p.target.response.length===0)return t.result.appendChild(mapp.utils.html.node`
          ...
          <li
            ...
            <span>${c.label}</span>`)})}},t.xhr.send()}function a(r){mapp.location.get({layer:t.mapview.layers[r.layer],table:r.table,id:r.id}).then(d=>d.flyTo())}function l(r){mapp.utils.xhr(`${t.mapview.host}/api/proxy?url=${encodeURIComponent(`https://maps.googleapis.com/maps/api/place/details/json?placeid=${r.id}&{GOOGLE}`)}`).then(d=>{s({label:r.label,source:r.source,lng:d.result.geometry.location.lng,lat:d.result.geometry.location.lat})})}function o(r){s({label:r.label,source:r.source,lng:r.marker[0],lat:r.marker[1]})}function n(r){s({label:r.label,source:r.source,lng:r.marker[0],lat:r.marker[1]})}function s(r){Object.assign(r,{layer:{mapview:t.mapview},Layers:[],hook:r.label});let d=[{title:r.label,value:r.source},{type:"pin",value:[r.lng,r.lat],srid:"4326",class:"display-none",location:r}];mapp.location.decorate(Object.assign(r,{infoj:d})),t.mapview.locations[r.hook]=r,r.flyTo()}};var Me={layers:H,locations:de,elements:Se,utils:De,gazetteer:qe,Dataview:ce,Tabview:pe};typeof window.mapp=="object"&&(window.mapp.ui=Me);var xi=Me;export{xi as default};
