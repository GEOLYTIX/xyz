mapp.utils.merge(mapp.dictionaries,{en:{layer_zoom_to_extent:"Zoom to filtered layer extent",layer_visibility:"Toggle visibility"},de:{layer_zoom_to_extent:"Zoom zum Ausma\xDF des gefilterten Datensatzes",layer_visibility:"Umschalten der Ansicht"},cn:{layer_zoom_to_extent:"\u7F29\u653E\u81F3\u76F8\u5E94\u7B5B\u9009\u8303\u56F4",layer_visibility:"\u5207\u6362\u53EF\u89C1\u6027"},pl:{layer_zoom_to_extent:"Poka\u017C zasi\u0119g warstwy",layer_visibility:"Widoczno\u015B\u0107"},ko:{layer_zoom_to_extent:"\uD544\uD130\uB41C \uB808\uC774\uC5B4\uD06C\uAE30\uC5D0 \uC90C(zoom)",layer_visibility:"\uD1A0\uAE00 \uAC00\uC2DC\uC131"},fr:{layer_zoom_to_extent:"Zoom sur l'\xE9tendue de la couche",layer_visibility:"Changer la visiblit\xE9"},ja:{layer_zoom_to_extent:"\u30D5\u30A3\u30EB\u30BF\u30FC\u3055\u308C\u305F\u30EC\u30A4\u30E4\u30FC\u7BC4\u56F2\u3092\u30BA\u30FC\u30E0\u306B",layer_visibility:"\u8868\u793A\u5207\u66FF"}});var _=e=>{let t=e.filter.zoomToExtent&&mapp.utils.html`
  ...
  <div
    ...
    <button 
      ...
      <div
        class="${`location ${e.class}`}">`,e.target=e.locationViewTarget,e.display&&mapp.ui.Dataview(e).then(()=>e.update())),e.chkbox=e.label&&mapp.ui.elements.chkbox({label:e.label,disabled:e.disabled,checked:!!e.display,onchange:t=>{if(e.display=t,e.locationViewTarget){if(!e.display){e.locationViewTarget.style.display="none";return}e.locationViewTarget.style.display="block",typeof e.update=="function"&&e.update(),typeof e.update!="function"&&mapp.ui.Dataview(e).then(()=>{typeof e.data=="object"?e.setData(e.data):e.update()});return}e.display?K(e):e.remove()}}),mapp.utils.html.node`
    ${e.chkbox||""}
  ...
  <div 
    ...
    ${e.content}`;var he=e=>mapp.utils.html.node`
  <label 
  ...
  <div 
    ...
    <button
      ...
      ${mapp.dictionary.draw_position}`}function pt(e,t){let l=Object.assign({},e,{type:"LineString",snap:e.edit.line.snap});return mapp.utils.html.node`
    <button
      class="flat wide bold primary-colour"
      onclick=${a=>t(a,l)}>
      ${mapp.dictionary.draw_line}`}function ut(e,t){let l=Object.assign({},e,{type:"Polygon",snap:e.edit.polygon.snap});return mapp.utils.html.node`
    <button
      class="flat wide bold primary-colour"
      onclick=${a=>t(a,l)}>
      ${mapp.dictionary.draw_polygon}`}function mt(e,t){e.edit.circle=Object.assign({units:"meter",radius:100,tooltip:{metric:"distance"}},typeof e.edit.circle=="object"&&e.edit.circle||{});let l={meter:s=>s,km:s=>s*1e3,miles:s=>s*1609.34,meter2:s=>Math.sqrt(s/Math.PI),km2:s=>Math.sqrt(s*1e6/Math.PI)},a=mapp.utils.html.node`
  ...
  <button 
    ...
    onchange=${s=>{let c=s.target.value;o?.set.has(c)&&o.set.delete(c)||o.set.add(c);for(let d=0;d<s.target.options.length;d++)s.target.options[d].style.backgroundColor=o.set.has(s.target.options[d].value)&&(o.selected||"#cae0b8")||"#ffffff";n(t,o.set),s.target.value=null}}>${o.options.map(s=>mapp.utils.html.node`
      <option>${s}`)}`;return r.value=null,o.multiple&&(r.multiple=!0),r}}var g={idle:600},Fe=e=>{Object.assign(g,e),g.idle!==0&&(window.onload=h,window.onmousemove=h,window.onmousedown=h,window.ontouchstart=h,window.onclick=h,window.onkeypress=h,h(),De())};function h(){g.locked||(g.timeout&&clearTimeout(g.timeout),g.timeout=setTimeout(qe,g.idle*1e3))}function qe(){g.locked=!0,g.renew&&clearTimeout(g.renew);let e=new XMLHttpRequest;e.open("GET",`${g.host}/api/user/cookie?destroy=true`),e.onload=t=>location.reload(),e.send()}function De(){g.renew=setTimeout(e,(g.idle-20)*1e3);function e(){let t=new XMLHttpRequest;t.open("GET",`${g.host}/api/user/cookie?renew=true`),t.onload=l=>{if(l.target.status===401)return qe();De()},t.send()}}var Ie=e=>{document.body.append(mapp.utils.html.node`
