mapp.utils.merge(mapp.dictionaries,{en:{layer_zoom_to_extent:"Zoom to filtered layer extent",layer_visibility:"Toggle visibility"},de:{layer_zoom_to_extent:"Zoom zum Ausma\xDF des gefilterten Datensatzes",layer_visibility:"Umschalten der Ansicht"},cn:{layer_zoom_to_extent:"\u7F29\u653E\u81F3\u76F8\u5E94\u7B5B\u9009\u8303\u56F4",layer_visibility:"\u5207\u6362\u53EF\u89C1\u6027"},pl:{layer_zoom_to_extent:"Poka\u017C zasi\u0119g warstwy",layer_visibility:"Widoczno\u015B\u0107"},ko:{layer_zoom_to_extent:"\uD544\uD130\uB41C \uB808\uC774\uC5B4\uD06C\uAE30\uC5D0 \uC90C(zoom)",layer_visibility:"\uD1A0\uAE00 \uAC00\uC2DC\uC131"},fr:{layer_zoom_to_extent:"Zoom sur l'\xE9tendue de la couche",layer_visibility:"Changer la visiblit\xE9"},ja:{layer_zoom_to_extent:"\u30D5\u30A3\u30EB\u30BF\u30FC\u3055\u308C\u305F\u30EC\u30A4\u30E4\u30FC\u7BC4\u56F2\u3092\u30BA\u30FC\u30E0\u306B",layer_visibility:"\u8868\u793A\u5207\u66FF"}});var w=e=>{let t=mapp.utils.html`
  ...
  <input
    ...
    <button
      ...
      <div class="mask-icon expander"></div>`,content:mapp.utils.html`${t}`})};var F=e=>{let t=Object.keys(e.reports).map(i=>{let a=e.reports[i];a.key=i,a.host=e.mapview.host;let o=`${a.host}/view?${mapp.utils.paramString({template:a.template,lat:mapp.hooks.current?.lat,lng:mapp.hooks.current?.lng,z:mapp.hooks.current?.z})}`;return mapp.utils.html`
      <a
        class="link-with-img"
        target="_blank"
        href="${o}">
    ...
    <div
      ...
      <div
        ...
        onclick=${s=>{s.target.classList.toggle("disabled"),s.target.classList.contains("disabled")?(e.filter.current[t.field]||(e.filter.current[t.field]={}),e.filter.current[t.field].ni||(e.filter.current[t.field].ni=[]),e.filter.current[t.field].ni.push(i[0])):(e.filter.current[t.field].ni.splice(e.filter.current[t.field].ni.indexOf(i[0]),1),e.filter.current[t.field].ni.length||delete e.filter.current[t.field]),e.reload()}}>${i[1].label||i[0]}`;l.push(mapp.utils.html`
    <div 
      data-id=${i[0]}
      class="contents">
      ...
      <div 
        data-id="cluster"
        class="contents">
        ${i}${a}`)}return e.style.legend=mapp.utils.html.node`<div class="grid">${l}`,e.style.legend};var A=e=>{let t=e.style.theme,l=[];return t.cat_arr.forEach(i=>{let a=Object.assign({},e.style.default,i.style&&i.style.icon||i.style||i),o=mapp.utils.html`
      <div
        style="height: 24px; width: 24px;">
        ${mapp.ui.elements.legendIcon(Object.assign({width:24,height:24},a))}`,n=mapp.utils.html`
    ...
    <div data-id="size-icons" class="contents">
      ${e.style.range.map((o,n)=>{let s=mapp.utils.svg.node`<svg height=50 width=50>
          ...
          <circle
            ...
            cx=25
            cy=25
            r='${23/l*(n+1)}'/>`,m=`data:image/svg+xml,${encodeURIComponent(i.serializeToString(s))}`,r=`
          grid-row:2;
          grid-column:${n+1};
  ...
  <div
    ...
    <button
      ...
      onclick = ${i=>{e.remove()}}>`),e.view=mapp.ui.elements.drawer({class:"location-view expanded",header:t});let l=e.view.appendChild(mapp.ui.locations.infoj(e));e.view.querySelector(".header").style.borderBottom=`3px solid ${e.record.colour}`,e.view.addEventListener("valChange",i=>{i.detail.value!=i.detail.newValue?i.detail.node.classList.add("val-changed"):(delete i.detail.newValue,i.detail.node.classList.remove("val-changed")),e.view.querySelector(".cloud-upload").style.display=e.infoj.some(a=>typeof a.newValue<"u")&&"inline-block"||"none"}),e.view.addEventListener("updateInfo",()=>{l.remove(),e.view.querySelector(".cloud-upload").style.display="none",e.view.classList.remove("disabled"),l=mapp.ui.locations.infoj(e),e.view.appendChild(l)})};mapp.utils.merge(mapp.dictionaries,{en:{location_clear_all:"Clear locations"},de:{location_clear_all:"Entferne Auswahl"},cn:{location_clear_all:"\uBAA8\uB4E0 \uC704\uCE58 \uC81C\uAC70"},pl:{location_clear_all:"Wyczy\u015B\u0107 selekcje"},ko:{location_clear_all:"\u6E05\u9664\u6240\u6709\u5730\u70B9"},fr:{location_clear_all:"Des\xE9lectionner tous les lieux."},ja:{location_clear_all:"\u5168\u30ED\u30B1\u30FC\u30B7\u30E7\u30F3\u3092\u30AF\u30EA\u30A2"}});var U=[{symbol:"A",colour:"#2E6F9E"},{symbol:"B",colour:"#EC602D"},{symbol:"C",colour:"#5B8C5A"},{symbol:"D",colour:"#B84444"},{symbol:"E",colour:"#514E7E"},{symbol:"F",colour:"#E7C547"},{symbol:"G",colour:"#368F8B"},{symbol:"H",colour:"#841C47"},{symbol:"I",colour:"#61A2D1"},{symbol:"J",colour:"#37327F"}],N=e=>{if(!e.mapview||!e.target)return;let t={node:e.target,mapview:e.mapview};return t.node.appendChild(mapp.utils.html.node`
    <button 
      class="tab-display bold primary-colour text-shadow"
      onclick=${l=>{Object.values(t.mapview.locations).forEach(i=>i.remove())}}>
      ${mapp.dictionary.location_clear_all}`),e.mapview.locations=new Proxy(e.mapview.locations,{set:function(l,i,a){let o=U.find(n=>!n.hook);return o.hook=a.hook,a.record=o,a.style={strokeColor:o.colour,fillColor:o.colour,fillOpacity:.2},a.Style=mapp.utils.style([{strokeColor:"#000",strokeOpacity:.1,strokeWidth:8},{strokeColor:"#000",strokeOpacity:.1,strokeWidth:6},{strokeColor:"#000",strokeOpacity:.1,strokeWidth:4},{strokeColor:a.style.strokeColor||"#000",strokeWidth:2,fillColor:a.style.fillColor||a.style.strokeColor||"#fff",fillOpacity:a.style.fillOpacity||.2}]),a.pinStyle=mapp.utils.style({icon:{type:"markerLetter",letter:o.symbol,color:a.style.strokeColor,scale:3,anchor:[.5,1]}}),Reflect.set(...arguments),mapp.ui.locations.view(a),Object.values(t.node.children).forEach(n=>n.classList.remove("expanded")),t.node.insertBefore(a.view,t.node.firstChild.nextSibling),a.view.dispatchEvent(new Event("addLocationView")),t.node.closest(".tab").style.display="block",t.node.closest(".tab").click(),!0},deleteProperty:function(l,i){Reflect.deleteProperty(...arguments);let a=U.find(o=>o.hook===i);return delete a.hook,setTimeout(()=>{if(t.node.children.length>1)return;let o=t.node.closest(".tab");o.style.display="none",o.previousElementSibling.click()},300),!0}}),t};var W=(e,t)=>{if(!e.infoj)return;let l=mapp.utils.html.node`<div class="location-view-grid">`,i={};for(let a of t||e.infoj){if(e.view&&e.view.classList.contains("disabled"))break;if(a.listview=l,a.type=a.type||"text",a.skipEntry||a.skipFalsyValue&&!a.value&&!a.edit||a.skipUndefinedValue&&typeof a.value>"u"&&!a.edit||a.skipNullValue&&a.value===null&&!a.edit)continue;a.nullValue&&a.value===null&&!a.edit&&(a.value=a.nullValue),a.group&&(i[a.group]||(i[a.group]=a.listview.appendChild(mapp.ui.elements.drawer({class:`lv-1 group ${a.expanded&&"expanded"||""}`,header:mapp.utils.html`
              <h3>${a.group}</h3>
    ...
    <div
      class="val"
      style="${`${e.css_val||""}`}">
      ${t}`};var X=e=>{let t=e.value.map(n=>mapp.utils.html`
    ...
    <div
      class="val"
      style="${`${e.css_val||""}`}">
      ${e.prefix}${t}${e.suffix}`};function Ne(e){let t=e.edit.options.find(a=>typeof a=="object"&&Object.values(a)[0]===e.value||a===e.value)||e.value;e.value=t&&typeof t=="object"&&Object.keys(t)[0]||t||e.value||" ";let l=e.edit.options.map(a=>({title:typeof a=="string"&&a||Object.keys(a)[0],option:typeof a=="string"&&a||Object.values(a)[0]}));return mapp.ui.elements.dropdown({span:e.value,entries:l,callback:(a,o)=>{e.newValue=o.option,e.location.view?.dispatchEvent(new CustomEvent("valChange",{detail:e}))}})}var S=e=>{e.query&&mapp.utils.xhr(`${e.location.layer.mapview.host}/api/query/${e.query}`).then(i=>{e.node.querySelector(".val").innerHTML=i[e.params.field]});let t=e.type!=="html"?e.value:"";e.edit&&(t=mapp.utils.html`
    <textarea
      style="auto; min-height: 50px;"
      onfocus=${i=>{i.target.style.height=i.target.scrollHeight+"px"}}
      onfocusout=${i=>{i.target.style.height="auto"}}
  ...
  <div
    class="val"
    style="${`${e.css_val||""}`}">${t}`;return!e.edit&&e.type==="html"&&(l.innerHTML=e.value||""),l};var oe={key:We,boolean:J,dataview:Z,date:x,datetime:x,documents:X,geometry:K,html:S,images:Y,integer:E,isoline_here:L,isoline_mapbox:L,json:Q,numeric:E,pin:ee,report:te,streetview:ae,tab:ie,text:le,textarea:S};function We(e){return mapp.utils.html.node`
  <div class="layer-key">
    <span>
      ${e.location.layer.name}`}var ne={view:H,listview:N,infoj:W,entries:oe};var re=async e=>{if(e.target=typeof e.target=="string"&&document.getElementById(e.target)||e.target,e.target=e.target instanceof HTMLElement&&e.target||mapp.utils.html.node`
      <div
      ...
      <div class="grid">
        ...
        ${e.target}`}return e.chart&&await Je(e),typeof e.columns<"u"&&(console.warn("Table dataviews should be configured inside a tables object"),e.table={columns:e.columns}),e.table&&await Ze(e),e.mapChange&&e.layer&&e.layer.mapview.Map.getTargetElement().addEventListener("changeEnd",()=>{e.layer&&!e.layer.display||e.tab&&!e.tab.classList.contains("active")||typeof e.mapChange=="function"&&e.mapChange()||e.update()}),e};async function Je(e){let t=e.target.appendChild(mapp.utils.html.node`<canvas>`);e.ChartJS=await mapp.ui.utils.Chart(t,mapp.utils.merge({type:"bar",options:{plugins:{legend:{display:!1},datalabels:{display:!1}}}},e.chart)),e.setData=l=>{e.noDataMask&&!l?(e.target.style.display="none",e.mask=!e.mask&&e.target.parentElement?.appendChild(mapp.utils.html.node`
        <div class="dataviewMask">No Data`)):(e.mask&&e.mask.remove(),delete e.mask,e.target.style.display="block"),l||(l={datasets:[{data:[]}]}),l.datasets||(l={datasets:[{data:l}]}),e.data=l,e.chart.datasets?.length&&l.datasets.forEach((i,a)=>Object.assign(i,e.chart.datasets[a])),l.labels=l.labels||e.chart.labels,e.ChartJS.data=l,e.ChartJS.update()}}async function Ze(e){e.Tabulator=await mapp.ui.utils.Tabulator(e.target,e.table),e.table.selectable&&e.Tabulator.on("rowClick",(t,l)=>{if(typeof e.rowSelect=="function"){e.rowSelect(t,l);return}let i=l.getData();!e.layer||!i[e.layer.qID]||(mapp.location.get({layer:e.layer,id:i[e.layer.qID]}),l.deselect())}),e.setData=t=>{!t&&e.data||(e.noDataMask&&!t?(e.target.style.display="none",e.mask=!e.mask&&e.target.parentElement?.appendChild(mapp.utils.html.node`
        <div class="dataviewMask">No Data`)):(e.mask&&e.mask.remove(),delete e.mask,e.target.style.display="block"),t=!t&&[]||t.length&&t||[t],e.Tabulator.setData(t),e.data=t,typeof e.setDataCallback=="function"&&e.setDataCallback(e))}}var se=e=>{!e.node||(e.tabs=e.node.appendChild(mapp.utils.html.node`<div class="tabs">`),e.panel=e.node.appendChild(mapp.utils.html.node`<div class="panel">`),e.id&&e.node.setAttribute("data-id",e.id),e.addTab=Xe,e.node.addEventListener("addTab",t=>e.addTab(t.detail)))};function Xe(e){let t=this;e.tab=mapp.utils.html.node`
    <div class="tab">
      <div
        class="header"
        style="${e.tab_style||""}"
  ...
  <label 
    ...
    <div></div>
    <span>${e.label}`;var pe={modify:Ke,draw:Ye};mapp.utils.merge(mapp.dictionaries,{en:{remove_last_vertex:"Remove last vertex"},de:{remove_last_vertex:"Remove last vertex"},cn:{remove_last_vertex:"\u5220\u9664\u6700\u540E\u4E00\u4E2A\u9876\u70B9"},pl:{remove_last_vertex:"Usu\u0144 ostatni wierzcho\u0142ek"},ko:{remove_last_vertex:"\uB9C8\uC9C0\uB9C9 \uC815\uC810(\uAF2D\uC9C0\uC810) \uC81C\uAC70"},fr:{remove_last_vertex:"Effacer le dernier point"},ja:{remove_last_vertex:"\u6700\u5F8C\u306E\u30D0\u30FC\u30C6\u30C3\u30AF\u30B9\u3092\u524A\u9664"}});function Ke(e){e&&e.preventDefault();let t=[];t.push(mapp.utils.html`
    <li
  ...
  <li
    ...
    <li
      onclick=${()=>this.interaction.finish()}>
      ${mapp.dictionary.cancel}`),this.popup({coords:this.interaction.vertices[this.interaction.vertices.length-1],content:mapp.utils.html.node`<ul>${l}`,autoPan:!0})}var me=e=>mapp.utils.html.node`
  <div 
  ...
  <button 
    ...
    <ul>${e.entries.map(t=>mapp.utils.html.node`
      <li onclick=${l=>{let i=l.target.closest("button.dropdown");i.classList.toggle("active"),i.querySelector("[data-id=header-span]").textContent=t.title,e.callback&&e.callback(l,t)}}>${t.title}`)}`;var ge=e=>{let t=new Set,l=new Set;return mapp.utils.html`
    <button 
      data-id=${e.data_id||"dropdown"}
      class="dropdown">
      <div class="head"
    ...
    <div 
      ...
      <div style="display: grid; grid-row-gap: 5px;">
        ${t}
        ${l}`})};var we=new XMLSerializer,_e=e=>{if(e.svg||e.type||e.layers){let t=e.layers&&Array.isArray(e.layers)&&e.layers.map(i=>`url(${i.svg})`).reverse().join(",")||`url(${e.svg||e.url||mapp.utils.svgSymbols[e.type](e)})`,l=`
      background-position: center;
      background-repeat: no-repeat;
      background-size: contain;
      ...
      <svg height=${e.height} width=${e.width}>
        <path d=${t}
          ...
          stroke-width=${e.strokeWidth||1}/>`,i=`data:image/svg+xml,${encodeURIComponent(we.serializeToString(l))}`,a=`
      background-position: center;
      background-repeat: no-repeat;
      background-size: contain;
      width: ${e.width}px;
      ...
      <svg height=${e.height} width=${e.width}>
        <rect
          ...
          stroke=${e.strokeColor}
          stroke-width=${e.strokeWidth||1}>`,l=`data:image/svg+xml,${encodeURIComponent(we.serializeToString(t))}`,i=`
      background-position: center;
      background-repeat: no-repeat;
      background-size: contain;
      width: ${e.width}px;
      height: ${e.height}px;
      background-image: url(${l});`;return mapp.utils.html`<div style=${i}>`}};var be=e=>{return mapp.utils.html.node`
    <div
      role="group"
      ...
      <input id="a"
        ...
        value=${e.val}
        step=${e.step||1}
        oninput=${t}>`;function t(l){l.target.closest(".input-range").style.setProperty(`--${l.target.id}`,l.target.value),l.target.closest(".input-range").querySelectorAll("input").forEach(i=>{i.id==l.target.id&&i!=l.target&&(i.value=l.target.value)}),e.callback&&e.callback(l)}};var ye=e=>{let t=mapp.utils.html.node`
    <div
      role="group"
      ...
      <input id="b" type="range"
        ...
        step=${e.step||1}
        value=${e.val_b}
        oninput=${l}/>`;function l(i){t.style.setProperty(`--${i.target.id}`,i.target.value),t.querySelectorAll("input").forEach(a=>{a.id==i.target.id&&a!=i.target&&(a.value=i.target.value)}),i.target.id==="a"&&typeof e.callback_a=="function"&&e.callback_a(i),i.target.id==="b"&&typeof e.callback_b=="function"&&e.callback_a(b)}return t};var $e={viewport:rt,download_json:st,download_csv:dt};function rt(e){return mapp.utils.html`
    <button
    ...
    <button
      class="flat"
      onclick=${()=>{e.Tabulator.download("csv",`${e.title||"table"}.csv`)}}>CSV`}var ke={card:de,chkbox:ce,contextMenu:pe,drawer:me,drawing:ue,dropdown:he,dropdown_multi:ge,isoline_params_here:fe,isoline_params_mapbox:ve,legendIcon:_e,slider:be,slider_ab:ye,toolbar_el:$e};var y=null;async function ct(){if(window.Chart){y=window.Chart;return}let e=await import("https://cdn.skypack.dev/chart.js@3.7.0");e.Chart.register(...e.registerables);let t=await import("https://cdn.skypack.dev/chartjs-plugin-datalabels@2.0.0");e.Chart.register(t.default);let l=await import("https://cdn.skypack.dev/chartjs-plugin-annotation@1.3.0");e.Chart.register(l.default),y=e.Chart}async function xe(e,t){return y||await ct(),new y(e,t)}var $=null;async function pt(){if(window.Tabulator){$=window.Tabulator;return}$=(await import("https://cdn.skypack.dev/pin/tabulator-tables@v5.0.10-xDu7yyLVhsxCKJbN5vfH/mode=imports/optimized/tabulator-tables.js")).TabulatorFull}async function Le(){return $||await pt(),new $(...arguments)}var u={idle:600},Se=e=>{Object.assign(u,e),u.idle!==0&&(window.onload=f,window.onmousemove=f,window.onmousedown=f,window.ontouchstart=f,window.onclick=f,window.onkeypress=f,f(),C())};function f(){u.locked||(u.timeout&&clearTimeout(u.timeout),u.timeout=setTimeout(Ee,u.idle*1e3))}function Ee(){u.locked=!0,u.renew&&clearTimeout(u.renew);let e=new XMLHttpRequest;e.open("GET",`${u.host}/api/user/cookie?destroy=true`),e.send();let t=mapp.utils.html.node`
    <form
      class="login"
    ...
    <div class="interfaceMask">${t}`)}function C(){u.renew=setTimeout(e,(u.idle-20)*1e3);function e(){let t=new XMLHttpRequest;t.open("GET",`${u.host}/api/user/cookie?renew=true`),t.onload=l=>{if(l.target.status===401)return Ee();C()},t.send()}}var Ce=e=>{document.body.append(mapp.utils.html.node`
    <div class="interfaceMask">
      <div class="bg-image" style=${`background-image:url(${e.target.src})`}>
      <button class="btn-close mask-icon close"
        onclick=${t=>t.target.parentElement.parentElement.remove()}>`)};var je=e=>{e.target.addEventListener("mousedown",l=>{l.preventDefault(),document.body.style.cursor="grabbing",window.addEventListener("mousemove",e.resizeEvent),window.addEventListener("mouseup",t)}),e.target.addEventListener("touchstart",l=>{l.preventDefault(),window.addEventListener("touchmove",e.resizeEvent),window.addEventListener("touchend",t)},{passive:!0});function t(){document.body.style.cursor="auto",window.removeEventListener("mousemove",e.resizeEvent),window.removeEventListener("touchmove",e.resizeEvent),window.removeEventListener("mouseup",t),window.removeEventListener("touchend",t)}};var ze={Chart:xe,Tabulator:Le,idleMask:Se,imagePreview:Ce,resizeHandler:je};function si(e){let t=e.getBoundingClientRect();return t.top>=0&&t.left>=0&&t.bottom<=(window.innerHeight||document.documentElement.clientHeight)&&t.right<=(window.innerWidth||document.documentElement.clientWidth)}function di(e,t){function l(a){let o=window.getComputedStyle(a);if(o.overflowY==="auto"||o.overflowY==="scroll"){t=a;return}a.parentElement&&l(a.parentElement)}!t&&l(e._positionElement.parentElement),e.config.onOpen.push(()=>{t.addEventListener("scroll",i,{passive:!0})}),e.config.onClose.push(()=>{t.removeEventListener("scroll",i)});function i(){e._positionCalendar()}}var De=e=>{let t=Object.assign({search:l,input:e.target.querySelector("input"),result:e.target.querySelector("ul"),sources:{glx:o,mapbox:n,google:s,opencage:m},select:a,createFeature:i,icon:{type:"markerColor",colorMarker:"#003D57",colorDot:"#939faa",anchor:[.5,1],scale:3}},e);t.input.placeholder=t.placeholder||"",t.input.addEventListener("keyup",r=>{!new Set([37,38,39,40,13]).has(r.keyCode||r.charCode)&&r.target.value.length>0&&t.search(r.target.value)}),t.input.addEventListener("keydown",r=>{let d=r.keyCode||r.charCode;if(d===46||d===8){t.xhr&&t.xhr.abort(),t.clear(),t.layer&&t.mapview.Map.removeLayer(t.layer);return}if(d===13){let p=r.target.value.split(",").map(parseFloat);p[1]>-90&&p[1]<90&&p[0]>-180&&p[0]<180&&(t.xhr&&t.xhr.abort(),t.clear(),t.createFeature({type:"Point",coordinates:[p[1],p[0]]})),t.result.querySelector("li").click()}}),t.input.addEventListener("focusout",()=>{t.xhr&&t.xhr.abort(),setTimeout(t.clear,400)}),t.clear=()=>{t.target.classList.remove("active"),t.result.innerHTML=""};function l(r,d={}){t.clear&&t.clear(),t.xhr&&t.xhr.abort(),t.xhr=new XMLHttpRequest,t.xhr.open("GET",t.mapview.host+"/api/gazetteer?"+mapp.utils.paramString({locale:t.mapview.locale.key,q:encodeURIComponent(r),source:d.source})),t.xhr.setRequestHeader("Content-Type","application/json"),t.xhr.responseType="json",t.xhr.onload=p=>{if(p.target.status===200){if(t.callback)return t.callback(p.target.response);if(p.target.response.length===0)return t.result.appendChild(mapp.utils.html.node`
