import{t as e}from"./chunk-DK3Fl9T5.js";async function t(e){if(!(!e.dynamic&&typeof e.create==`function`)){if(typeof e.create==`function`){e.create();return}if(typeof e.target==`string`&&(e.target=document.getElementById(e.target)),!(e.target instanceof HTMLElement)){console.warn(`Dataviews require a HTMLHtmlElement target`);return}return i(e)instanceof Error?e.err:(typeof e.label==`string`&&(e.chkbox=mapp.ui.elements.chkbox({checked:!!e.display,data_id:e.key,disabled:e.disabled,label:e.label,onchange:t=>{e.display=t,e.display?e.show():e.hide()}})),e.update??=a,e.show??=n,e.hide??=r,mapp.ui.utils.dataview.Toolbar(e),mapp.ui.utils.dataview.mapChange(e),e)}}async function n(){this.display=!0,this.create===void 0?(this.create=()=>{mapp.ui.utils[this.dataview].create(this)},this.create(),typeof this.update==`function`&&this.update()):this.dynamic?(typeof this.create==`function`&&await this.create(),typeof this.update==`function`&&this.update()):this.reload&&typeof this.update==`function`&&this.update(),this.btnRow?.style.setProperty(`display`,`flex`),this.target.style.display=`block`}function r(){this.display=!1,this.target.style.display=`none`,this.btnRow?.style.setProperty(`display`,`none`)}function i(e){if(e.key??=e.query||e.title||e.label,e.dataview||(e.chart&&(e.dataview=`chartjs`),e.columns!==void 0&&(console.warn(`Table dataviews should be configured inside a tables object`),e.table={columns:e.columns},e.dataview=`tabulator`),e.table&&(e.dataview=`tabulator`)),!Object.hasOwn(mapp.ui.utils,e.dataview))return e.err=Error(`mapp.ui.utils.${e.dataview} doesnt exist`),console.error(e.err),e.update=()=>console.warn(`Unable to update ${e.key} dataview.`),e.err;if(typeof mapp.ui.utils[e.dataview].create!=`function`)return e.err=Error(`mapp.ui.utils.${e.dataview}.create() method doesn't exist`),console.error(e.err),e.err}async function a(){if(this.create===void 0&&(this.create=function(){mapp.ui.utils[this.dataview].create(this)},this.create()),!this.query)return;let e=mapp.utils.queryParams(this),t=mapp.utils.paramString(e);this.host??=this.layer?.mapview?.host;let n=await mapp.utils.xhr(`${this.host||mapp.host}/api/query?${t}`);if(!(n instanceof Error)){if(typeof this.responseFunction==`function`){this.responseFunction(n);return}typeof this.setData==`function`&&this.setData(n)}}function o(e){typeof e==`string`&&(e={text:e}),e.title??=mapp.dictionary.information,e.icon??=`info`,e.data_id??=`alert`,e.class??=`alert-confirm box-shadow no-select`;let t=e.icon===``?``:mapp.utils.html`<span class="material-symbols-outlined notranslate">${e.icon}</span>`;return e.header=mapp.utils.html`${t}${e.title}`,e.innerContent??=mapp.utils.html`<p>${e.text}</p>`,e.content=mapp.utils.html`
  ...
  <button
    ...
    <h3>${mapp.dictionary.circle_config}</h3>`,content:n,popout:t.popout,dialog:t.dialog}),t.drawer)}function be(e,t){let n=e.unitsConfig[t];e.units=n.option,e.radius??=n.min,e.radius=e.radius>n.max?n.max:e.radius,mapp.utils.render(e.rangeSlider,mapp.ui.elements.slider({callback:t=>{e.radius=Number.parseFloat(t)},max:n.max,min:n.min,val:e.radius}))}function xe(e){return e.draw.locator={label:mapp.dictionary.draw_position,layer:e,type:`Point`,...e.draw.locator},e.draw.locator.btn=mapp.utils.html.node`
    <button
      class="action wide"
  ...
  <svg
    ...
    width: ${e.width}px;
    height: ${e.height}px;
    background-image: url(${r});`;return mapp.utils.html`<div style=${i}>`}function We(e){e.fillOpacity??=1,e.strokeWidth??=1,e.lineDash??=``;let t=mapp.utils.svg.node`
  <svg
    height=${e.height}
    width=${e.width}>
    <rect
    ...
    <a 
      class="switch"
      href=${e.register}>${e.login_registration_link}</a>`;function o(){a.disabled=!(n.validity.valid&&i.validity.valid),r.textContent=a.disabled?e.login_invalid:``}}function qe(e){e.placeholder??=``,e.data_id??=`numeric-input`,e.numericChecks??=Je;let t=mapp.utils.formatNumericValue(e),n=`text-align: center; width: ${e.dynamicWidth?t.length+1.3+`ch`:`100%`}`;return mapp.utils.html.node`<input
    data-id=${e.data_id}
    type="text"
    style=${n}
  ...
  <input
    ...
    <div class="searchbox">
      ${e.input}
      ${e.list}`,e.target.append(e.node),e.input.addEventListener(`input`,t=>e.searchFunction(t)),e.input.addEventListener(`focus`,t=>e.searchFunction(t)),e}function rt(e){e.group_id??=e.data_id||`slider`,e.data_id=`a`,e.rangeInput=`rangeInput`,e.step??=1,e.value??=e.val,e.title??=``,e.style??=``;let t=mapp.ui.elements.numericInput(e);return e.sliderElement=mapp.utils.html.node`
    <div
      role="group"
      data-id=${e.group_id}
      title=${e.title}
    ...
    <div
      ...
      <div class="actions">
      ${r}
      </div>`;document.body.append(a);function o(e,n){a.classList.add(`ease-out`),setTimeout(function(){a.remove()},1200),typeof n?.callback==`function`&&n.callback(e),t(e?.target.value)}})}function ot(e={}){if(e.data_id??=`ui-elements-tooltip`,!e.content){console.warn(`mapp.ui.elements.tooltip: content for ${e.data_id} not provided.`);return}return e.node=mapp.utils.html.node`<span 
    data-id=${e.data_id}
    title=${e.content}
    class="tooltip material-symbols-outlined notranslate">help</span>`,e.node}function st(e,t){return e.localeInput=mapp.utils.html.node`<input
  ...
  <div>
    ...
    <div style="display:flex;">${e.localeInput}
      <button style="font-size: 1.5em;"
        onclick=${async()=>await lt(e,t)}>
        <span class="notranslate material-symbols-outlined">save</span>
      </button>
    </div>
    ${e.ulLocales}`,x(e,t),e.panel}async function ct(e,t,n){e.panel.classList.add(`disabled`),t.locale.name=n,await mapp.utils.userLocale.remove(t.locale),x(e,t),e.panel.classList.remove(`disabled`)}async function lt(e,t){e.panel.classList.add(`disabled`),t.locale.name=encodeURIComponent(e.localeInput.value),await mapp.utils.userLocale.putLocale(t)===t.locale.name&&(x(e,t),e.panel.classList.remove(`disabled`))}async function x(e,t){let n=await mapp.utils.userLocale.list(t.locale.workspace);if(!n.length){mapp.utils.render(e.ulLocales,mapp.utils.html`<div>`);return}let r=n.map(n=>{let r=mapp.utils.html`<button
    onclick=${async r=>await ct(e,t,n.name)}>
    ...
    <ul>${r}`)}globalThis.__varlockThrowOnMissingKeys=!0;var ut={alert:o,btnPanel:s,card:c,chkbox:l,confirm:u,contextMenu:ee,control:re,dialog:oe,drawer:le,drawerDialog:f,drawing:pe,dropdown:Se,helpDialog:ke,jsoneditor:Ae,layerStyle:h,legendIcon:Be,locales:Ge,loginForm:Ke,numericInput:qe,pills:Ye,radio:Qe,reportLink:et,registerForm:$e,searchbox:nt,slider:rt,slider_ab:it,toast:at,tooltip:ot,userLocale:st},dt=e=>{e={...e,...mapp.ui.elements.searchbox({name:`gazetteer-search-input`,placeholder:e.placeholder,searchFunction:t,target:e.target})};function t(t){if(e.list.innerHTML=``,!t.target.value.length)return;let n=t.target.value.split(`,`).map(parseFloat);if(n.length===2&&n.every(e=>typeof e==`number`&&!Number.isNaN(Number(e))&&Number.isFinite(Number(e)))){let[t,r]=n;if(t>=-90&&t<=90&&r>=-180&&r<=180){e.list.appendChild(mapp.utils.html.node`
        <li onclick=${t=>{mapp.utils.gazetteer.getLocation({label:`Latitude:${n[0]}, Longitude:${n[1]}`,lat:n[0],lng:n[1],source:`Coordinates`},e)}}><span>Latitude:${n[0]}, Longitude:${n[1]}</span>`);return}else e.list.appendChild(mapp.utils.html.node`
          <li style="color: red;">
            <span>${mapp.dictionary.invalid_lat_long_range}</span>`)}e.provider&&(Object.hasOwn(mapp.utils.gazetteer,e.provider)?mapp.utils.gazetteer[e.provider](t.target.value,e):console.warn(`Requested gazetteer service not available`)),mapp.utils.gazetteer.datasets(t.target.value,e)}},S={applyFilter:pt,boolean:_t,date:w,datetime:w,in:St,integer:bt,like:gt,match:gt,ni:St,null:vt,numeric:bt,removeFilter:mt,resetFilter:ht,generateMinMax:yt},ft;function pt(e){clearTimeout(ft),ft=setTimeout(()=>{e.reload(),e.style?.theme&&mapp.ui.layers.legends[e.style.theme.type]?.(e),e.filter.list?.forEach(e=>{e.histogram?.update?.()}),e.mapview?.target.dispatchEvent(new Event(`changeEnd`))},500)}function C(e,t){e.filter.current[t.field]&&(Object.hasOwn(e.filter.current[t.field],t.type)&&delete e.filter.current[t.field][t.type],Object.hasOwn(e.filter.current,t.field)&&delete e.filter.current[t.field],Number.isNaN(Number(t.Min))&&delete t.min,Number.isNaN(Number(t.Max))&&delete t.max),mapp.ui.layers.filters.applyFilter(e)}function mt(e,t){C(e,t),t.li?.classList.remove(`selected`),t.card?.remove(),t.histogram?.removeChangeEnd?.(),delete t.selected,delete t.card,e.filter.list?.some(e=>e.card)||(e.filter.clearAll instanceof HTMLElement&&e.filter.clearAll.style.setProperty(`display`,`none`),e.filter.resetAll instanceof HTMLElement&&e.filter.resetAll.style.setProperty(`display`,`none`),e.filter.feature_count instanceof HTMLElement&&e.filter.feature_count.style.setProperty(`display`,`none`))}async function ht(e,t){if(C(e,t),!t.card)return;t.content=[await mapp.ui.layers.filters[t.type](e,t)].flat();let n=mapp.ui.elements.card(t);t.card.replaceWith(n),t.card=n}function gt(e,t){return mapp.utils.html.node`
  <input
    type="text"
    onkeyup=${n=>{n.target.value.length?(e.filter.current[t.field]??={},e.filter.current[t.field][t.type]=encodeURIComponent(`${t.leading_wildcard&&`%`||``}${n.target.value}`)):delete e.filter.current[t.field],S.applyFilter(e)}}>`}function _t(e,t){function n(n){e.filter.current[t.field]={boolean:n},S.applyFilter(e)}return n(!1),mapp.ui.elements.chkbox({label:t.label||t.title||`chkbox`,onchange:n})}function vt(e,t){function n(n){e.filter.current[t.field]={null:n},S.applyFilter(e)}return n(!1),mapp.ui.elements.chkbox({label:t.label||t.title||`chkbox`,onchange:n})}async function yt(e,t){let n=mapp.utils.queryParams({layer:e,queryparams:{table:!0,template:`field_minmax`,field:t.field},viewport:e.filter.viewport});n.filter=structuredClone(e.filter.current),delete n.filter[t.field];let r=mapp.utils.paramString(n),i=await mapp.utils.xhr(`${e.mapview.host}/api/query?${r}`),a=Number.isNaN(Number(t.min))?i.minmax[0]:t.min,o=Number.isNaN(Number(t.max))?i.minmax[1]:t.max;e.filter?.viewport&&(a=i.minmax[0],o=i.minmax[1]),t.min=t.type===`integer`?Math.round(a):Number.parseFloat(a),t.max=t.type===`integer`?Math.round(o):Number.parseFloat(o),t.min=t.val_a<t.min?t.val_a:t.min,t.max=t.val_b>t.max?t.val_b:t.max}async function bt(e,t){let n=e.infoj.find(e=>e.field===t.field);if(Object.assign(t,n),delete t.val_a,delete t.val_b,t.Min??=Number.isFinite(t.min)?t.min:void 0,t.Max??=Number.isFinite(t.max)?t.max:void 0,(Number.isNaN(Number(t.max))||Number.isNaN(Number(t.min)))&&await yt(e,t),Number.isNaN(Number(t.min))||Number.isNaN(Number(t.max)))return mapp.utils.html.node`<div>${mapp.dictionary.no_data_filter}</div>`;t.step??=t.type===`integer`?1:.01,e.filter.current[t.field]=Object.assign({gte:Number(t.min),lte:Number(t.max)},e.filter.current[t.field]),S.applyFilter(e);let r=t.prefix||t.suffix?`(${(t.prefix||t.suffix).trim()})`:``;return t.label_a??=`${mapp.dictionary.layer_filter_greater_than} ${r}`,t.label_b??=`${mapp.dictionary.layer_filter_less_than} ${r}`,t.val_a=e.filter.current?.[t.field]?.gte,t.val_b=e.filter.current?.[t.field]?.lte,t.callback_a=n=>{e.filter.current[t.field].gte=n,t.val_a=Number(n),S.applyFilter(e)},t.callback_b=n=>{e.filter.current[t.field].lte=n,t.val_b=Number(n),S.applyFilter(e)},t.slider=mapp.ui.elements.slider_ab(t),await xt(t,e),mapp.utils.html`
    ${t.histogram?.container}
    ${t.slider}`}async function xt(e,t){if(e.histogram&&(e.histogram===!0&&(e.histogram={}),e.histogram.container??=mapp.utils.html.node`<div>`,e.histogram.dataview??=`histogram`,e.histogram.options??={tooltip:!0},e.histogram.target??=e.histogram.container,e.histogram.layer??=t,e.histogram.queryparams??={field:e.field,table:!0,filter:!0},e.histogram.viewport??=t.filter.viewport,e.histogram.update||await mapp.ui.Dataview(e.histogram),e.histogram.viewport)){t.mapview.Map.getTargetElement().addEventListener(`changeEnd`,n),e.histogram.removeChangeEnd=()=>{t.mapview.Map.getTargetElement().removeEventListener(`changeEnd`,n)};function n(){e.histogram.update()}}}async function St(e,t){if(C(e,t),!Array.isArray(t[t.type])){let n=structuredClone(e.filter?.current);delete n?.[t.field];let r=await mapp.utils.xhr(`${e.mapview.host}/api/query?`+mapp.utils.paramString({field:t.field,filter:n,layer:e.key,locale:e.mapview.locale.key,table:e.tableCurrent(),template:`distinct_values`}));if(!r)return console.warn(`Distinct values query did not return any values for field ${t.field}`),mapp.utils.html.node`<div>${mapp.dictionary.no_data_filter}</div>`;t[t.type]=[r].flat().map(e=>e[t.field]).filter(e=>e!==null)}let n=new Set(e.filter?.current[t.field]?.[t.type]||[]);if(t.dropdown||t.dropdown_pills||t.dropdown_search){let r=mapp.ui.elements.dropdown({callback:async(n,r)=>{r.length?Object.assign(e.filter.current,{[t.field]:{[t.type]:r}}):delete e.filter.current[t.field],S.applyFilter(e)},field:t.field,search:t.dropdown_search,entries:t[t.type].map(e=>({option:e,selected:n.has(e),title:e})),inputfilter:!0,keepPlaceholder:t.dropdown_pills,maxHeight:300,multi:!0,pills:t.dropdown_pills,placeholder:t.placeholder||mapp.dictionary.layer_filter_dropdown_select});return mapp.utils.html.node`<div class="filter">${r}`}if(t.searchbox){let n=mapp.utils.html.node`<div class="filter">`,r=mapp.ui.elements.pills({pills:e.filter?.current[t.field]?.[t.type],addCallback:(n,r)=>{Object.assign(e.filter.current,{[t.field]:{[t.type]:[...r]}}),S.applyFilter(e)},removeCallback:(n,r)=>{r.size===0?(i.input.value=null,delete e.filter.current[t.field]):Object.assign(e.filter.current,{[t.field]:{[t.type]:[...r]}}),S.applyFilter(e)},target:n}),i=mapp.ui.elements.searchbox({placeholder:mapp.dictionary.filter_searchbox_placeholder,searchFunction:e=>{if(i.list.innerHTML=``,!e.target.value)return;let n=e.target.value,a=t[t.type].filter(e=>e.toString().toLowerCase().startsWith(n.toLowerCase()));if(!a.length){i.list.append(mapp.utils.html.node`
    ...
    <div style="
      ...
      <label>Date after
        ${n}</label>
      <label>Date before
        ${r}</label>`}function Ct(e){let t=e.style.theme;t.legend??={},t.legend.grid=[],e.style.theme.style??={},e.style.theme.style.width??=24,e.style.theme.style.height??=24;let n=mapp.ui.elements.legendIcon(e.style.theme.style);return t.legend.grid.push(mapp.utils.html`
    <div 
      class="contents">
      ${n}<div class="label" style="grid-column: 2";>${e.style.theme.label}`),t.legend.node=mapp.utils.html.node`
    <div class="legend">
    <div class="contents-wrapper grid">${t.legend.grid}`,e.style.legend??=t.legend.node,e.style.legend&&e.style.legend.replaceChildren(...t.legend.node.children),t.legend.node}function T(e,t,n){if(e.field??=t.field,e.disabled??=n.filter?.current[e.field]?.ni?.includes(e.value),e.disabled||(e.style??=e._style),n.featureFields&&t.distribution===`count`){let r={value:n.featureFields[e.field]?.[e.value]};if(e.count=mapp.utils.formatNumericValue(r),!e.disabled&&!t.legend?.showEmptyCat&&!e.count)return}let r=mapp.ui.elements.legendIcon({height:24,width:24,...e._style||e.style}),i=mapp.utils.html`<div
    ...
    data-id="cluster"
    class="contents">
    ${t}${n}`}function O(e){e.legend??={},e.legend.layout??=`grid`;let t=[`contents-wrapper`,e.legend.layout];e.legend.nowrap&&t.push(`nowrap`),e.legend.classList=t.join(` `)}function k(){return mapp.utils.html`<div
    class="switch-all"
    ...
    </button>.`}function Et(e){let t=e.style.theme;O(t),t.legend.grid=t.categories.map(n=>T(n,t,e)).filter(e=>e!==void 0),e.style.cluster&&t.legend.grid.push(D(e));let n=k();return t.legend.node=mapp.utils.html.node`
    <div class="legend">
      ${n}
      <div class=${t.legend.classList}>
        ${t.legend.grid}`,e.style.legend??=t.legend.node,e.style.legend&&e.style.legend.replaceChildren(...t.legend.node.children),t.legend.node}function A(e){e.style.theme.filterOnly=!0;let t=e.style.theme;if(O(t),!t.hideLegend){t.legend.grid=[];for(let n of t.categories)n.label=n.values?.join(`, `),n.label!==void 0&&t.legend.grid.push(T(n,t,e));return e.style.cluster&&t.legend.grid.push(D(e)),t.legend.node=mapp.utils.html.node`
    <div class="legend">
      <div class=${t.legend.classList}>
        ${t.legend.grid}`,e.style.legend??=t.legend.node,e.style.legend&&e.style.legend.replaceChildren(...t.legend.node.children),e.L.once(`postrender`,()=>{t.key===e.style.theme.key&&A(e)}),t.legend.node}}function Dt(e){let t=e.style.theme;t.filterOnly=!0,O(t),t.legend??={};let n=t.categories.filter(e=>e.value!==void 0).map(n=>{let r=`contents ${t.legend?.horizontal?`horizontal`:``}`,i=mapp.ui.elements.legendIcon({height:24,width:24,...n._style||n.style});n.label??=n.value;let a=`label switch ${n.disabled?`disabled`:``}`;return mapp.utils.html`<div 
        data-id=${n.value}
        class=${r}>
        <div 
          style="height: 24px; width: 24px; grid-column: 1;">
        ...
        <div 
          ...
          style="grid-column: 2;"
          onclick=${t=>E(t,e,n)}>
          ${n.label}`}),r=k();return t.legend.node=mapp.utils.html.node`
    <div class="legend">
      ${r}
      <div class=${t.legend.classList}>
        ${n}`,e.style.legend??=t.legend.node,e.style.legend&&e.style.legend.replaceChildren(...t.legend.node.children),t.legend.node}var Ot={basic:Ct,categorized:Et,distributed:A,graduated:Dt};function kt(e){if(!e.target)return;let t={add:At,createGroup:jt,groups:{},node:e.target};return typeof e.layers==`object`&&Object.values(e.layers).length&&Object.values(e.layers).forEach(e=>t.add(e)),t}function At(e){if(!e.hidden){if(mapp.ui.layers.view(e),!e.group){this.node.append(e.view),this.node.dispatchEvent(new CustomEvent(`addLayerView`,{detail:e}));return}this.groups[e.group]||this.createGroup(e),this.groups[e.group].addLayer(e),this.node.dispatchEvent(new CustomEvent(`addLayerView`,{detail:e}))}}function jt(e){let t={list:[]};this.groups[e.group]=t,t.hideLayers=mapp.utils.html.node`<button
  class="notranslate material-symbols-outlined active"
    ...
    onclick=${e=>{e.target.style.visibility=`hidden`,t.list.filter(e=>e.display).forEach(e=>e.hide())}}>visibility_off`,t.meta=mapp.utils.html.node`<div class="meta no-select">`,t.drawer=mapp.ui.elements.drawer({class:`layer-group ${e.groupClassList||``}`,content:t.meta,data_id:e.group,header:mapp.utils.html`
       <h2>${e.group}</h2>
       ${t.hideLayers}`}),this.node.appendChild(t.drawer),t.chkVisibleLayer=Nt,t.addLayer=Mt}function Mt(e){if(e.groupmeta){let t=mapp.utils.html.node`<div>`;t.innerHTML=e.groupmeta,mapp.utils.render(this.meta,t)}this.list.push(e),this.drawer.appendChild(e.view),this.chkVisibleLayer(),e.showCallbacks.push(()=>this.chkVisibleLayer()),e.hideCallbacks.push(()=>this.chkVisibleLayer())}function Nt(){this.hideLayers.style.visibility=this.list.some(e=>e.display)?`visible`:`hidden`}function Pt(e){let t=[];for(let[n,r]of Object.entries(e.dataviews))if(!(typeof r==`string`||r===!0||r===!1)){if(Object.assign(r,{host:e.mapview.host,key:n,layer:e}),Ft(r),r.tabview&&mapp.utils.mobile()||(r.label??=r.title||r.key,mapp.ui.utils.dataviewDialog(r),r.target=r.target instanceof HTMLElement?r.target:mapp.utils.html.node`
    ...
    <p style="display:none">${e.filter.count} ${e.filter.count_meta}`,e.filter.viewport_description??=e.filter.viewport?mapp.dictionary.filter_in_viewport:mapp.dictionary.filter_not_in_viewport,e.filter.viewport_description=mapp.utils.html.node`<p style="display:none"><i>${e.filter.viewport_description}</i>`,e.filter.content=[e.filter.dropdown,e.filter.clearAll,e.filter.resetAll,e.filter.feature_count,e.filter.viewport_description],e.filter.dialog&&(e.filter.filter_list=mapp.utils.html.node`
      <div id=filter-list class=filter-list>`,e.filter.content=[mapp.utils.html`<p class=bold>${e.name}</p>`,e.filter.feature_count,e.filter.viewport_description,e.filter.clearAll,e.filter.resetAll,e.filter.dropdown,e.filter.filter_list],e.filter.content=mapp.utils.html.node`${e.filter.content}`,e.filter.dialog&&(e.filter.dialog={btn_label:`Filtering`,title:`Filtering`,icon:`filter_alt`,view:e.filter.filter_list,...e.filter.dialog}),e.filter.clearAll.style.display=`inline-block`,e.filter.resetAll.style.display=`inline-block`),e.filter.drawer={drawer:e.filter.drawer,data_id:`filter-drawer`,class:e.filter.classList||``,header:mapp.utils.html`
        <h3>${mapp.dictionary.filter_header}</h3>`,content:e.filter.dialog?.btn||e.filter.content,popout:e.filter.popout,dialog:e.filter.dialog,view:e.view,layer:e},e.filter.view=mapp.ui.elements.drawer(e.filter.drawer),e.filter.view}var j={numeric:`numeric`,integer:`integer`,text:`like`,date:`date`,datetime:`datetime`,boolean:`boolean`};function Rt(e){let t=[];e.filter.include??=[],e.filter.exclude??=[];for(let n of e.infoj)if(!(n.skipEntry===!0||n.field===void 0||e.filter.exclude.includes(n.field))){if(e.filter.includeAll||e.filter.include.includes(n.field)||n.filter===!0){if(n.type??=`text`,!Object.hasOwn(j,n.type))continue;Object.keys(n.filter||{}).length||(n.filter=j[n.type])}typeof n.filter==`string`&&(n.filter={type:n.filter}),!(!n.filter?.type||!Object.hasOwn(mapp.ui.layers.filters,n.filter.type))&&(n.filter.title??=n.title,n.filter.field??=n.field,t.push(structuredClone(n.filter)))}return t}function M(e){e.display&&(clearTimeout(e.filter.debounce),e.filter.debounce=setTimeout(async()=>{for(let t of e.filter.list){if(!Object.hasOwn(e.filter.current,t.field)||t.min===void 0||t.max===void 0||t.Min&&t.Max||(await mapp.ui.layers.filters.generateMinMax(e,t),t.max-t.min===0))continue;let n=mapp.ui.elements.slider_ab(t);t.slider.replaceWith(n),t.slider=n}mapp.ui.utils.locationCount(e).then(t=>{let n=mapp.utils.formatNumericValue({value:t});e.filter.count.innerText=n,e.filter.feature_count.style.setProperty(`display`,`block`)})},1e3))}function zt(e){e.multi_filter&&(mapp.layer.multi_filter||(delete e.filter.hidden,e.filter.dialog=!0,e.filter.drawer=!1,e.filter.includeAll=!0))}var Bt=e=>{let t={layer:e.key,mapview:e.mapview,target:mapp.utils.html.node`<div>`,...e.gazetteer};return mapp.ui.Gazetteer(t),t.target};function Vt(e){e.jsonEditor===!0&&(e.jsonEditor={});let t=mapp.utils.html.node`<button class="wide flat" data-id="jsonEditor">JSON Editor</button>`;return t.addEventListener(`click`,async()=>{t.classList.toggle(`active`);let n=mapp.utils.jsonParser(e),r=mapp.utils.html.node`<div>`,i=await mapp.ui.utils.layerJSE(r,{json:n},{className:`text-button`,onClick:()=>Ht(i,e),text:`Update Layer`,title:`Update Layer`,type:`button`});e.jsonEditor.dialog=mapp.ui.elements.dialog({header:mapp.utils.html.node`<div>`,css_style:`width: 500px; height 300px;`,containedCentre:!0,contained:!0,closeBtn:!0,onClose:()=>{t.classList.toggle(`active`)},content:r})}),e.removeCallbacks.push(e=>{e.jsonEditor?.dialog?.close()}),t}async function Ht(e,t){let n=e.get(),r=JSON.parse(n.text);await t.update(r)}function Ut(e){let t=mapp.utils.html.node`<p data-id="meta" class="meta no-select">`;return t.innerHTML=e.meta,t}function Wt(e){let t=[];for(let[n,r]of Object.entries(e.reports)){if(typeof r==`boolean`)continue;r.key=n,r.label??=n,r.layer=e,r.template&&(r.report={template:r.template});let i=mapp.ui.elements.reportLink(r);t.push(i)}return e.reports.dialog&&(e.reports.dialog={btn_label:`Reports`,title:`Reports`,icon:`summarize`,...e.reports.dialog}),mapp.ui.elements.drawer({drawer:e.reports.drawer,class:e.reports.classList||``,data_id:`reports-drawer`,header:mapp.utils.html`
      <h3>Reports</h3>`,content:mapp.utils.html`${t}`,popout:e.reports.popout,dialog:e.reports.dialog,layer:e})}function Gt(e){if(e.style.hidden)return;e.style.elements??=[`labels`,`label`,`hovers`,`hover`,`themes`,`theme`,`icon_scaling`,`opacitySlider`];let t=mapp.ui.elements.layerStyle.panel(e);if(!t)return;e.style.classList??=``;let n=mapp.utils.html`
    <h3>${mapp.dictionary.layer_style_header}</h3>`;return e.style.dialog&&(e.style.dialog={btn_label:`Styling`,title:`Styling`,icon:`brush`,...e.style.dialog}),e.style.drawer=mapp.ui.elements.drawer({drawer:e.style.drawer,data_id:`style-drawer`,class:e.style.classList,header:n,content:t,popout:e.style.popout,dialog:e.style.dialog,layer:e}),e.style.drawer}var Kt={dataviews:Pt,draw:It,filter:Lt,gazetteer:Bt,jsonEditor:Vt,meta:Ut,reports:Wt,style:Gt};function qt(e){if(e.view===null)return;Jt(e);let t=mapp.utils.html`
    <h2>${e.name}</h2>
    ${e.viewConfig.headerBtn}`;e.view=mapp.ui.elements.drawer({drawer:e.viewConfig.drawer,data_id:e.key,class:e.viewConfig.classList,header:t,content:e.viewConfig.content,popout:e.viewConfig.popoutBtn}),e.changeEndCallbacks.push(Yt)}function Jt(e){e.viewConfig={displayToggle:!0,zoomBtn:!0,zoomToFilteredExtentBtn:!0,hideDisabled:!1,...e.viewConfig},e.viewConfig.drawer??=e.drawer,e.zoomBtn===!1&&(delete e.viewConfig.zoomBtn,delete e.zoomBtn),e.viewConfig.popoutBtn??=e.popout,e.tables||(delete e.viewConfig.zoomBtn,delete e.viewConfig.hideDisabled),e.viewConfig.panelOrder??=e.panelOrder,delete e.panelOrder,e.viewConfig.keyOrder??=[`meta`,`style`,`filter`,`dataviews`,`draw`],e.viewConfig.content=Object.keys(e).sort((t,n)=>{let r=e.viewConfig.keyOrder.indexOf(t),i=e.viewConfig.keyOrder.indexOf(n);return r===-1&&(r=1/0),i===-1&&(i=1/0),r-i}).map(t=>mapp.ui.layers?.panels?.[t]?.(e)).filter(e=>e!==void 0),Array.isArray(e.viewConfig.panelOrder)&&(console.warn(`Layer [${e.key}] has the legacy panelOrder config with inverted logic; please use viewConfig.keyOrder array!`),e.viewConfig.content.sort((t,n)=>e.viewConfig.panelOrder.indexOf(t.dataset?.id)<e.viewConfig.panelOrder.indexOf(n.dataset?.id)?1:-1)),e.viewConfig.classList??=e.classList||``,e.viewConfig.classList+=` layer-view `,e.viewConfig.classList+=e.viewConfig.content.length?``:` empty `,Array.isArray(e.viewConfig.headerOrder)||(e.viewConfig.headerOrder=[`zoomToFilteredExtentBtn`,`zoomBtn`,`popoutBtn`,`displayToggle`].filter(t=>e.viewConfig[t])),e.viewConfig.headerBtn??=[];for(let t of e.viewConfig.headerOrder){let n=mapp.ui.layers.viewHeader[t]?.(e);e.viewConfig.headerBtn.push(n)}}function Yt(e){if(!e.tableCurrent)return;let t=Array.from(e.view.querySelectorAll(`:scope > :not(.header)`));e.tableCurrent()?(e.viewConfig.hideDisabled&&e.view.classList.remove(`display-none`),e.zoomBtn instanceof HTMLElement&&e.zoomBtn.style.setProperty(`display`,`none`),e.view.querySelector(`[data-id="display-toggle"]`)?.classList.remove(`disabled`),t.forEach(e=>{e.disabled=!1,e.classList.remove(`disabled`)})):(e.table||e.tables)&&(e.zoomBtn instanceof HTMLElement&&e.zoomBtn.style.removeProperty(`display`),e.view.classList.remove(`expanded`),[...e.view.querySelectorAll(`.expanded`)].forEach(e=>e.classList.remove(`expanded`)),e.view.querySelector(`[data-id="display-toggle"]`)?.classList.add(`disabled`),t.forEach(e=>{e.disabled=!0,e.classList.add(`disabled`)}),e.viewConfig.hideDisabled&&e.view.classList.add(`display-none`))}var Xt={zoomToFilteredExtentBtn:Zt,zoomBtn:Qt,popoutBtn:en,displayToggle:tn};function Zt(e){let t=Object.keys(e.filter.current).length?``:`display: none;`,n=mapp.utils.html.node`<button
    data-id=zoomToFilteredExtentBtn
    title=${mapp.dictionary.layer_zoom_to_extent}
    style=${t}
    class="notranslate material-symbols-outlined"
    onclick=${async t=>{t.target.disabled=!await e.zoomToExtent(),e.show()}}>filter_alt`;function r(e){Object.keys(e.filter.current).length?n.style.display=`inline-block`:n.style.display=`none`}return e.changeEndCallbacks.push(r),n}function Qt(e){let t=mapp.utils.html.node`<button
    data-id="zoom-to"
    title=${mapp.dictionary.zoom_to}
    class="notranslate material-symbols-outlined"
    onclick=${()=>$t(e)}>arrows_input`;function n(e){if(e.tableCurrent()){t.style.display=`none`;return}t.style.display=`block`}return e.changeEndCallbacks.push(n),t}function $t(e){let t=Object.entries(e.tables).find(e=>!!e[1])[0],n=Object.entries(e.tables).reverse().find(e=>!!e[1])[0],r=e.mapview.Map.getView();r.getZoom()<t?r.setZoom(t):r.setZoom(n),e.show()}function en(e){return mapp.utils.html.node`<button
    data-id="popout-btn"
    class="notranslate material-symbols-outlined">open_in_new`}function tn(e){let t=`notranslate material-symbols-outlined toggle ${e.zoomDisplay||e.display?`toggle-on`:``}`,n=mapp.utils.html.node`<button
    data-id=display-toggle
    title=${mapp.dictionary.layer_visibility}
    class=${t}
    onclick=${t=>{t.target.classList.toggle(`toggle-on`)?e.show():e.hide()}}>`;return e.showCallbacks.push(()=>{n.classList.add(`toggle-on`)}),e.hideCallbacks.push(()=>{!e.zoomDisplay&&n.classList.remove(`toggle-on`)}),n}var nn={filters:S,legends:Ot,listview:kt,panels:Kt,view:qt,viewHeader:Xt};function rn(e){if(e.edit)return mapp.ui.elements.chkbox({checked:e.newValue===void 0?e.value:e.newValue,disabled:!e.edit,label:e.label||e.title,onchange:t=>{e.newValue=t,e.location.view?.dispatchEvent(new CustomEvent(`valChange`,{detail:e}))}});e.label??=``;let t=e.value?`check`:`close`;return mapp.utils.html.node`
    <div class="link-with-img">
      <div class="notranslate material-symbols-outlined">${t}</div>
      <span>${e.label}`}var N={documents:sn,image:an,images:on},P=e=>N[e.type](e);function an(e){if(e.value){let t=e.edit&&mapp.utils.html`<button 
      ...
      <input
        type="file"
        accept="image/*;capture=camera" onchange=${t=>{F(t,e)}}>`}function on(e){let t=[];if(e.value?.map(n=>{let r=e.edit&&mapp.utils.html`<button title="${mapp.dictionary.delete}"
      class="notranslate material-symbols-outlined color-danger delete"
      ...
      <input
        type="file"
        accept="image/*;capture=camera" onchange=${t=>{F(t,e)}}>`;t.push(n)}if(t.length)return mapp.utils.html.node`<div class="images-grid">${t}`}function sn(e){let t=[];if(e.value?.map(n=>{let r=e.edit&&mapp.utils.html`<button 
      title="${mapp.dictionary.delete}"
      class="notranslate material-symbols-outlined color-danger delete"
      data-name=${n.replace(/^.*\//,``).replace(/\.([\w-]{3})/,``)}
      data-href=${n}
      ...
      <input type="file"
        ...
        onchange=${t=>F(t,e)}>`;t.unshift(n)}if(t.length)return mapp.utils.html.node`<div>${t}`}async function F(e,t){let n=new FileReader,r=e.type===`drop`?e.dataTransfer?.files[0]:e.target.files[0];if(!r)return;t.location.view?.classList.add(`disabled`);let i={documents:ln,image:I,images:I};n.onload=e=>i[t.type](e,t,r),n.readAsDataURL(r)}function I(e,t,n){let r=new Image;r.onload=async()=>cn(t,r,n),r.src=e.target.result}async function cn(e,t,n){let r=document.createElement(`canvas`);e.max_size??=1024,t.width>t.height&&t.width>e.max_size?(t.height*=e.max_size/t.width,t.width=e.max_size):t.height>e.max_size&&(t.width*=e.max_size/t.height,t.height=e.max_size),r.width=t.width,r.height=t.height,r.getContext(`2d`).drawImage(t,0,0,t.width,t.height);let i=await z(e,{public_id:n.name.replace(/^.*\//,``).replace(/\.([\w-]{3})/,``)+e.suffix_date?`@${Date.now()}`:``});if(!i)return;let a=new FormData;a.append(`file`,r.toDataURL(`image/jpeg`,.5));let o=await fetch(i,{body:a,method:`post`});if(!o||o.error){let e=`Cloudinary Image upload failed! ${o?.error?.message?`Error: ${o.error.message}`:``}`;mapp.ui.elements.alert({text:e});return}let s=await o.json();e.type===`image`?e.value=s.secure_url:e.value=Array.isArray(e.value)?e.value.concat([s.secure_url]):[s.secure_url],R(e)}async function ln(e,t,n){let r=new Date,i=`${r.getMonth()+1}-${r.getDate()}T${r.getHours()}:${r.getMinutes()}`,a=n.name.substring(n.name.lastIndexOf(`.`)),o=await z(t,{public_id:`${n.name.replace(a,``)}-${i}${a}`});if(!o)return;let s=new FormData;s.append(`file`,e.target.result.toString());let c=await fetch(o,{body:s,method:`post`});if(!c||c.error){let e=c?.error?.message?`Error: ${c.error.message}`:``;mapp.ui.elements.alert(`Cloudinary document upload failed! ${e}`);return}let l=await c.json();t.value=Array.isArray(t.value)?t.value.concat([l.secure_url]):[l.secure_url],R(t)}async function L(e,t,n){if(!await mapp.ui.elements.confirm({text:n}))return;let r=await z(t,{destroy:!0,public_id:decodeURIComponent(e.target.dataset.name)});if(!r)return;await fetch(r,{method:`post`});let i=new Set(t.value);i.delete(e.target.dataset.src||e.target.dataset.href),t.type===`image`?t.value=null:t.value=i.size?Array.from(i):null,R(t)}async function R(e){e.location.view?.classList.add(`disabled`),await mapp.utils.xhr({body:JSON.stringify({[e.field]:e.value}),method:`POST`,url:`${e.location.layer.mapview.host}/api/query?`+mapp.utils.paramString({id:e.location.id,layer:e.location.layer.key,locale:e.location.layer.mapview.locale.key,table:e.location.table,template:e.location.updateTemplate})}),e.node.replaceChildren(mapp.utils.html.node`<div class="label">${e.title}`,N[e.type](e)),e.location.view?.classList.remove(`disabled`)}async function z(e,t){let n=mapp.utils.paramString({...t,folder:e.cloudinary_folder}),r=await mapp.utils.xhr({responseType:`text`,url:`${e.location.layer.mapview.host}/api/sign/cloudinary?${n}`});return r instanceof Error&&console.error(r),r}function un(e){if(e.value!==void 0&&(e.data=e.value),e.data?.length===0&&(e.data=null),e.layer??=e.location.layer,dn(e),e.target instanceof HTMLElement){if(!e.display&&typeof e.create==`function`)return mapp.utils.html.node`
    ${e.chkbox||``}
    ...
    class="val"
    style=${e.css_val}>
    ${t}`}function pn(e){if(e.format??=`GeoJSON`,e.mapview??=e.location.layer.mapview,typeof e.value==`string`&&(e.value=JSON.parse(e.value)),e.srid??=e.location.layer.srid,e.zIndex??=e.location.layer.zIndex+1||99,e.display??=!1,e.edit?.draw&&(e.draw=e.edit.draw),e._edit?.draw&&delete e.draw,!e.value&&!e.draw&&!e.api){e.location.removeLayer(e);return}e.style!==null&&(e.style={...e.location.style,...e.style},e.Style??=mapp.utils.style(e.style)),e.show??=hn,e.hide??=gn,e.getExtent??=mn,e.modify??=yn,e.label??=`Geometry`,e.disabled=!e.value&&!e.api||e.error,e.chkbox=mapp.ui.elements.chkbox({checked:e.display,data_id:`chkbox-${e.key}`,disabled:e.disabled,label:e.label,onchange:t=>{t?e.show():e.hide()}}),Array.isArray(e.elements)||(e.elements=e.api_elements||[],bn(e),_n(e)),e.edit||delete e.elements;let t=mapp.ui.elements.legendIcon({height:24,width:24,...e.style});return e.display&&e.show(),mapp.utils.html.node`
    <div class="flex-spacer">${e.chkbox}${t}</div>
    ${e.elements}`}function mn(){if(this.display&&this.L)return this.L.getSource().getExtent()}async function hn(){if(this.disabled)return;this.location.removeLayer(this),this.display=!0;let e=this.chkbox?.querySelector(`input`);if(e&&(e.checked=!0),!this.value&&this.api){if(this.blocking&&this.location.view?.classList.add(`disabled`),await this.api(this),this.blocking)return;this.value||(this.disabled=!0)}this.value&&(this.L=this.location.layer.mapview.geometry(this),this.location.Layers.push(this))}function gn(){this.display=!1,this.location.removeLayer(this)}function _n(e){e.edit&&e.value&&(e.edit.modify_label??=mapp.dictionary.modify_geometry,e.edit.modify_btn=mapp.utils.html.node`
    <button
      class="action wide"
      data-id="modify-btn"
    ...
    <span style="color: var(--color-danger)">${e.edit.delete_label}</span>`))}async function vn(e){if(await e.location.confirmUpdate(t)===null)return;t();function t(){e.display=!1,e.newValue=null,e.location.update(n)}function n(){delete e.elements,e.location.renderLocationView()}}async function yn(){let e=this;if(await e.location.confirmUpdate(()=>{e.location.renderLocationView()})===null){e.edit.modify_btn.classList.remove(`active`);return}if(!e.hideHelp){let e={content:mapp.utils.html.node`<li>
        ...
        <ul>${mapp.dictionary.edit_dialog_save}</ul>`,header:mapp.utils.html`<h3>${mapp.dictionary.edit_dialog_title}</h3>`};mapp.ui.elements.helpDialog(e)}!e.display&&e.show(),e.location.layer.mapview.Map.removeLayer(e.L);let t=e.L.getSource().getFeatures()[0].clone();e.location.layer.mapview.interactions.modify({Feature:t,layer:e.location.layer,snap:e.edit.snap,srid:e.srid,callback:t=>{e.edit.modify_btn.classList.remove(`active`),V(t,e)}})}function bn(e){e.draw&&(Object.keys(e.draw).forEach(t=>{if(e.draw[t]===!0&&(e.draw[t]={}),mapp.ui.elements.drawing[t]){e.draw[t].callback??=n=>{e.draw[t].btn.classList.remove(`active`),V(n,e)};let n=mapp.ui.elements.drawing[t](e);e.draw.dialog&&e.edit?(e.draw.content??=[],n instanceof HTMLElement&&e.draw.content.push(n)):n instanceof HTMLElement&&e.elements.push(n)}}),mapp.ui.elements.drawer({layer:e,key:`draw`,name:`Draw`,icon:`new_label`}),e.edit&&e.draw?.dialog?.btn&&e.elements.push(e.draw.dialog.btn))}function V(e,t){if(delete t.mapview.interaction,mapp.ui.elements.helpDialog(),!e){setTimeout(()=>{!t.mapview.interaction&&t.mapview.interactions.highlight()},400),t.location.renderLocationView();return}t.newValue=e.geometry,t.location.update(n);function n(){delete t.elements,t.location.renderLocationView()}}function xn(e){e.css_val??=``,e.inline&&console.warn(`${e.key} - json entries cannot be inline, please remove inline true from entry`);let t=mapp.utils.html.node`<div
    class="val"
    style=${e.css_val}>`;e.data=e.newValue||e.value;function n(e){return e.filter(e=>e.text!==`tree`).filter(e=>e.text!==`text`).filter(e=>e.text!==`table`).filter(e=>e.type!==`separator`).filter(e=>e.className!==`jse-undo`).filter(e=>e.className!==`jse-redo`).filter(e=>e.className!==`jse-search`).filter(e=>e.className!==`jse-contextmenu`).filter(e=>e.className!==`jse-sort`).filter(e=>e.className!==`jse-transform`)}e.jsoneditor=mapp.ui.elements.jsoneditor({data:e.data,props:{mode:`text`,onChange:r,onRenderMenu:n,readOnly:!e.edit},target:t});function r(t,n,r){r.contentErrors||(e.newValue=JSON.parse(t.text),e.location.view?.dispatchEvent(new CustomEvent(`valChange`,{detail:e})))}return t}function Sn(e){e.hideLayer&&e.location.layer.hide();let t=`layer-key ${e.location.layer.display?`active`:``}`;return mapp.utils.html.node`<div>
    <button 
      class=${t}
      title="${mapp.dictionary.layer_visibility}"
      onclick="${t=>{t.target.classList.toggle(`active`)?e.location.layer.show():e.location.layer.hide()}}">${e.location.layer.name}`}function H(e){return Cn(e),e.mapview??=e.location.layer.mapview,wn(e),e.zIndex??=e.location.layer.zIndex++,Tn(e),e.panel??=mapp.utils.html.node`<div class="entry-layer">`,e.panel}function Cn(e){e.value!==void 0&&(e.data=e.value,e.data?.length===0&&(e.data=null)),(e.featureSet||e.featureLookup)&&(!e.query&&!e.data?(e.disabled=!0,e.display_toggle?.classList.add(`disabled`)):(e.disabled=!1,e.display_toggle?.classList.remove(`disabled`)))}function wn(e){if(!e.layer)return;e.key=e.layer;let t=structuredClone(e.mapview.locale.layers.find(t=>t.key===e.layer));if(!t){console.warn(`Layer [${e.layer}] not found in mapview.locale`);return}let n=Object.keys(t).filter(t=>!Object.hasOwn(e,t)).reduce((e,n)=>(e[n]=t[n],e),{});Object.assign(e,n),e.params??={},e.params.layer_template??=e.layer,delete e.layer}async function Tn(e){if(e.L&&e.display){e.show();return}else if(e.L)return;e.source_key=e.key,await mapp.layer.decorate(e),e.L&&(e.location.Layers.push(e),e.key+=`-${e.location.id}`,e.L.set(`key`,e.key),e.show=En,e.hide=On,e.mapview.layers[e.key]=e,e.display_toggle=mapp.ui.elements.chkbox({checked:e.display,data_id:`${e.key}-display`,disabled:e.disabled,label:e.name,onchange:t=>{t?e.show():e.hide()}}),e.tables&&(e.tableCurrent()||e.display_toggle.classList.add(`disabled`),e.changeEndCallbacks.push(()=>{e.tableCurrent()?e.display_toggle.classList.remove(`disabled`):e.display_toggle.classList.add(`disabled`)})),e.panel.append(e.display_toggle),e.meta&&e.panel.append(mapp.ui.layers.panels.meta(e)),e.style.elements??=[`labels`,`label`,`hovers`,`hover`,`icon_scaling`,`opacitySlider`,`themes`,`theme`],e.style.panel=mapp.ui.elements.layerStyle.panel(e),e.style.default&&(e.style.default={...e.location?.style,...e.style.default}),e.style.panel?.style.setProperty(`display`,`none`),e.style.panel&&e.panel.append(e.style.panel),e.location.removeCallbacks.push(()=>{typeof e.remove==`function`&&e.remove()}),e.display&&e.show())}async function En(){let e=this;if(e.query){let t=mapp.utils.queryParams(e),n=mapp.utils.paramString(t);if(e.data=await mapp.utils.xhr(`${e.mapview.host}/api/query?${n}`),!e.data){e.disabled=!0,e.display_toggle?.classList.add(`disabled`),e.panel.replaceChildren(e.display_toggle);return}}Dn(e),e.style.panel?.style.setProperty(`display`,`block`),e.display=!0;try{e.mapview.Map.addLayer(e.L)}catch{}typeof e.reload==`function`&&e.reload(),e.showCallbacks?.forEach(t=>typeof t==`function`&&t(e))}function Dn(e){if(e.data){if(e.featureSet){let t=Array.isArray(e.data)?e.data:[e.data];e.featureSet=new Set(t);return}if(e.featureLookup){e.featureLookup=Array.isArray(e.data)?e.data:[e.data];return}e.features=e.data,e.setSource(e.features)}}function On(){this.display=!1,this.style.panel?.style.setProperty(`display`,`none`),this.mapview.Map.removeLayer(this.L),this.hideCallbacks?.forEach(e=>typeof e==`function`&&e(this))}globalThis.__varlockThrowOnMissingKeys=!0;function U(e){let t=mapp.ui.elements.reportLink(e);return mapp.utils.html.node`<div class="val">${t}`}function W(e){if(e.edit)return kn(e);if(!(e.value===null||Number.isNaN(Number(e.value))))return mapp.utils.formatNumericValue(e),mapp.utils.html.node`<div 
    class="val"
    style=${e.css_val}
    >${e.stringValue}`}function kn(e){e.edit.callback??=()=>{e.location.view?.dispatchEvent(new CustomEvent(`valChange`,{detail:e}))},e.edit.min??=e.min||-2147483648,e.edit.max??=e.max||2147483647,e.edit.step??=e.step||e.type===`integer`?1:.1,Object.assign(e,e.edit);let t=e.value??e.newValue;return e.edit.range&&!Number.isNaN(Number(t))&&t>e.min&&t<e.max?mapp.ui.elements.slider(e):mapp.ui.elements.numericInput(e)}var An=e=>(e.pills??=e.value||[],mapp.ui.elements.pills(e),e.container);function jn(e){if(!Array.isArray(e.value)){console.warn(`Entry type pin requires a value array.`);return}return e.srid??=e.location.layer.srid,e.display??=e.display===void 0,e.zIndex??=1/0,e.Style??=e.style?mapp.utils.style(e.style):e.location.pinStyle,e.show??=Mn,e.hide??=Nn,e.getExtent??=Pn,e.geometry={coordinates:e.value,type:`Point`},e.display&&e.show(),e.label??=mapp.dictionary.pin,e.chkbox??=mapp.ui.elements.chkbox({checked:e.display,label:e.label,onchange:t=>{t?e.show():e.hide()}}),e.chkbox}function Mn(){this.disabled||(this.location.removeLayer(this),this.display=!0,this.L=this.location.layer.mapview.geoJSON(this),this.location.Layers.push(this))}function Nn(){this.display=!1,this.location.removeLayer(this)}function Pn(){return this.buffer??=0,this.buffer=Number.parseInt(this.buffer)||200,ol.extent.buffer(this.L.getSource().getExtent(),this.buffer)}function Fn(e){return e.textarea||(e.textarea=mapp.utils.html.node`<textarea
      class="val"
      style="auto; min-height: 50px;">`,e.callback??=async(e,t)=>{t.textarea.value=e},mapp.utils.ping(e),e.location.removeCallbacks.push(()=>{e.callback=!1})),e.textarea}function In(e){if(!e.query){console.warn(`You must provide a query to use "type": "query_button".`);return}return e.label??=`Run query:${e.query}`,mapp.utils.html.node`
    <button 
      class="flat wide bold"
      onclick=${()=>Ln(e)}>${e.label}`}async function Ln(e){e.updated_fields&&(console.warn(`entry.updated_fields is deprecated, please use entry.dependents instead.`),e.dependents??=e.updated_fields),e.location.view.classList.add(`disabled`),e.queryparams??={},e.queryparams.template=e.query;let t=mapp.utils.paramString(mapp.utils.queryParams(e));e.host??=e.location.layer.mapview.host+`/api/query`;let n=await mapp.utils.xhr(`${e.host}?${t}`);if(n instanceof Error){mapp.ui.elements.alert({text:mapp.dictionary.query_failed}),e.location.view.classList.remove(`disabled`);return}e.value=n,e.alert&&mapp.ui.elements.alert({text:e.alert}),e.reload&&e.location.layer.reload(),e.dependents&&await e.location.syncFields(e.dependents),e.location.view.dispatchEvent(new Event(`updateInfo`)),e.location.view.classList.remove(`disabled`)}var Rn=e=>{let t=document.querySelector(`[data-id=${e.target}]`);return e.tab_style=`border-bottom: 3px solid ${e.location.style.strokeColor}`,t.dispatchEvent(new CustomEvent(`addTab`,{detail:e})),e.display&&e.show(),mapp.ui.elements.chkbox({checked:!!e.display,label:e.label,onchange:t=>{e.display=t,e.display?e.show():e.remove()}})};function zn(e){if(!(!e.edit&&!e.value))return e.edit?Bn(e):mapp.utils.html.node`
    <div
      class="val"
      style=${e.css_val}>
      ${e.prefix}${e.value}${e.suffix}`}function Bn(e){return e.edit.options?(e.container=mapp.utils.html.node`<div>${mapp.dictionary.loading}`,Hn(e),e.container):mapp.utils.html.node`<input
    type="text"
    maxlength=${e.edit.maxlength}
    value="${e.newValue||e.value||``}"
    placeholder="${e.edit.placeholder||``}"
    onkeyup=${t=>Vn(t,e)}>`}function Vn(e,t){t.newValue=t.edit.arraySeparator?e.target.value.split(t.edit.arraySeparator):e.target.value,t.location.view?.dispatchEvent(new CustomEvent(`valChange`,{detail:t}))}async function Hn(e){if(e.edit.dynamic===!0&&(e.edit.options=[]),!e.edit.options.length){(e.jsonb_field||e.json_field)&&(e.edit.query??=`distinct_values_json`),e.edit.query??=`distinct_values`;let t=mapp.utils.paramString({field:e.json_field||e.jsonb_field||e.field,filter:e.location.layer.filter?.current,id:e.location.id,key:e.jsonb_key||e.json_key,layer:e.location.layer.key,locale:e.location.layer.mapview.locale.key,table:e.location.layer.tableCurrent(),template:e.edit.query}),n=await mapp.utils.xhr(`${e.location.layer.mapview.host}/api/query?${t}`);if(n===null){e.container.innerHTML=`${mapp.dictionary.no_options_available}`;return}e.edit.options=[n].flat().map(e=>Object.values(e)[0])}if(e.edit.radio){Un(e);return}let t=e.edit.options.map(e=>({option:e===null?null:typeof e==`string`&&e||Object.values(e)[0],title:e===null?null:typeof e==`string`&&e||Object.keys(e)[0]})),n=t.find(t=>t.option===e.value),r=e.value===void 0?e.edit.placeholder:e.value;mapp.utils.render(e.container,mapp.ui.elements.dropdown({callback:(t,n)=>{e.newValue=n.option,e.location.view?.dispatchEvent(new CustomEvent(`valChange`,{detail:e}))},entries:t,placeholder:r,span:n?.title||e.value}))}function Un(e){let t=e.edit.options.map(n=>{let r={data_id:n,name:e.field,label:n,onchange:()=>{e.newValue=t.find(e=>e.querySelector(`input[type="radio"]:checked`)).querySelector(`input[type="radio"]`).dataset.id,e.location.view?.dispatchEvent(new CustomEvent(`valChange`,{detail:e}))}};return mapp.ui.elements.radio(r)});if(e.value){let n=t.find(t=>t.querySelector(`input[type="radio"]`).dataset.id===e.value);n&&(n.querySelector(`input[type="radio"]`).checked=!0)}mapp.utils.render(e.container,mapp.utils.html.node`${t}`)}function G(e){let t=e.type===`html`?``:e.value;e.edit&&(t=mapp.utils.html`
    <textarea
  ...
  <div
    class="val"
    style="${e.css_val}">${t}`;return!e.edit&&e.type===`html`&&(n.innerHTML=e.value||``),n}function Wn(e){let t,n=e.value?.toString().replace(`.`,`:`);return n=n&&n.length<3&&`${n}:00`||n,t=e.edit?mapp.utils.html.node`
      <input
        type="time"
        value=${n}
    ...
    <div
      class="val"
      style="${e.css_val}">
      ${t}`}function Gn(e){return mapp.utils.html.node`
        <div
          class="label"
          style=${e.css_title}
          title=${e.tooltip}>${e.title}`}function Kn(){console.warn(`The type:defaults entry method has been deprecated.`)}function K(e){return console.warn(`The type:${e.type} entry method has been deprecated in favour of the type:layer entry method.`),e.type=`layer`,e.qID??=`id`,e.name??=e.label,H(e)}var qn={boolean:rn,dataview:un,date:B,datetime:B,defaults:Kn,documents:P,geometry:pn,html:G,image:P,images:P,integer:W,json:xn,key:Sn,layer:H,link:U,mvt_clone:K,numeric:W,pills:An,pin:jn,ping:Fn,query_button:In,report:U,tab:Rn,text:zn,textarea:G,time:Wn,title:Gn,vector_layer:K},q;function Jn(e,t){if(!e.infoj)return;let n=mapp.utils.html.node`<div class="location-view-grid">`;q={},t??=e?.layer?.infoj_order;let r=Array.isArray(t)?t.map(i).filter(e=>e!==void 0):e.infoj;function i(t){if(typeof t==`string`){let n=e.infoj.find(e=>new Set([e.key,e.field,e.query]).has(t));return n||console.warn(`infoj_order field: "${t}" not found in location.infoj. Please add entry.key, entry.field, or entry.query to the entry.`),n}else if(typeof t==`object`)return t.location=e,t}let a=0;for(let t of r){if(t.key??=t.field||a++,e.view?.classList.contains(`disabled`))break;if(t.listview=n,t.type??=`text`,t.edit===!0&&(t.edit={}),Yn(t),Xn(t),Zn(t)||(Qn(t),$n(t),er(t),tr(t),nr(t),rr(t)))continue;if(!Object.hasOwn(mapp.ui.locations.entries,t.type)){console.error(`entry.type:${t.type} method not found.`);continue}let r=mapp.ui.locations.entries[t.type]?.(t);r&&t.node.append(r),ar(t)}return n}function Yn(e){e.jsonb_field&&e.jsonb_key&&e.value!==null&&typeof e.value==`object`&&e.value.jsonb&&(e.value=e.value.jsonb[e.jsonb_field][e.jsonb_key])}function Xn(e){if(e.objectAssignFromField&&e.objectMergeFromField){console.warn(`${e.key}: objectAssignFromField and objectMergeFromField cannot both be used on the same entry.`);return}let t=e.objectAssignFromField||e.objectMergeFromField||e.json_field;if(!t)return;let n=e.location.infoj.find(e=>e.field===t);if(n&&typeof n.value==`object`){if(e.json_field){if(!e.json_key){console.warn(`json_field requires entry.json_key to be specified`);return}if(!n.value)return;e.value=n.value[e.json_key]}e.objectAssignFromField&&Object.assign(e,n.value),e.objectMergeFromField&&mapp.utils.merge(e,n.value)}}function Zn(e){if(e.skipEntry||e.skipFalsyValue&&!e.value&&!e.edit||e.skipUndefinedValue&&e.value===void 0&&!e.edit||e.skipNullValue&&e.value===null&&!e.edit)return!0}function Qn(e){e.nullValue!==void 0&&(e.edit||(e.value??=e.nullValue))}function $n(e){e.default!==void 0&&e.edit&&(e.newValue=e.default,e.location.view?.dispatchEvent(new CustomEvent(`valChange`,{detail:e})))}function er(e){e.group&&(q[e.group]||(q[e.group]=e.listview.appendChild(mapp.ui.elements.drawer({data_id:`location-infoj-group`,class:`group ${e.groupClassList||``}`,header:mapp.utils.html`
          <h3>${e.group}</h3>`}))),e.listview=q[e.group])}function tr(e){let t=`contents ${e.type} ${e.class||``} ${e.inline&&`inline`||``}`;e.node=e.listview.appendChild(mapp.utils.html.node`
  <div
    data-type=${e.type}
    class=${t}>`)}function nr(e){e.title&&e.node.append(mapp.ui.locations.entries.title(e))}function rr(e){if(e.query)if(e.queryCheck??=e.run,e.queryCallback??=ir,e.runOnce||e.queryCheck){delete e.runOnce,e.host??=e.location.layer?.mapview?.host||mapp.host;let t=mapp.utils.queryParams(e),n=mapp.utils.paramString(t);return e.url=`${e.host}/api/query?${n}`,e.blocking&&e.location.view?.classList.add(`loading`),mapp.utils.xhr(e).then(t=>e.queryCallback(t,e)),!0}else e.field&&!e.xhr&&console.warn(`field:"${e.field}" has a query:"${e.query}" which is not set to run, please add the run or runOnce flag to the entry.`)}function ir(e,t){if(t.blocking&&t.location.view?.classList.remove(`loading`),e?t.value=t.field?e[t.field]:e:t.value=t.nullValue||null,Zn(t)){t.node.remove();return}let n=mapp.ui.locations.entries[t.type]?.(t);n&&t.node.appendChild(n)}function ar(e){if(!e.tooltip)return;let t=e.node.querySelector(`label`)||e.node.querySelector(`div.label`);t&&(e.tooltipElement||(e.tooltipElement=mapp.ui.elements.tooltip({content:e.tooltip}),t.append(e.tooltipElement)))}var or=[{symbol:`A`,colour:`#2E6F9E`},{symbol:`B`,colour:`#EC602D`},{symbol:`C`,colour:`#5B8C5A`},{symbol:`D`,colour:`#B84444`},{symbol:`E`,colour:`#514E7E`},{symbol:`F`,colour:`#E7C547`},{symbol:`G`,colour:`#368F8B`},{symbol:`H`,colour:`#841C47`},{symbol:`I`,colour:`#61A2D1`},{symbol:`J`,colour:`#37327F`},{symbol:`K`,colour:`#FFA500`},{symbol:`L`,colour:`#7FFF00`},{symbol:`M`,colour:`#FFD700`},{symbol:`N`,colour:`#00CED1`},{symbol:`O`,colour:`#9932CC`},{symbol:`P`,colour:`#FF69B4`},{symbol:`Q`,colour:`#008B8B`},{symbol:`R`,colour:`#8A2BE2`},{symbol:`S`,colour:`#FF4500`},{symbol:`T`,colour:`#00FF00`},{symbol:`U`,colour:`#FF00FF`},{symbol:`V`,colour:`#708090`},{symbol:`W`,colour:`#F4A460`},{symbol:`X`,colour:`#000080`},{symbol:`Y`,colour:`#D2691E`},{symbol:`Z`,colour:`#8FBC8F`}];function sr(e){e.mapview||console.warn(`A mapview is required in the locations listview params argument.`),e.target||console.warn(`A target element is required in the locations listview params argument.`);let t=e.mapview.locale;t.locations??={},t.locations.pinStyle??={anchor:[.5,1],scale:3,type:`markerLetter`},t.locations.style??={fillColor:`#fff`,fillOpacity:.1,strokeColor:`#fff`},t.locations.records??=t.listview_records||structuredClone(or),t.locations.clearAll=e.target.appendChild(mapp.utils.html.node`
    <button
    ...
    `);async function i(){e.infoj.some(e=>e.newValue!==void 0)&&!await mapp.ui.elements.confirm({text:mapp.dictionary.location_close_without_save})||e.remove()}e.view=mapp.ui.elements.drawer({data_id:`location-${e.layer.key}-${e.id}`,class:`location-view raised expanded`,header:r}),e.viewEntries=e.view.appendChild(mapp.ui.locations.infoj(e)),e.view.querySelector(`.header`).style.borderBottom=`3px solid ${e.record.colour}`,e.view.addEventListener(`valChange`,dr),e.renderLocationView=fr,e.view.addEventListener(`render`,()=>e.renderLocationView()),e.view.addEventListener(`updateInfo`,()=>{e.editToggle&&(e.editToggle.classList.remove(`toggle-on`),e.removeEdits()),e.layer?.dataviews&&Object.values(e.layer.dataviews).forEach(e=>{e.display===!0&&e.update()}),e.renderLocationView()})}function ur(e){if(e.layer?.toggleLocationViewEdits===!1)return e.removeEdits(),!1;if(!e.layer?.toggleLocationViewEdits||!e.infoj.some(e=>e.edit))return!1;!e.new&&e.removeEdits();let t=`notranslate material-symbols-outlined ${e.new?`toggle-on`:``}`;return e.editToggle=mapp.utils.html.node`<button
    title="Enable edits"
    class=${t}
    onclick=${n}>edit`,!0;async function n(t){await e.confirmUpdate()===void 0&&(t.target.classList.toggle(`toggle-on`)?e.restoreEdits():e.removeEdits(),e.renderLocationView())}}function dr(e){let t=e.detail,n=t.location;if(typeof t.valChangeMethod==`function`){t.valChangeMethod(t);return}if(t.value==t.newValue?(delete t.newValue,t.node?.classList.remove(`val-changed`)):t.node?.classList.add(`val-changed`),n.infoj.some(e=>e.invalid)){n.view.querySelector(`.btn-save`).style.display=`none`;return}n.view.querySelector(`.btn-save`).style.display=n.infoj.some(e=>e.newValue!==void 0)?`inline-block`:`none`}function fr(){let e=this;e.view.querySelector(`.btn-save`).style.display=`none`,e.viewEntries.remove(),e.view.classList.remove(`disabled`),e.viewEntries=e.view.appendChild(mapp.ui.locations.infoj(e))}var pr={entries:qn,infoj:Jn,listview:sr,view:lr};function mr(e){if(e.node)return e.tabs=e.node.appendChild(mapp.utils.html.node`<div class="tabs">`),e.panel=e.node.appendChild(mapp.utils.html.node`<div class="panel">`),e.id&&(e.node.dataset.id=e.id),e.addTab=hr,e.node.addEventListener(`addTab`,t=>e.addTab(t.detail)),e}function hr(e){if(e.tab&&!e.dynamic)return;let t=this;e.layer?.dataviews?.[e.key]&&(e.disableTabClose??=e.layer.dataviews.hide),e.activate??=a,e.location?e.location.removeCallbacks.push(()=>e.remove()):e.layer&&(e.layer.showCallbacks.push(()=>{e.display&&e.show()}),e.layer.hideCallbacks.push(()=>{e.remove()})),e.label??=e.title||e.key||`Tab`,e.tab_btn=mapp.utils.html.node`<button
    ...
    overflow: hidden !important`;Object.assign(e.dataview_dialog,{header:mapp.utils.html.node`<h1> ${e.label}`,data_id:`${e.key}-dataviews-dialog`,target:document.getElementById(`Map`),height:`auto`,left:`5%`,top:`0.5em`,class:`box-shadow tabview`,css_style:t,containedCentre:!0,contained:!0,headerDrag:!0,closeBtn:!0,onClose:()=>{e.chkbox&&(e.chkbox.querySelector(`input`).checked=!1)}}),mapp.ui.elements.dialog(e.dataview_dialog);let n=e.dataview_dialog.node.querySelector(`.content`);n.classList.add(`panel`),e.target=n,e.dataview_dialog.dialog.close(),e.show=()=>{e.update(),e.toolbar&&e.dataview_dialog.node.querySelector(`.btn-row`).style.removeProperty(`display`),e.chart&&(e.dataview_dialog.node.appendChild(e.target.querySelector(`canvas`)),e.dataview_dialog.node.removeChild(e.target)),e.dataview_dialog.dialog.show()},e.hide=()=>{e.dataview_dialog.dialog.close()},e.location?.removeCallbacks?.push?.(()=>e.dataview_dialog.dialog.close())}var Er={create:Dr};async function Dr(e){e.query??=`histogram`,e.options??={};let t=`column-gap: ${e.options.columnGap||10}px`;e.node=mapp.utils.html.node`<div class="histogram" style=${t}>`,e.options.width&&(e.node.style.width=`${e.options.width}px`),e.options.height&&(e.node.style.height=`${e.options.height}px`),e.options.title&&e.target.append(mapp.utils.html.node`<h3 class="histogram-title">${e.options.title}`),e.setData=Or,e.target.append(e.node),e.options.xlabel&&(e.xlabel=mapp.utils.html.node`<div class="histogram-xlabel">${e.options.xlabel}`,e.options.width&&(e.xlabel.style.width=`${e.options.width}px`),e.target.append(e.xlabel)),e.options.caption&&(e.caption=mapp.utils.html.node`<span class="histogram-caption">${e.options.caption}`,e.target.append(e.caption))}function Or(e){this.data=e,kr(this)}function kr(e){if(!Array.isArray(e.data)){mapp.utils.render(e.node,mapp.utils.html`<div>`);return}let t=Math.max(...e.data.map(e=>e.count));e.options.height??=100;let n=e.data.map(n=>{let r=`height: ${Math.round(e.options.height*n.count/t)}px`,i=``;return e.options.tooltip&&(i=mapp.utils.html`<div class="bucket-info">
      ...
      style=${r}>
      ${i}`}),r=e.options.ylabel?mapp.utils.html`<div class="ylabel">${e.options.ylabel}`:``;mapp.utils.render(e.node,mapp.utils.html`${r}${n}`)}var Y={idle:600};function Ar(e){Object.assign(Y,e),Y.idle!==0&&(window.onload=X,window.onmousemove=X,window.onmousedown=X,window.ontouchstart=X,window.onclick=X,window.onkeypress=X,X(),jr())}function X(){Y.locked||(Y.timeout&&clearTimeout(Y.timeout),Y.timeout=setTimeout(Z,Y.idle*1e3))}function Z(){Y.locked=!0,Y.renew&&clearTimeout(Y.renew);let e={url:`${Y.host}/api/user/cookie?destroy=true`};e.onLoad=e=>location.reload(),mapp.utils.xhr(e)}function jr(){Y.renew=setTimeout(Mr,(Y.idle-20)*1e3)}function Mr(){let e={url:`${Y.host}/api/user/cookie?renew=true`};e.onLoad=e=>{if(e.target.status===401)return Z();jr()},mapp.utils.xhr(e)}function Nr(e){let t=mapp.utils.html.node`
  <div class="interface-mask">
    ...
    <button 
      ...
      onclick=${e=>e.target.closest(`.interface-mask`).remove()}>close</button>`;document.body.append(t)}var Pr={create:Fr,toolbar:{copyToClipboard:Rr,csvupload:zr,jsonfile:Lr,sqlinsert:Br}};async function Fr(e){e.props??={mainMenuBar:!1,mode:`text`,readOnly:!0},e.jsoneditor=await mapp.ui.elements.jsoneditor(e),e.setData=Ir,e.data&&e.setData(e.data)}function Ir(e){this.data=e;let t={json:e};this.jsoneditor.set(t)}function Lr(e){return e.title??=e.label||`Unknown`,mapp.utils.html.node`<button onclick=${()=>{let t={filename:`${e.title}.json`,text:JSON.stringify(e.data,null,2),type:`application/json`};mapp.utils.textFile(t)}}>Download</button>`}function Rr(e){return mapp.utils.html.node`<button onclick=${()=>{let t=JSON.stringify(e.data,null,2);mapp.utils.copyToClipboard(t)}}>Copy to Clipboard</button>`}function zr(e){return e.toolbar.csvupload.label??=`CSV Upload`,e.toolbar.csvupload.input=mapp.utils.html.node`<input 
...
`)}`});return}e.setData(n)}}>`,e.toolbar.csvupload.input}function Br(e){return e.props={mainMenuBar:!1,mode:`text`},mapp.utils.html.node`<button onclick=${()=>{let t=e.jsoneditor.get();if(t.text===``)return;let n=mapp.utils.paramString(e.toolbar.sqlinsert),r=`${mapp.host}/api/query?${n}`;return mapp.utils.xhr({body:t.text,method:`POST`,url:r})}}>SQL Insert</button>`}async function Vr(e,t,n){return await mapp.ui.elements.jsoneditor({props:{onRenderMenu:e=>Hr(e,n),content:t,mode:`text`},target:e})}function Hr(e,t){return e.push(t),e.filter(e=>e.text!==`text`).filter(e=>e.text!==`tree`).filter(e=>e.text!==`table`).filter(e=>e.type!==`separator`).filter(e=>e.className!==`jse-undo`).filter(e=>e.className!==`jse-redo`).filter(e=>e.className!==`jse-search`).filter(e=>e.className!==`jse-contextmenu`).filter(e=>e.className!==`jse-sort`).filter(e=>e.className!==`jse-transform`)}async function Ur(e){if(Array.isArray(e.Features)&&!e.filter?.viewport)return e.Features.length;let t={layer:e,queryparams:{}};t.viewport=e.filter?.viewport||e.queryparams?.viewport;let n=mapp.utils.queryParams(t),r=mapp.utils.paramString({...n,filter:e.filter?.current,layer:e.key,table:e.tableCurrent(),qID:e.qID,template:`location_count`}),i=await mapp.utils.xhr(`${e.mapview.host}/api/query?${r}`);if(i instanceof Error)if(e.filter?.viewport){console.warn(`locationCount failed on viewport.`);return}else return e.filter.viewport=!0,await Ur(e);return i}var Q={},$={X:{min:75,lockSize:200,grid:`gridTemplateColumns`},Y:{min:0,lockSize:50,grid:`gridTemplateRows`},gridTemplate:{X:`auto`,Z:`10px`,Y:`auto`}};function Wr(e){e.resizeEvent??=Gr.bind(e),e.stopResize=qr.bind(e),e.axis??=`X`,e.doubleClickClose&&(e.target.ondblclick=t=>Kr(t,e)),e.target.addEventListener(`mousedown`,t=>{t.preventDefault(),document.body.style.cursor=`grabbing`,globalThis.addEventListener(`mousemove`,e.resizeEvent),globalThis.addEventListener(`mouseup`,e.stopResize)}),e.target.addEventListener(`touchstart`,t=>{t.preventDefault(),globalThis.addEventListener(`touchmove`,e.resizeEvent),globalThis.addEventListener(`touchend`,e.stopResize)},{passive:!0}),Q.gridTemplateColumns=getComputedStyle(document.body).gridTemplateColumns,Q.gridTemplateRows=getComputedStyle(document.body).gridTemplateRows}function Gr(e){let t=this.axis;$.X.size=e.touches?.[0]?.pageX||e.pageX,$.Y.size=globalThis.innerHeight-(e.touches?.[0]?.pageY||e.pageY);let n=$[t];if(n.size<n.min){$.gridTemplate[t]=`0px`,document.body.style[n.grid]=Object.values($.gridTemplate).join(` `);return}n.size<n.lockSize||(t===`X`&&$.X.size>globalThis.innerWidth/2&&($.X.size=globalThis.innerWidth/2),t===`Y`&&($.Y.size>globalThis.innerHeight-10&&($.Y.size=globalThis.innerHeight),OL.style.marginTop=`-${$.Y.size/2}px`),$.gridTemplate[t]=`${n.size}px`,document.body.style[n.grid]=Object.values($.gridTemplate).join(` `),$.gridTemplate={X:`auto`,Z:`10px`,Y:`auto`})}function Kr(e,t){if($.X.size=e.touches?.[0]?.pageX||e.pageX,$.Y.size=globalThis.innerHeight-(e.touches?.[0]?.pageY||e.pageY),$[t.axis].size<$[t.axis].min){document.body.style[$[t.axis].grid]=Q[$[t.axis].grid];return}$.gridTemplate[t.axis]=`0px`,document.body.style[$[t.axis].grid]=Object.values($.gridTemplate).join(` `),$.gridTemplate={X:`auto`,Z:`10px`,Y:`auto`}}function qr(){document.body.style.cursor=`auto`,globalThis.removeEventListener(`mousemove`,this.resizeEvent),globalThis.removeEventListener(`touchmove`,this.resizeEvent),globalThis.removeEventListener(`mouseup`,this.stopResize),globalThis.removeEventListener(`touchend`,this.stopResize)}var Jr={cssColour:gr,dataview:yr,dataviewDialog:Tr,histogram:Er,idleLogout:Ar,imagePreview:Nr,Json:Pr,layerJSE:Vr,locationCount:Ur,resizeHandler:Wr};globalThis.__varlockThrowOnMissingKeys=!0;var Yr={Dataview:t,elements:ut,Gazetteer:dt,layers:nn,locations:pr,Tabview:mr,utils:Jr};globalThis.ui=Yr,globalThis.mapp??={},mapp.ui=Yr;
