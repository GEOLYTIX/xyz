import{t as e}from"./chunk-DK3Fl9T5.js";async function t(e){if(!(!e.dynamic&&typeof e.create==`function`)){if(typeof e.create==`function`){e.create();return}if(typeof e.target==`string`&&(e.target=document.getElementById(e.target)),!(e.target instanceof HTMLElement)){console.warn(`Dataviews require a HTMLHtmlElement target`);return}return i(e)instanceof Error?e.err:(typeof e.label==`string`&&(e.chkbox=mapp.ui.elements.chkbox({checked:!!e.display,data_id:e.key,disabled:e.disabled,label:e.label,onchange:t=>{e.display=t,e.display?e.show():e.hide()}})),e.update??=a,e.show??=n,e.hide??=r,mapp.ui.utils.dataview.Toolbar(e),mapp.ui.utils.dataview.mapChange(e),e)}}async function n(){this.display=!0,this.create===void 0?(this.create=()=>{mapp.ui.utils[this.dataview].create(this)},this.create(),typeof this.update==`function`&&this.update()):this.dynamic?(typeof this.create==`function`&&await this.create(),typeof this.update==`function`&&this.update()):this.reload&&typeof this.update==`function`&&this.update(),this.btnRow?.style.setProperty(`display`,`flex`),this.target.style.display=`block`}function r(){this.display=!1,this.target.style.display=`none`,this.btnRow?.style.setProperty(`display`,`none`)}function i(e){if(e.key??=e.query||e.title||e.label,e.dataview||(e.chart&&(e.dataview=`chartjs`),e.columns!==void 0&&(console.warn(`Table dataviews should be configured inside a tables object`),e.table={columns:e.columns},e.dataview=`tabulator`),e.table&&(e.dataview=`tabulator`)),!Object.hasOwn(mapp.ui.utils,e.dataview))return e.err=Error(`mapp.ui.utils.${e.dataview} doesnt exist`),console.error(e.err),e.update=()=>console.warn(`Unable to update ${e.key} dataview.`),e.err;if(typeof mapp.ui.utils[e.dataview].create!=`function`)return e.err=Error(`mapp.ui.utils.${e.dataview}.create() method doesn't exist`),console.error(e.err),e.err}async function a(){if(this.create===void 0&&(this.create=function(){mapp.ui.utils[this.dataview].create(this)},this.create()),!this.query)return;let e=mapp.utils.queryParams(this),t=mapp.utils.paramString(e);this.host??=this.layer?.mapview?.host;let n=await mapp.utils.xhr(`${this.host||mapp.host}/api/query?${t}`);if(!(n instanceof Error)){if(typeof this.responseFunction==`function`){this.responseFunction(n);return}typeof this.setData==`function`&&this.setData(n)}}function o(e){typeof e==`string`&&(e={text:e}),e.title??=mapp.dictionary.information,e.icon??=`info`,e.data_id??=`alert`,e.class??=`alert-confirm box-shadow no-select`;let t=e.icon===``?``:mapp.utils.html`<span class="material-symbols-outlined notranslate">${e.icon}</span>`;return e.header=mapp.utils.html`${t}${e.title}`,e.innerContent??=mapp.utils.html`<p>${e.text}</p>`,e.content=mapp.utils.html`
  ...
  <div
    ...
    <button
      class="action wide"
      onclick=${t=>m(t,e,e.draw.point)}>
      <span class="notranslate material-symbols-outlined">add_location_alt</span>
      ${e.draw.point.label}`,e.draw.point.btn}function me(e){let t={content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
    ...
    <button
      class="action wide"
      onclick=${t=>m(t,e,e.draw.line)}>
      <span class="notranslate material-symbols-outlined">polyline</span>
      ${e.draw.line.label}`,e.draw.line.btn}function he(e){let t={content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
    ...
    <button
      class="action wide"
      onclick=${t=>m(t,e,e.draw.polygon)}>
      <span class="notranslate material-symbols-outlined">activity_zone</span>
      ${e.draw.polygon.label}`,e.draw.polygon.btn}function ge(e){let t={content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
  ...
  <button
    class="action wide"
    onclick=${t=>m(t,e,e.draw.rectangle)}>
    <span class="notranslate material-symbols-outlined">rectangle</span>
    ${e.draw.rectangle.label}`,e.draw.rectangle.btn}function _e(e){let t={content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
  ...
  <button
    class="action wide"
    onclick=${t=>m(t,e,e.draw.circle_2pt)}>
    <span class="notranslate material-symbols-outlined">outbound</span>
    ${e.draw.circle_2pt.label}`,e.draw.circle_2pt.btn}function ve(e){let t={helpDialog:{content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
    ...
    style="display: grid; grid-template-columns: 100px 1fr; align-items: center;">
      ...
      <div style="grid-column: 2;">
        ${mapp.ui.elements.dropdown({callback:(e,n)=>h(t,n.option),entries:Object.values(t.unitsConfig),placeholder:t.unitsConfig[t.units].title})}`,t.rangeSlider=mapp.utils.html.node`<div>`,h(t,t.units),t.btn=mapp.utils.html.node`<button
    class="action wide"
    onclick=${n=>m(n,e,t)}>
    ...
    <h3>${mapp.dictionary.circle_config}</h3>`,content:n,popout:t.popout,dialog:t.dialog}),t.drawer)}function h(e,t){let n=e.unitsConfig[t];e.units=n.option,e.radius??=n.min,e.radius=e.radius>n.max?n.max:e.radius,mapp.utils.render(e.rangeSlider,mapp.ui.elements.slider({callback:t=>{e.radius=Number.parseFloat(t)},max:n.max,min:n.min,val:e.radius}))}function ye(e){return e.draw.locator={label:mapp.dictionary.draw_position,type:`Point`,...e.draw.locator,layer:e},e.draw.locator.btn=mapp.utils.html.node`
    <button
      class="action wide"
      ...
      ${e.draw.locator.label}`,e.draw.locator.btn}function be(e){e.selectedTitles=new Set,e.selectedOptions=new Set,e.placeholder??=e.span||``,e.entries=e.entries?.filter?.(e=>e.option!==``),we(e),Te(e),e.onChange??=Se,e.options=Ce(e);let t=e.selectedTitles.size?Array.from(e.selectedTitles).join(`, `):e.placeholder,n=e.keepPlaceholder?e.placeholder:t;return e.placeHolderOption=mapp.utils.html.node`
    ...
    ${e.pills?.container}
    ${e.search||e.select}`,e.node}function xe(e){e.target.selectedIndex=0}function Se(e,t){let n=e.target.selectedIndex;e.target.selectedIndex=0;let r=t.entries[n-1];if(r.selected=!r.selected,t.multi){if(t.options[n].classList.toggle(`selected`)?(t.selectedTitles.add(r.title),t.selectedOptions.add(r.option),t.pills?.add(r.title)):(t.selectedTitles.delete(r.title),t.selectedOptions.delete(r.option),t.pills?.remove(r.title)),!t.pills&&!t.keepPlaceholder){let e=t.selectedTitles?.size&&Array.from(t.selectedTitles).join(`, `);t.placeHolderOption.textContent=e||t.placeholder}t.callback?.(e,[...t.selectedOptions],r);return}t.options.forEach(e=>{e.classList.remove(`selected`),e.style.backgroundColor=`var(--color-base-secondary)`}),t.options[n].classList.add(`selected`),t.options[n].style.removeProperty(`background-color`),t.keepPlaceholder||(t.placeHolderOption.textContent=r.title),t.callback?.(e,r)}function Ce(e){return e.entries.map(t=>(t.li=mapp.utils.html.node`<option
        value=${t.option}>
        ${t.title||t.label||t.field}`,t.selected&&(t.li.classList.add(`selected`),e.selectedTitles.add(t.title),e.selectedOptions.add(t.option),e.pills?.add(t.title)),t.li))}function we(e){e.pills&&=mapp.ui.elements.pills({addCallback:(t,n)=>{e.callback?.(null,[...n])},pills:[...e.selectedTitles],removeCallback:(t,n)=>{let r=e.entries.findIndex(e=>e.option===t),i=e.entries.findIndex(e=>e.option===t)+1;e.entries.find(e=>e.option===t).selected=!1,e.options[i].classList.remove(`selected`),e.selectedTitles.delete(r.title),e.selectedOptions.delete(r.option),e.callback?.(null,[...n])}})}function Te(e){if(!e.search)return;let t=`${e.field}-search-options`,n=e.placeholder||`Enter search term...`,r=mapp.utils.html.node`<input
    placeholder=${n}
    type="search" list=${t}
    onInput=${t=>Ee(t,e)}
    onfocus="this.placeholder=''"
    onblur=${e=>e.target.placeholder=n}>`;e.searchOptions=[];for(let t of e.entries){let n=mapp.utils.html.node`
      <option value=${t.option} data-title=${t.title}>${t.title}`;e.searchOptions.push(n)}let i=mapp.utils.html.node`<datalist id=${t}>${e.searchOptions}`;return e.search=mapp.utils.html.node`${r}${i}`,e.search}function Ee(e,t){t.entries.forEach(n=>{n.title===e.target.value&&(n.selected=!n.selected,n.selected?(t.selectedTitles.add(n.title),t.selectedOptions.add(n.option),t.pills?.add(n.title)):(t.selectedTitles.delete(n.title),t.selectedOptions.delete(n.option),t.pills?.remove(n.title)),e.target.value=``,e.target.dispatchEvent(new Event(`blur`)),t.pills||t.callback?.(e,[...t.selectedOptions]))})}var g;function De(e){g?.node.remove(),e&&(mapp.user?.hideHelp||(g={closeBtn:!0,contained:!0,css_style:`padding: 0.5em;`,headerDrag:!0,height:`auto`,left:`6em`,minimizeBtn:!0,target:document.getElementById(`Map`),top:`3em`,...e},mapp.ui.elements.dialog(g)))}async function Oe(e){let t=await mapp.utils.esmImport(`vanilla-jsoneditor@3.3.1`);return e.content??={json:{}},typeof e.data==`object`&&(e.content.json=e.data),t.createJSONEditor({props:{content:e.content,...e.props},target:e.target})}var _={hover:Ae,hovers:je,icon_scaling:Le,label:Me,labels:Ne,opacitySlider:Pe,panel:ke,theme:Fe,themes:Ie};function ke(e){if(!e.style)return;e.style.elements??=Object.keys(e.style);let t=[];for(let n of e.style.elements){if(!Object.hasOwn(e.style,n)||!Object.hasOwn(_,n))continue;let r=_[n](e);r&&t.push(r)}if(t.length)return e.style.view=mapp.utils.html.node`<div>${t}`,e.style.view}function Ae(e){if(e.style.hover&&!e.style.hover.hidden)return mapp.ui.elements.chkbox({checked:!!e.style.hover.display,data_id:`hoverCheckbox-${e.key}`,label:e.style.hovers&&mapp.dictionary.layer_style_display_hover||e.style.hover.title||mapp.dictionary.layer_style_display_hover,onchange:t=>{e.style.hover.display=t}})}function je(e){if(!e.style.hover||e.style.hover.hidden||!e.style.hovers||Object.keys(e.style.hovers).length<2)return;let t=Object.keys(e.style.hovers).filter(t=>!e.style.hovers[t].hidden).map(t=>({option:t,title:e.style.hovers[t].title||t}));return mapp.ui.elements.dropdown({callback:(t,n)=>{let r=e.style.hover.display;e.style.hover=e.style.hovers[n.option],e.style.hover.method??=mapp.layer.featureHover,e.style.hover.display=r},data_id:`hoversDropdown-${e.key}`,entries:t,placeholder:e.style.hover.title})}function Me(e){if(!e.style.label||e.style.label.hidden)return;if(e.style.labelCheckbox)return e.style.labelCheckbox;let t=e.style.labels&&mapp.dictionary.layer_style_display_labels;return t??=e.style.label.title||mapp.dictionary.layer_style_display_labels,e.style.labelCheckbox=mapp.ui.elements.chkbox({checked:!!e.style.label.display,data_id:`labelCheckbox-${e.key}`,label:t,onchange:t=>{e.style.label.display=t,e.reload()}}),e.changeEndCallbacks.push(v),e.style.labelCheckbox}function v(e){if(e.style.label.minZoom||e.style.label.maxZoom){let t=e.mapview.Map.getView().getZoom();t<=e.style.label.minZoom||t>=e.style.label.maxZoom?e.style.labelCheckbox?.classList.add(`disabled`):e.style.labelCheckbox?.classList.remove(`disabled`)}}function Ne(e){if(!e.style.label||e.style.label.hidden||!e.style.labels||Object.keys(e.style.labels).length<2)return;let t=Object.keys(e.style.labels).filter(t=>!e.style.labels[t].hidden).map(t=>({option:t,title:e.style.labels[t].title||t}));return mapp.utils.html`
  <div>${mapp.dictionary.layer_style_select_label}</div>
  ${mapp.ui.elements.dropdown({callback:n,data_id:`labelsDropdown`,entries:t,placeholder:e.style.label.title})}`;function n(t,n){let r=e.style.label.display;e.style.label=e.style.labels[n.option],e.style.label.display=r,e.reload(),v(e)}}function Pe(e){return mapp.ui.elements.slider({callback:t=>{e.L.setOpacity(Number.parseFloat(t/100))},data_id:`opacitySlider`,label:`${mapp.dictionary.layer_style_opacity}`,max:100,min:0,val:Number.parseInt(e.L.getOpacity()*100)})}function Fe(e){if(!e.style.theme)return;e.style.labels&&Object.hasOwn(e.style.labels,e.style.theme?.setLabel)&&(e.style.label=e.style.labels[e.style.theme.setLabel]),e.style.hovers&&Object.hasOwn(e.style.hovers,e.style.theme?.setHover)&&(e.style.hover=e.style.hovers[e.style.theme.setHover]);let t=[];return e.style.theme?.meta&&(e.style.theme.meta_node=mapp.utils.html.node`<p>${e.style.theme.meta}`,t.push(e.style.theme.meta_node)),Object.hasOwn(mapp.ui.layers.legends,e.style.theme?.type)&&(e.style.legend=mapp.utils.html.node`<div class="legend">`,t.push(e.style.legend),Object.hasOwn(mapp.layer.featureFields.distribution,e.style.theme.distribution)||e.showCallbacks.push(()=>{mapp.ui.layers.legends[e.style.theme.type](e)})),mapp.utils.html.node`<div data-id="layerTheme">${t}`}function Ie(e){if(!e.style.themes||Object.keys(e.style.themes).length<2)return;let t=Object.keys(e.style.themes).map(t=>({option:t,title:e.style.themes[t].title||t}));function n(t,n){e.style.theme=e.style.themes[n.option],e.style.theme.setLabel&&e.style.labels&&(e.style.label=e.style.labels[e.style.theme.setLabel]),e.style.theme.setHover&&e.style.hovers&&(e.style.hover=e.style.hovers[e.style.theme.setHover],e.style.hover.method??=mapp.layer.featureHover);let r=mapp.ui.layers.panels.style(e);e.style.panel&&e.style.panel.replaceChildren(...r.children),e.view?.querySelector(`[data-id=style-drawer]`).replaceChildren(...r.children),e.reload()}return mapp.utils.html`<div>
    ${mapp.dictionary.layer_style_select_theme}
    ${mapp.ui.elements.dropdown({callback:n,data_id:`themesDropdown`,entries:t,placeholder:e.style.theme.title})}`}function Le(e){if(!e.style.icon_scaling||e.style.icon_scaling.hidden)return;let t=[];if(e.style.icon_scaling.fields){let n=Object.keys(e.style.icon_scaling.fields).map(t=>({option:e.style.icon_scaling.fields[t].field,title:e.style.icon_scaling.fields[t].title||t,key:t}));n.push({title:mapp.dictionary.icon_scaling_no_scaling}),e.style.icon_scaling.placeholder??=mapp.dictionary.icon_scaling_select_one,e.style.icon_scaling.title??=mapp.dictionary.icon_scaling_title;let r=Object.keys(e.style.icon_scaling.fields).find(t=>e.style.icon_scaling.fields[t].field===e.style.icon_scaling.field)||Object.keys(e.style.icon_scaling.fields)[0];t.push(mapp.utils.html`
        <h3>${e.style.icon_scaling.title}`,mapp.ui.elements.dropdown({data_id:`iconScalingFieldsDropdown`,entries:n,callback:(t,n)=>{n.option?Object.assign(e.style.icon_scaling,{...e.style.icon_scaling.fields[n.key]}):delete e.style.icon_scaling.field,e.reload(),e.L.changed()},placeholder:e.style.icon_scaling.fields[r].title||r||e.style.icon_scaling.placeholder}))}else e.style.icon_scaling.field&&t.push(mapp.ui.elements.chkbox({checked:!!e.style.icon_scaling.field,data_id:`iconScalingFieldCheckbox-${e.key}`,label:mapp.dictionary.icon_scaling_field,onchange:t=>{t?(e.style.icon_scaling.field=e.style.icon_scaling._field,delete e.style.icon_scaling._field):(e.style.icon_scaling._field=e.style.icon_scaling.field,delete e.style.icon_scaling.field),e.L.changed()}}));return e.style.icon_scaling.icon&&(e.style.default.scale??=1,e.style.icon_scaling.maxScale??=e.style.default.scale*3,e.style.icon_scaling.minScale??=.1,e.style.icon_scaling.icon_scaling_label??=mapp.dictionary.icon_scaling_label,t.push(mapp.ui.elements.slider({callback:t=>{e.style.default.scale=Number.parseFloat(t),clearTimeout(e.style.timeout),e.style.timeout=setTimeout(()=>e.L.changed(),400)},data_id:`iconScalingSlider`,label:e.style.icon_scaling.icon_scaling_label,max:e.style.icon_scaling.maxScale,min:e.style.icon_scaling.minScale,step:e.style.default.scale/10,val:e.style.default.scale}))),e.style.icon_scaling.clusterScale&&e.style.cluster?.clusterScale&&(e.style.cluster.clusterScale??=1,t.push(mapp.ui.elements.slider({callback:t=>{e.style.cluster.clusterScale=Number.parseFloat(t),clearTimeout(e.style.timeout),e.style.timeout=setTimeout(()=>e.L.changed(),400)},data_id:`iconScalingClusterSlider`,label:mapp.dictionary.icon_scaling_cluster,max:e.style.cluster.clusterScale*3,min:0,step:e.style.cluster.clusterScale/10,val:e.style.cluster.clusterScale}))),e.style.icon_scaling.zoomInScale&&(e.style.zoomInScale??=1,t.push(mapp.ui.elements.slider({callback:t=>{e.style.zoomInScale=Number.parseFloat(t),clearTimeout(e.style.timeout),e.style.timeout=setTimeout(()=>e.L.changed(),400)},data_id:`iconScalingZoomInSlider`,label:mapp.dictionary.icon_scaling_zoom_in,max:e.style.zoomInScale*3,min:0,step:e.style.zoomInScale/10,val:e.style.zoomInScale}))),e.style.icon_scaling.zoomOutScale&&(e.style.zoomOutScale??=1,t.push(mapp.ui.elements.slider({callback:t=>{e.style.zoomOutScale=Number.parseFloat(t),clearTimeout(e.style.timeout),e.style.timeout=setTimeout(()=>e.L.changed(),400)},data_id:`iconScalingZoomOutSlider`,label:mapp.dictionary.icon_scaling_zoom_out,max:e.style.zoomOutScale*3,min:0,step:e.style.zoomOutScale/10,val:e.style.zoomOutScale}))),mapp.utils.html.node`<div>${t}`}var Re=new XMLSerializer,y=24;function ze(e){let t=[];return Array.isArray(e.icon)?t.push(Be(e)):(e.icon||e.svg||e.type)&&t.push(Ue(e)),e.fillColor&&t.push(Ke(e)),e.strokeColor&&!e.fillColor&&t.push(Ge(e)),mapp.utils.html.node`<div class="legend-icon">${t}`}function Be(e){let t=e.width||y,n=e.height||y,r=document.createElement(`canvas`);if(r.width=t,r.height=n,!e.bitmap_icons)return Ve(r,e,t,n),r;let i=e.icon.map(e=>mapp.utils.iconUrl(e));return mapp.utils.svgBitmap.requestBitmaps(i).then(()=>Ve(r,e,t,n)),r}function Ve(e,t,n,r){let i=t.icon.length;function a(){if(--i)return;let a=t.icon.map(e=>e.legendStyle?.getImage()).filter(Boolean).map(e=>{let t=e.getImageSize()||[n,r],i=e.getScale()||1;return[t[0]*i,t[1]*i]}),s=Math.max(...a.map(e=>e[0])),c=Math.max(...a.map(e=>e[1])),l=S(n,r,s,c),u=[(s||n)*l,(c||r)*l];t.icon.forEach(e=>{e.legendStyle&&=new ol.style.Style({image:He(e,t,o,l)})});let d=ol.render.toContext(e.getContext(`2d`),{pixelRatio:1,size:u});t.icon.forEach(e=>{d.setStyle(e.legendStyle),d.drawGeometry(new ol.geom.Point([u[0]*.5,u[1]*.5]))})}let o=t.icon[0].legendScale||1;for(let e of t.icon){if(!mapp.utils.iconUrl(e)){console.warn(`legendIcon: icon has no url, the svg or template may be missing/invalid: `,e);continue}let n=He(e,t,o);e.legendStyle=new ol.style.Style({image:n});let r=n.getImage();n.getImageState()===2?a():(r.addEventListener(`load`,a),n.load())}return e}function He(e,t,n,r){return mapp.utils.imageStyle({anchor:e.legendAnchor,bitmap:t.bitmap_icons,scale:n*(e.scale||1)*(r||1),url:mapp.utils.iconUrl(e)})}function Ue(e){let t=mapp.utils.iconUrl(e.icon||e);return t||console.warn(`legendIcon: icon has no url, the svg or template may be missing/invalid: `,e.icon||e),e.bitmap_icons&&t?.startsWith(`data:`)?We(t,e):b(t,e.width||y,e.height||y)}function We(e,t){let n=t.width||y,r=t.height||y,i=`width: ${n}px; height: ${r}px;`,a=mapp.utils.html.node`<canvas
    style=${i}>`;return mapp.utils.svgBitmap.requestBitmaps([e]).then(()=>{let t=mapp.utils.svgBitmap.iconBitmap(e);if(!t){a.replaceWith(b(e,n,r));return}a.width=t.image.width,a.height=t.image.height;let i=t.image.width/t.pixelRatio,o=t.image.height/t.pixelRatio,s=S(n,r,i,o);a.style.width=`${i*s}px`,a.style.height=`${o*s}px`,a.getContext(`2d`).drawImage(t.image,0,0)}),a}function Ge(e){let t=`M 0,${e.height/2} L ${e.width/2},${e.height/2} ${e.width/2},${e.height/2} ${e.width},${e.height/2}`;return e.strokeWidth??=1,e.lineDash??=``,b(x(mapp.utils.svg.node`
  <svg
    height=${e.height}
    width=${e.width}>
    <path
      ...
      stroke-dasharray=${e.lineDash} />`),e.width,e.height)}function Ke(e){return e.fillOpacity??=1,e.strokeWidth??=1,e.lineDash??=``,b(x(mapp.utils.svg.node`
  <svg
    height=${e.height}
    width=${e.width}>
    <rect
  ...
  <div>
    ...
    ${e.ulLocales}`,T(e,t),e.panel}async function ut(e,t,n){e.panel.classList.add(`disabled`),t.locale.name=n,await mapp.utils.userLocale.remove(t.locale),T(e,t),e.panel.classList.remove(`disabled`)}async function dt(e,t){e.panel.classList.add(`disabled`),t.locale.name=encodeURIComponent(e.localeInput.value),await mapp.utils.userLocale.putLocale(t)===t.locale.name&&(T(e,t),e.panel.classList.remove(`disabled`))}async function T(e,t){let n=await mapp.utils.userLocale.list(t.locale.workspace);if(!n.length){mapp.utils.render(e.ulLocales,mapp.utils.html`<div>`);return}let r=n.map(n=>{let r=mapp.utils.html`<button
  ...
  <input
    ...
    <div style="
      ...
      <label>Date after
        ${n}</label>
      <label>Date before
        ${r}</label>`}function Ct(e){let t=e.style.theme;t.legend??={},t.legend.grid=[],e.style.theme.style??={},e.style.theme.style.width??=24,e.style.theme.style.height??=24,e.style.theme.style.bitmap_icons??=e.style.bitmap_icons;let n=mapp.ui.elements.legendIcon(e.style.theme.style);return t.legend.grid.push(mapp.utils.html`
    <div 
      class="contents">
      ${n}<div class="label" style="grid-column: 2";>${e.style.theme.label}`),t.legend.node=mapp.utils.html.node`
    <div class="legend">
    <div class="contents-wrapper grid">${t.legend.grid}`,e.style.legend??=t.legend.node,e.style.legend&&e.style.legend.replaceChildren(...t.legend.node.children),t.legend.node}function j(e,t,n){if(e.field??=t.field,e.disabled??=n.filter?.current[e.field]?.ni?.includes(e.value),e.disabled||(e.style??=e._style),n.featureFields&&t.distribution===`count`){let r={value:n.featureFields[e.field]?.[e.value]};if(e.count=mapp.utils.formatNumericValue(r),!e.disabled&&!t.legend?.showEmptyCat&&!e.count)return}let r=mapp.ui.elements.legendIcon({bitmap_icons:n.style.bitmap_icons,height:24,width:24,...e._style||e.style}),i=mapp.utils.html`<div
    ...
    data-id="cluster"
    class="contents">
    ${t}${n}`}function P(e){e.legend??={},e.legend.layout??=`grid`;let t=[`contents-wrapper`,e.legend.layout];e.legend.nowrap&&t.push(`nowrap`),e.legend.classList=t.join(` `)}function F(){return mapp.utils.html`<div
    class="switch-all"
    ...
    </button>.`}function Et(e){let t=e.style.theme;P(t),t.legend.grid=t.categories.map(n=>j(n,t,e)).filter(e=>e!==void 0),e.style.cluster&&t.legend.grid.push(N(e));let n=F();return t.legend.node=mapp.utils.html.node`
    <div class="legend">
      ${n}
      <div class=${t.legend.classList}>
        ${t.legend.grid}`,e.style.legend??=t.legend.node,e.style.legend&&e.style.legend.replaceChildren(...t.legend.node.children),t.legend.node}function I(e){e.style.theme.filterOnly=!0;let t=e.style.theme;if(P(t),!t.hideLegend){t.legend.grid=[];for(let n of t.categories)n.label=n.values?.join(`, `),n.label!==void 0&&t.legend.grid.push(j(n,t,e));return e.style.cluster&&t.legend.grid.push(N(e)),t.legend.node=mapp.utils.html.node`
    <div class="legend">
      <div class=${t.legend.classList}>
        ${t.legend.grid}`,e.style.legend??=t.legend.node,e.style.legend&&e.style.legend.replaceChildren(...t.legend.node.children),e.L.once(`postrender`,()=>{t.key===e.style.theme.key&&I(e)}),t.legend.node}}function Dt(e){let t=e.style.theme;t.filterOnly=!0,P(t),t.legend??={};let n=t.categories.filter(e=>e.value!==void 0).map(n=>{let r=`contents ${t.legend?.horizontal?`horizontal`:``}`,i=mapp.ui.elements.legendIcon({bitmap_icons:e.style.bitmap_icons,height:24,width:24,...n._style||n.style});n.label??=n.value;let a=`label switch ${n.disabled?`disabled`:``}`;return mapp.utils.html`<div 
        data-id=${n.value}
        class=${r}>
        <div 
          style="height: 24px; width: 24px; grid-column: 1;">
  ...
  class="notranslate material-symbols-outlined active"
    ...
    data-id=resetall
    class="flat underline"
    onclick=${t=>{e.filter.list.forEach(t=>mapp.ui.layers.filters.resetFilter(e,t))}}>${mapp.dictionary.filter_reset_all}`,e.filter.count=mapp.utils.html.node`<span class="bold">`,e.changeEndCallbacks.push(R),e.showCallbacks.push(R),e.hideCallbacks.push(e=>{e.filter.clearAll?.checkVisibility()&&e.filter.feature_count.style.setProperty(`display`,`none`)}),e.filter.count_meta??=mapp.dictionary.filter_count,e.filter.feature_count=mapp.utils.html.node`
    ...
    <div class="link-with-img">
      ...
      data-src=${e.value}
      onclick=${t=>U(t,e,mapp.dictionary.remove_image_confirm)}>delete`;return mapp.utils.html.node`<div class="img-item"><img
      style="width: 100%"
      src=${e.value}
      ...
      <input
        type="file"
        accept="image/*;capture=camera" onchange=${t=>{V(t,e)}}>`}function on(e){let t=[];if(e.value?.map(n=>{let r=e.edit&&mapp.utils.html`<button title="${mapp.dictionary.delete}"
      class="notranslate material-symbols-outlined color-danger delete"
      data-name=${n.replace(/^.*\//,``).replace(/\.([\w-]{3})/,``)}
      data-src=${n}
      onclick=${t=>U(t,e,mapp.dictionary.remove_image_confirm)}>delete`;t.push(mapp.utils.html`<div class="img-item"><img
      src=${n}
      onclick=${mapp.ui.utils.imagePreview}>
      ...
      <input
        type="file"
        accept="image/*;capture=camera" onchange=${t=>{V(t,e)}}>`;t.push(n)}if(t.length)return mapp.utils.html.node`<div class="images-grid">${t}`}function sn(e){let t=[];if(e.value?.map(n=>{let r=e.edit&&mapp.utils.html`<button
      title="${mapp.dictionary.delete}"
      class="notranslate material-symbols-outlined color-danger delete"
      data-name=${n.replace(/^.*\//,``).replace(/\.([\w-]{3})/,``)}
      data-href=${n}
      onclick=${t=>U(t,e,mapp.dictionary.remove_document_confirm)}>delete`,i=n.replace(/^.*\//,``).replace(/\.([\w-]{3})/,``);t.push(mapp.utils.html`<div class="link-with-img">
      ...
      <input type="file"
        ...
        onchange=${t=>V(t,e)}>`;t.unshift(n)}if(t.length)return mapp.utils.html.node`<div>${t}`}async function V(e,t){let n=new FileReader,r=e.type===`drop`?e.dataTransfer?.files[0]:e.target.files[0];if(!r)return;t.location.view?.classList.add(`disabled`);let i={documents:ln,image:H,images:H};n.onload=e=>i[t.type](e,t,r),n.readAsDataURL(r)}function H(e,t,n){let r=new Image;r.onload=async()=>cn(t,r,n),r.src=e.target.result}async function cn(e,t,n){let r=document.createElement(`canvas`);e.max_size??=1024,t.width>t.height&&t.width>e.max_size?(t.height*=e.max_size/t.width,t.width=e.max_size):t.height>e.max_size&&(t.width*=e.max_size/t.height,t.height=e.max_size),r.width=t.width,r.height=t.height,r.getContext(`2d`).drawImage(t,0,0,t.width,t.height);let i=await G(e,{public_id:n.name.replace(/^.*\//,``).replace(/\.([\w-]{3})/,``)+e.suffix_date?`@${Date.now()}`:``});if(!i)return;let a=new FormData;a.append(`file`,r.toDataURL(`image/jpeg`,.5));let o=await fetch(i,{body:a,method:`post`});if(!o.ok){mapp.ui.elements.alert(`Cloudinary Image upload failed!`),e.location.view?.classList.remove(`disabled`);return}let s=await o.json();e.type===`image`?e.value=s.secure_url:e.value=Array.isArray(e.value)?e.value.concat([s.secure_url]):[s.secure_url],W(e)}async function ln(e,t,n){let r=new Date,i=`${r.getMonth()+1}-${r.getDate()}T${r.getHours()}:${r.getMinutes()}`,a=n.name.substring(n.name.lastIndexOf(`.`)),o=await G(t,{public_id:`${n.name.replace(a,``)}-${i}${a}`});if(!o)return;let s=new FormData;s.append(`file`,e.target.result.toString());let c=await fetch(o,{body:s,method:`post`});if(!c.ok){mapp.ui.elements.alert(`Cloudinary Document upload failed!`),t.location.view?.classList.remove(`disabled`);return}let l=await c.json();t.value=Array.isArray(t.value)?t.value.concat([l.secure_url]):[l.secure_url],W(t)}async function U(e,t,n){if(!await mapp.ui.elements.confirm({text:n}))return;let r=await G(t,{destroy:!0,public_id:decodeURIComponent(e.target.dataset.name)});if(!r)return;await fetch(r,{method:`post`});let i=new Set(t.value);i.delete(e.target.dataset.src||e.target.dataset.href),t.type===`image`?t.value=null:t.value=i.size?Array.from(i):null,W(t)}async function W(e){e.location.view?.classList.add(`disabled`),await mapp.utils.xhr({body:JSON.stringify({[e.field]:e.value}),method:`POST`,url:`${e.location.layer.mapview.host}/api/query?`+mapp.utils.paramString({id:e.location.id,layer:e.location.layer.key,locale:e.location.layer.mapview.locale.key,table:e.location.table,template:e.location.updateTemplate})}),e.node.replaceChildren(mapp.utils.html.node`<div class="label">${e.title}`,z[e.type](e)),e.location.view?.classList.remove(`disabled`)}async function G(e,t){let n=mapp.utils.paramString({...t,folder:e.cloudinary_folder}),r=await mapp.utils.xhr({responseType:`text`,url:`${e.location.layer.mapview.host}/api/sign/cloudinary?${n}`});return r instanceof Error&&(console.error(r),e.location.view?.classList.remove(`disabled`)),r}function un(e){if(e.value!==void 0&&(e.data=e.value),e.data?.length===0&&(e.data=null),e.layer??=e.location.layer,dn(e),e.target instanceof HTMLElement){if(!e.display&&typeof e.create==`function`)return mapp.utils.html.node`
    ${e.chkbox||``}
    ...
    <div
      ...
      ${t}`}function Yn(e){return mapp.utils.html.node`
        <div
          class="label"
          style=${e.css_title}
          title=${e.tooltip}>${e.title}`}function Xn(){console.warn(`The type:defaults entry method has been deprecated.`)}function Zn(e){return console.warn(`The type:${e.type} entry method has been deprecated in favour of the type:layer entry method.`),e.type=`layer`,e.qID??=`id`,e.name??=e.label,Cn(e)}var Qn={boolean:rn,dataview:un,date:K,datetime:K,defaults:Xn,documents:B,geometry:pn,html:qn,image:B,images:B,integer:jn,json:xn,key:Sn,layer:Cn,link:An,mvt_clone:Zn,numeric:jn,pills:Nn,pin:Pn,ping:Rn,query_button:zn,report:An,tab:Vn,text:Hn,textarea:qn,time:Jn,title:Yn,vector_layer:Zn},J;function $n(e,t){if(!e.infoj)return;let n=mapp.utils.html.node`<div class="location-view-grid">`;J={},t??=e?.layer?.infoj_order;let r=Array.isArray(t)?t.map(i).filter(e=>e!==void 0):e.infoj;function i(t){if(typeof t==`string`){let n=e.infoj.find(e=>new Set([e.key,e.field,e.query]).has(t));return n||console.warn(`infoj_order field: "${t}" not found in location.infoj. Please add entry.key, entry.field, or entry.query to the entry.`),n}else if(typeof t==`object`)return t.location=e,t}let a=0;for(let t of r){if(t.key??=t.field||a++,e.view?.classList.contains(`disabled`))break;if(t.listview=n,t.type??=`text`,t.edit===!0&&(t.edit={}),er(t),tr(t),nr(t)||(rr(t),ir(t),ar(t),or(t),sr(t),cr(t)))continue;if(!Object.hasOwn(mapp.ui.locations.entries,t.type)){console.error(`entry.type:${t.type} method not found.`);continue}let r=mapp.ui.locations.entries[t.type]?.(t);r&&t.node.append(r),ur(t)}return n}function er(e){e.jsonb_field&&e.jsonb_key&&e.value!==null&&typeof e.value==`object`&&e.value.jsonb&&(e.value=e.value.jsonb[e.jsonb_field][e.jsonb_key])}function tr(e){if(e.objectAssignFromField&&e.objectMergeFromField){console.warn(`${e.key}: objectAssignFromField and objectMergeFromField cannot both be used on the same entry.`);return}let t=e.objectAssignFromField||e.objectMergeFromField||e.json_field;if(!t)return;let n=e.location.infoj.find(e=>e.field===t);if(n&&typeof n.value==`object`){if(e.json_field){if(!e.json_key){console.warn(`json_field requires entry.json_key to be specified`);return}if(!n.value)return;e.value=n.value[e.json_key]}e.objectAssignFromField&&Object.assign(e,n.value),e.objectMergeFromField&&mapp.utils.merge(e,n.value)}}function nr(e){if(e.skipEntry||e.skipFalsyValue&&!e.value&&!e.edit||e.skipUndefinedValue&&e.value===void 0&&!e.edit||e.skipNullValue&&e.value===null&&!e.edit)return!0}function rr(e){e.nullValue!==void 0&&(e.edit||(e.value??=e.nullValue))}function ir(e){e.default!==void 0&&e.edit&&(e.value||(e.newValue=e.default,e.location.view?.dispatchEvent(new CustomEvent(`valChange`,{detail:e}))))}function ar(e){e.group&&(J[e.group]||(J[e.group]=e.listview.appendChild(mapp.ui.elements.drawer({data_id:`location-infoj-group`,class:`group ${e.groupClassList||``}`,header:mapp.utils.html`
          <h3>${e.group}</h3>`}))),e.listview=J[e.group])}function or(e){let t=`contents ${e.type} ${e.class||``} ${e.inline&&`inline`||``}`;e.node=e.listview.appendChild(mapp.utils.html.node`
  <div
    data-type=${e.type}
    class=${t}>`)}function sr(e){e.title&&e.node.append(mapp.ui.locations.entries.title(e))}function cr(e){if(e.query)if(e.queryCheck??=e.run,e.queryCallback??=lr,e.runOnce||e.queryCheck){delete e.runOnce,e.host??=e.location.layer?.mapview?.host||mapp.host;let t=mapp.utils.queryParams(e),n=mapp.utils.paramString(t);return e.url=`${e.host}/api/query?${n}`,e.blocking&&e.location.view?.classList.add(`loading`),mapp.utils.xhr(e).then(t=>e.queryCallback(t,e)),!0}else e.field&&!e.xhr&&console.warn(`field:"${e.field}" has a query:"${e.query}" which is not set to run, please add the run or runOnce flag to the entry.`)}function lr(e,t){if(t.blocking&&t.location.view?.classList.remove(`loading`),e?t.value=t.field?e[t.field]:e:t.value=t.nullValue||null,nr(t)){t.node.remove();return}let n=mapp.ui.locations.entries[t.type]?.(t);n&&t.node.appendChild(n)}function ur(e){if(!e.tooltip)return;let t=e.node.querySelector(`label`)||e.node.querySelector(`div.label`);t&&(e.tooltipElement||(e.tooltipElement=mapp.ui.elements.tooltip({content:e.tooltip}),t.append(e.tooltipElement)))}var dr=[{symbol:`A`,colour:`#2E6F9E`},{symbol:`B`,colour:`#EC602D`},{symbol:`C`,colour:`#5B8C5A`},{symbol:`D`,colour:`#B84444`},{symbol:`E`,colour:`#514E7E`},{symbol:`F`,colour:`#E7C547`},{symbol:`G`,colour:`#368F8B`},{symbol:`H`,colour:`#841C47`},{symbol:`I`,colour:`#61A2D1`},{symbol:`J`,colour:`#37327F`},{symbol:`K`,colour:`#FFA500`},{symbol:`L`,colour:`#7FFF00`},{symbol:`M`,colour:`#FFD700`},{symbol:`N`,colour:`#00CED1`},{symbol:`O`,colour:`#9932CC`},{symbol:`P`,colour:`#FF69B4`},{symbol:`Q`,colour:`#008B8B`},{symbol:`R`,colour:`#8A2BE2`},{symbol:`S`,colour:`#FF4500`},{symbol:`T`,colour:`#00FF00`},{symbol:`U`,colour:`#FF00FF`},{symbol:`V`,colour:`#708090`},{symbol:`W`,colour:`#F4A460`},{symbol:`X`,colour:`#000080`},{symbol:`Y`,colour:`#D2691E`},{symbol:`Z`,colour:`#8FBC8F`}];function fr(e){e.mapview||console.warn(`A mapview is required in the locations listview params argument.`),e.target||console.warn(`A target element is required in the locations listview params argument.`);let t=e.mapview.locale;t.locations??={},t.locations.pinStyle??={anchor:[.5,1],scale:3,type:`markerLetter`},t.locations.style??={fillColor:`#fff`,fillOpacity:.1,strokeColor:`#fff`},t.locations.records??=t.listview_records||structuredClone(dr),t.locations.clearAll=e.target.appendChild(mapp.utils.html.node`
    <button
    ...
    `);async function i(){e.infoj.some(e=>e.newValue!==void 0)&&!await mapp.ui.elements.confirm({text:mapp.dictionary.location_close_without_save})||e.remove()}e.view=mapp.ui.elements.drawer({data_id:`location-${e.layer.key}-${e.id}`,class:`location-view raised expanded`,header:r}),e.viewEntries=e.view.appendChild(mapp.ui.locations.infoj(e)),e.view.querySelector(`.header`).style.borderBottom=`3px solid ${e.record.colour}`,e.view.addEventListener(`valChange`,gr),e.renderLocationView=_r,e.view.addEventListener(`render`,()=>e.renderLocationView()),e.view.addEventListener(`updateInfo`,()=>{e.editToggle&&(e.editToggle.classList.remove(`toggle-on`),e.removeEdits()),e.layer?.dataviews&&Object.values(e.layer.dataviews).forEach(e=>{e.display===!0&&e.update()}),e.renderLocationView()})}function hr(e){if(e.layer?.toggleLocationViewEdits===!1)return e.removeEdits(),!1;if(!e.layer?.toggleLocationViewEdits||!e.infoj.some(e=>e.edit))return!1;!e.new&&e.removeEdits();let t=`notranslate material-symbols-outlined ${e.new?`toggle-on`:``}`;return e.editToggle=mapp.utils.html.node`<button
    title="Enable edits"
    class=${t}
    onclick=${n}>edit`,!0;async function n(t){await e.confirmUpdate()===void 0&&(t.target.classList.toggle(`toggle-on`)?e.restoreEdits():e.removeEdits(),e.renderLocationView())}}function gr(e){let t=e.detail,n=t.location;if(typeof t.valChangeMethod==`function`){t.valChangeMethod(t);return}if(t.value==t.newValue?(delete t.newValue,t.node?.classList.remove(`val-changed`)):t.node?.classList.add(`val-changed`),n.infoj.some(e=>e.invalid)){n.view.querySelector(`.btn-save`).style.display=`none`;return}n.view.querySelector(`.btn-save`).style.display=n.infoj.some(e=>e.newValue!==void 0)?`inline-block`:`none`}function _r(){let e=this;e.view.querySelector(`.btn-save`).style.display=`none`,e.viewEntries.remove(),e.view.classList.remove(`disabled`),e.viewEntries=e.view.appendChild(mapp.ui.locations.infoj(e))}var vr={entries:Qn,infoj:$n,listview:fr,view:mr};function yr(e){if(e.node)return e.tabs=e.node.appendChild(mapp.utils.html.node`<div class="tabs">`),e.panel=e.node.appendChild(mapp.utils.html.node`<div class="panel">`),e.id&&(e.node.dataset.id=e.id),e.addTab=br,e.node.addEventListener(`addTab`,t=>e.addTab(t.detail)),e}function br(e){if(e.tab&&!e.dynamic)return;let t=this;e.layer?.dataviews?.[e.key]&&(e.disableTabClose??=e.layer.dataviews.hide),e.activate??=a,e.location?e.location.removeCallbacks.push(()=>e.remove()):e.layer&&(e.layer.showCallbacks.push(()=>{e.display&&e.show()}),e.layer.hideCallbacks.push(()=>{e.remove()})),e.label??=e.title||e.key||`Tab`,e.tab_btn=mapp.utils.html.node`<button
    ...
    overflow: hidden !important`;Object.assign(e.dataview_dialog,{header:mapp.utils.html.node`<h1> ${e.label}`,data_id:`${e.key}-dataviews-dialog`,target:document.getElementById(`Map`),height:`auto`,left:`5%`,top:`0.5em`,class:`box-shadow tabview`,css_style:t,containedCentre:!0,contained:!0,headerDrag:!0,closeBtn:!0,onClose:()=>{e.chkbox&&(e.chkbox.querySelector(`input`).checked=!1)}}),mapp.ui.elements.dialog(e.dataview_dialog);let n=e.dataview_dialog.node.querySelector(`.content`);n.classList.add(`panel`),e.target=n,e.dataview_dialog.dialog.close(),e.show=()=>{e.update(),e.toolbar&&e.dataview_dialog.node.querySelector(`.btn-row`).style.removeProperty(`display`),e.chart&&(e.dataview_dialog.node.appendChild(e.target.querySelector(`canvas`)),e.dataview_dialog.node.removeChild(e.target)),e.dataview_dialog.dialog.show()},e.hide=()=>{e.dataview_dialog.dialog.close()},e.location?.removeCallbacks?.push?.(()=>e.dataview_dialog.dialog.close())}var jr={create:Mr};async function Mr(e){e.query??=`histogram`,e.options??={};let t=`column-gap: ${e.options.columnGap||10}px`;e.node=mapp.utils.html.node`<div class="histogram" style=${t}>`,e.options.width&&(e.node.style.width=`${e.options.width}px`),e.options.height&&(e.node.style.height=`${e.options.height}px`),e.options.title&&e.target.append(mapp.utils.html.node`<h3 class="histogram-title">${e.options.title}`),e.setData=Nr,e.target.append(e.node),e.options.xlabel&&(e.xlabel=mapp.utils.html.node`<div class="histogram-xlabel">${e.options.xlabel}`,e.options.width&&(e.xlabel.style.width=`${e.options.width}px`),e.target.append(e.xlabel)),e.options.caption&&(e.caption=mapp.utils.html.node`<span class="histogram-caption">${e.options.caption}`,e.target.append(e.caption))}function Nr(e){this.data=e,Pr(this)}function Pr(e){if(!Array.isArray(e.data)){mapp.utils.render(e.node,mapp.utils.html`<div>`);return}let t=Math.max(...e.data.map(e=>e.count));e.options.height??=100;let n=e.data.map(n=>{let r=`height: ${Math.round(e.options.height*n.count/t)}px`,i=``;return e.options.tooltip&&(i=mapp.utils.html`<div class="bucket-info">
      ...
      style=${r}>
      ${i}`}),r=e.options.ylabel?mapp.utils.html`<div class="ylabel">${e.options.ylabel}`:``;mapp.utils.render(e.node,mapp.utils.html`${r}${n}`)}var X={idle:600};function Fr(e){Object.assign(X,e),X.idle!==0&&(window.onload=Z,window.onmousemove=Z,window.onmousedown=Z,window.ontouchstart=Z,window.onclick=Z,window.onkeypress=Z,Z(),Lr())}function Z(){X.locked||(X.timeout&&clearTimeout(X.timeout),X.timeout=setTimeout(Ir,X.idle*1e3))}function Ir(){X.locked=!0,X.renew&&clearTimeout(X.renew);let e={url:`${X.host}/api/user/cookie?destroy=true`};e.onLoad=e=>location.reload(),mapp.utils.xhr(e)}function Lr(){X.renew=setTimeout(Rr,(X.idle-20)*1e3)}function Rr(){let e={url:`${X.host}/api/user/cookie`};e.onLoad=e=>{if(e.target.status===401)return Ir();Lr()},mapp.utils.xhr(e)}function zr(e){let t=mapp.utils.html.node`
  <div class="interface-mask">
    ...
    <button 
      ...
      onclick=${e=>e.target.closest(`.interface-mask`).remove()}>close</button>`;document.body.append(t)}var Br={create:Vr,toolbar:{copyToClipboard:Wr,csvupload:Gr,jsonfile:Ur,sqlinsert:Kr}};async function Vr(e){e.props??={mainMenuBar:!1,mode:`text`,readOnly:!0},e.jsoneditor=await mapp.ui.elements.jsoneditor(e),e.setData=Hr,e.data&&e.setData(e.data)}function Hr(e){this.data=e;let t={json:e};this.jsoneditor.set(t)}function Ur(e){return e.title??=e.label||`Unknown`,mapp.utils.html.node`<button onclick=${()=>{let t={filename:`${e.title}.json`,text:JSON.stringify(e.data,null,2),type:`application/json`};mapp.utils.textFile(t)}}>Download</button>`}function Wr(e){return mapp.utils.html.node`<button onclick=${()=>{let t=JSON.stringify(e.data,null,2);mapp.utils.copyToClipboard(t)}}>Copy to Clipboard</button>`}function Gr(e){return e.toolbar.csvupload.label??=`CSV Upload`,e.toolbar.csvupload.input=mapp.utils.html.node`<input 
...
`)}`});return}e.setData(n)}}>`,e.toolbar.csvupload.input}function Kr(e){return e.props={mainMenuBar:!1,mode:`text`},mapp.utils.html.node`<button onclick=${()=>{let t=e.jsoneditor.get();if(t.text===``)return;let n=mapp.utils.paramString(e.toolbar.sqlinsert),r=`${mapp.host}/api/query?${n}`;return mapp.utils.xhr({body:t.text,method:`POST`,url:r})}}>SQL Insert</button>`}async function qr(e,t,n){return await mapp.ui.elements.jsoneditor({props:{onRenderMenu:e=>Jr(e,n),content:t,mode:`text`},target:e})}function Jr(e,t){return e.push(t),e.filter(e=>e.text!==`text`).filter(e=>e.text!==`tree`).filter(e=>e.text!==`table`).filter(e=>e.type!==`separator`).filter(e=>e.className!==`jse-undo`).filter(e=>e.className!==`jse-redo`).filter(e=>e.className!==`jse-search`).filter(e=>e.className!==`jse-contextmenu`).filter(e=>e.className!==`jse-sort`).filter(e=>e.className!==`jse-transform`)}async function Yr(e){if(Array.isArray(e.Features)&&!e.filter?.viewport)return e.Features.length;let t={layer:e,queryparams:{}};t.viewport=e.filter?.viewport||e.queryparams?.viewport;let n=mapp.utils.queryParams(t),r=mapp.utils.paramString({...n,filter:e.filter?.current,layer:e.key,table:e.tableCurrent(),qID:e.qID,template:`location_count`}),i=await mapp.utils.xhr(`${e.mapview.host}/api/query?${r}`);if(i instanceof Error)if(e.filter?.viewport){console.warn(`locationCount failed on viewport.`);return}else return e.filter.viewport=!0,await Yr(e);return i}var Q={},$={X:{min:75,lockSize:200,grid:`gridTemplateColumns`},Y:{min:0,lockSize:50,grid:`gridTemplateRows`},gridTemplate:{X:`auto`,Z:`10px`,Y:`auto`}};function Xr(e){e.resizeEvent??=Zr.bind(e),e.stopResize=$r.bind(e),e.axis??=`X`,e.doubleClickClose&&(e.target.ondblclick=t=>Qr(t,e)),e.target.addEventListener(`mousedown`,t=>{t.preventDefault(),document.body.style.cursor=`grabbing`,globalThis.addEventListener(`mousemove`,e.resizeEvent),globalThis.addEventListener(`mouseup`,e.stopResize)}),e.target.addEventListener(`touchstart`,t=>{t.preventDefault(),globalThis.addEventListener(`touchmove`,e.resizeEvent),globalThis.addEventListener(`touchend`,e.stopResize)},{passive:!0}),Q.gridTemplateColumns=getComputedStyle(document.body).gridTemplateColumns,Q.gridTemplateRows=getComputedStyle(document.body).gridTemplateRows}function Zr(e){let t=this.axis;$.X.size=e.touches?.[0]?.pageX||e.pageX,$.Y.size=globalThis.innerHeight-(e.touches?.[0]?.pageY||e.pageY);let n=$[t];if(n.size<n.min){$.gridTemplate[t]=`0px`,document.body.style[n.grid]=Object.values($.gridTemplate).join(` `);return}n.size<n.lockSize||(t===`X`&&$.X.size>globalThis.innerWidth/2&&($.X.size=globalThis.innerWidth/2),t===`Y`&&($.Y.size>globalThis.innerHeight-10&&($.Y.size=globalThis.innerHeight),OL.style.marginTop=`-${$.Y.size/2}px`),$.gridTemplate[t]=`${n.size}px`,document.body.style[n.grid]=Object.values($.gridTemplate).join(` `),$.gridTemplate={X:`auto`,Z:`10px`,Y:`auto`})}function Qr(e,t){if($.X.size=e.touches?.[0]?.pageX||e.pageX,$.Y.size=globalThis.innerHeight-(e.touches?.[0]?.pageY||e.pageY),$[t.axis].size<$[t.axis].min){document.body.style[$[t.axis].grid]=Q[$[t.axis].grid];return}$.gridTemplate[t.axis]=`0px`,document.body.style[$[t.axis].grid]=Object.values($.gridTemplate).join(` `),$.gridTemplate={X:`auto`,Z:`10px`,Y:`auto`}}function $r(){document.body.style.cursor=`auto`,globalThis.removeEventListener(`mousemove`,this.resizeEvent),globalThis.removeEventListener(`touchmove`,this.resizeEvent),globalThis.removeEventListener(`mouseup`,this.stopResize),globalThis.removeEventListener(`touchend`,this.stopResize)}var ei={Dataview:t,elements:ft,Gazetteer:pt,layers:nn,locations:vr,Tabview:yr,utils:{cssColour:xr,dataview:wr,dataviewDialog:Ar,histogram:jr,idleLogout:Fr,imagePreview:zr,Json:Br,layerJSE:qr,locationCount:Yr,resizeHandler:Xr}};globalThis.ui=ei,globalThis.mapp??={},mapp.ui=ei;
