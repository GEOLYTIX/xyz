mapp.utils.merge(mapp.dictionaries,{en:{layer_zoom_to_extent:"Zoom to filtered layer extent",layer_visibility:"Toggle visibility"},de:{layer_zoom_to_extent:"Zoom zum Ausma\xDF des gefilterten Datensatzes",layer_visibility:"Umschalten der Ansicht"},cn:{layer_zoom_to_extent:"\u7F29\u653E\u81F3\u76F8\u5E94\u7B5B\u9009\u8303\u56F4",layer_visibility:"\u5207\u6362\u53EF\u89C1\u6027"},pl:{layer_zoom_to_extent:"Poka\u017C zasi\u0119g warstwy",layer_visibility:"Widoczno\u015B\u0107"},ko:{layer_zoom_to_extent:"\uD544\uD130\uB41C \uB808\uC774\uC5B4\uD06C\uAE30\uC5D0 \uC90C(zoom)",layer_visibility:"\uD1A0\uAE00 \uAC00\uC2DC\uC131"},fr:{layer_zoom_to_extent:"Zoom sur l'\xE9tendue de la couche",layer_visibility:"Changer la visiblit\xE9"},ja:{layer_zoom_to_extent:"\u30D5\u30A3\u30EB\u30BF\u30FC\u3055\u308C\u305F\u30EC\u30A4\u30E4\u30FC\u7BC4\u56F2\u3092\u30BA\u30FC\u30E0\u306B",layer_visibility:"\u8868\u793A\u5207\u66FF"}});var _=e=>{let t=e.filter.zoomToExtent&&mapp.utils.html`
  ...
  <div class="dropdown active">
    ...
    <div
      ...
      <div
        ...
        style="grid-column: 2;"
        onclick=${r=>{!e.filter||(r.target.classList.toggle("disabled"),r.target.classList.contains("disabled")?(e.filter.current[t.field]||(e.filter.current[t.field]={}),e.filter.current[t.field].ni||(e.filter.current[t.field].ni=[]),e.filter.current[t.field].ni.push(i[1].keys||i[0]),e.filter.current[t.field].ni=e.filter.current[t.field].ni.flat()):(Array.isArray(i[1].keys)?i[1].keys.forEach(s=>{e.filter.current[t.field].ni.splice(e.filter.current[t.field].ni.indexOf(s),1)}):e.filter.current[t.field].ni.splice(e.filter.current[t.field].ni.indexOf(i[0]),1),e.filter.current[t.field].ni.length||delete e.filter.current[t.field]),e.style.filter?(e.style.filter[t.field]=e.filter.current[t.field],e.style.filter[t.field]||delete e.style.filter[t.field],e.L.changed()):e.reload())}}>${i[1].label||i[0]}`;l.push(mapp.utils.html`
    <div 
      data-id=${i[0]}
      class="contents">
  ...
  <div
    ...
    <div
      class="val"
      style="${`${e.css_val||""}`}">${t}`};var le=e=>{let t=e.location.layer.mapview;e.Layer=t.layers[e.layer];let l=mapp.ui.elements.chkbox({label:e.label||"MVT Clone",data_id:`${e.field}-chkbox`,checked:!!e.display,onchange:n=>{n?o():(e.display=!1,t.Map.removeLayer(e.L),e.Layer.clones.delete(e.L),e.legend?.remove())}});e.display&&o();let i=mapp.utils.html.node`<div>${l}`;function a(){if(!!e.Layer.tables){if(e.Layer.tableCurrent()===null)return i.querySelector("input").disabled=!0;i.querySelector("input").disabled=!1}}return a(),t.Map.getTargetElement().addEventListener("changeEnd",a),e.location.removeCallbacks.push(()=>{t.Map.getTargetElement().removeEventListener("changeEnd",a)}),i;async function o(){if(e.L){t.Map.addLayer(e.L),e.Layer.clones.add(e.L);return}let n=mapp.utils.queryParams(e),r=mapp.utils.paramString(n),s=await mapp.utils.xhr(`${e.host||e.location.layer.mapview.host}/api/query?${r}`);if(!e.location.remove)return;let c={style:{default:e.Layer.style.default},featureLookup:s};e.theme&&(c.style.theme=e.theme,e.legend=e.node.appendChild(mapp.utils.html.node`
        <div class="legend">
          ${e.theme.title||""}
          ${mapp.ui.layers.legends[e.theme.type](c)}`)),e.L=new ol.layer.VectorTile({source:e.Layer.L.getSource(),renderBuffer:200,zIndex:e.zIndex,style:mapp.layer.Style(c)}),e.Layer.clones.add(e.L),t.Map.addLayer(e.L),e.location.removeCallbacks.push(()=>{e.location.layer.mapview.Map.removeLayer(e.L),e.Layer.clones.delete(e.L)})}};var j=e=>{let t=!isNaN(e.value)&&e.type==="integer"?parseInt(e.value).toLocaleString("en-GB",{maximumFractionDigits:0}):parseFloat(e.value).toLocaleString("en-GB",{maximumFractionDigits:2});return e.edit&&(e.edit.range?t=mapp.ui.elements.slider({min:e.edit.range.min,max:e.edit.range.max,val:e.value,callback:i=>{t.value=parseFloat(i.target.value),e.newValue=t.value,e.location.view?.dispatchEvent(new CustomEvent("valChange",{detail:e}))}}):t=mapp.utils.html.node`
      <input
        type="number"
        value="${e.value||""}"
