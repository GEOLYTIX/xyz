mapp.utils.merge(mapp.dictionaries,{en:{layer_zoom_to_extent:"Zoom to filtered layer extent",layer_visibility:"Toggle visibility"},de:{layer_zoom_to_extent:"Zoom zum Ausma\xDF des gefilterten Datensatzes",layer_visibility:"Umschalten der Ansicht"},cn:{layer_zoom_to_extent:"\u7F29\u653E\u81F3\u76F8\u5E94\u7B5B\u9009\u8303\u56F4",layer_visibility:"\u5207\u6362\u53EF\u89C1\u6027"},pl:{layer_zoom_to_extent:"Poka\u017C zasi\u0119g warstwy",layer_visibility:"Widoczno\u015B\u0107"},ko:{layer_zoom_to_extent:"\uD544\uD130\uB41C \uB808\uC774\uC5B4\uD06C\uAE30\uC5D0 \uC90C(zoom)",layer_visibility:"\uD1A0\uAE00 \uAC00\uC2DC\uC131"},fr:{layer_zoom_to_extent:"Zoom sur l'\xE9tendue de la couche",layer_visibility:"Changer la visiblit\xE9"},ja:{layer_zoom_to_extent:"\u30D5\u30A3\u30EB\u30BF\u30FC\u3055\u308C\u305F\u30EC\u30A4\u30E4\u30FC\u7BC4\u56F2\u3092\u30BA\u30FC\u30E0\u306B",layer_visibility:"\u8868\u793A\u5207\u66FF"}});var y=e=>{let t=mapp.utils.html`
  ...
  <div
    ...
    <div
      class="val"
      style="${`${e.css_val||""}`}">
      ${t}`};var de={key:Ke,boolean:X,dataview:K,date:x,datetime:x,defaults:Q,documents:Y,geometry:ee,html:E,images:te,integer:S,isoline_here:L,isoline_mapbox:L,json:ae,numeric:S,pin:ie,report:le,streetview:oe,tab:ne,text:re,textarea:E,time:se};function Ke(e){return mapp.utils.html.node`
  <div class="layer-key">
    <span>
  ...
  <input
    type="text"
    placeholder="Filter"
    oninput=${n=>{let r=t.getColumn().getField();n.target.value.length?e.layer.filter.current[r]={[o.type]:encodeURIComponent(n.target.value)}:delete e.layer.filter.current[r],e.layer.reload(),e.update()}}>`}function at(e,t,i,a,l,o){return mapp.utils.html.node`<span>
    <input
      style="width: 50%"
      type="number"
      placeholder="Min"
    ...
    <input
      ...
      type="number"
      placeholder="Max"
      oninput=${n=>{let r=t.getColumn().getField();n.target.value.length?e.layer.filter.current[r]=Object.assign(e.layer.filter.current[r]||{},{lte:Number(n.target.value)}):delete e.layer.filter.current[r],e.layer.reload(),e.update()}}></input>`}var ue=e=>{!e.node||(e.tabs=e.node.appendChild(mapp.utils.html.node`<div class="tabs">`),e.panel=e.node.appendChild(mapp.utils.html.node`<div class="panel">`),e.id&&e.node.setAttribute("data-id",e.id),e.addTab=it,e.node.addEventListener("addTab",t=>e.addTab(t.detail)))};function it(e){let t=this;e.tab=mapp.utils.html.node`
    <div class="tab">
      <div
        class="header"
        style="${e.tab_style||""}"
  ...
  <button 
    ...
    <div
      ...
      <input id="b" type="range"
        ...
        step=${e.step||1}
        value=${e.val_b}
        oninput=${i}/>`;function i(a){t.style.setProperty(`--${a.target.id}`,a.target.value),t.querySelectorAll("input").forEach(l=>{l.id==a.target.id&&l!=a.target&&(l.value=a.target.value)}),a.target.id==="a"&&typeof e.callback_a=="function"&&e.callback_a(a),a.target.id==="b"&&typeof e.callback_b=="function"&&e.callback_a(b)}return t};var Se={viewport:gt,download_json:ft,download_csv:vt};function gt(e){return mapp.utils.html`
    <button
    ...
    <button
      class="flat"
      onclick=${()=>{e.Tabulator.download("csv",`${e.title||"table"}.csv`)}}>Download as CSV`}var Ee={card:me,chkbox:he,contextMenu:ge,drawer:fe,drawing:ve,dropdown:we,dropdown_multi:be,isoline_params_here:_e,isoline_params_mapbox:ye,legendIcon:ke,slider:xe,slider_ab:Le,toolbar_el:Se};var C,v=null;async function Ce(e,t){return v?new v(e,t):(C||(C=new Promise(async i=>{if(window.Chart){v=window.Chart,i();return}Promise.all([import("https://cdn.skypack.dev/chart.js@3.7.0"),import("https://cdn.skypack.dev/chartjs-plugin-datalabels@2.0.0"),import("https://cdn.skypack.dev/chartjs-plugin-annotation@1.4.0")]).then(a=>{a[0].Chart.register(...a[0].registerables),a[0].Chart.register(a[1].default),a[0].Chart.register(a[2].default),v=a[0].Chart,i()}).catch(a=>{console.error(a.message),alert("Failed to load Chart.js library. Please reload the browser.")})})),await C,new v(e,t))}var j,w=null;async function je(){return w?new w(...arguments):(j||(j=new Promise(async e=>{if(window.Tabulator){w=window.Tabulator,e();return}Promise.all([import("https://cdn.skypack.dev/-/tabulator-tables@v5.2.7-uFL2B6RLQMMWIcn9QymG/dist=es2019,mode=imports/optimized/tabulator-tables.js")]).then(t=>{w=t[0].TabulatorFull,e()}).catch(t=>{console.error(t.message),alert("Failed to load Chart.js library. Please reload the browser.")})})),await j,new w(...arguments))}var u={idle:600},Oe=e=>{Object.assign(u,e),u.idle!==0&&(window.onload=f,window.onmousemove=f,window.onmousedown=f,window.ontouchstart=f,window.onclick=f,window.onkeypress=f,f(),z())};function f(){u.locked||(u.timeout&&clearTimeout(u.timeout),u.timeout=setTimeout(ze,u.idle*1e3))}function ze(){u.locked=!0,u.renew&&clearTimeout(u.renew);let e=new XMLHttpRequest;e.open("GET",`${u.host}/api/user/cookie?destroy=true`),e.send();let t=mapp.utils.html.node`
    <form
      class="login"
