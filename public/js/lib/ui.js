import{t as e}from"./chunk-DK3Fl9T5.js";async function t(e){if(!(!e.dynamic&&typeof e.create==`function`)){if(typeof e.create==`function`){e.create();return}if(typeof e.target==`string`&&(e.target=document.getElementById(e.target)),!(e.target instanceof HTMLElement)){console.warn(`Dataviews require a HTMLHtmlElement target`);return}return i(e)instanceof Error?e.err:(typeof e.label==`string`&&(e.chkbox=mapp.ui.elements.chkbox({checked:!!e.display,data_id:e.key,disabled:e.disabled,label:e.label,onchange:t=>{e.display=t,e.display?e.show():e.hide()}})),e.update??=a,e.show??=n,e.hide??=r,mapp.ui.utils.dataview.Toolbar(e),mapp.ui.utils.dataview.mapChange(e),e)}}async function n(){this.display=!0,this.create===void 0?(this.create=()=>{mapp.ui.utils[this.dataview].create(this)},this.create(),typeof this.update==`function`&&this.update()):this.dynamic?(typeof this.create==`function`&&await this.create(),typeof this.update==`function`&&this.update()):this.reload&&typeof this.update==`function`&&this.update(),this.btnRow?.style.setProperty(`display`,`flex`),this.target.style.display=`block`}function r(){this.display=!1,this.target.style.display=`none`,this.btnRow?.style.setProperty(`display`,`none`)}function i(e){if(e.key??=e.query||e.title||e.label,e.dataview||(e.chart&&(e.dataview=`chartjs`),e.columns!==void 0&&(console.warn(`Table dataviews should be configured inside a tables object`),e.table={columns:e.columns},e.dataview=`tabulator`),e.table&&(e.dataview=`tabulator`)),!Object.hasOwn(mapp.ui.utils,e.dataview))return e.err=Error(`mapp.ui.utils.${e.dataview} doesnt exist`),console.error(e.err),e.update=()=>console.warn(`Unable to update ${e.key} dataview.`),e.err;if(typeof mapp.ui.utils[e.dataview].create!=`function`)return e.err=Error(`mapp.ui.utils.${e.dataview}.create() method doesn't exist`),console.error(e.err),e.err}async function a(){if(this.create===void 0&&(this.create=function(){mapp.ui.utils[this.dataview].create(this)},this.create()),!this.query)return;let e=mapp.utils.queryParams(this),t=mapp.utils.paramString(e);this.host??=this.layer?.mapview?.host;let n=await mapp.utils.xhr(`${this.host||mapp.host}/api/query?${t}`);if(!(n instanceof Error)){if(typeof this.responseFunction==`function`){this.responseFunction(n);return}typeof this.setData==`function`&&this.setData(n)}}function o(e){typeof e==`string`&&(e={text:e}),e.title??=mapp.dictionary.information,e.icon??=`info`,e.data_id??=`alert`,e.class??=`alert-confirm box-shadow no-select`;let t=e.icon===``?``:mapp.utils.html`<span class="material-symbols-outlined notranslate">${e.icon}</span>`;return e.header=mapp.utils.html`${t}${e.title}`,e.innerContent??=mapp.utils.html`<p>${e.text}</p>`,e.content=mapp.utils.html`
  ...
  <div
    ...
    <button
      class="action wide"
      onclick=${t=>f(t,e,e.draw.point)}>
      <span class="notranslate material-symbols-outlined">add_location_alt</span>
      ${e.draw.point.label}`,e.draw.point.btn}function ge(e){let t={content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
    ...
    <button
      class="action wide"
      onclick=${t=>f(t,e,e.draw.line)}>
      <span class="notranslate material-symbols-outlined">polyline</span>
      ${e.draw.line.label}`,e.draw.line.btn}function _e(e){let t={content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
    ...
    <button
      class="action wide"
      onclick=${t=>f(t,e,e.draw.polygon)}>
      <span class="notranslate material-symbols-outlined">activity_zone</span>
      ${e.draw.polygon.label}`,e.draw.polygon.btn}function ve(e){let t={content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
  ...
  <button
    class="action wide"
    onclick=${t=>f(t,e,e.draw.rectangle)}>
    <span class="notranslate material-symbols-outlined">rectangle</span>
    ${e.draw.rectangle.label}`,e.draw.rectangle.btn}function ye(e){let t={content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
  ...
  <button
    class="action wide"
    onclick=${t=>f(t,e,e.draw.circle_2pt)}>
    <span class="notranslate material-symbols-outlined">outbound</span>
    ${e.draw.circle_2pt.label}`,e.draw.circle_2pt.btn}function be(e){let t={helpDialog:{content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
    ...
    style="display: grid; grid-template-columns: 100px 1fr; align-items: center;">
      ...
      <div style="grid-column: 2;">
        ${mapp.ui.elements.dropdown({callback:(e,n)=>xe(t,n.option),entries:Object.values(t.unitsConfig),placeholder:t.unitsConfig[t.units].title})}`,t.rangeSlider=mapp.utils.html.node`<div>`,xe(t,t.units),t.btn=mapp.utils.html.node`<button
    class="action wide"
    onclick=${n=>f(n,e,t)}>
    ...
    <h3>${mapp.dictionary.circle_config}</h3>`,content:n,popout:t.popout,dialog:t.dialog}),t.drawer)}function xe(e,t){let n=e.unitsConfig[t];e.units=n.option,e.radius??=n.min,e.radius=e.radius>n.max?n.max:e.radius,mapp.utils.render(e.rangeSlider,mapp.ui.elements.slider({callback:t=>{e.radius=Number.parseFloat(t)},max:n.max,min:n.min,val:e.radius}))}function Se(e){return e.draw.locator={label:mapp.dictionary.draw_position,type:`Point`,...e.draw.locator,layer:e},e.draw.locator.btn=mapp.utils.html.node`
    <button
      class="action wide"
      ...
      ${e.draw.locator.label}`,e.draw.locator.btn}function Ce(e){e.selectedTitles=new Set,e.selectedOptions=new Set,e.placeholder??=e.span||``,e.entries=e.entries?.filter?.(e=>e.option!==``),De(e),Oe(e),e.onChange??=Te,e.options=Ee(e);let t=e.selectedTitles.size?Array.from(e.selectedTitles).join(`, `):e.placeholder,n=e.keepPlaceholder?e.placeholder:t;return e.placeHolderOption=mapp.utils.html.node`
  ...
  <div>
    ...
    <ul>${r}`)}var dt={alert:o,btnPanel:s,card:c,chkbox:l,confirm:u,contextMenu:ee,control:re,dialog:oe,drawer:ue,drawerDialog:d,drawing:me,dropdown:Ce,helpDialog:Ae,jsoneditor:je,layerStyle:m,legendIcon:Ve,locales:Ke,loginForm:qe,numericInput:Je,pills:Xe,radio:$e,reportLink:tt,registerForm:et,searchbox:rt,slider:it,slider_ab:at,toast:ot,tooltip:st,userLocale:ct},ft=e=>{e={...e,...mapp.ui.elements.searchbox({name:`gazetteer-search-input`,placeholder:e.placeholder,searchFunction:t,target:e.target})};function t(t){if(e.list.innerHTML=``,!t.target.value.length)return;let n=t.target.value.split(`,`).map(parseFloat);if(n.length===2&&n.every(e=>typeof e==`number`&&!Number.isNaN(Number(e))&&Number.isFinite(Number(e)))){let[t,r]=n;if(t>=-90&&t<=90&&r>=-180&&r<=180){e.list.appendChild(mapp.utils.html.node`
        <li onclick=${t=>{mapp.utils.gazetteer.getLocation({label:`Latitude:${n[0]}, Longitude:${n[1]}`,lat:n[0],lng:n[1],source:`Coordinates`},e)}}><span>Latitude:${n[0]}, Longitude:${n[1]}</span>`);return}else e.list.appendChild(mapp.utils.html.node`
          <li style="color: red;">
            <span>${mapp.dictionary.invalid_lat_long_range}</span>`)}e.provider&&(Object.hasOwn(mapp.utils.gazetteer,e.provider)?mapp.utils.gazetteer[e.provider](t.target.value,e):console.warn(`Requested gazetteer service not available`)),mapp.utils.gazetteer.datasets(t.target.value,e)}};function pt(e){e.style?.legend&&mapp.layer.featureFields.distributionReady(e)&&mapp.ui.layers.legends[e.style.theme?.type]?.(e)}var S={applyFilter:mt,boolean:_t,date:wt,datetime:wt,in:Ct,integer:bt,like:T,match:T,ni:Ct,null:vt,numeric:bt,removeFilter:ht,resetFilter:gt,generateMinMax:yt},C;function mt(e){clearTimeout(C),C=setTimeout(()=>{e.reload(),mapp.ui.layers.drawLegend(e),e.filter.list?.forEach(e=>{e.histogram?.update?.()}),e.mapview?.target.dispatchEvent(new Event(`changeEnd`))},500)}function w(e,t){e.filter.current[t.field]&&(Object.hasOwn(e.filter.current[t.field],t.type)&&delete e.filter.current[t.field][t.type],delete e.filter.current[t.field],Number.isNaN(Number(t.Min))&&delete t.min,Number.isNaN(Number(t.Max))&&delete t.max,mapp.ui.layers.filters.applyFilter(e))}function ht(e,t){w(e,t),t.li?.classList.remove(`selected`),t.card?.remove(),t.histogram?.removeChangeEnd?.(),delete t.selected,delete t.card,e.filter.list?.some(e=>e.card)||(e.filter.clearAll instanceof HTMLElement&&e.filter.clearAll.style.setProperty(`display`,`none`),e.filter.resetAll instanceof HTMLElement&&e.filter.resetAll.style.setProperty(`display`,`none`),e.filter.feature_count instanceof HTMLElement&&e.filter.feature_count.style.setProperty(`display`,`none`))}async function gt(e,t){if(w(e,t),!t.card)return;t.content=[await mapp.ui.layers.filters[t.type](e,t)].flat();let n=mapp.ui.elements.card(t);t.card.replaceWith(n),t.card=n}function T(e,t){return mapp.utils.html.node`
  <input
    type="text"
    onkeyup=${n=>{n.target.value.length?(e.filter.current[t.field]??={},e.filter.current[t.field][t.type]=encodeURIComponent(`${t.leading_wildcard&&`%`||``}${n.target.value}`)):delete e.filter.current[t.field],S.applyFilter(e)}}>`}function _t(e,t){function n(n){e.filter.current[t.field]={boolean:n},S.applyFilter(e)}return n(!1),mapp.ui.elements.chkbox({label:t.label||t.title||`chkbox`,onchange:n})}function vt(e,t){function n(n){e.filter.current[t.field]={null:n},S.applyFilter(e)}return n(!1),mapp.ui.elements.chkbox({label:t.label||t.title||`chkbox`,onchange:n})}async function yt(e,t){let n=mapp.utils.queryParams({layer:e,queryparams:{table:!0,template:`field_minmax`,field:t.field},viewport:e.filter.viewport});n.filter=structuredClone(e.filter.current),delete n.filter[t.field];let r=mapp.utils.paramString(n),i=await mapp.utils.xhr(`${e.mapview.host}/api/query?${r}`),a=Number.isNaN(Number(t.min))?i.minmax[0]:t.min,o=Number.isNaN(Number(t.max))?i.minmax[1]:t.max;e.filter?.viewport&&(a=i.minmax[0],o=i.minmax[1]),t.min=t.type===`integer`?Math.round(a):Number.parseFloat(a),t.max=t.type===`integer`?Math.round(o):Number.parseFloat(o),t.min=t.val_a<t.min?t.val_a:t.min,t.max=t.val_b>t.max?t.val_b:t.max}async function bt(e,t){if(t={...e.infoj.find(e=>e.field===t.field),...t},delete t.val_a,delete t.val_b,t.Min??=Number.isFinite(t.min)?t.min:void 0,t.Max??=Number.isFinite(t.max)?t.max:void 0,(Number.isNaN(Number(t.max))||Number.isNaN(Number(t.min)))&&await yt(e,t),Number.isNaN(Number(t.min))||Number.isNaN(Number(t.max)))return mapp.utils.html.node`<div>${mapp.dictionary.no_data_filter}</div>`;t.step??=t.type===`integer`?1:.01,e.filter.current[t.field]=Object.assign({gte:Number(t.min),lte:Number(t.max)},e.filter.current[t.field]),(t.Min!==void 0||t.Max!==void 0)&&S.applyFilter(e);let n=t.prefix||t.suffix?`(${(t.prefix||t.suffix).trim()})`:``;return t.label_a??=`${mapp.dictionary.layer_filter_greater_than} ${n}`,t.label_b??=`${mapp.dictionary.layer_filter_less_than} ${n}`,t.val_a=e.filter.current?.[t.field]?.gte,t.val_b=e.filter.current?.[t.field]?.lte,t.callback_a=n=>{e.filter.current[t.field].gte=n,t.val_a=Number(n),S.applyFilter(e)},t.callback_b=n=>{e.filter.current[t.field].lte=n,t.val_b=Number(n),S.applyFilter(e)},t.slider=mapp.ui.elements.slider_ab(t),await xt(t,e),mapp.utils.html`
    ${t.histogram?.container}
    ${t.slider}`}async function xt(e,t){if(e.histogram&&(e.histogram===!0&&(e.histogram={}),e.histogram.container??=mapp.utils.html.node`<div>`,e.histogram.dataview??=`histogram`,e.histogram.options??={tooltip:!0},e.histogram.target??=e.histogram.container,e.histogram.layer??=t,e.histogram.queryparams??={field:e.field,table:!0,filter:!0},e.histogram.viewport??=t.filter.viewport,e.histogram.update||await mapp.ui.Dataview(e.histogram),e.histogram.viewport)){t.mapview.Map.getTargetElement().addEventListener(`changeEnd`,n),e.histogram.removeChangeEnd=()=>{t.mapview.Map.getTargetElement().removeEventListener(`changeEnd`,n)};function n(){e.histogram.update()}}}function St(e){return e.tableCurrent()||Object.values(e.tables??{}).find(e=>!!e)}async function Ct(e,t){if(w(e,t),!Array.isArray(t[t.type])){let n=structuredClone(e.filter?.current);delete n?.[t.field];let r=await mapp.utils.xhr(`${e.mapview.host}/api/query?`+mapp.utils.paramString({field:t.field,filter:n,layer:e.key,locale:e.mapview.locale.key,table:St(e),template:`distinct_values`}));if(!r||r instanceof Error)return console.warn(`Distinct values query did not return any values for field ${t.field}`),mapp.utils.html.node`<div>${mapp.dictionary.no_data_filter}</div>`;t[t.type]=[r].flat().map(e=>e[t.field]).filter(e=>e!=null)}let n=new Set(e.filter?.current[t.field]?.[t.type]||[]);if(t.dropdown||t.dropdown_pills||t.dropdown_search){let r=mapp.ui.elements.dropdown({callback:async(n,r)=>{r.length?Object.assign(e.filter.current,{[t.field]:{[t.type]:r}}):delete e.filter.current[t.field],S.applyFilter(e)},field:t.field,search:t.dropdown_search,entries:t[t.type].map(e=>({option:e,selected:n.has(e),title:e})),inputfilter:!0,keepPlaceholder:t.dropdown_pills,maxHeight:300,multi:!0,pills:t.dropdown_pills,placeholder:t.placeholder||mapp.dictionary.layer_filter_dropdown_select});return mapp.utils.html.node`<div class="filter">${r}`}if(t.searchbox){let n=mapp.utils.html.node`<div class="filter">`,r=mapp.ui.elements.pills({pills:e.filter?.current[t.field]?.[t.type],addCallback:(n,r)=>{Object.assign(e.filter.current,{[t.field]:{[t.type]:[...r]}}),S.applyFilter(e)},removeCallback:(n,r)=>{r.size===0?(i.input.value=null,delete e.filter.current[t.field]):Object.assign(e.filter.current,{[t.field]:{[t.type]:[...r]}}),S.applyFilter(e)},target:n}),i=mapp.ui.elements.searchbox({placeholder:mapp.dictionary.filter_searchbox_placeholder,searchFunction:e=>{if(i.list.innerHTML=``,!e.target.value)return;let n=e.target.value,a=t[t.type].filter(e=>e.toString().toLowerCase().startsWith(n.toLowerCase()));if(!a.length){i.list.append(mapp.utils.html.node`
    ...
    <div style="
      ...
      <label>Date after
        ${n}</label>
      <label>Date before
        ${r}</label>`}function E(e,t,n){if(e.field??=t.field,e.disabled??=n.filter?.current[e.field]?.ni?.includes(e.value),e.disabled||(e.style??=e._style),n.featureFields&&t.distribution===`count`){let r={value:n.featureFields[e.field]?.[e.value]};if(e.count=mapp.utils.formatNumericValue(r),!e.disabled&&!t.legend?.showEmptyCat&&!e.count)return}let r=mapp.ui.elements.legendIcon({height:24,width:24,...e._style||e.style}),i=mapp.utils.html`<div
    ...
    data-id="cluster"
    class="contents">
    ${t}${n}`}function k(e){e.legend??={},e.legend.layout??=`grid`;let t=[`contents-wrapper`,e.legend.layout];e.legend.nowrap&&t.push(`nowrap`),e.legend.classList=t.join(` `)}function A(e,t){if(!e.style.legend){e.style.legend=t;return}e.style.legend.replaceChildren(...t.children)}function j(){return mapp.utils.html`<div
    class="switch-all"
    ...
    </button>.`}function Dt(e){let t=e.style.theme;t.legend??={},t.legend.grid=[],e.style.theme.style??={},e.style.theme.style.width??=24,e.style.theme.style.height??=24;let n=mapp.ui.elements.legendIcon(e.style.theme.style);return t.legend.grid.push(mapp.utils.html`
    <div 
      class="contents">
      ${n}<div class="label" style="grid-column: 2";>${e.style.theme.label}`),t.legend.node=mapp.utils.html.node`
    <div class="legend">
    <div class="contents-wrapper grid">${t.legend.grid}`,A(e,t.legend.node),t.legend.node}function Ot(e){if(!mapp.layer.featureFields.distributionReady(e))return;let t=e.style.theme;k(t),t.legend.grid=t.categories.map(n=>E(n,t,e)).filter(e=>e!==void 0),e.style.cluster&&t.legend.grid.push(O(e));let n=j();return t.legend.node=mapp.utils.html.node`
    <div class="legend">
      ${n}
      <div class=${t.legend.classList}>
        ${t.legend.grid}`,A(e,t.legend.node),t.legend.node}var M=new WeakSet,N=new WeakMap;function P(e){e.style.theme.filterOnly=!0;let t=e.style.theme;if(k(t),!t.hideLegend){t.legend.grid=[];for(let n of t.categories)n.label=n.values?.join(`, `),n.label!==void 0&&t.legend.grid.push(E(n,t,e));return e.style.cluster&&t.legend.grid.push(O(e)),t.legend.node=mapp.utils.html.node`
    <div class="legend">
      <div class=${t.legend.classList}>
        ${t.legend.grid}`,A(e,t.legend.node),N.set(t,I(t)),F(e,t),t.legend.node}}function F(e,t){M.has(e)||(M.add(e),e.L.once(`postrender`,()=>{if(M.delete(e),t===e.style.theme){if(I(t)===N.get(t)){F(e,t);return}P(e)}}))}function I(e){return e.categories.reduce((e,t)=>e+(t.values?.length||0),0)}function kt(e){if(!mapp.layer.featureFields.distributionReady(e))return;let t=e.style.theme;t.filterOnly=!0,k(t),t.legend??={};let n=t.categories.filter(e=>e.value!==void 0).map(n=>{let r=`contents ${t.legend?.horizontal?`horizontal`:``}`,i=mapp.ui.elements.legendIcon({height:24,width:24,...n._style||n.style});n.label??=n.value;let a=`label switch ${n.disabled?`disabled`:``}`;return mapp.utils.html`<div 
        data-id=${n.value}
        class=${r}>
        <div 
          style="height: 24px; width: 24px; grid-column: 1;">
  ...
  class="notranslate material-symbols-outlined active"
    ...
    data-id=resetall
    class="flat underline"
    onclick=${t=>{e.filter.list.forEach(t=>mapp.ui.layers.filters.resetFilter(e,t))}}>${mapp.dictionary.filter_reset_all}`,e.filter.count=mapp.utils.html.node`<span class="bold">`,e.changeEndCallbacks.push(L),e.showCallbacks.push(L),e.hideCallbacks.push(e=>{e.filter.clearAll?.checkVisibility()&&e.filter.feature_count.style.setProperty(`display`,`none`)}),e.filter.count_meta??=mapp.dictionary.filter_count,e.filter.feature_count=mapp.utils.html.node`
    ...
    <div class="link-with-img">
      ...
      data-src=${e.value}
      onclick=${t=>H(t,e,mapp.dictionary.remove_image_confirm)}>delete`;return mapp.utils.html.node`<div class="img-item"><img
      style="width: 100%"
      src=${e.value}
      ...
      <input
        type="file"
        accept="image/*;capture=camera" onchange=${t=>{B(t,e)}}>`}function un(e){let t=[];if(e.value?.map(n=>{let r=e.edit&&mapp.utils.html`<button title="${mapp.dictionary.delete}"
      class="notranslate material-symbols-outlined color-danger delete"
      data-name=${n.replace(/^.*\//,``).replace(/\.([\w-]{3})/,``)}
      data-src=${n}
      onclick=${t=>H(t,e,mapp.dictionary.remove_image_confirm)}>delete`;t.push(mapp.utils.html`<div class="img-item"><img
      src=${n}
      onclick=${mapp.ui.utils.imagePreview}>
      ...
      <input
        type="file"
        accept="image/*;capture=camera" onchange=${t=>{B(t,e)}}>`;t.push(n)}if(t.length)return mapp.utils.html.node`<div class="images-grid">${t}`}function dn(e){let t=[];if(e.value?.map(n=>{let r=e.edit&&mapp.utils.html`<button
      title="${mapp.dictionary.delete}"
      class="notranslate material-symbols-outlined color-danger delete"
      data-name=${n.replace(/^.*\//,``).replace(/\.([\w-]{3})/,``)}
      data-href=${n}
      ...
      <input type="file"
        ...
        onchange=${t=>B(t,e)}>`;t.unshift(n)}if(t.length)return mapp.utils.html.node`<div>${t}`}async function B(e,t){let n=new FileReader,r=e.type===`drop`?e.dataTransfer?.files[0]:e.target.files[0];if(!r)return;t.location.view?.classList.add(`disabled`);let i={documents:pn,image:V,images:V};n.onload=e=>i[t.type](e,t,r),n.readAsDataURL(r)}function V(e,t,n){let r=new Image;r.onload=async()=>fn(t,r,n),r.src=e.target.result}async function fn(e,t,n){let r=document.createElement(`canvas`);e.max_size??=1024,t.width>t.height&&t.width>e.max_size?(t.height*=e.max_size/t.width,t.width=e.max_size):t.height>e.max_size&&(t.width*=e.max_size/t.height,t.height=e.max_size),r.width=t.width,r.height=t.height,r.getContext(`2d`).drawImage(t,0,0,t.width,t.height);let i=await W(e,{public_id:n.name.replace(/^.*\//,``).replace(/\.([\w-]{3})/,``)+e.suffix_date?`@${Date.now()}`:``});if(!i)return;let a=new FormData;a.append(`file`,r.toDataURL(`image/jpeg`,.5));let o=await fetch(i,{body:a,method:`post`});if(!o.ok){mapp.ui.elements.alert(`Cloudinary Image upload failed!`),e.location.view?.classList.remove(`disabled`);return}let s=await o.json();e.type===`image`?e.value=s.secure_url:e.value=Array.isArray(e.value)?e.value.concat([s.secure_url]):[s.secure_url],U(e)}async function pn(e,t,n){let r=new Date,i=`${r.getMonth()+1}-${r.getDate()}T${r.getHours()}:${r.getMinutes()}`,a=n.name.substring(n.name.lastIndexOf(`.`)),o=await W(t,{public_id:`${n.name.replace(a,``)}-${i}${a}`});if(!o)return;let s=new FormData;s.append(`file`,e.target.result.toString());let c=await fetch(o,{body:s,method:`post`});if(!c.ok){mapp.ui.elements.alert(`Cloudinary Document upload failed!`),t.location.view?.classList.remove(`disabled`);return}let l=await c.json();t.value=Array.isArray(t.value)?t.value.concat([l.secure_url]):[l.secure_url],U(t)}async function H(e,t,n){if(!await mapp.ui.elements.confirm({text:n}))return;let r=await W(t,{destroy:!0,public_id:decodeURIComponent(e.target.dataset.name)});if(!r)return;await fetch(r,{method:`post`});let i=new Set(t.value);i.delete(e.target.dataset.src||e.target.dataset.href),t.type===`image`?t.value=null:t.value=i.size?Array.from(i):null,U(t)}async function U(e){e.location.view?.classList.add(`disabled`),await mapp.utils.xhr({body:JSON.stringify({[e.field]:e.value}),method:`POST`,url:`${e.location.layer.mapview.host}/api/query?`+mapp.utils.paramString({id:e.location.id,layer:e.location.layer.key,locale:e.location.layer.mapview.locale.key,table:e.location.table,template:e.location.updateTemplate})}),e.node.replaceChildren(mapp.utils.html.node`<div class="label">${e.title}`,R[e.type](e)),e.location.view?.classList.remove(`disabled`)}async function W(e,t){let n=mapp.utils.paramString({...t,folder:e.cloudinary_folder}),r=await mapp.utils.xhr({responseType:`text`,url:`${e.location.layer.mapview.host}/api/sign/cloudinary?${n}`});return r instanceof Error&&(console.error(r),e.location.view?.classList.remove(`disabled`)),r}function mn(e){if(e.value!==void 0&&(e.data=e.value),e.data?.length===0&&(e.data=null),e.layer??=e.location.layer,hn(e),e.target instanceof HTMLElement){if(!e.display&&typeof e.create==`function`)return mapp.utils.html.node`
    ${e.chkbox||``}
    ...
    <button 
      class=${t}
      title="${mapp.dictionary.layer_visibility}"
      onclick="${t=>{t.target.classList.toggle(`active`)?e.location.layer.show():e.location.layer.hide()}}">${e.location.layer.name}`}function Dn(e){return On(e),e.mapview??=e.location.layer.mapview,kn(e),e.zIndex??=e.location.layer.zIndex++,An(e),e.panel??=mapp.utils.html.node`<div class="entry-layer">`,e.panel}function On(e){e.value!==void 0&&(e.data=e.value,e.data?.length===0&&(e.data=null)),(e.featureSet||e.featureLookup)&&(!e.query&&!e.data?(e.disabled=!0,e.display_toggle?.classList.add(`disabled`)):(e.disabled=!1,e.display_toggle?.classList.remove(`disabled`)))}function kn(e){if(!e.layer)return;e.key=e.layer;let t=structuredClone(e.mapview.locale.layers.find(t=>t.key===e.layer));if(!t){console.warn(`Layer [${e.layer}] not found in mapview.locale`);return}let n=Object.keys(t).filter(t=>!Object.hasOwn(e,t)).reduce((e,n)=>(e[n]=t[n],e),{});Object.assign(e,n),e.params??={},e.params.layer_template??=e.layer,delete e.layer}async function An(e){if(e.L&&e.display){e.show();return}else if(e.L)return;e.source_key=e.key,await mapp.layer.decorate(e),e.L&&(e.location.Layers.push(e),e.key+=`-${e.location.id}`,e.L.set(`key`,e.key),e.show=jn,e.hide=Nn,e.mapview.layers[e.key]=e,e.display_toggle=mapp.ui.elements.chkbox({checked:e.display,data_id:`${e.key}-display`,disabled:e.disabled,label:e.name,onchange:t=>{t?e.show():e.hide()}}),e.tables&&(e.tableCurrent()||e.display_toggle.classList.add(`disabled`),e.changeEndCallbacks.push(()=>{e.tableCurrent()?e.display_toggle.classList.remove(`disabled`):e.display_toggle.classList.add(`disabled`)})),e.panel.append(e.display_toggle),e.meta&&e.panel.append(mapp.ui.layers.panels.meta(e)),e.style.elements??=[`labels`,`label`,`hovers`,`hover`,`icon_scaling`,`opacitySlider`,`themes`,`theme`],e.style.panel=mapp.ui.elements.layerStyle.panel(e),e.style.default&&(e.style.default={...e.location?.style,...e.style.default}),e.style.panel?.style.setProperty(`display`,`none`),e.style.panel&&e.panel.append(e.style.panel),e.location.removeCallbacks.push(()=>{typeof e.remove==`function`&&e.remove()}),e.display&&e.show())}async function jn(){let e=this;if(e.query){let t=mapp.utils.queryParams(e),n=mapp.utils.paramString(t);if(e.data=await mapp.utils.xhr(`${e.mapview.host}/api/query?${n}`),!e.data){e.disabled=!0,e.display_toggle?.classList.add(`disabled`),e.panel.replaceChildren(e.display_toggle);return}}Mn(e),e.style.panel?.style.setProperty(`display`,`block`),e.display=!0;try{e.mapview.Map.addLayer(e.L)}catch{}typeof e.reload==`function`&&e.reload(),e.showCallbacks?.forEach(t=>typeof t==`function`&&t(e))}function Mn(e){if(e.data){if(e.featureSet){let t=Array.isArray(e.data)?e.data:[e.data];e.featureSet=new Set(t);return}if(e.featureLookup){e.featureLookup=Array.isArray(e.data)?e.data:[e.data];return}e.features=e.data,e.setSource(e.features)}}function Nn(){this.display=!1,this.style.panel?.style.setProperty(`display`,`none`),this.mapview.Map.removeLayer(this.L),this.hideCallbacks?.forEach(e=>typeof e==`function`&&e(this))}function Pn(e){let t=mapp.ui.elements.reportLink(e);return mapp.utils.html.node`<div class="val">${t}`}function Fn(e){if(e.edit)return In(e);if(!(e.value===null||Number.isNaN(Number(e.value))))return mapp.utils.formatNumericValue(e),mapp.utils.html.node`<div 
    class="val"
    style=${e.css_val}
    >${e.stringValue}`}function In(e){e.edit.callback??=()=>{e.location.view?.dispatchEvent(new CustomEvent(`valChange`,{detail:e}))},e.edit.min??=e.min||-2147483648,e.edit.max??=e.max||2147483647,e.edit.step??=e.step||e.type===`integer`?1:.1,Object.assign(e,e.edit);let t=e.value??e.newValue;return e.edit.range&&!Number.isNaN(Number(t))&&t>e.min&&t<e.max?mapp.ui.elements.slider(e):mapp.ui.elements.numericInput(e)}var Ln=e=>(e.pills??=e.value||[],mapp.ui.elements.pills(e),e.container);function Rn(e){if(!Array.isArray(e.value)){console.warn(`Entry type pin requires a value array.`);return}return e.srid??=e.location.layer.srid,e.display??=e.display===void 0,e.zIndex??=1/0,e.Style??=e.style?mapp.utils.style(e.style):e.location.pinStyle,e.show??=zn,e.hide??=Bn,e.getExtent??=Vn,e.geometry={coordinates:e.value,type:`Point`},e.display&&e.show(),e.label??=mapp.dictionary.pin,e.chkbox??=mapp.ui.elements.chkbox({checked:e.display,label:e.label,onchange:t=>{t?e.show():e.hide()}}),e.chkbox}function zn(){this.disabled||(this.location.removeLayer(this),this.display=!0,this.L=this.location.layer.mapview.geoJSON(this),this.location.Layers.push(this))}function Bn(){this.display=!1,this.location.removeLayer(this)}function Vn(){return this.buffer??=0,this.buffer=Number.parseInt(this.buffer)||200,ol.extent.buffer(this.L.getSource().getExtent(),this.buffer)}function Hn(e){return e.textarea||(e.textarea=mapp.utils.html.node`<textarea
      class="val"
      style="auto; min-height: 50px;">`,e.callback??=async(e,t)=>{t.textarea.value=e},mapp.utils.ping(e),e.location.removeCallbacks.push(()=>{e.callback=!1})),e.textarea}function Un(e){if(!e.query){console.warn(`You must provide a query to use "type": "query_button".`);return}return e.label??=`Run query:${e.query}`,mapp.utils.html.node`
    <button 
      class="flat wide bold"
      onclick=${()=>Wn(e)}>${e.label}`}async function Wn(e){e.updated_fields&&(console.warn(`entry.updated_fields is deprecated, please use entry.dependents instead.`),e.dependents??=e.updated_fields),e.location.view.classList.add(`disabled`),e.queryparams??={},e.queryparams.template=e.query;let t=mapp.utils.paramString(mapp.utils.queryParams(e));e.host??=e.location.layer.mapview.host+`/api/query`;let n=await mapp.utils.xhr(`${e.host}?${t}`);if(n instanceof Error){mapp.ui.elements.alert({text:mapp.dictionary.query_failed}),e.location.view.classList.remove(`disabled`);return}e.value=n,e.alert&&mapp.ui.elements.alert({text:e.alert}),e.reload&&e.location.layer.reload(),e.dependents&&await e.location.syncFields(e.dependents),e.location.view.dispatchEvent(new Event(`updateInfo`)),e.location.view.classList.remove(`disabled`)}var Gn=e=>{let t=document.querySelector(`[data-id=${e.target}]`);return e.tab_style=`border-bottom: 3px solid ${e.location.style.strokeColor}`,t.dispatchEvent(new CustomEvent(`addTab`,{detail:e})),e.display&&e.show(),mapp.ui.elements.chkbox({checked:!!e.display,label:e.label,onchange:t=>{e.display=t,e.display?e.show():e.remove()}})};function Kn(e){if(!(!e.edit&&!e.value))return e.edit?qn(e):mapp.utils.html.node`
    <div
      class="val"
      style=${e.css_val}>
      ${e.prefix}${e.value}${e.suffix}`}function qn(e){return e.edit.options?(e.container=mapp.utils.html.node`<div>${mapp.dictionary.loading}`,Yn(e),e.container):mapp.utils.html.node`<input
    type="text"
    maxlength=${e.edit.maxlength}
    value="${e.newValue||e.value||``}"
    placeholder="${e.edit.placeholder||``}"
    onkeyup=${t=>Jn(t,e)}>`}function Jn(e,t){t.newValue=t.edit.arraySeparator?e.target.value.split(t.edit.arraySeparator):e.target.value,t.location.view?.dispatchEvent(new CustomEvent(`valChange`,{detail:t}))}async function Yn(e){if(e.edit.dynamic===!0&&(e.edit.options=[]),!e.edit.options.length){(e.jsonb_field||e.json_field)&&(e.edit.query??=`distinct_values_json`),e.edit.query??=`distinct_values`;let t=mapp.utils.paramString({field:e.json_field||e.jsonb_field||e.field,filter:e.location.layer.filter?.current,id:e.location.id,key:e.jsonb_key||e.json_key,layer:e.location.layer.key,locale:e.location.layer.mapview.locale.key,table:e.location.layer.tableCurrent(),template:e.edit.query}),n=await mapp.utils.xhr(`${e.location.layer.mapview.host}/api/query?${t}`);if(n===null){e.container.innerHTML=`${mapp.dictionary.no_options_available}`;return}e.edit.options=[n].flat().map(e=>Object.values(e)[0])}if(e.edit.radio){Xn(e);return}let t=e.edit.options.map(e=>({option:e===null?null:typeof e==`string`&&e||Object.values(e)[0],title:e===null?null:typeof e==`string`&&e||Object.keys(e)[0]})),n=t.find(t=>t.option===e.value),r=e.value===void 0?e.edit.placeholder:e.value;mapp.utils.render(e.container,mapp.ui.elements.dropdown({callback:(t,n)=>{e.newValue=n.option,e.location.view?.dispatchEvent(new CustomEvent(`valChange`,{detail:e}))},entries:t,placeholder:r,span:n?.title||e.value}))}function Xn(e){let t=e.edit.options.map(n=>{let r={data_id:n,name:e.field,label:n,onchange:()=>{e.newValue=t.find(e=>e.querySelector(`input[type="radio"]:checked`)).querySelector(`input[type="radio"]`).dataset.id,e.location.view?.dispatchEvent(new CustomEvent(`valChange`,{detail:e}))}};return mapp.ui.elements.radio(r)});if(e.value){let n=t.find(t=>t.querySelector(`input[type="radio"]`).dataset.id===e.value);n&&(n.querySelector(`input[type="radio"]`).checked=!0)}mapp.utils.render(e.container,mapp.utils.html.node`${t}`)}function Zn(e){let t=e.type===`html`?``:e.value;e.edit&&(t=mapp.utils.html`
    <textarea
  ...
  <div
    class="val"
    style="${e.css_val}">${t}`;return!e.edit&&e.type===`html`&&(n.innerHTML=e.value||``),n}function Qn(e){let t,n=e.value?.toString().replace(`.`,`:`);return n=n&&n.length<3&&`${n}:00`||n,t=e.edit?mapp.utils.html.node`
      <input
        type="time"
        value=${n}
    ...
    <div
      class="val"
      style="${e.css_val}">
      ${t}`}function $n(e){return mapp.utils.html.node`
        <div
          class="label"
          style=${e.css_title}
          title=${e.tooltip}>${e.title}`}function er(){console.warn(`The type:defaults entry method has been deprecated.`)}function q(e){return console.warn(`The type:${e.type} entry method has been deprecated in favour of the type:layer entry method.`),e.type=`layer`,e.qID??=`id`,e.name??=e.label,Dn(e)}var tr={boolean:cn,dataview:mn,date:G,datetime:G,defaults:er,documents:z,geometry:_n,html:Zn,image:z,images:z,integer:Fn,json:Tn,key:En,layer:Dn,link:Pn,mvt_clone:q,numeric:Fn,pills:Ln,pin:Rn,ping:Hn,query_button:Un,report:Pn,tab:Gn,text:Kn,textarea:Zn,time:Qn,title:$n,vector_layer:q},J;function nr(e,t){if(!e.infoj)return;let n=mapp.utils.html.node`<div class="location-view-grid">`;J={},t??=e?.layer?.infoj_order;let r=Array.isArray(t)?t.map(i).filter(e=>e!==void 0):e.infoj;function i(t){if(typeof t==`string`){let n=e.infoj.find(e=>new Set([e.key,e.field,e.query]).has(t));return n||console.warn(`infoj_order field: "${t}" not found in location.infoj. Please add entry.key, entry.field, or entry.query to the entry.`),n}else if(typeof t==`object`)return t.location=e,t}let a=0;for(let t of r){if(t.key??=t.field||a++,e.view?.classList.contains(`disabled`))break;if(t.listview=n,t.type??=`text`,t.edit===!0&&(t.edit={}),rr(t),ir(t),ar(t)||(or(t),sr(t),cr(t),lr(t),ur(t),dr(t)))continue;if(!Object.hasOwn(mapp.ui.locations.entries,t.type)){console.error(`entry.type:${t.type} method not found.`);continue}let r=mapp.ui.locations.entries[t.type]?.(t);r&&t.node.append(r),pr(t)}return n}function rr(e){e.jsonb_field&&e.jsonb_key&&e.value!==null&&typeof e.value==`object`&&e.value.jsonb&&(e.value=e.value.jsonb[e.jsonb_field][e.jsonb_key])}function ir(e){if(e.objectAssignFromField&&e.objectMergeFromField){console.warn(`${e.key}: objectAssignFromField and objectMergeFromField cannot both be used on the same entry.`);return}let t=e.objectAssignFromField||e.objectMergeFromField||e.json_field;if(!t)return;let n=e.location.infoj.find(e=>e.field===t);if(n&&typeof n.value==`object`){if(e.json_field){if(!e.json_key){console.warn(`json_field requires entry.json_key to be specified`);return}if(!n.value)return;e.value=n.value[e.json_key]}e.objectAssignFromField&&Object.assign(e,n.value),e.objectMergeFromField&&mapp.utils.merge(e,n.value)}}function ar(e){if(e.skipEntry||e.skipFalsyValue&&!e.value&&!e.edit||e.skipUndefinedValue&&e.value===void 0&&!e.edit||e.skipNullValue&&e.value===null&&!e.edit)return!0}function or(e){e.nullValue!==void 0&&(e.edit||(e.value??=e.nullValue))}function sr(e){e.default!==void 0&&e.edit&&(e.value||(e.newValue=e.default,e.location.view?.dispatchEvent(new CustomEvent(`valChange`,{detail:e}))))}function cr(e){e.group&&(J[e.group]||(J[e.group]=e.listview.appendChild(mapp.ui.elements.drawer({data_id:`location-infoj-group`,class:`group ${e.groupClassList||``}`,header:mapp.utils.html`
          <h3>${e.group}</h3>`}))),e.listview=J[e.group])}function lr(e){let t=`contents ${e.type} ${e.class||``} ${e.inline&&`inline`||``}`;e.node=e.listview.appendChild(mapp.utils.html.node`
  <div
    data-type=${e.type}
    class=${t}>`)}function ur(e){e.title&&e.node.append(mapp.ui.locations.entries.title(e))}function dr(e){if(e.query)if(e.queryCheck??=e.run,e.queryCallback??=fr,e.runOnce||e.queryCheck){delete e.runOnce,e.host??=e.location.layer?.mapview?.host||mapp.host;let t=mapp.utils.queryParams(e),n=mapp.utils.paramString(t);return e.url=`${e.host}/api/query?${n}`,e.blocking&&e.location.view?.classList.add(`loading`),mapp.utils.xhr(e).then(t=>e.queryCallback(t,e)),!0}else e.field&&!e.xhr&&console.warn(`field:"${e.field}" has a query:"${e.query}" which is not set to run, please add the run or runOnce flag to the entry.`)}function fr(e,t){if(t.blocking&&t.location.view?.classList.remove(`loading`),e?t.value=t.field?e[t.field]:e:t.value=t.nullValue||null,ar(t)){t.node.remove();return}let n=mapp.ui.locations.entries[t.type]?.(t);n&&t.node.appendChild(n)}function pr(e){if(!e.tooltip)return;let t=e.node.querySelector(`label`)||e.node.querySelector(`div.label`);t&&(e.tooltipElement||(e.tooltipElement=mapp.ui.elements.tooltip({content:e.tooltip}),t.append(e.tooltipElement)))}var mr=[{symbol:`A`,colour:`#2E6F9E`},{symbol:`B`,colour:`#EC602D`},{symbol:`C`,colour:`#5B8C5A`},{symbol:`D`,colour:`#B84444`},{symbol:`E`,colour:`#514E7E`},{symbol:`F`,colour:`#E7C547`},{symbol:`G`,colour:`#368F8B`},{symbol:`H`,colour:`#841C47`},{symbol:`I`,colour:`#61A2D1`},{symbol:`J`,colour:`#37327F`},{symbol:`K`,colour:`#FFA500`},{symbol:`L`,colour:`#7FFF00`},{symbol:`M`,colour:`#FFD700`},{symbol:`N`,colour:`#00CED1`},{symbol:`O`,colour:`#9932CC`},{symbol:`P`,colour:`#FF69B4`},{symbol:`Q`,colour:`#008B8B`},{symbol:`R`,colour:`#8A2BE2`},{symbol:`S`,colour:`#FF4500`},{symbol:`T`,colour:`#00FF00`},{symbol:`U`,colour:`#FF00FF`},{symbol:`V`,colour:`#708090`},{symbol:`W`,colour:`#F4A460`},{symbol:`X`,colour:`#000080`},{symbol:`Y`,colour:`#D2691E`},{symbol:`Z`,colour:`#8FBC8F`}];function hr(e){e.mapview||console.warn(`A mapview is required in the locations listview params argument.`),e.target||console.warn(`A target element is required in the locations listview params argument.`);let t=e.mapview.locale;t.locations??={},t.locations.pinStyle??={anchor:[.5,1],scale:3,type:`markerLetter`},t.locations.style??={fillColor:`#fff`,fillOpacity:.1,strokeColor:`#fff`},t.locations.records??=t.listview_records||structuredClone(mr),t.locations.clearAll=e.target.appendChild(mapp.utils.html.node`
    <button
    ...
    `);async function i(){e.infoj.some(e=>e.newValue!==void 0)&&!await mapp.ui.elements.confirm({text:mapp.dictionary.location_close_without_save})||e.remove()}e.view=mapp.ui.elements.drawer({data_id:`location-${e.layer.key}-${e.id}`,class:`location-view raised expanded`,header:r}),e.viewEntries=e.view.appendChild(mapp.ui.locations.infoj(e)),e.view.querySelector(`.header`).style.borderBottom=`3px solid ${e.record.colour}`,e.view.addEventListener(`valChange`,yr),e.renderLocationView=br,e.view.addEventListener(`render`,()=>e.renderLocationView()),e.view.addEventListener(`updateInfo`,()=>{e.editToggle&&(e.editToggle.classList.remove(`toggle-on`),e.removeEdits()),e.layer?.dataviews&&Object.values(e.layer.dataviews).forEach(e=>{e.display===!0&&e.update()}),e.renderLocationView()})}function vr(e){if(e.layer?.toggleLocationViewEdits===!1)return e.removeEdits(),!1;if(!e.layer?.toggleLocationViewEdits||!e.infoj.some(e=>e.edit))return!1;!e.new&&e.removeEdits();let t=`notranslate material-symbols-outlined ${e.new?`toggle-on`:``}`;return e.editToggle=mapp.utils.html.node`<button
    title="Enable edits"
    class=${t}
    onclick=${n}>edit`,!0;async function n(t){await e.confirmUpdate()===void 0&&(t.target.classList.toggle(`toggle-on`)?e.restoreEdits():e.removeEdits(),e.renderLocationView())}}function yr(e){let t=e.detail,n=t.location;if(typeof t.valChangeMethod==`function`){t.valChangeMethod(t);return}if(t.value==t.newValue?(delete t.newValue,t.node?.classList.remove(`val-changed`)):t.node?.classList.add(`val-changed`),n.infoj.some(e=>e.invalid)){n.view.querySelector(`.btn-save`).style.display=`none`;return}n.view.querySelector(`.btn-save`).style.display=n.infoj.some(e=>e.newValue!==void 0)?`inline-block`:`none`}function br(){let e=this;e.view.querySelector(`.btn-save`).style.display=`none`,e.viewEntries.remove(),e.view.classList.remove(`disabled`),e.viewEntries=e.view.appendChild(mapp.ui.locations.infoj(e))}var xr={entries:tr,infoj:nr,listview:hr,view:_r};function Sr(e){if(e.node)return e.tabs=e.node.appendChild(mapp.utils.html.node`<div class="tabs">`),e.panel=e.node.appendChild(mapp.utils.html.node`<div class="panel">`),e.id&&(e.node.dataset.id=e.id),e.addTab=Cr,e.node.addEventListener(`addTab`,t=>e.addTab(t.detail)),e}function Cr(e){if(e.tab&&!e.dynamic)return;let t=this;e.layer?.dataviews?.[e.key]&&(e.disableTabClose??=e.layer.dataviews.hide),e.activate??=a,e.location?e.location.removeCallbacks.push(()=>e.remove()):e.layer&&(e.layer.showCallbacks.push(()=>{e.display&&e.show()}),e.layer.hideCallbacks.push(()=>{e.remove()})),e.label??=e.title||e.key||`Tab`,e.tab_btn=mapp.utils.html.node`<button
    ...
    overflow: hidden !important`;Object.assign(e.dataview_dialog,{header:mapp.utils.html.node`<h1> ${e.label}`,data_id:`${e.key}-dataviews-dialog`,target:document.getElementById(`Map`),height:`auto`,left:`5%`,top:`0.5em`,class:`box-shadow tabview`,css_style:t,containedCentre:!0,contained:!0,headerDrag:!0,closeBtn:!0,onClose:()=>{e.chkbox&&(e.chkbox.querySelector(`input`).checked=!1)}}),mapp.ui.elements.dialog(e.dataview_dialog);let n=e.dataview_dialog.node.querySelector(`.content`);n.classList.add(`panel`),e.target=n,e.dataview_dialog.dialog.close(),e.show=()=>{e.update(),e.toolbar&&e.dataview_dialog.node.querySelector(`.btn-row`).style.removeProperty(`display`),e.chart&&(e.dataview_dialog.node.appendChild(e.target.querySelector(`canvas`)),e.dataview_dialog.node.removeChild(e.target)),e.dataview_dialog.dialog.show()},e.hide=()=>{e.dataview_dialog.dialog.close()},e.location?.removeCallbacks?.push?.(()=>e.dataview_dialog.dialog.close())}var Pr={create:Fr};async function Fr(e){e.query??=`histogram`,e.options??={};let t=`column-gap: ${e.options.columnGap||10}px`;e.node=mapp.utils.html.node`<div class="histogram" style=${t}>`,e.options.width&&(e.node.style.width=`${e.options.width}px`),e.options.height&&(e.node.style.height=`${e.options.height}px`),e.options.title&&e.target.append(mapp.utils.html.node`<h3 class="histogram-title">${e.options.title}`),e.setData=Ir,e.target.append(e.node),e.options.xlabel&&(e.xlabel=mapp.utils.html.node`<div class="histogram-xlabel">${e.options.xlabel}`,e.options.width&&(e.xlabel.style.width=`${e.options.width}px`),e.target.append(e.xlabel)),e.options.caption&&(e.caption=mapp.utils.html.node`<span class="histogram-caption">${e.options.caption}`,e.target.append(e.caption))}function Ir(e){this.data=e,Lr(this)}function Lr(e){if(!Array.isArray(e.data)){mapp.utils.render(e.node,mapp.utils.html`<div>`);return}let t=Math.max(...e.data.map(e=>e.count));e.options.height??=100;let n=e.data.map(n=>{let r=`height: ${Math.round(e.options.height*n.count/t)}px`,i=``;return e.options.tooltip&&(i=mapp.utils.html`<div class="bucket-info">
      ...
      style=${r}>
      ${i}`}),r=e.options.ylabel?mapp.utils.html`<div class="ylabel">${e.options.ylabel}`:``;mapp.utils.render(e.node,mapp.utils.html`${r}${n}`)}var X={idle:600};function Rr(e){Object.assign(X,e),X.idle!==0&&(window.onload=Z,window.onmousemove=Z,window.onmousedown=Z,window.ontouchstart=Z,window.onclick=Z,window.onkeypress=Z,Z(),Br())}function Z(){X.locked||(X.timeout&&clearTimeout(X.timeout),X.timeout=setTimeout(zr,X.idle*1e3))}function zr(){X.locked=!0,X.renew&&clearTimeout(X.renew);let e={url:`${X.host}/api/user/cookie?destroy=true`};e.onLoad=e=>location.reload(),mapp.utils.xhr(e)}function Br(){X.renew=setTimeout(Vr,(X.idle-20)*1e3)}function Vr(){let e={url:`${X.host}/api/user/cookie`};e.onLoad=e=>{if(e.target.status===401)return zr();Br()},mapp.utils.xhr(e)}function Hr(e){let t=mapp.utils.html.node`
  <div class="interface-mask">
    ...
    <button 
      ...
      onclick=${e=>e.target.closest(`.interface-mask`).remove()}>close</button>`;document.body.append(t)}var Ur={create:Wr,toolbar:{copyToClipboard:qr,csvupload:Jr,jsonfile:Kr,sqlinsert:Yr}};async function Wr(e){e.props??={mainMenuBar:!1,mode:`text`,readOnly:!0},e.jsoneditor=await mapp.ui.elements.jsoneditor(e),e.setData=Gr,e.data&&e.setData(e.data)}function Gr(e){this.data=e;let t={json:e};this.jsoneditor.set(t)}function Kr(e){return e.title??=e.label||`Unknown`,mapp.utils.html.node`<button onclick=${()=>{let t={filename:`${e.title}.json`,text:JSON.stringify(e.data,null,2),type:`application/json`};mapp.utils.textFile(t)}}>Download</button>`}function qr(e){return mapp.utils.html.node`<button onclick=${()=>{let t=JSON.stringify(e.data,null,2);mapp.utils.copyToClipboard(t)}}>Copy to Clipboard</button>`}function Jr(e){return e.toolbar.csvupload.label??=`CSV Upload`,e.toolbar.csvupload.input=mapp.utils.html.node`<input 
...
`)}`});return}e.setData(n)}}>`,e.toolbar.csvupload.input}function Yr(e){return e.props={mainMenuBar:!1,mode:`text`},mapp.utils.html.node`<button onclick=${()=>{let t=e.jsoneditor.get();if(t.text===``)return;let n=mapp.utils.paramString(e.toolbar.sqlinsert),r=`${mapp.host}/api/query?${n}`;return mapp.utils.xhr({body:t.text,method:`POST`,url:r})}}>SQL Insert</button>`}async function Xr(e,t,n){return await mapp.ui.elements.jsoneditor({props:{onRenderMenu:e=>Zr(e,n),content:t,mode:`text`},target:e})}function Zr(e,t){return e.push(t),e.filter(e=>e.text!==`text`).filter(e=>e.text!==`tree`).filter(e=>e.text!==`table`).filter(e=>e.type!==`separator`).filter(e=>e.className!==`jse-undo`).filter(e=>e.className!==`jse-redo`).filter(e=>e.className!==`jse-search`).filter(e=>e.className!==`jse-contextmenu`).filter(e=>e.className!==`jse-sort`).filter(e=>e.className!==`jse-transform`)}async function Qr(e){if(Array.isArray(e.Features)&&!e.filter?.viewport)return e.Features.length;let t={layer:e,queryparams:{}};t.viewport=e.filter?.viewport||e.queryparams?.viewport;let n=mapp.utils.queryParams(t),r=mapp.utils.paramString({...n,filter:e.filter?.current,layer:e.key,table:e.tableCurrent(),geom:e.geomCurrent(),qID:e.qID,template:`location_count`}),i=await mapp.utils.xhr(`${e.mapview.host}/api/query?${r}`);if(i instanceof Error)if(e.filter?.viewport){console.warn(`locationCount failed on viewport.`);return}else return e.filter.viewport=!0,await Qr(e);return i}var Q={},$={X:{min:75,lockSize:200,grid:`gridTemplateColumns`},Y:{min:0,lockSize:50,grid:`gridTemplateRows`},gridTemplate:{X:`auto`,Z:`10px`,Y:`auto`}};function $r(e){e.resizeEvent??=ei.bind(e),e.stopResize=ni.bind(e),e.axis??=`X`,e.doubleClickClose&&(e.target.ondblclick=t=>ti(t,e)),e.target.addEventListener(`mousedown`,t=>{t.preventDefault(),document.body.style.cursor=`grabbing`,globalThis.addEventListener(`mousemove`,e.resizeEvent),globalThis.addEventListener(`mouseup`,e.stopResize)}),e.target.addEventListener(`touchstart`,t=>{t.preventDefault(),globalThis.addEventListener(`touchmove`,e.resizeEvent),globalThis.addEventListener(`touchend`,e.stopResize)},{passive:!0}),Q.gridTemplateColumns=getComputedStyle(document.body).gridTemplateColumns,Q.gridTemplateRows=getComputedStyle(document.body).gridTemplateRows}function ei(e){let t=this.axis;$.X.size=e.touches?.[0]?.pageX||e.pageX,$.Y.size=globalThis.innerHeight-(e.touches?.[0]?.pageY||e.pageY);let n=$[t];if(n.size<n.min){$.gridTemplate[t]=`0px`,document.body.style[n.grid]=Object.values($.gridTemplate).join(` `);return}n.size<n.lockSize||(t===`X`&&$.X.size>globalThis.innerWidth/2&&($.X.size=globalThis.innerWidth/2),t===`Y`&&($.Y.size>globalThis.innerHeight-10&&($.Y.size=globalThis.innerHeight),OL.style.marginTop=`-${$.Y.size/2}px`),$.gridTemplate[t]=`${n.size}px`,document.body.style[n.grid]=Object.values($.gridTemplate).join(` `),$.gridTemplate={X:`auto`,Z:`10px`,Y:`auto`})}function ti(e,t){if($.X.size=e.touches?.[0]?.pageX||e.pageX,$.Y.size=globalThis.innerHeight-(e.touches?.[0]?.pageY||e.pageY),$[t.axis].size<$[t.axis].min){document.body.style[$[t.axis].grid]=Q[$[t.axis].grid];return}$.gridTemplate[t.axis]=`0px`,document.body.style[$[t.axis].grid]=Object.values($.gridTemplate).join(` `),$.gridTemplate={X:`auto`,Z:`10px`,Y:`auto`}}function ni(){document.body.style.cursor=`auto`,globalThis.removeEventListener(`mousemove`,this.resizeEvent),globalThis.removeEventListener(`touchmove`,this.resizeEvent),globalThis.removeEventListener(`mouseup`,this.stopResize),globalThis.removeEventListener(`touchend`,this.stopResize)}var ri={Dataview:t,elements:dt,Gazetteer:ft,layers:sn,locations:xr,Tabview:Sr,utils:{cssColour:wr,dataview:Dr,dataviewDialog:Nr,histogram:Pr,idleLogout:Rr,imagePreview:Hr,Json:Ur,layerJSE:Xr,locationCount:Qr,resizeHandler:$r}};globalThis.ui=ri,globalThis.mapp??={},mapp.ui=ri;
