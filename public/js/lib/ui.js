mapp.utils.merge(mapp.dictionaries,{en:{layer_zoom_to_extent:"Zoom to filtered layer extent",layer_visibility:"Toggle visibility"},de:{layer_zoom_to_extent:"Zoom zum Ausma\xDF des gefilterten Datensatzes",layer_visibility:"Umschalten der Ansicht"},cn:{layer_zoom_to_extent:"\u7F29\u653E\u81F3\u76F8\u5E94\u7B5B\u9009\u8303\u56F4",layer_visibility:"\u5207\u6362\u53EF\u89C1\u6027"},pl:{layer_zoom_to_extent:"Poka\u017C zasi\u0119g warstwy",layer_visibility:"Widoczno\u015B\u0107"},ko:{layer_zoom_to_extent:"\uD544\uD130\uB41C \uB808\uC774\uC5B4\uD06C\uAE30\uC5D0 \uC90C(zoom)",layer_visibility:"\uD1A0\uAE00 \uAC00\uC2DC\uC131"},fr:{layer_zoom_to_extent:"Zoom sur l'\xE9tendue de la couche",layer_visibility:"Changer la visiblit\xE9"},ja:{layer_zoom_to_extent:"\u30D5\u30A3\u30EB\u30BF\u30FC\u3055\u308C\u305F\u30EC\u30A4\u30E4\u30FC\u7BC4\u56F2\u3092\u30BA\u30FC\u30E0\u306B",layer_visibility:"\u8868\u793A\u5207\u66FF"}});var y=e=>{let t=mapp.utils.html`
  ...
  <div class="dropdown active">
    ...
    <div class="legend">
      ...
      <div class="mask-icon expander"></div>`,content:t}),e.style.drawer};var G=e=>{let t=e.style.theme,i=[];if(e.filter&&i.push(mapp.utils.html`
    <div
      class="switch-all"
    ...
    <div 
      ...
      <div 
        data-id="cluster"
        class="contents">
        ${a}${l}`)}return e.style.legend=mapp.utils.html.node`<div class="grid">${i}`,e.style.legend};var N=e=>{let t=e.style.theme,i=[];return t.cat_arr.forEach(a=>{let l=Object.assign({},e.style.default,a.style&&a.style.icon||a.style||a),o=mapp.utils.html`
      <div
        style="height: 24px; width: 24px; grid-column: 1;">
        ${mapp.ui.elements.legendIcon(Object.assign({width:24,height:24},l))}`,n=mapp.utils.html`
  ...
  <div
    ...
    <div
      class="images-grid">${t}`;return a;async function l(n){e.location.view?.classList.add("disabled");let r=new FileReader,s=n.target.files[0];!s||(r.onload=c=>{let d=new Image;d.onload=async()=>{let h=mapp.utils.html.node`<canvas>`,p=1024,u=d.width,g=d.height;u>g&&u>p?(g*=p/u,u=p):g>p&&(u*=p/g,g=p),h.width=u,h.height=g,h.getContext("2d").drawImage(d,0,0,u,g);let v=h.toDataURL("image/jpeg",.5),f=await mapp.utils.xhr({method:"POST",requestHeader:{"Content-Type":"application/octet-stream"},url:`${e.location.layer.mapview.host}/api/provider/cloudinary?${mapp.utils.paramString({public_id:s.name.replace(/.*\//,"").replace(/\.([\w-]{3})/,""),resource_type:"image"})}`,body:mapp.utils.dataURLtoBlob(v)});await mapp.utils.xhr(`${e.location.layer.mapview.host}/api/query?`+mapp.utils.paramString({template:"set_field_array",locale:e.location.layer.mapview.locale.key,layer:e.location.layer.key,table:e.location.table,qID:e.location.layer.qID,id:e.location.id,action:"append",field:e.field,value:f.secure_url}));let Ae=mapp.utils.html.node`
          <div>
            <img
              src=${f.secure_url}
              onclick=${mapp.ui.utils.imagePreview}>
  ...
  <div
    ...
    <div
      class="val"
      style="${`${e.css_val||""}`}">
      ${t}`};var ce={key:Qe,boolean:K,dataview:X,date:k,datetime:k,defaults:Q,documents:Y,geometry:ee,html:E,images:te,integer:S,isoline_here:L,isoline_mapbox:L,json:ae,mvt_clone:ie,numeric:S,pin:le,report:oe,streetview:ne,tab:re,text:se,textarea:E,time:de};function Qe(e){return mapp.utils.html.node`
  <div class="layer-key">
    <span>
      ${e.location.layer.name}`}var pe={view:U,listview:J,infoj:Z,entries:ce};var ue=async e=>{if(e.target=typeof e.target=="string"&&document.getElementById(e.target)||e.target,e.target=e.target instanceof HTMLElement&&e.target||mapp.utils.html.node`<div class="dataview-target">`,e.update=async()=>{if(!e.query)return;let t=mapp.utils.queryParams(e),i=mapp.utils.paramString(t),a=await mapp.utils.xhr(`${e.host||e.location.layer.mapview.host}/api/query?${i}`);if(!(a instanceof Error)){if(typeof e.responseFunction=="function")return e.responseFunction(a);typeof e.setData=="function"&&e.setData(a)}},e.toolbar){e.target=mapp.utils.html.node`<div class="dataview-target">`;let t=Object.keys(e.toolbar).map(i=>mapp.ui.elements.toolbar_el[i](e));e.panel=mapp.utils.html.node`
      <div class="flex-col">
  ...
  <input
    ...
    <input
      ...
      type="number"
      placeholder="Max"
      oninput=${n=>{let r=t.getColumn().getField();n.target.value.length?e.layer.filter.current[r]=Object.assign(e.layer.filter.current[r]||{},{lte:Number(n.target.value)}):delete e.layer.filter.current[r],e.layer.reload(),e.update()}}></input>`}function lt(e){return(t,i,a,l,o)=>{console.log(e.Tabulator);let n=mapp.utils.html`
    <input 
      type="number" 
      placeholder="Min" 
      onchange=${r}
      onblur=${s}>`;function r(p){console.log(p);let u=t.getColumn().getField();console.log(p.target.value);let v=e.Tabulator.getFilters().find(f=>f.field===u&&f.type==">=");typeof v>"u"||v===null?(console.log("Filter does not exist yet"),e.Tabulator.addFilter(u,">=",Number(p.target.value))):(console.log("Filter already exists, so update"),e.Tabulator.removeFilter(...Object.values(v)),e.Tabulator.addFilter(u,">=",Number(p.target.value)))}function s(p){console.log(p)}let c=mapp.utils.html`
    <input 
      type="number" 
      placeholder="Max" 
      onchange=${d}
      onblur=${h}>`;function d(p){console.log(p);let u=t.getColumn().getField();console.log(p.target.value);let v=e.Tabulator.getFilters().find(f=>f.field===u&&f.type=="<=");typeof v>"u"||v===null?(console.log("Filter does not exist yet"),e.Tabulator.addFilter(u,"<=",Number(p.target.value))):(console.log("Filter already exists, so update"),e.Tabulator.removeFilter(...Object.values(v)),e.Tabulator.addFilter(u,">=",Number(p.target.value)))}function h(p){console.log(p)}return mapp.utils.html.node`
    <div><div style="display: flex;">${n}${c}`}}var me=e=>{!e.node||(e.tabs=e.node.appendChild(mapp.utils.html.node`<div class="tabs">`),e.panel=e.node.appendChild(mapp.utils.html.node`<div class="panel">`),e.id&&e.node.setAttribute("data-id",e.id),e.addTab=ot,e.node.addEventListener("addTab",t=>e.addTab(t.detail)))};function ot(e){let t=this;e.tab=mapp.utils.html.node`
    <div class="tab">
      <div
        class="header"
        style="${e.tab_style||""}"
  ...
  <label 
    ...
    <div></div>
    <span>${e.label}`;var fe={modify:nt,draw:rt};mapp.utils.merge(mapp.dictionaries,{en:{remove_last_vertex:"Remove last vertex"},de:{remove_last_vertex:"Remove last vertex"},cn:{remove_last_vertex:"\u5220\u9664\u6700\u540E\u4E00\u4E2A\u9876\u70B9"},pl:{remove_last_vertex:"Usu\u0144 ostatni wierzcho\u0142ek"},ko:{remove_last_vertex:"\uB9C8\uC9C0\uB9C9 \uC815\uC810(\uAF2D\uC9C0\uC810) \uC81C\uAC70"},fr:{remove_last_vertex:"Effacer le dernier point"},ja:{remove_last_vertex:"\u6700\u5F8C\u306E\u30D0\u30FC\u30C6\u30C3\u30AF\u30B9\u3092\u524A\u9664"}});function nt(e){e&&e.preventDefault();let t=[];t.push(mapp.utils.html`
    <li
  ...
  <button 
    ...
    <div 
      ...
      <div style="display: grid; grid-row-gap: 5px;">
        ${t}
        ${i}`})};var xe=new XMLSerializer,ke=e=>{if(e.svg||e.type||e.layers){let t=e.layers&&Array.isArray(e.layers)&&e.layers.map(a=>`url(${a.svg})`).reverse().join(",")||`url(${e.svg||e.url||mapp.utils.svgSymbols[e.type](e)})`,i=`
      background-position: center;
      background-repeat: no-repeat;
      background-size: contain;
    ...
    <button
      class="flat"
      onclick=${()=>{e.Tabulator.download("csv",`${e.title||"table"}.csv`)}}>Download as CSV`}var Ce={card:he,chkbox:ge,contextMenu:fe,drawer:ve,drawing:we,dropdown:be,dropdown_multi:_e,isoline_params_here:ye,isoline_params_mapbox:$e,legendIcon:ke,slider:Le,slider_ab:Se,toolbar_el:Ee};var C,b=null;async function je(e,t){return b?new b(e,t):(C||(C=new Promise(async i=>{if(window.Chart){b=window.Chart,i();return}Promise.all([import("https://cdn.skypack.dev/chart.js@3.7.1"),import("https://cdn.skypack.dev/chartjs-plugin-datalabels@2.0.0"),import("https://cdn.skypack.dev/chartjs-plugin-annotation@1.4.0")]).then(a=>{a[0].Chart.register(...a[0].registerables),a[0].Chart.register(a[1].default),a[0].Chart.register(a[2].default),b=a[0].Chart,i()}).catch(a=>{console.error(a.message),alert("Failed to load Chart.js library. Please reload the browser.")})})),await C,new b(e,t))}var j,_=null;async function ze(){return _?new _(...arguments):(j||(j=new Promise(async e=>{if(window.Tabulator){_=window.Tabulator,e();return}Promise.all([import("https://cdn.skypack.dev/pin/tabulator-tables@v5.3.0-3P9dSwNRL29tgc0Vmk59/mode=imports,min/optimized/tabulator-tables.js")]).then(t=>{_=t[0].TabulatorFull,e()}).catch(t=>{console.error(t.message),alert("Failed to load Chart.js library. Please reload the browser.")})})),await j,new _(...arguments))}var m={idle:600},De=e=>{Object.assign(m,e),m.idle!==0&&(window.onload=w,window.onmousemove=w,window.onmousedown=w,window.ontouchstart=w,window.onclick=w,window.onkeypress=w,w(),Te())};function w(){m.locked||(m.timeout&&clearTimeout(m.timeout),m.timeout=setTimeout(Oe,m.idle*1e3))}function Oe(){m.locked=!0,m.renew&&clearTimeout(m.renew);let e=new XMLHttpRequest;e.open("GET",`${m.host}/api/user/cookie?destroy=true`),e.onload=t=>location.reload(),e.send()}function Te(){m.renew=setTimeout(e,(m.idle-20)*1e3);function e(){let t=new XMLHttpRequest;t.open("GET",`${m.host}/api/user/cookie?renew=true`),t.onload=i=>{if(i.target.status===401)return Oe();Te()},t.send()}}var qe=e=>{document.body.append(mapp.utils.html.node`
    <div class="interface-mask">
      <div class="bg-image" style=${`background-image:url(${e.target.src})`}>
      <button class="btn-close mask-icon close"
        onclick=${t=>t.target.parentElement.parentElement.remove()}>`)};var Me=e=>{e.target.addEventListener("mousedown",i=>{i.preventDefault(),document.body.style.cursor="grabbing",window.addEventListener("mousemove",e.resizeEvent),window.addEventListener("mouseup",t)}),e.target.addEventListener("touchstart",i=>{i.preventDefault(),window.addEventListener("touchmove",e.resizeEvent),window.addEventListener("touchend",t)},{passive:!0});function t(){document.body.style.cursor="auto",window.removeEventListener("mousemove",e.resizeEvent),window.removeEventListener("touchmove",e.resizeEvent),window.removeEventListener("mouseup",t),window.removeEventListener("touchend",t)}};var Fe={Chart:je,Tabulator:ze,idleLogout:De,imagePreview:qe,resizeHandler:Me};var Ie=e=>{let t=Object.assign({input:e.target.querySelector("input"),result:e.target.querySelector("ul"),sources:{glx:a,mapbox:o,google:l}},e);t.input.placeholder=t.placeholder||"",t.input.oninput=r=>{t.xhr&&t.xhr.abort(),t.result.innerHTML="",r.target.value.length>0&&i(r.target.value)},t.input.addEventListener("keydown",r=>{(r.keyCode||r.charCode)===13&&t.result.querySelector("li").click()});function i(r){let s=r.split(",").map(parseFloat);if(s.length===2&&s.every(c=>!isNaN(c))){t.result.appendChild(mapp.utils.html.node`
      <li 
