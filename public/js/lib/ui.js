import{t as e}from"./chunk-DK3Fl9T5.js";async function t(e){if(!(!e.dynamic&&typeof e.create==`function`)){if(typeof e.create==`function`){e.create();return}if(typeof e.target==`string`&&(e.target=document.getElementById(e.target)),!(e.target instanceof HTMLElement)){console.warn(`Dataviews require a HTMLHtmlElement target`);return}return i(e)instanceof Error?e.err:(typeof e.label==`string`&&(e.chkbox=mapp.ui.elements.chkbox({checked:!!e.display,data_id:e.key,disabled:e.disabled,label:e.label,onchange:t=>{e.display=t,e.display?e.show():e.hide()}})),e.update??=a,e.show??=n,e.hide??=r,mapp.ui.utils.dataview.Toolbar(e),mapp.ui.utils.dataview.mapChange(e),e)}}async function n(){this.display=!0,this.create===void 0?(this.create=()=>{mapp.ui.utils[this.dataview].create(this)},this.create(),typeof this.update==`function`&&this.update()):this.dynamic?(typeof this.create==`function`&&await this.create(),typeof this.update==`function`&&this.update()):this.reload&&typeof this.update==`function`&&this.update(),this.btnRow?.style.setProperty(`display`,`flex`),this.target.style.display=`block`}function r(){this.display=!1,this.target.style.display=`none`,this.btnRow?.style.setProperty(`display`,`none`)}function i(e){if(e.key??=e.query||e.title||e.label,e.dataview||(e.chart&&(e.dataview=`chartjs`),e.columns!==void 0&&(console.warn(`Table dataviews should be configured inside a tables object`),e.table={columns:e.columns},e.dataview=`tabulator`),e.table&&(e.dataview=`tabulator`)),!Object.hasOwn(mapp.ui.utils,e.dataview))return e.err=Error(`mapp.ui.utils.${e.dataview} doesnt exist`),console.error(e.err),e.update=()=>console.warn(`Unable to update ${e.key} dataview.`),e.err;if(typeof mapp.ui.utils[e.dataview].create!=`function`)return e.err=Error(`mapp.ui.utils.${e.dataview}.create() method doesn't exist`),console.error(e.err),e.err}async function a(){if(this.create===void 0&&(this.create=function(){mapp.ui.utils[this.dataview].create(this)},this.create()),!this.query)return;let e=mapp.utils.queryParams(this),t=mapp.utils.paramString(e);this.host??=this.layer?.mapview?.host;let n=await mapp.utils.xhr(`${this.host||mapp.host}/api/query?${t}`);if(!(n instanceof Error)){if(typeof this.responseFunction==`function`){this.responseFunction(n);return}typeof this.setData==`function`&&this.setData(n)}}function o(e){typeof e==`string`&&(e={text:e}),e.title??=mapp.dictionary.information,e.icon??=`info`,e.data_id??=`alert`,e.class??=`alert-confirm box-shadow no-select`;let t=e.icon===``?``:mapp.utils.html`<span class="material-symbols-outlined notranslate">${e.icon}</span>`;return e.header=mapp.utils.html`${t}${e.title}`,e.innerContent??=mapp.utils.html`<p>${e.text}</p>`,e.content=mapp.utils.html`
  ...
  ${mapp.ui.elements.dropdown({callback:n,data_id:`labelsDropdown`,entries:t,placeholder:e.style.label.title})}`;function n(t,n){let r=e.style.label.display;e.style.label=e.style.labels[n.option],e.style.label.display=r,e.reload(),_(e)}}function Fe(e){return mapp.ui.elements.slider({callback:t=>{e.L.setOpacity(Number.parseFloat(t/100))},data_id:`opacitySlider`,label:`${mapp.dictionary.layer_style_opacity}`,max:100,min:0,val:Number.parseInt(e.L.getOpacity()*100)})}function Ie(e){if(!e.style.theme)return;e.style.labels&&Object.hasOwn(e.style.labels,e.style.theme?.setLabel)&&(e.style.label=e.style.labels[e.style.theme.setLabel]),e.style.hovers&&Object.hasOwn(e.style.hovers,e.style.theme?.setHover)&&(e.style.hover=e.style.hovers[e.style.theme.setHover]);let t=[];return e.style.theme?.meta&&(e.style.theme.meta_node=mapp.utils.html.node`<p>${e.style.theme.meta}`,t.push(e.style.theme.meta_node)),Object.hasOwn(mapp.ui.layers.legends,e.style.theme?.type)&&(e.showCallbacks.push(()=>{mapp.ui.layers.legends[e.style.theme.type](e)}),e.style.legend=mapp.utils.html.node`<div class="legend">`,e.style.legend&&t.push(e.style.legend),e.display&&mapp.ui.layers.legends[e.style.theme.type](e)),mapp.utils.html.node`<div data-id="layerTheme">${t}`}function Le(e){if(!e.style.themes||Object.keys(e.style.themes).length<2)return;let t=Object.keys(e.style.themes).map(t=>({option:t,title:e.style.themes[t].title||t}));function n(t,n){e.style.theme=e.style.themes[n.option],e.style.theme.setLabel&&e.style.labels&&(e.style.label=e.style.labels[e.style.theme.setLabel]),e.style.theme.setHover&&e.style.hovers&&(e.style.hover=e.style.hovers[e.style.theme.setHover],e.style.hover.method??=mapp.layer.featureHover);let r=mapp.ui.layers.panels.style(e);e.style.panel&&e.style.panel.replaceChildren(...r.children),e.view?.querySelector(`[data-id=style-drawer]`).replaceChildren(...r.children),Object.hasOwn(mapp.ui.layers.legends,e.style.theme?.type)&&mapp.ui.layers.legends[e.style.theme.type](e),e.reload()}return mapp.utils.html`<div>
  ...
  <svg
    ...
    width: ${e.width}px;
    height: ${e.height}px;
    background-image: url(${r});`;return mapp.utils.html`<div style=${i}>`}function Ue(e){e.fillOpacity??=1,e.strokeWidth??=1,e.lineDash??=``;let t=mapp.utils.svg.node`
  <svg
    height=${e.height}
    width=${e.width}>
    <rect
    ...
    <a 
      class="switch"
      href=${e.register}>${e.login_registration_link}</a>`;function o(){a.disabled=!(n.validity.valid&&i.validity.valid),r.textContent=a.disabled?e.login_invalid:``}}function Ke(e){e.placeholder??=``,e.data_id??=`numeric-input`,e.numericChecks??=qe;let t=mapp.utils.formatNumericValue(e),n=`text-align: center; width: ${e.dynamicWidth?t.length+1.3+`ch`:`100%`}`;return mapp.utils.html.node`<input
    data-id=${e.data_id}
    type="text"
    style=${n}
  ...
  <input
    ...
    <div class="searchbox">
      ${e.input}
      ${e.list}`,e.target.append(e.node),e.input.addEventListener(`input`,t=>e.searchFunction(t)),e.input.addEventListener(`focus`,t=>e.searchFunction(t)),e}function nt(e){e.group_id??=e.data_id||`slider`,e.data_id=`a`,e.rangeInput=`rangeInput`,e.step??=1,e.value??=e.val,e.title??=``,e.style??=``;let t=mapp.ui.elements.numericInput(e);return e.sliderElement=mapp.utils.html.node`
    <div
      role="group"
      data-id=${e.group_id}
      title=${e.title}
    ...
    <div
      ...
      <div class="actions">
      ${r}
      </div>`;document.body.append(a);function o(e,n){a.classList.add(`ease-out`),setTimeout(function(){a.remove()},1200),typeof n?.callback==`function`&&n.callback(e),t(e?.target.value)}})}function at(e={}){if(e.data_id??=`ui-elements-tooltip`,!e.content){console.warn(`mapp.ui.elements.tooltip: content for ${e.data_id} not provided.`);return}return e.node=mapp.utils.html.node`<span 
    data-id=${e.data_id}
    title=${e.content}
    class="tooltip material-symbols-outlined notranslate">help</span>`,e.node}function ot(e,t){return e.localeInput=mapp.utils.html.node`<input
  ...
  <div>
    ...
    <div style="display:flex;">${e.localeInput}
      <button style="font-size: 1.5em;"
        onclick=${async()=>await ct(e,t)}>
        <span class="notranslate material-symbols-outlined">save</span>
      </button>
    </div>
    ${e.ulLocales}`,S(e,t),e.panel}async function st(e,t,n){e.panel.classList.add(`disabled`),t.locale.name=n,await mapp.utils.userLocale.remove(t.locale),S(e,t),e.panel.classList.remove(`disabled`)}async function ct(e,t){e.panel.classList.add(`disabled`),t.locale.name=encodeURIComponent(e.localeInput.value),await mapp.utils.userLocale.putLocale(t)===t.locale.name&&(S(e,t),e.panel.classList.remove(`disabled`))}async function S(e,t){let n=await mapp.utils.userLocale.list(t.locale.workspace);if(!n.length){mapp.utils.render(e.ulLocales,mapp.utils.html`<div>`);return}let r=n.map(n=>{let r=mapp.utils.html`<button
    onclick=${async r=>await st(e,t,n.name)}>
    ...
    <ul>${r}`)}var lt={alert:o,btnPanel:s,card:c,chkbox:l,confirm:u,contextMenu:ee,control:re,dialog:oe,drawer:le,drawerDialog:f,drawing:pe,dropdown:xe,helpDialog:Oe,jsoneditor:ke,layerStyle:g,legendIcon:ze,locales:We,loginForm:Ge,numericInput:Ke,pills:Je,radio:Ze,reportLink:$e,registerForm:Qe,searchbox:tt,slider:nt,slider_ab:rt,toast:it,tooltip:at,userLocale:ot},ut=e=>{e={...e,...mapp.ui.elements.searchbox({name:`gazetteer-search-input`,placeholder:e.placeholder,searchFunction:t,target:e.target})};function t(t){if(e.list.innerHTML=``,!t.target.value.length)return;let n=t.target.value.split(`,`).map(parseFloat);if(n.length===2&&n.every(e=>typeof e==`number`&&!Number.isNaN(Number(e))&&Number.isFinite(Number(e)))){let[t,r]=n;if(t>=-90&&t<=90&&r>=-180&&r<=180){e.list.appendChild(mapp.utils.html.node`
        <li onclick=${t=>{mapp.utils.gazetteer.getLocation({label:`Latitude:${n[0]}, Longitude:${n[1]}`,lat:n[0],lng:n[1],source:`Coordinates`},e)}}><span>Latitude:${n[0]}, Longitude:${n[1]}</span>`);return}else e.list.appendChild(mapp.utils.html.node`
          <li style="color: red;">
            <span>${mapp.dictionary.invalid_lat_long_range}</span>`)}e.provider&&(Object.hasOwn(mapp.utils.gazetteer,e.provider)?mapp.utils.gazetteer[e.provider](t.target.value,e):console.warn(`Requested gazetteer service not available`)),mapp.utils.gazetteer.datasets(t.target.value,e)}},C={applyFilter:dt,boolean:ht,date:E,datetime:E,in:xt,integer:vt,like:mt,match:mt,ni:xt,null:gt,numeric:vt,removeFilter:ft,resetFilter:pt,generateMinMax:_t},w;function dt(e){clearTimeout(w),w=setTimeout(()=>{e.reload(),e.style?.theme&&mapp.ui.layers.legends[e.style.theme.type]?.(e),e.filter.list?.forEach(e=>{e.histogram?.update?.()}),e.mapview?.target.dispatchEvent(new Event(`changeEnd`))},500)}function T(e,t){e.filter.current[t.field]&&(Object.hasOwn(e.filter.current[t.field],t.type)&&delete e.filter.current[t.field][t.type],delete e.filter.current[t.field],Number.isNaN(Number(t.Min))&&delete t.min,Number.isNaN(Number(t.Max))&&delete t.max,mapp.ui.layers.filters.applyFilter(e))}function ft(e,t){T(e,t),t.li?.classList.remove(`selected`),t.card?.remove(),t.histogram?.removeChangeEnd?.(),delete t.selected,delete t.card,e.filter.list?.some(e=>e.card)||(e.filter.clearAll instanceof HTMLElement&&e.filter.clearAll.style.setProperty(`display`,`none`),e.filter.resetAll instanceof HTMLElement&&e.filter.resetAll.style.setProperty(`display`,`none`),e.filter.feature_count instanceof HTMLElement&&e.filter.feature_count.style.setProperty(`display`,`none`))}async function pt(e,t){if(T(e,t),!t.card)return;t.content=[await mapp.ui.layers.filters[t.type](e,t)].flat();let n=mapp.ui.elements.card(t);t.card.replaceWith(n),t.card=n}function mt(e,t){return mapp.utils.html.node`
  <input
    type="text"
    onkeyup=${n=>{n.target.value.length?(e.filter.current[t.field]??={},e.filter.current[t.field][t.type]=encodeURIComponent(`${t.leading_wildcard&&`%`||``}${n.target.value}`)):delete e.filter.current[t.field],C.applyFilter(e)}}>`}function ht(e,t){function n(n){e.filter.current[t.field]={boolean:n},C.applyFilter(e)}return n(!1),mapp.ui.elements.chkbox({label:t.label||t.title||`chkbox`,onchange:n})}function gt(e,t){function n(n){e.filter.current[t.field]={null:n},C.applyFilter(e)}return n(!1),mapp.ui.elements.chkbox({label:t.label||t.title||`chkbox`,onchange:n})}async function _t(e,t){let n=mapp.utils.queryParams({layer:e,queryparams:{table:!0,template:`field_minmax`,field:t.field},viewport:e.filter.viewport});n.filter=structuredClone(e.filter.current),delete n.filter[t.field];let r=mapp.utils.paramString(n),i=await mapp.utils.xhr(`${e.mapview.host}/api/query?${r}`),a=Number.isNaN(Number(t.min))?i.minmax[0]:t.min,o=Number.isNaN(Number(t.max))?i.minmax[1]:t.max;e.filter?.viewport&&(a=i.minmax[0],o=i.minmax[1]),t.min=t.type===`integer`?Math.round(a):Number.parseFloat(a),t.max=t.type===`integer`?Math.round(o):Number.parseFloat(o),t.min=t.val_a<t.min?t.val_a:t.min,t.max=t.val_b>t.max?t.val_b:t.max}async function vt(e,t){if(t={...e.infoj.find(e=>e.field===t.field),...t},delete t.val_a,delete t.val_b,t.Min??=Number.isFinite(t.min)?t.min:void 0,t.Max??=Number.isFinite(t.max)?t.max:void 0,(Number.isNaN(Number(t.max))||Number.isNaN(Number(t.min)))&&await _t(e,t),Number.isNaN(Number(t.min))||Number.isNaN(Number(t.max)))return mapp.utils.html.node`<div>${mapp.dictionary.no_data_filter}</div>`;t.step??=t.type===`integer`?1:.01,e.filter.current[t.field]=Object.assign({gte:Number(t.min),lte:Number(t.max)},e.filter.current[t.field]),(t.Min!==void 0||t.Max!==void 0)&&C.applyFilter(e);let n=t.prefix||t.suffix?`(${(t.prefix||t.suffix).trim()})`:``;return t.label_a??=`${mapp.dictionary.layer_filter_greater_than} ${n}`,t.label_b??=`${mapp.dictionary.layer_filter_less_than} ${n}`,t.val_a=e.filter.current?.[t.field]?.gte,t.val_b=e.filter.current?.[t.field]?.lte,t.callback_a=n=>{e.filter.current[t.field].gte=n,t.val_a=Number(n),C.applyFilter(e)},t.callback_b=n=>{e.filter.current[t.field].lte=n,t.val_b=Number(n),C.applyFilter(e)},t.slider=mapp.ui.elements.slider_ab(t),await yt(t,e),mapp.utils.html`
    ${t.histogram?.container}
    ${t.slider}`}async function yt(e,t){if(e.histogram&&(e.histogram===!0&&(e.histogram={}),e.histogram.container??=mapp.utils.html.node`<div>`,e.histogram.dataview??=`histogram`,e.histogram.options??={tooltip:!0},e.histogram.target??=e.histogram.container,e.histogram.layer??=t,e.histogram.queryparams??={field:e.field,table:!0,filter:!0},e.histogram.viewport??=t.filter.viewport,e.histogram.update||await mapp.ui.Dataview(e.histogram),e.histogram.viewport)){t.mapview.Map.getTargetElement().addEventListener(`changeEnd`,n),e.histogram.removeChangeEnd=()=>{t.mapview.Map.getTargetElement().removeEventListener(`changeEnd`,n)};function n(){e.histogram.update()}}}function bt(e){return e.tableCurrent()||Object.values(e.tables??{}).find(e=>!!e)}async function xt(e,t){if(T(e,t),!Array.isArray(t[t.type])){let n=structuredClone(e.filter?.current);delete n?.[t.field];let r=await mapp.utils.xhr(`${e.mapview.host}/api/query?`+mapp.utils.paramString({field:t.field,filter:n,layer:e.key,locale:e.mapview.locale.key,table:bt(e),template:`distinct_values`}));if(!r||r instanceof Error)return console.warn(`Distinct values query did not return any values for field ${t.field}`),mapp.utils.html.node`<div>${mapp.dictionary.no_data_filter}</div>`;t[t.type]=[r].flat().map(e=>e[t.field]).filter(e=>e!=null)}let n=new Set(e.filter?.current[t.field]?.[t.type]||[]);if(t.dropdown||t.dropdown_pills||t.dropdown_search){let r=mapp.ui.elements.dropdown({callback:async(n,r)=>{r.length?Object.assign(e.filter.current,{[t.field]:{[t.type]:r}}):delete e.filter.current[t.field],C.applyFilter(e)},field:t.field,search:t.dropdown_search,entries:t[t.type].map(e=>({option:e,selected:n.has(e),title:e})),inputfilter:!0,keepPlaceholder:t.dropdown_pills,maxHeight:300,multi:!0,pills:t.dropdown_pills,placeholder:t.placeholder||mapp.dictionary.layer_filter_dropdown_select});return mapp.utils.html.node`<div class="filter">${r}`}if(t.searchbox){let n=mapp.utils.html.node`<div class="filter">`,r=mapp.ui.elements.pills({pills:e.filter?.current[t.field]?.[t.type],addCallback:(n,r)=>{Object.assign(e.filter.current,{[t.field]:{[t.type]:[...r]}}),C.applyFilter(e)},removeCallback:(n,r)=>{r.size===0?(i.input.value=null,delete e.filter.current[t.field]):Object.assign(e.filter.current,{[t.field]:{[t.type]:[...r]}}),C.applyFilter(e)},target:n}),i=mapp.ui.elements.searchbox({placeholder:mapp.dictionary.filter_searchbox_placeholder,searchFunction:e=>{if(i.list.innerHTML=``,!e.target.value)return;let n=e.target.value,a=t[t.type].filter(e=>e.toString().toLowerCase().startsWith(n.toLowerCase()));if(!a.length){i.list.append(mapp.utils.html.node`
    ...
    <div style="
      ...
      <label>Date after
        ${n}</label>
      <label>Date before
        ${r}</label>`}function St(e){let t=e.style.theme;t.legend??={},t.legend.grid=[],e.style.theme.style??={},e.style.theme.style.width??=24,e.style.theme.style.height??=24;let n=mapp.ui.elements.legendIcon(e.style.theme.style);return t.legend.grid.push(mapp.utils.html`
    <div 
      class="contents">
      ${n}<div class="label" style="grid-column: 2";>${e.style.theme.label}`),t.legend.node=mapp.utils.html.node`
    <div class="legend">
    <div class="contents-wrapper grid">${t.legend.grid}`,e.style.legend??=t.legend.node,e.style.legend&&e.style.legend.replaceChildren(...t.legend.node.children),t.legend.node}function D(e,t,n){if(e.field??=t.field,e.disabled??=n.filter?.current[e.field]?.ni?.includes(e.value),e.disabled||(e.style??=e._style),n.featureFields&&t.distribution===`count`){let r={value:n.featureFields[e.field]?.[e.value]};if(e.count=mapp.utils.formatNumericValue(r),!e.disabled&&!t.legend?.showEmptyCat&&!e.count)return}let r=mapp.ui.elements.legendIcon({height:24,width:24,...e._style||e.style}),i=mapp.utils.html`<div
    ...
    </button>.`}function Tt(e){let t=e.style.theme;A(t),t.legend.grid=t.categories.map(n=>D(n,t,e)).filter(e=>e!==void 0),e.style.cluster&&t.legend.grid.push(k(e));let n=j();return t.legend.node=mapp.utils.html.node`
    <div class="legend">
      ${n}
    ...
    <div class="legend">
      ${r}
      <div class=${t.legend.classList}>
        ${n}`,e.style.legend??=t.legend.node,e.style.legend&&e.style.legend.replaceChildren(...t.legend.node.children),t.legend.node}var Dt={basic:St,categorized:Tt,distributed:M,graduated:Et};function Ot(e){if(!e.target)return;let t={add:kt,createGroup:At,groups:{},node:e.target};return typeof e.layers==`object`&&Object.values(e.layers).length&&Object.values(e.layers).forEach(e=>t.add(e)),t}function kt(e){if(!e.hidden){if(mapp.ui.layers.view(e),!e.group){this.node.append(e.view),this.node.dispatchEvent(new CustomEvent(`addLayerView`,{detail:e}));return}this.groups[e.group]||this.createGroup(e),this.groups[e.group].addLayer(e),this.node.dispatchEvent(new CustomEvent(`addLayerView`,{detail:e}))}}function At(e){let t={list:[]};this.groups[e.group]=t,t.hideLayers=mapp.utils.html.node`<button
  class="notranslate material-symbols-outlined active"
    ...
    onclick=${e=>{e.target.style.visibility=`hidden`,t.list.filter(e=>e.display).forEach(e=>e.hide())}}>visibility_off`,t.meta=mapp.utils.html.node`<div class="meta no-select">`,t.drawer=mapp.ui.elements.drawer({class:`layer-group ${e.groupClassList||``}`,content:t.meta,data_id:e.group,header:mapp.utils.html`
       <h2>${e.group}</h2>
       ${t.hideLayers}`}),this.node.appendChild(t.drawer),t.chkVisibleLayer=Mt,t.addLayer=jt}function jt(e){if(e.groupmeta){let t=mapp.utils.html.node`<div>`;t.innerHTML=e.groupmeta,mapp.utils.render(this.meta,t)}this.list.push(e),this.drawer.appendChild(e.view),this.chkVisibleLayer(),e.showCallbacks.push(()=>this.chkVisibleLayer()),e.hideCallbacks.push(()=>this.chkVisibleLayer())}function Mt(){this.hideLayers.style.visibility=this.list.some(e=>e.display)?`visible`:`hidden`}function Nt(e){let t=[];for(let[n,r]of Object.entries(e.dataviews))if(!(typeof r==`string`||r===!0||r===!1)){if(Object.assign(r,{host:e.mapview.host,key:n,layer:e}),r.target??=r._target,r._target??=r.target,Pt(r),r.tabview&&mapp.utils.mobile()||(r.label??=r.title||r.key,mapp.ui.utils.dataviewDialog(r),r.target=r.target instanceof HTMLElement?r.target:mapp.utils.html.node`
    ...
    data-id=resetall
    class="flat underline"
    onclick=${t=>{e.filter.list.forEach(t=>mapp.ui.layers.filters.resetFilter(e,t))}}>${mapp.dictionary.filter_reset_all}`,e.filter.count=mapp.utils.html.node`<span class="bold">`,e.changeEndCallbacks.push(N),e.showCallbacks.push(N),e.hideCallbacks.push(e=>{e.filter.clearAll?.checkVisibility()&&e.filter.feature_count.style.setProperty(`display`,`none`)}),e.filter.count_meta??=mapp.dictionary.filter_count,e.filter.feature_count=mapp.utils.html.node`
    <p style="display:none">${e.filter.count} ${e.filter.count_meta}`,e.filter.viewport_description??=e.filter.viewport?mapp.dictionary.filter_in_viewport:mapp.dictionary.filter_not_in_viewport,e.filter.viewport_description=mapp.utils.html.node`<p style="display:none"><i>${e.filter.viewport_description}</i>`,e.filter.content=[e.filter.dropdown,e.filter.clearAll,e.filter.resetAll,e.filter.feature_count,e.filter.viewport_description],e.filter.dialog&&(e.filter.filter_list=mapp.utils.html.node`
      <div id=filter-list class=filter-list>`,e.filter.content=[mapp.utils.html`<p class=bold>${e.name}</p>`,e.filter.feature_count,e.filter.viewport_description,e.filter.clearAll,e.filter.resetAll,e.filter.dropdown,e.filter.filter_list],e.filter.content=mapp.utils.html.node`${e.filter.content}`,e.filter.dialog&&(e.filter.dialog={btn_label:`Filtering`,title:`Filtering`,icon:`filter_alt`,view:e.filter.filter_list,...e.filter.dialog}),e.filter.clearAll.style.display=`inline-block`,e.filter.resetAll.style.display=`inline-block`),e.filter.drawer={drawer:e.filter.drawer,data_id:`filter-drawer`,class:e.filter.classList||``,header:mapp.utils.html`
        <h3>${mapp.dictionary.filter_header}</h3>`,content:e.filter.dialog?.btn||e.filter.content,popout:e.filter.popout,dialog:e.filter.dialog,view:e.view,layer:e},e.filter.view=mapp.ui.elements.drawer(e.filter.drawer),e.filter.view}var Lt={numeric:`numeric`,integer:`integer`,text:`like`,date:`date`,datetime:`datetime`,boolean:`boolean`};function Rt(e){let t=[];e.filter.include??=[],e.filter.exclude??=[];for(let n of e.infoj){if(n.type??=`text`,zt(e,n))continue;let r=n.filter===!0||n.filter===void 0||!Object.keys(n.filter).length;if(r&&(n.filter=Lt[n.type]),typeof n.filter==`string`&&(n.filter={type:n.filter}),!n.filter?.type||!Object.hasOwn(mapp.ui.layers.filters,n.filter.type)){!r&&console.warn(`${n.field}: ${n.filter?.type} is not a valid filter type`);continue}n.filter.title??=n.title,n.filter.field??=n.field,t.push(structuredClone(n.filter))}return t}function zt(e,t){return t.skipEntry===!0||t.field===void 0||t.filter===!1||e.filter.exclude.includes(t.field)?!0:t.filter===void 0?!e.filter.includeAll&&!e.filter.include.includes(t.field):!1}function N(e){e.display&&(clearTimeout(e.filter.debounce),e.filter.debounce=setTimeout(async()=>{let t=JSON.stringify(e.filter.current);if(Bt(e,t)){e.filter.feature_count.style.setProperty(`display`,`block`);return}for(let t of e.filter.list){if(!Object.hasOwn(e.filter.current,t.field)||t.min===void 0||t.max===void 0||t.Min&&t.Max||(await mapp.ui.layers.filters.generateMinMax(e,t),t.max-t.min===0))continue;let n=mapp.ui.elements.slider_ab(t);t.slider.replaceWith(n),t.slider=n}mapp.ui.utils.locationCount(e).then(n=>{if(n===void 0)return;e.filter.countedFilter=t;let r=mapp.utils.formatNumericValue({value:n});e.filter.count.innerText=r,e.filter.feature_count.style.setProperty(`display`,`block`)})},1e3))}function Bt(e,t){return e.filter.viewport?!1:t===e.filter.countedFilter}function Vt(e){e.multi_filter&&(mapp.layer.multi_filter||(delete e.filter.hidden,e.filter.dialog=!0,e.filter.drawer=!1,e.filter.includeAll=!e.filter.include?.length))}var Ht=e=>{let t={layer:e.key,mapview:e.mapview,target:mapp.utils.html.node`<div>`,...e.gazetteer};return mapp.ui.Gazetteer(t),t.target};function Ut(e){e.jsonEditor===!0&&(e.jsonEditor={});let t=mapp.utils.html.node`<button class="wide flat" data-id="jsonEditor">JSON Editor</button>`;return t.addEventListener(`click`,async()=>{t.classList.toggle(`active`);let n=mapp.utils.jsonParser(e),r=mapp.utils.html.node`<div>`,i=await mapp.ui.utils.layerJSE(r,{json:n},{className:`text-button`,onClick:()=>Wt(i,e),text:`Update Layer`,title:`Update Layer`,type:`button`});e.jsonEditor.dialog=mapp.ui.elements.dialog({header:mapp.utils.html.node`<div>`,css_style:`width: 500px; height 300px;`,containedCentre:!0,contained:!0,closeBtn:!0,onClose:()=>{t.classList.toggle(`active`)},content:r})}),e.removeCallbacks.push(e=>{e.jsonEditor?.dialog?.close()}),t}async function Wt(e,t){let n=e.get(),r=JSON.parse(n.text);await t.update(r)}function Gt(e){let t=mapp.utils.html.node`<p data-id="meta" class="meta no-select">`;return t.innerHTML=e.meta,t}function Kt(e){let t=[];for(let[n,r]of Object.entries(e.reports)){if(typeof r==`boolean`)continue;r.key=n,r.label??=n,r.layer=e,r.template&&(r.report={template:r.template});let i=mapp.ui.elements.reportLink(r);t.push(i)}return e.reports.dialog&&(e.reports.dialog={btn_label:`Reports`,title:`Reports`,icon:`summarize`,...e.reports.dialog}),mapp.ui.elements.drawer({drawer:e.reports.drawer,class:e.reports.classList||``,data_id:`reports-drawer`,header:mapp.utils.html`
      <h3>Reports</h3>`,content:mapp.utils.html`${t}`,popout:e.reports.popout,dialog:e.reports.dialog,layer:e})}function qt(e){if(e.style.hidden)return;e.style.elements??=[`labels`,`label`,`hovers`,`hover`,`themes`,`theme`,`icon_scaling`,`opacitySlider`];let t=mapp.ui.elements.layerStyle.panel(e);if(!t)return;e.style.classList??=``;let n=mapp.utils.html`
    <h3>${mapp.dictionary.layer_style_header}</h3>`;return e.style.dialog&&(e.style.dialog={btn_label:`Styling`,title:`Styling`,icon:`brush`,...e.style.dialog}),e.style.drawer=mapp.ui.elements.drawer({drawer:e.style.drawer,data_id:`style-drawer`,class:e.style.classList,header:n,content:t,popout:e.style.popout,dialog:e.style.dialog,layer:e}),e.style.drawer}var Jt={dataviews:Nt,draw:Ft,filter:It,gazetteer:Ht,jsonEditor:Ut,meta:Gt,reports:Kt,style:qt};function Yt(e){if(e.view===null)return;Xt(e);let t=mapp.utils.html`
    <h2>${e.name}</h2>
    ${e.viewConfig.headerBtn}`;e.view=mapp.ui.elements.drawer({drawer:e.viewConfig.drawer,data_id:e.key,class:e.viewConfig.classList,header:t,content:e.viewConfig.content,popout:e.viewConfig.popoutBtn}),e.changeEndCallbacks.push(Zt)}function Xt(e){e.viewConfig={displayToggle:!0,zoomBtn:!0,zoomToFilteredExtentBtn:!0,hideDisabled:!1,...e.viewConfig},e.viewConfig.drawer??=e.drawer,e.zoomBtn===!1&&(delete e.viewConfig.zoomBtn,delete e.zoomBtn),e.viewConfig.popoutBtn??=e.popout,e.tables||(delete e.viewConfig.zoomBtn,delete e.viewConfig.hideDisabled),e.viewConfig.panelOrder??=e.panelOrder,delete e.panelOrder,e.viewConfig.keyOrder??=[`meta`,`style`,`filter`,`dataviews`,`draw`],e.viewConfig.content=Object.keys(e).sort((t,n)=>{let r=e.viewConfig.keyOrder.indexOf(t),i=e.viewConfig.keyOrder.indexOf(n);return r===-1&&(r=1/0),i===-1&&(i=1/0),r-i}).map(t=>mapp.ui.layers?.panels?.[t]?.(e)).filter(e=>e!==void 0),Array.isArray(e.viewConfig.panelOrder)&&(console.warn(`Layer [${e.key}] has the legacy panelOrder config with inverted logic; please use viewConfig.keyOrder array!`),e.viewConfig.content.sort((t,n)=>e.viewConfig.panelOrder.indexOf(t.dataset?.id)<e.viewConfig.panelOrder.indexOf(n.dataset?.id)?1:-1)),e.viewConfig.classList??=e.classList||``,e.viewConfig.classList+=` layer-view `,e.viewConfig.classList+=e.viewConfig.content.length?``:` empty `,Array.isArray(e.viewConfig.headerOrder)||(e.viewConfig.headerOrder=[`zoomToFilteredExtentBtn`,`zoomBtn`,`popoutBtn`,`displayToggle`].filter(t=>e.viewConfig[t])),e.viewConfig.headerBtn??=[];for(let t of e.viewConfig.headerOrder){let n=mapp.ui.layers.viewHeader[t]?.(e);e.viewConfig.headerBtn.push(n)}}function Zt(e){if(!e.tableCurrent)return;let t=Array.from(e.view.querySelectorAll(`:scope > :not(.header)`));e.tableCurrent()?(e.viewConfig.hideDisabled&&e.view.classList.remove(`display-none`),e.zoomBtn instanceof HTMLElement&&e.zoomBtn.style.setProperty(`display`,`none`),e.view.querySelector(`[data-id="display-toggle"]`)?.classList.remove(`disabled`),t.forEach(t=>{!e.display&&t.classList.contains(`disable-on-hide`)||(t.disabled=!1,t.classList.remove(`disabled`))})):(e.table||e.tables)&&(e.zoomBtn instanceof HTMLElement&&e.zoomBtn.style.removeProperty(`display`),e.view.classList.remove(`expanded`),[...e.view.querySelectorAll(`.expanded`)].forEach(e=>e.classList.remove(`expanded`)),e.view.querySelector(`[data-id="display-toggle"]`)?.classList.add(`disabled`),t.forEach(e=>{e.disabled=!0,e.classList.add(`disabled`)}),e.viewConfig.hideDisabled&&e.view.classList.add(`display-none`))}var Qt={zoomToFilteredExtentBtn:$t,zoomBtn:en,popoutBtn:nn,displayToggle:rn};function $t(e){let t=Object.keys(e.filter.current).length?``:`display: none;`,n=mapp.utils.html.node`<button
    data-id=zoomToFilteredExtentBtn
    title=${mapp.dictionary.layer_zoom_to_extent}
    style=${t}
    class="notranslate material-symbols-outlined"
    onclick=${async t=>{t.target.disabled=!await e.zoomToExtent(),e.show()}}>filter_alt`;function r(e){Object.keys(e.filter.current).length?n.style.display=`inline-block`:n.style.display=`none`}return e.changeEndCallbacks.push(r),n}function en(e){let t=mapp.utils.html.node`<button
    data-id="zoom-to"
    title=${mapp.dictionary.zoom_to}
    class="notranslate material-symbols-outlined"
    onclick=${()=>tn(e)}>arrows_input`;function n(e){if(e.tableCurrent()){t.style.display=`none`;return}t.style.display=`block`}return e.changeEndCallbacks.push(n),t}function tn(e){let t=Object.entries(e.tables).find(e=>!!e[1])[0],n=Object.entries(e.tables).reverse().find(e=>!!e[1])[0],r=e.mapview.Map.getView();r.getZoom()<t?r.setZoom(t):r.setZoom(n),e.show()}function nn(e){return mapp.utils.html.node`<button
    data-id="popout-btn"
    class="notranslate material-symbols-outlined">open_in_new`}function rn(e){let t=`notranslate material-symbols-outlined toggle ${e.zoomDisplay||e.display?`toggle-on`:``}`,n=mapp.utils.html.node`<button
    data-id=display-toggle
    title=${mapp.dictionary.layer_visibility}
    class=${t}
    onclick=${t=>{t.target.classList.toggle(`toggle-on`)?e.show():e.hide()}}>`;return e.showCallbacks.push(()=>{n.classList.add(`toggle-on`)}),e.hideCallbacks.push(()=>{!e.zoomDisplay&&n.classList.remove(`toggle-on`)}),n}var an={filters:C,legends:Dt,listview:Ot,panels:Jt,view:Yt,viewHeader:Qt};function on(e){if(e.edit)return mapp.ui.elements.chkbox({checked:e.newValue===void 0?e.value:e.newValue,disabled:!e.edit,label:e.label||e.title,onchange:t=>{e.newValue=t,e.location.view?.dispatchEvent(new CustomEvent(`valChange`,{detail:e}))}});e.label??=``;let t=e.value?`check`:`close`;return mapp.utils.html.node`
    <div class="link-with-img">
      <div class="notranslate material-symbols-outlined">${t}</div>
      <span>${e.label}`}var P={documents:ln,image:sn,images:cn},F=e=>P[e.type](e);function sn(e){if(e.value){let t=e.edit&&mapp.utils.html`<button
      ...
      data-src=${e.value}
      onclick=${t=>R(t,e,mapp.dictionary.remove_image_confirm)}>delete`;return mapp.utils.html.node`<div class="img-item"><img
      style="width: 100%"
      src=${e.value}
      ...
      <input
        type="file"
        accept="image/*;capture=camera" onchange=${t=>{I(t,e)}}>`}function cn(e){let t=[];if(e.value?.map(n=>{let r=e.edit&&mapp.utils.html`<button title="${mapp.dictionary.delete}"
      class="notranslate material-symbols-outlined color-danger delete"
      data-name=${n.replace(/^.*\//,``).replace(/\.([\w-]{3})/,``)}
      data-src=${n}
      onclick=${t=>R(t,e,mapp.dictionary.remove_image_confirm)}>delete`;t.push(mapp.utils.html`<div class="img-item"><img
      src=${n}
      onclick=${mapp.ui.utils.imagePreview}>
      ...
      <input
        type="file"
        accept="image/*;capture=camera" onchange=${t=>{I(t,e)}}>`;t.push(n)}if(t.length)return mapp.utils.html.node`<div class="images-grid">${t}`}function ln(e){let t=[];if(e.value?.map(n=>{let r=e.edit&&mapp.utils.html`<button
      title="${mapp.dictionary.delete}"
      class="notranslate material-symbols-outlined color-danger delete"
      data-name=${n.replace(/^.*\//,``).replace(/\.([\w-]{3})/,``)}
      data-href=${n}
      ...
      <input type="file"
        ...
        onchange=${t=>I(t,e)}>`;t.unshift(n)}if(t.length)return mapp.utils.html.node`<div>${t}`}async function I(e,t){let n=new FileReader,r=e.type===`drop`?e.dataTransfer?.files[0]:e.target.files[0];if(!r)return;t.location.view?.classList.add(`disabled`);let i={documents:dn,image:L,images:L};n.onload=e=>i[t.type](e,t,r),n.readAsDataURL(r)}function L(e,t,n){let r=new Image;r.onload=async()=>un(t,r,n),r.src=e.target.result}async function un(e,t,n){let r=document.createElement(`canvas`);e.max_size??=1024,t.width>t.height&&t.width>e.max_size?(t.height*=e.max_size/t.width,t.width=e.max_size):t.height>e.max_size&&(t.width*=e.max_size/t.height,t.height=e.max_size),r.width=t.width,r.height=t.height,r.getContext(`2d`).drawImage(t,0,0,t.width,t.height);let i=await B(e,{public_id:n.name.replace(/^.*\//,``).replace(/\.([\w-]{3})/,``)+e.suffix_date?`@${Date.now()}`:``});if(!i)return;let a=new FormData;a.append(`file`,r.toDataURL(`image/jpeg`,.5));let o=await fetch(i,{body:a,method:`post`});if(!o.ok){mapp.ui.elements.alert(`Cloudinary Image upload failed!`),e.location.view?.classList.remove(`disabled`);return}let s=await o.json();e.type===`image`?e.value=s.secure_url:e.value=Array.isArray(e.value)?e.value.concat([s.secure_url]):[s.secure_url],z(e)}async function dn(e,t,n){let r=new Date,i=`${r.getMonth()+1}-${r.getDate()}T${r.getHours()}:${r.getMinutes()}`,a=n.name.substring(n.name.lastIndexOf(`.`)),o=await B(t,{public_id:`${n.name.replace(a,``)}-${i}${a}`});if(!o)return;let s=new FormData;s.append(`file`,e.target.result.toString());let c=await fetch(o,{body:s,method:`post`});if(!c.ok){mapp.ui.elements.alert(`Cloudinary Document upload failed!`),t.location.view?.classList.remove(`disabled`);return}let l=await c.json();t.value=Array.isArray(t.value)?t.value.concat([l.secure_url]):[l.secure_url],z(t)}async function R(e,t,n){if(!await mapp.ui.elements.confirm({text:n}))return;let r=await B(t,{destroy:!0,public_id:decodeURIComponent(e.target.dataset.name)});if(!r)return;await fetch(r,{method:`post`});let i=new Set(t.value);i.delete(e.target.dataset.src||e.target.dataset.href),t.type===`image`?t.value=null:t.value=i.size?Array.from(i):null,z(t)}async function z(e){e.location.view?.classList.add(`disabled`),await mapp.utils.xhr({body:JSON.stringify({[e.field]:e.value}),method:`POST`,url:`${e.location.layer.mapview.host}/api/query?`+mapp.utils.paramString({id:e.location.id,layer:e.location.layer.key,locale:e.location.layer.mapview.locale.key,table:e.location.table,template:e.location.updateTemplate})}),e.node.replaceChildren(mapp.utils.html.node`<div class="label">${e.title}`,P[e.type](e)),e.location.view?.classList.remove(`disabled`)}async function B(e,t){let n=mapp.utils.paramString({...t,folder:e.cloudinary_folder}),r=await mapp.utils.xhr({responseType:`text`,url:`${e.location.layer.mapview.host}/api/sign/cloudinary?${n}`});return r instanceof Error&&(console.error(r),e.location.view?.classList.remove(`disabled`)),r}function fn(e){if(e.value!==void 0&&(e.data=e.value),e.data?.length===0&&(e.data=null),e.layer??=e.location.layer,pn(e),e.target instanceof HTMLElement){if(!e.display&&typeof e.create==`function`)return mapp.utils.html.node`
    ${e.chkbox||``}
    ...
    <div
      ...
      ${t}`}function qn(e){return mapp.utils.html.node`
        <div
          class="label"
          style=${e.css_title}
          title=${e.tooltip}>${e.title}`}function Jn(){console.warn(`The type:defaults entry method has been deprecated.`)}function q(e){return console.warn(`The type:${e.type} entry method has been deprecated in favour of the type:layer entry method.`),e.type=`layer`,e.qID??=`id`,e.name??=e.label,U(e)}var Yn={boolean:on,dataview:fn,date:V,datetime:V,defaults:Jn,documents:F,geometry:hn,html:K,image:F,images:F,integer:G,json:Cn,key:wn,layer:U,link:W,mvt_clone:q,numeric:G,pills:Mn,pin:Nn,ping:Ln,query_button:Rn,report:W,tab:Bn,text:Vn,textarea:K,time:Kn,title:qn,vector_layer:q},J;function Xn(e,t){if(!e.infoj)return;let n=mapp.utils.html.node`<div class="location-view-grid">`;J={},t??=e?.layer?.infoj_order;let r=Array.isArray(t)?t.map(i).filter(e=>e!==void 0):e.infoj;function i(t){if(typeof t==`string`){let n=e.infoj.find(e=>new Set([e.key,e.field,e.query]).has(t));return n||console.warn(`infoj_order field: "${t}" not found in location.infoj. Please add entry.key, entry.field, or entry.query to the entry.`),n}else if(typeof t==`object`)return t.location=e,t}let a=0;for(let t of r){if(t.key??=t.field||a++,e.view?.classList.contains(`disabled`))break;if(t.listview=n,t.type??=`text`,t.edit===!0&&(t.edit={}),Zn(t),Qn(t),$n(t)||(er(t),tr(t),nr(t),rr(t),ir(t),ar(t)))continue;if(!Object.hasOwn(mapp.ui.locations.entries,t.type)){console.error(`entry.type:${t.type} method not found.`);continue}let r=mapp.ui.locations.entries[t.type]?.(t);r&&t.node.append(r),sr(t)}return n}function Zn(e){e.jsonb_field&&e.jsonb_key&&e.value!==null&&typeof e.value==`object`&&e.value.jsonb&&(e.value=e.value.jsonb[e.jsonb_field][e.jsonb_key])}function Qn(e){if(e.objectAssignFromField&&e.objectMergeFromField){console.warn(`${e.key}: objectAssignFromField and objectMergeFromField cannot both be used on the same entry.`);return}let t=e.objectAssignFromField||e.objectMergeFromField||e.json_field;if(!t)return;let n=e.location.infoj.find(e=>e.field===t);if(n&&typeof n.value==`object`){if(e.json_field){if(!e.json_key){console.warn(`json_field requires entry.json_key to be specified`);return}if(!n.value)return;e.value=n.value[e.json_key]}e.objectAssignFromField&&Object.assign(e,n.value),e.objectMergeFromField&&mapp.utils.merge(e,n.value)}}function $n(e){if(e.skipEntry||e.skipFalsyValue&&!e.value&&!e.edit||e.skipUndefinedValue&&e.value===void 0&&!e.edit||e.skipNullValue&&e.value===null&&!e.edit)return!0}function er(e){e.nullValue!==void 0&&(e.edit||(e.value??=e.nullValue))}function tr(e){e.default!==void 0&&e.edit&&(e.value||(e.newValue=e.default,e.location.view?.dispatchEvent(new CustomEvent(`valChange`,{detail:e}))))}function nr(e){e.group&&(J[e.group]||(J[e.group]=e.listview.appendChild(mapp.ui.elements.drawer({data_id:`location-infoj-group`,class:`group ${e.groupClassList||``}`,header:mapp.utils.html`
          <h3>${e.group}</h3>`}))),e.listview=J[e.group])}function rr(e){let t=`contents ${e.type} ${e.class||``} ${e.inline&&`inline`||``}`;e.node=e.listview.appendChild(mapp.utils.html.node`
  <div
    data-type=${e.type}
    class=${t}>`)}function ir(e){e.title&&e.node.append(mapp.ui.locations.entries.title(e))}function ar(e){if(e.query)if(e.queryCheck??=e.run,e.queryCallback??=or,e.runOnce||e.queryCheck){delete e.runOnce,e.host??=e.location.layer?.mapview?.host||mapp.host;let t=mapp.utils.queryParams(e),n=mapp.utils.paramString(t);return e.url=`${e.host}/api/query?${n}`,e.blocking&&e.location.view?.classList.add(`loading`),mapp.utils.xhr(e).then(t=>e.queryCallback(t,e)),!0}else e.field&&!e.xhr&&console.warn(`field:"${e.field}" has a query:"${e.query}" which is not set to run, please add the run or runOnce flag to the entry.`)}function or(e,t){if(t.blocking&&t.location.view?.classList.remove(`loading`),e?t.value=t.field?e[t.field]:e:t.value=t.nullValue||null,$n(t)){t.node.remove();return}let n=mapp.ui.locations.entries[t.type]?.(t);n&&t.node.appendChild(n)}function sr(e){if(!e.tooltip)return;let t=e.node.querySelector(`label`)||e.node.querySelector(`div.label`);t&&(e.tooltipElement||(e.tooltipElement=mapp.ui.elements.tooltip({content:e.tooltip}),t.append(e.tooltipElement)))}var cr=[{symbol:`A`,colour:`#2E6F9E`},{symbol:`B`,colour:`#EC602D`},{symbol:`C`,colour:`#5B8C5A`},{symbol:`D`,colour:`#B84444`},{symbol:`E`,colour:`#514E7E`},{symbol:`F`,colour:`#E7C547`},{symbol:`G`,colour:`#368F8B`},{symbol:`H`,colour:`#841C47`},{symbol:`I`,colour:`#61A2D1`},{symbol:`J`,colour:`#37327F`},{symbol:`K`,colour:`#FFA500`},{symbol:`L`,colour:`#7FFF00`},{symbol:`M`,colour:`#FFD700`},{symbol:`N`,colour:`#00CED1`},{symbol:`O`,colour:`#9932CC`},{symbol:`P`,colour:`#FF69B4`},{symbol:`Q`,colour:`#008B8B`},{symbol:`R`,colour:`#8A2BE2`},{symbol:`S`,colour:`#FF4500`},{symbol:`T`,colour:`#00FF00`},{symbol:`U`,colour:`#FF00FF`},{symbol:`V`,colour:`#708090`},{symbol:`W`,colour:`#F4A460`},{symbol:`X`,colour:`#000080`},{symbol:`Y`,colour:`#D2691E`},{symbol:`Z`,colour:`#8FBC8F`}];function lr(e){e.mapview||console.warn(`A mapview is required in the locations listview params argument.`),e.target||console.warn(`A target element is required in the locations listview params argument.`);let t=e.mapview.locale;t.locations??={},t.locations.pinStyle??={anchor:[.5,1],scale:3,type:`markerLetter`},t.locations.style??={fillColor:`#fff`,fillOpacity:.1,strokeColor:`#fff`},t.locations.records??=t.listview_records||structuredClone(cr),t.locations.clearAll=e.target.appendChild(mapp.utils.html.node`
    <button
    ...
    `);async function i(){e.infoj.some(e=>e.newValue!==void 0)&&!await mapp.ui.elements.confirm({text:mapp.dictionary.location_close_without_save})||e.remove()}e.view=mapp.ui.elements.drawer({data_id:`location-${e.layer.key}-${e.id}`,class:`location-view raised expanded`,header:r}),e.viewEntries=e.view.appendChild(mapp.ui.locations.infoj(e)),e.view.querySelector(`.header`).style.borderBottom=`3px solid ${e.record.colour}`,e.view.addEventListener(`valChange`,pr),e.renderLocationView=mr,e.view.addEventListener(`render`,()=>e.renderLocationView()),e.view.addEventListener(`updateInfo`,()=>{e.editToggle&&(e.editToggle.classList.remove(`toggle-on`),e.removeEdits()),e.layer?.dataviews&&Object.values(e.layer.dataviews).forEach(e=>{e.display===!0&&e.update()}),e.renderLocationView()})}function fr(e){if(e.layer?.toggleLocationViewEdits===!1)return e.removeEdits(),!1;if(!e.layer?.toggleLocationViewEdits||!e.infoj.some(e=>e.edit))return!1;!e.new&&e.removeEdits();let t=`notranslate material-symbols-outlined ${e.new?`toggle-on`:``}`;return e.editToggle=mapp.utils.html.node`<button
    title="Enable edits"
    class=${t}
    onclick=${n}>edit`,!0;async function n(t){await e.confirmUpdate()===void 0&&(t.target.classList.toggle(`toggle-on`)?e.restoreEdits():e.removeEdits(),e.renderLocationView())}}function pr(e){let t=e.detail,n=t.location;if(typeof t.valChangeMethod==`function`){t.valChangeMethod(t);return}if(t.value==t.newValue?(delete t.newValue,t.node?.classList.remove(`val-changed`)):t.node?.classList.add(`val-changed`),n.infoj.some(e=>e.invalid)){n.view.querySelector(`.btn-save`).style.display=`none`;return}n.view.querySelector(`.btn-save`).style.display=n.infoj.some(e=>e.newValue!==void 0)?`inline-block`:`none`}function mr(){let e=this;e.view.querySelector(`.btn-save`).style.display=`none`,e.viewEntries.remove(),e.view.classList.remove(`disabled`),e.viewEntries=e.view.appendChild(mapp.ui.locations.infoj(e))}var hr={entries:Yn,infoj:Xn,listview:lr,view:dr};function gr(e){if(e.node)return e.tabs=e.node.appendChild(mapp.utils.html.node`<div class="tabs">`),e.panel=e.node.appendChild(mapp.utils.html.node`<div class="panel">`),e.id&&(e.node.dataset.id=e.id),e.addTab=_r,e.node.addEventListener(`addTab`,t=>e.addTab(t.detail)),e}function _r(e){if(e.tab&&!e.dynamic)return;let t=this;e.layer?.dataviews?.[e.key]&&(e.disableTabClose??=e.layer.dataviews.hide),e.activate??=a,e.location?e.location.removeCallbacks.push(()=>e.remove()):e.layer&&(e.layer.showCallbacks.push(()=>{e.display&&e.show()}),e.layer.hideCallbacks.push(()=>{e.remove()})),e.label??=e.title||e.key||`Tab`,e.tab_btn=mapp.utils.html.node`<button
    ...
    overflow: hidden !important`;Object.assign(e.dataview_dialog,{header:mapp.utils.html.node`<h1> ${e.label}`,data_id:`${e.key}-dataviews-dialog`,target:document.getElementById(`Map`),height:`auto`,left:`5%`,top:`0.5em`,class:`box-shadow tabview`,css_style:t,containedCentre:!0,contained:!0,headerDrag:!0,closeBtn:!0,onClose:()=>{e.chkbox&&(e.chkbox.querySelector(`input`).checked=!1)}}),mapp.ui.elements.dialog(e.dataview_dialog);let n=e.dataview_dialog.node.querySelector(`.content`);n.classList.add(`panel`),e.target=n,e.dataview_dialog.dialog.close(),e.show=()=>{e.update(),e.toolbar&&e.dataview_dialog.node.querySelector(`.btn-row`).style.removeProperty(`display`),e.chart&&(e.dataview_dialog.node.appendChild(e.target.querySelector(`canvas`)),e.dataview_dialog.node.removeChild(e.target)),e.dataview_dialog.dialog.show()},e.hide=()=>{e.dataview_dialog.dialog.close()},e.location?.removeCallbacks?.push?.(()=>e.dataview_dialog.dialog.close())}var Or={create:kr};async function kr(e){e.query??=`histogram`,e.options??={};let t=`column-gap: ${e.options.columnGap||10}px`;e.node=mapp.utils.html.node`<div class="histogram" style=${t}>`,e.options.width&&(e.node.style.width=`${e.options.width}px`),e.options.height&&(e.node.style.height=`${e.options.height}px`),e.options.title&&e.target.append(mapp.utils.html.node`<h3 class="histogram-title">${e.options.title}`),e.setData=Ar,e.target.append(e.node),e.options.xlabel&&(e.xlabel=mapp.utils.html.node`<div class="histogram-xlabel">${e.options.xlabel}`,e.options.width&&(e.xlabel.style.width=`${e.options.width}px`),e.target.append(e.xlabel)),e.options.caption&&(e.caption=mapp.utils.html.node`<span class="histogram-caption">${e.options.caption}`,e.target.append(e.caption))}function Ar(e){this.data=e,jr(this)}function jr(e){if(!Array.isArray(e.data)){mapp.utils.render(e.node,mapp.utils.html`<div>`);return}let t=Math.max(...e.data.map(e=>e.count));e.options.height??=100;let n=e.data.map(n=>{let r=`height: ${Math.round(e.options.height*n.count/t)}px`,i=``;return e.options.tooltip&&(i=mapp.utils.html`<div class="bucket-info">
      ...
      style=${r}>
      ${i}`}),r=e.options.ylabel?mapp.utils.html`<div class="ylabel">${e.options.ylabel}`:``;mapp.utils.render(e.node,mapp.utils.html`${r}${n}`)}var X={idle:600};function Mr(e){Object.assign(X,e),X.idle!==0&&(window.onload=Z,window.onmousemove=Z,window.onmousedown=Z,window.ontouchstart=Z,window.onclick=Z,window.onkeypress=Z,Z(),Pr())}function Z(){X.locked||(X.timeout&&clearTimeout(X.timeout),X.timeout=setTimeout(Nr,X.idle*1e3))}function Nr(){X.locked=!0,X.renew&&clearTimeout(X.renew);let e={url:`${X.host}/api/user/cookie?destroy=true`};e.onLoad=e=>location.reload(),mapp.utils.xhr(e)}function Pr(){X.renew=setTimeout(Fr,(X.idle-20)*1e3)}function Fr(){let e={url:`${X.host}/api/user/cookie`};e.onLoad=e=>{if(e.target.status===401)return Nr();Pr()},mapp.utils.xhr(e)}function Ir(e){let t=mapp.utils.html.node`
