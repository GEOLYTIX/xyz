mapp.utils.merge(mapp.dictionaries,{en:{layer_zoom_to_extent:"Zoom to filtered layer extent",layer_visibility:"Toggle visibility"},de:{layer_zoom_to_extent:"Zoom zum Ausma\xDF des gefilterten Datensatzes",layer_visibility:"Umschalten der Ansicht"},cn:{layer_zoom_to_extent:"\u7F29\u653E\u81F3\u76F8\u5E94\u7B5B\u9009\u8303\u56F4",layer_visibility:"\u5207\u6362\u53EF\u89C1\u6027"},pl:{layer_zoom_to_extent:"Poka\u017C zasi\u0119g warstwy",layer_visibility:"Widoczno\u015B\u0107"},ko:{layer_zoom_to_extent:"\uD544\uD130\uB41C \uB808\uC774\uC5B4\uD06C\uAE30\uC5D0 \uC90C(zoom)",layer_visibility:"\uD1A0\uAE00 \uAC00\uC2DC\uC131"},fr:{layer_zoom_to_extent:"Zoom sur l'\xE9tendue de la couche",layer_visibility:"Changer la visiblit\xE9"},ja:{layer_zoom_to_extent:"\u30D5\u30A3\u30EB\u30BF\u30FC\u3055\u308C\u305F\u30EC\u30A4\u30E4\u30FC\u7BC4\u56F2\u3092\u30BA\u30FC\u30E0\u306B",layer_visibility:"\u8868\u793A\u5207\u66FF"}});var _=e=>{let t=e.filter.zoomToExtent&&mapp.utils.html`
  ...
  <div class="dropdown active">
    ...
    <div 
      ...
      <div 
        data-id="cluster"
        class="contents">
        ${a}${l}`)}return e.style.legend=mapp.utils.html.node`<div class="grid">${i}`,e.style.legend};var N=e=>(e.style.legend=mapp.utils.html.node`<div>`,e.style.legend);var k=e=>{let t=e.style.theme,i=[];return t.cat_arr.forEach(a=>{let l=Object.assign({},e.style.default,a.style&&a.style.icon||a.style||a),o=mapp.utils.html`
      <div
        style="height: 24px; width: 24px; grid-column: 1;">
        ${mapp.ui.elements.legendIcon(Object.assign({width:24,height:24},l))}`,n=mapp.utils.html`
  ...
  <div class="layer-key">
    <span>
      ${e.location.layer.name}`}var ue={view:U,listview:J,infoj:Z,entries:pe};var me=async e=>{if(typeof e.target=="string"&&(e.target=document.getElementById(e.target)),!e.target){console.warn("Dataview creation requires a target key-value"),console.log(e);return}if(e.update=async()=>{if(!e.query)return;let t=mapp.utils.queryParams(e),i=mapp.utils.paramString(t),a=await mapp.utils.xhr(`${e.host||e.location.layer.mapview.host}/api/query?${i}`);if(!(a instanceof Error)){if(typeof e.responseFunction=="function")return e.responseFunction(a);typeof e.setData=="function"&&e.setData(a)}},e.toolbar&&e.target instanceof HTMLElement){let t=mapp.utils.html.node`<div class="dataview-target">`,i=Object.keys(e.toolbar).map(a=>mapp.ui.elements.toolbar_el[a](e));e.panel=e.target.appendChild(mapp.utils.html.node`
        ...
        <div class="dataview-mask">No Data`)):(e.mask&&e.mask.remove(),delete e.mask,e.target.style.display="block"),t=!t&&[]||t.length&&t||[t],e.Tabulator.setData(t),e.data=t,typeof e.setDataCallback=="function"&&e.setDataCallback(e))}}var fe=e=>{!e.node||(e.tabs=e.node.appendChild(mapp.utils.html.node`<div class="tabs">`),e.panel=e.node.appendChild(mapp.utils.html.node`<div class="panel">`),e.id&&e.node.setAttribute("data-id",e.id),e.addTab=nt,e.node.addEventListener("addTab",t=>e.addTab(t.detail)))};function nt(e){let t=this;e.layer&&(e.layer.showCallbacks.push(()=>{e.display&&e.show()}),e.layer.hideCallbacks.push(()=>{e.remove()})),e.tab=mapp.utils.html.node`
    <div class="tab">
      <div
        class="header"
        style="${e.tab_style||""}"
  ...
  <div 
    ...
    <button
      class="flat wide bold primary-colour"
      onclick=${a=>t(a,i)}>
      ${mapp.dictionary.draw_polygon}`}function ft(e,t){let i=Object.assign({},e,{type:"Circle",geometryFunction:ol.interaction.Draw.createRegularPolygon(33),tooltip:e.edit.circle.tooltip});return mapp.utils.html.node`
    <button
      class="flat wide bold primary-colour"
      onclick=${a=>t(a,i)}>
  ...
  <button 
    ...
    <button
      class="flat"
      onclick=${()=>{e.Tabulator.download("csv",`${e.title||"table"}.csv`)}}>Download as CSV`}var ze={card:he,chkbox:ge,contextMenu:ve,drawer:be,drawing:we,dropdown:_e,dropdown_multi:ye,isoline_params_here:$e,isoline_params_mapbox:xe,legendIcon:Le,modal:Ee,slider:Se,slider_ab:Ce,toolbar_el:je};var T,v=null;async function Te(e,t){return v?new v(e,t):(T||(T=new Promise(async i=>{if(window.Chart){v=window.Chart,i();return}Promise.all([import("https://cdn.skypack.dev/chart.js@3.7.1"),import("https://cdn.skypack.dev/chartjs-plugin-datalabels@2.0.0"),import("https://cdn.skypack.dev/chartjs-plugin-annotation@1.4.0")]).then(a=>{a[0].Chart.register(...a[0].registerables),a[0].Chart.register(a[1].default),a[0].Chart.register(a[2].default),v=a[0].Chart,i()}).catch(a=>{console.error(a.message),alert("Failed to load Chart.js library. Please reload the browser.")})})),await T,new v(e,t))}var O,b=null;async function Oe(){return b?new b(...arguments):(O||(O=new Promise(async e=>{if(window.Tabulator){b=window.Tabulator,e();return}Promise.all([import("https://cdn.skypack.dev/pin/tabulator-tables@v5.3.0-3P9dSwNRL29tgc0Vmk59/mode=imports,min/optimized/tabulator-tables.js")]).then(t=>{b=t[0].TabulatorFull,e()}).catch(t=>{console.error(t.message),alert("Failed to load Tabulator library. Please reload the browser.")})})),await O,new b(...arguments))}var De={headerFilter:{like:yt,numeric:$t,set:xt}};function yt(e){return(t,i,a,l,o)=>{let n=t.getColumn().getField();function r(s){let c=e.Tabulator.getFilters().find(d=>d.field===n&&d.type==">=");c&&(e.Tabulator.removeFilter(...Object.values(c)),o.layer&&delete e.layer.filter.current[n]),s.target.value.length&&(e.Tabulator.setFilter([{field:n,type:"like",value:s.target.value}]),o.layer&&(e.layer.filter.current[n]={[o.type]:encodeURIComponent(s.target.value)},e.layer.reload(),e.update()))}return mapp.utils.html.node`<span>
      <input
        type="text"
        placeholder="Filter"
        oninput=${r}
    ...
    onchange=${s=>{let c=s.target.value;o?.set.has(c)&&o.set.delete(c)||o.set.add(c);for(let d=0;d<s.target.options.length;d++)s.target.options[d].style.backgroundColor=o.set.has(s.target.options[d].value)&&(o.selected||"#cae0b8")||"#ffffff";n(t,o.set),s.target.value=null}}>${o.options.map(s=>mapp.utils.html.node`
      <option>${s}`)}`;return r.value=null,o.multiple&&(r.multiple=!0),r}}var f={idle:600},Fe=e=>{Object.assign(f,e),f.idle!==0&&(window.onload=g,window.onmousemove=g,window.onmousedown=g,window.ontouchstart=g,window.onclick=g,window.onkeypress=g,g(),Me())};function g(){f.locked||(f.timeout&&clearTimeout(f.timeout),f.timeout=setTimeout(qe,f.idle*1e3))}function qe(){f.locked=!0,f.renew&&clearTimeout(f.renew);let e=new XMLHttpRequest;e.open("GET",`${f.host}/api/user/cookie?destroy=true`),e.onload=t=>location.reload(),e.send()}function Me(){f.renew=setTimeout(e,(f.idle-20)*1e3);function e(){let t=new XMLHttpRequest;t.open("GET",`${f.host}/api/user/cookie?renew=true`),t.onload=i=>{if(i.target.status===401)return qe();Me()},t.send()}}var Ie=e=>{document.body.append(mapp.utils.html.node`
    <div class="interface-mask">
      <div class="bg-image" style=${`background-image:url(${e.target.src})`}>
      <button class="btn-close mask-icon close"
        onclick=${t=>t.target.parentElement.parentElement.remove()}>`)};var Ae=e=>{e.target.addEventListener("mousedown",i=>{i.preventDefault(),document.body.style.cursor="grabbing",window.addEventListener("mousemove",e.resizeEvent),window.addEventListener("mouseup",t)}),e.target.addEventListener("touchstart",i=>{i.preventDefault(),window.addEventListener("touchmove",e.resizeEvent),window.addEventListener("touchend",t)},{passive:!0});function t(){document.body.style.cursor="auto",window.removeEventListener("mousemove",e.resizeEvent),window.removeEventListener("touchmove",e.resizeEvent),window.removeEventListener("mouseup",t),window.removeEventListener("touchend",t)}};var Pe={Chart:Te,Tabulator:Oe,tabulator:De,idleLogout:Fe,imagePreview:Ie,resizeHandler:Ae};var Re=e=>{let t=Object.assign({input:e.target.querySelector("input"),result:e.target.querySelector("ul"),sources:{glx:a,mapbox:o,google:l}},e);t.input.placeholder=t.placeholder||"",t.input.oninput=r=>{t.xhr&&t.xhr.abort(),t.result.innerHTML="",r.target.value.length>0&&i(r.target.value)},t.input.addEventListener("keydown",r=>{(r.keyCode||r.charCode)===13&&t.result.querySelector("li").click()});function i(r){let s=r.split(",").map(parseFloat);if(s.length===2&&s.every(c=>!isNaN(c))){t.result.appendChild(mapp.utils.html.node`
      <li 
