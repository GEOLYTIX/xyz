export default (function () {
    ...
    mapp.ui.layers.panels.s3bucket_upload = layer => {
        ...
        layer.s3bucket_upload.input = mapp.utils.html.node`
                <input type=file class="flat bold wide primary-colour type="file" accept=".json,.csv"
                onchange=${upload}>`

        async function upload(e) {

            //Getting the file from the input and setting the size
            const csvFile = new Blob([layer.s3bucket_upload.input.files[0]], { type: 'text/csv' });
                ...
                var uploadRes = await mapp.utils.xhr({
                    ...
                    url: `${layer.mapview.host}/api/provider/s3?` +
                        mapp.utils.paramString({
                            command: 'upload',
                            filename: layer.s3bucket_upload.input.files[0].name
                        }),
                    body: csvFile
                });

                if (uploadRes.$metadata.httpStatusCode === 200)
                    alert(`File ${layer.s3bucket_upload.input.files[0].name} successfully uploaded`)
        ...
        return mapp.utils.html.node`${layer.s3bucket_upload.input}`
    }
}
)()