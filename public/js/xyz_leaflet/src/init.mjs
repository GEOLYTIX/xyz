...
export default xyz => {
  ...
  xyz.init = params => {
    ...
    // Set XYZ map DOM.
    xyz.map_dom = document.getElementById(params.map_id);

    xyz.map = xyz.L.map(xyz.map_dom, {
      renderer: xyz.L.svg(),
    ...
    xyz.L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(xyz.map);
  
    xyz.map.setView([ params.view_lat, params.view_lng ], 10);
