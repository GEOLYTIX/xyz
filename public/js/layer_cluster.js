...
function addClusterToLayer(cluster, layer) {
    ...
    if (layer.style.theme && layer.style.theme.filter && layer.style.theme.filter.length > 0) {

        // Filter all cluster features
        cluster = cluster.filter(f => {

            // Map the infoj keys.
            Object.keys(f.properties.cat).map(key => {

                // Check whether key is found in filter array.
                if (layer.style.theme.filter.indexOf(key) >= 0) {

                    // Subtract the key value count from the cluster count.
                    f.properties.count -= f.properties.cat[key];

                    // Delete the key from the feature.
                    delete f.properties.cat[key];
                }
            });

            // Return the feature to cluster if count is more than 0.  
            if (f.properties.count > 0) return f;
        });
    }

    // Check whether filterOther is true.
    if (layer.style.theme && layer.style.theme.filterOther) {

        // Filter cluster features.
        cluster = cluster.filter(f => {

            // Map infoj of cluster feature.
            Object.keys(f.properties.cat).map(key => {

                // Check whether the key is not found in cat theme.
                if (!layer.style.theme.cat[key]) {
...
function clusterMouseClick(e, layer) {
    ...
    let count = e.layer.feature.properties.count,
        ...
        xhr.onload = () => {
            ...
            // Data is returned and the layer is still current.
            //if (layer.xhr.status === 200) return addClusterToLayer(JSON.parse(layer.xhr.responseText), layer);
