  ...
  await codi.describe('Mapview test', async () => {
    ...
    await codi.it('should merge dictionaries correctly', () => {
      ...
      codi.assertTrue(
        ...
        'English dictionary should have new entries',
      );

      // Assert that the merged dictionaries have the expected properties
      codi.assertEqual(
        mapp.dictionaries.en.toolbar_zoom_in,
        'Zoom in',
        'English dictionary should have the correct value for toolbar_zoom_in',
      );
      codi.assertEqual(
        mapp.dictionaries.de.toolbar_zoom_in,
        'Zoom rein',
        'German dictionary should have the correct value for toolbar_zoom_in',
      );
    });

    //Testcase: Ensure User refreshes cookie
    await codi.it(
      'should refresh cookie and get user with updated credentials',
      async () => {
        ...
        if (mapp.user !== null) {
          ...
          codi.assertNotEqual(
            mapp.user,
            currentUser,
            'User object should be updated after refreshing cookie',
          );

          // Assert that the user object has the expected properties
          codi.assertTrue(
            mapp.user.hasOwnProperty('email'),
            'User object should have the "email" property',
          );
          codi.assertTrue(
            mapp.user.hasOwnProperty('language'),
            'User object should have the "language" property',
          );
          codi.assertTrue(
            mapp.user.hasOwnProperty('roles'),
            'User object should have the "roles" property',
          );
        }
      },
    );
    ...
    await codi.it('should set the language correctly', () => {
      ...
      mapp.language =
        mapp.hooks.current.language || mapp.user?.language || mapp.language;

      // Assert that the language has been set correctly
      if (mapp.hooks.current.language) {
        codi.assertEqual(
          mapp.language,
          mapp.hooks.current.language,
          'Language should be set to the value from mapp.hooks.current.language',
        );
      } else if (mapp.user?.language) {
        codi.assertEqual(
          mapp.language,
          mapp.user.language,
          'Language should be set to the value from mapp.user.language',
        );
      } else {
        codi.assertEqual(
          mapp.language,
          currentLanguage,
          'Language should remain unchanged if no overrides are present',
        );
      }
    });
    ...
    await codi.it('should restore scroll if supported', () => {
      ...
      if ('scrollRestoration' in history) {
        ...
        codi.assertEqual(
          ...
          'auto',
          'Scroll restoration should be set to "auto"',
        );
      } else {
        // Assert that scroll restoration is not supported
        codi.assertFalse(
          'scrollRestoration' in history,
          'Scroll restoration is not supported',
        );
      }
    });
    ...
    await codi.it('should set help text from dictionary', () => {
      tabs.forEach((tab) => {
        ...
        codi.assertEqual(
          tab.title,
          mapp.dictionary[tab.dataset.id],
          'Title should be set from the dictionary',
        );
      });
    });
    ...
    await codi.it('should handle tab click events', () => {
      tabs.forEach((tab) => {
        ...
        codi.assertTrue(
          ...
          'Clicked tab should have the active class',
        );
        ...
        codi.assertTrue(
          ...
          'Corresponding panel should have the active class',
        );
      ...
      const layersTab = Array.from(tabs).find(
        (tab) => tab.dataset.id === 'layers',
      );
      ...
      const localesDropdown = mapp.ui.elements.dropdown({
        ...
        callback: (e, entry) => {
          window.location.assign(
            `${mapp.host}?template=test_view&integrity=true&locale=${entry.key}`,
          );
        },
      });
    ...
    await codi.it(
      'should retrieve accessible locales from Workspace API',
      async () => {
    ...
    await codi.it(
      'should retrieve locale with list of layers from Workspace API',
      async () => {
        // Assert that locale is not an instance of Error
        codi.assertFalse(
          locale instanceof Error,
          'Locale should not be an instance of Error',
        );

        // Assert that locale has the expected properties
        codi.assertTrue(
          locale.hasOwnProperty('name'),
          'Locale should have a "name" property',
        );
        codi.assertTrue(
          locale.hasOwnProperty('key'),
          'Locale should have a "key" property',
        );
        codi.assertTrue(
          locale.hasOwnProperty('layers'),
          'Locale should have a "layers" property',
        );
      },
    );
    ...
    if (locale instanceof Error) {
      await codi.it(
        'should display an error dialog if locale is an instance of Error',
        async () => {
          ...
          codi.assertNotEqual(
            errorDialog,
            null,
            'Error dialog should be appended to the document body',
          );

          // Assert that the error dialog contains the correct message
          codi.assertEqual(
            errorDialog.textContent.trim(),
            mapp.dictionary.no_locales,
            'Error dialog should display the correct message',
          );
        },
      );
    }
    ...
    if (locales.length > 1) {
      await codi.it(
        'should add locale dropdown to layers panel if multiple locales are accessible',
        async () => {
          // Assert that the locale dropdown is appended to the layers tab
          const dropdown = layersTab.querySelector(
            '[data-id="locales-dropdown"]',
          );
          codi.assertNotEqual(
            dropdown,
            null,
            'Locale dropdown should be appended to the layers tab',
          );
        },
      );
    }
    ...
    await codi.it('should create a mapview', async () => {
      ...
      codi.assertTrue(
        mapview.hasOwnProperty('host'),
        'Mapview should have a "host" property',
      );
      codi.assertTrue(
        mapview.hasOwnProperty('target'),
        'Mapview should have a "target" property',
      );
      codi.assertTrue(
        mapview.hasOwnProperty('locale'),
        'Mapview should have a "locale" property',
      );
      codi.assertTrue(
        mapview.hasOwnProperty('hooks'),
        'Mapview should have a "hooks" property',
      );
      codi.assertTrue(
        mapview.hasOwnProperty('scrollWheelZoom'),
        'Mapview should have a "scrollWheelZoom" property',
      );
      codi.assertTrue(
        mapview.hasOwnProperty('attribution'),
        'Mapview should have an "attribution" property',
      );
    });
    ...
    await codi.it('should add layers to mapview', async () => {
      ...
      codi.assertTrue(
        Object.keys(mapview.layers).length > 0,
        'Mapview should have layers',
      );
    });

    //Testcase: Create a gazetteer if in locale
    await codi.it(
      'should create a gazetteer if available in the locale',
      async () => {
        if (mapview.locale.gazetteer) {
          ...
          const locationsTabElement = document.querySelector(
            '[data-id=locations]',
          );
          codi.assertEqual(
            locationsTabElement.style.display,
            'none',
            'Locations tab should be hidden',
          );
        }
      },
    );
    ...
    if (mapview.locale.gazetteer) {
      await codi.it('should create a locations listview', async () => {
        ...
        codi.assertNotEqual(
          listview,
          null,
          'Locations listview should be created',
        );
      });
    }
    ...
    await codi.it('should begin highlight interaction', async () => {
      ...
      codi.assertTrue(
        mapview.interactions.hasOwnProperty('highlight'),
        'Mapview should have a "highlight" interaction',
      );
    });
...
<div class="mobile-display-none" style="height: 60px;">`);
    ...
    await codi.it('should show the tabview when showTab is called', () => {
      ...
      codi.assertFalse(
        tabview.classList.contains('desktop-display-none'),
        'Tabview should not have the "desktop-display-none" class after showTab is called',
      );
      ...
      codi.assertEqual(
        ...
        'auto 10px 50px',
        'Grid template rows should be set to "auto 10px 50px" after showTab is called',
      );
    });

    //Testcase: Should hide the tabview
    await codi.it(
      'should hide the tabview when removeLastTab is called',
      () => {
        ...
        codi.assertTrue(
          tabview.classList.contains('desktop-display-none'),
          'Tabview should have the "desktop-display-none" class after removeLastTab is called',
        );
        ...
        codi.assertEqual(
          document.body.style.gridTemplateRows,
          'auto 0px 0px',
          'Grid template rows should be set to "auto 0 0" after removeLastTab is called',
        );

        // Assert that the margin top of the map target element is set to 0
        codi.assertEqual(
          mapview.Map.getTargetElement().style.marginTop,
          '0px',
          'Margin top of the map target element should be set to 0 after removeLastTab is called',
        );
      },
    );
  });
