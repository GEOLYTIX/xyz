export default _xyz => {
  ...
  function init(params) {
    ...
    _xyz.locations.listview.node.addEventListener('passover', ()=>{

      setTimeout(()=>{

        //Lambs blood painted door, I shall pass
        if (_xyz.locations.listview.node.children.length > 1) return
        
        const tab = _xyz.locations.listview.node.closest('.tab')
    ...
    _xyz.locations.listview.add = location => {
      ...
      _xyz.locations.listview.node.appendChild(location.view)
      _xyz.locations.listview.node.closest('.tab').style.display = 'block'
      _xyz.locations.listview.node.closest('.tab').click()
    }
