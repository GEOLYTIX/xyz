export default _xyz => {
  ...
  function settings(entry) {
    ...
    if (entry.edit.isoline_mapbox && !entry.edit.isoline_mapbox.geometry && (entry.edit.isoline_mapbox.minutes || (!entry.edit.isoline_mapbox.minutes && entry.value))) return _xyz.utils.wire()`<div>`;

    if (entry.edit.isoline_mapbox && entry.edit.isoline_mapbox.geometry && entry.edit.isoline_mapbox.minutes && !entry.value) return _xyz.utils.wire()`<div>`;
