export default _xyz => {
    ...
    function create (entry) {
        ...
        const geo = `${entry.location.geometry.coordinates[1]},${entry.location.geometry.coordinates[0]}`
        ...
        xhr.open('GET', _xyz.host + '/api/provider/here?' +
            'url=isoline.router.hereapi.com/v8/isolines' +
        ...
        xhr.onload = e => {
            ...
            if (e.target.status !== 200 || !e.target.response.isolines) {
                entry.location.view && entry.location.view.classList.remove('disabled')
                console.log(e.target.response)
                return alert(_xyz.language.here_error)
            }

            let coordinates = _xyz.utils.isoline_here_decode(e.target.response.isolines[0].polygons[0].outer)

            entry.newValue = {
                type: 'Polygon',
                coordinates: [coordinates.polyline.map(p => {return p.reverse()})]
            }
