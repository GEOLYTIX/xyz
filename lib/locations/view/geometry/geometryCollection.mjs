    ...
    entry.value.features.map(feature => {
        ...
        let f = _xyz.mapview.geoJSON({
            ...
            style: new ol.style.Style({
                image: _xyz.mapview.icon(Object.assign({}, style, {
                    scale: style.marker ? (style.marker.scale || 1) : 1,
                    type: style.marker ? (style.marker.type || 'target') : 'target', 
                    fillColor: _xyz.utils.Chroma(style.color || style.strokeColor).alpha(1)
                })),
