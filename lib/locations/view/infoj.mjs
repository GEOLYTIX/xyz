export default _xyz => location => {
  ...
  for (const entry of location.infoj) {
    ...
    if (entry.title) {
      ...
      entry.title_div = _xyz.utils.wire()`
      <div
    ...
    if (!entry.edit && !entry.displayValue && !entry.query) continue
    ...
    if (!entry.inline && !(entry.type === 'integer' ^ entry.type === 'numeric' ^ entry.type === 'date')) {
      ...
      entry.val = _xyz.utils.wire()`<div class="val ${entry.class || ''}" style="grid-column: 1 / 3;">`;
    ...
    // Else create val table cell inline.
      ...
      entry.val = _xyz.utils.wire()`<div class="val num ${entry.class || ''}" style="grid-column: 2;">`;
    ...
    if (entry.query) {
      ...
      entry.promise = _xyz.dataviews.query(entry);
      entry.promise.then(response => {

        entry.val.textContent = response[entry.key];
      });
