export default _xyz => location => {
  ...
  for (const entry of location.infoj) {
    ...
    // // Continue if the entry is not editable and empty.
    // if ((!entry.edit && !entry.value)

    //   // Or continue if the entry is not a query and empty.
    //   || (!entry.query && !entry.value)) continue
