export default function(params){
  ...
  function pointerMove(e) {
    ...
    mapview.Map.forEachFeatureAtPixel(e.pixel,
      (F, L) => {
        ...
        let candidate = {
          key: `${L.get('key') || ol.util.getUid(L)}!${F.get('id') || ol.util.getUid(F)}`,
          F, L
        }
  ...
  function highlight(feature, e) {
    ...
    // Set highlight flag on OL feature.
    typeof feature.F.set === 'function' && feature.F.set('highlight', true)
    ...
    interaction.current.layer.hover && interaction.current.layer.hover(feature.F)

    // Style the feature itself if possible.
    if (typeof interaction.current.F.setStyle === 'function') {
