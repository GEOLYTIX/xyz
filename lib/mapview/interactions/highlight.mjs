export default function(params){
  ...
  mapview.Map.on('click', singleClick)
  ...
  function finish() {
    ...
    mapview.Map.un('click', singleClick)
  ...
  function pointerMove(e) {
    ...
    mapview.Map.forEachFeatureAtPixel(e.pixel,
      (F, L) => {
        ...
        interaction.candidates[candidate.key] = candidate
      },
      {
    ...
    if (!Object.keys(interaction.candidates).length) {
    ...
    // console.log(Object.keys(interaction.candidates))
    // console.log(interaction.candidateKeys)

    // Check whether set of candidate keys is equal to interaction.candidateKeys
    if (mapp.utils.areSetsEqual(interaction.candidateKeys, new Set(Object.keys(interaction.candidates)))) {
    ...
    interaction.candidateKeys = new Set(Object.keys(interaction.candidates))

    // Find candidate from key which is not in candidates set.
    let candidate = interaction.candidates[Object.keys(interaction.candidates).find(key => !interaction.candidateKeys.has(key))]

    // Assign candidate from first key if candidate is falsy.
    candidate = candidate
      || interaction.current?.key && interaction.candidates[interaction.current?.key]
      || interaction.candidates[Object.keys(interaction.candidates)[0]]
  ...
  function highlight(feature, e) {
    ...
    if (e.type !== 'pointermove' && e.originalEvent.pointerType !== 'mouse') {
      interaction.getLocation(interaction.current)
      clear()
      return;
    }
    ...
    typeof interaction.current.layer.hover === 'function' && interaction.current.layer.hover(feature.F)
  ...
  function singleClick(e) {
    ...
    if (e.dragging) {
      drag && clearTimeout(drag)
      drag = setTimeout(()=>{
        e.originalEvent.pointerType === 'touch' && pointerMove(e)
      }, 400)
      return;
    }
    ...
    if (e.originalEvent.pointerType === 'touch') return pointerMove(e)
