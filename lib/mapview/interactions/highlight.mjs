...
export default function highlight(params) {
  ...
  */
  function pointerMove(e) {
    ...
    // TODO: evaluate getFeaturesAtPixel https://openlayers.org/en/latest/apidoc/module-ol_Map-Map.html#getFeaturesAtPixel
    ...
    if (!Object.keys(mapview.interaction.candidates).length) {
    ...
    //console.log(Object.keys(mapview.interaction.candidates));

    if (
      mapp.utils.areSetsEqual(
        mapview.interaction.candidateKeys,
        new Set(Object.keys(mapview.interaction.candidates)),
      )
    ) {
    ...
    let feature =
      mapview.interaction.candidates[
        Object.keys(mapview.interaction.candidates).find(
    ...
    feature ??=
      ...
      mapview.interaction.candidates[mapview.interaction.current?.key];

    feature ??=
      mapview.interaction.candidates[
        Object.keys(mapview.interaction.candidates)[0]
      ];

    // Assign new Set of candidate keys to mapview.interaction.
    mapview.interaction.candidateKeys = new Set(
      Object.keys(mapview.interaction.candidates),
    );
  ...
  function featureCallback(F, L) {
    ...
    mapview.interaction.candidates[key] = { F, key, L };
  }
