...
export default function decorate(mapview) {
  ...
  const initialZoom =
    mapp.hooks?.current?.z || mapview.view?.z || mapview.locale.minZoom || 0;
  ...
  if (mapview.locale.showScaleBar) {
    console.warn(
      'locale.showScaleBar is deprecated. Use locale.ScaleLine.metric or locale.ScaleLine.imperial instead.',
    );
  ...
  if (mapview.locale.scalebar) {
    console.warn(
      'locale.scalebar is deprecated. Use locale.ScaleLine.metric or locale.ScaleLine.imperial instead.',
    );
