...
import renderComplete from './renderComplete.mjs'
...
export default (mapview) => {
  ...
  const muObserver = new MutationObserver(()=>{
    if (document.body.contains(mapview.target)) {
      console.log('existe')
      muObserver.disconnect();
    }
  });
  muObserver.observe(document.body, { childList: true });
