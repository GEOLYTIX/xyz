export default _xyz => layer => {
  ...
  function loader(extent) {
    ...
    layer.xhr.onload = (e) => {
      ...
      for (var i = 1; i < n; i++) {
        if (i < n / 2) {
          layer.colorBins.push(
            layer.colorMin + ((layer.colorAvg - layer.colorMin) / (n / 2)) * i
          );
        }
        ...
        if (i > n / 2) {
          layer.colorBins.push(
            layer.colorAvg +
              ((layer.colorMax - layer.colorAvg) / (n / 2)) * (i - n / 2)
          );
        }
      }
      ...
      if (layer.style.legend.size_min)
        layer.style.legend.size_min.textContent = layer.sizeMin.toLocaleString(
          "en-GB",
          { maximumFractionDigits: 0 }
        );
      if (layer.style.legend.size_avg)
        layer.style.legend.size_avg.textContent = layer.sizeAvg.toLocaleString(
          "en-GB",
          { maximumFractionDigits: 0 }
        );
      if (layer.style.legend.size_max)
        layer.style.legend.size_max.textContent = layer.sizeMax.toLocaleString(
          "en-GB",
          { maximumFractionDigits: 0 }
        );
      ...
      if (layer.grid_ratio) {
        if (layer.style.legend.color_min)
          layer.style.legend.color_min.textContent =
            layer.colorMin.toLocaleString("en-GB", {
              maximumFractionDigits: 0,
              style: "percent",
            });
        if (layer.style.legend.color_avg)
          layer.style.legend.color_avg.textContent =
            layer.colorAvg.toLocaleString("en-GB", {
              maximumFractionDigits: 0,
              style: "percent",
            });
        if (layer.style.legend.color_max)
          layer.style.legend.color_max.textContent =
            layer.colorMax.toLocaleString("en-GB", {
              maximumFractionDigits: 0,
              style: "percent",
            });
      } else {
