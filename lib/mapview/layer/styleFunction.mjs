export default _xyz => layer => feature => {
  ...
  const Z = _xyz.mapview.getZoom()

  const properties = feature.getProperties()

  if(layer.featureLookup) { // modify properties
  ...
  //const style = _xyz.utils.cloneDeep(_style)
  ...
  if (geometryType === 'Point') {
    ...
    style.icon = _xyz.utils.cloneDeep(style.icon || style.marker || style)
  ...
  if (theme && theme.type === 'categorized') {
    ...
    var catStyle = theme.cat && _xyz.utils.cloneDeep(theme.cat[catValue]?.style || theme.cat[catValue])
    ...
    if (catStyle) {
      ...
      catStyle.icon = geometryType === 'Point' && (catStyle.icon || catStyle.marker || catStyle)
  ...
  if (theme && theme.type === 'graduated') {
    ...
    if (catValue || catValue === 0) {
      ...
      for (let i = 0; i < theme.cat_arr.length; i++) {
        ...
        var catStyle = _xyz.utils.cloneDeep(theme.cat_arr[i].style || theme.cat_arr[i]);
      }
  ...
  if (style.label) {
    ...
    style.label.text = properties.label || style.label.count && properties.count > 1 && properties.count

    !style.label?.display && delete style.label

    style.label?.minZoom > Z && delete style.label

    style.label?.maxZoom < Z && delete style.label
  }
