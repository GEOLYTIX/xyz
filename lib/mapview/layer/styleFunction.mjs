export default _xyz => layer => feature => {
  ...
  if (layer.highlight === feature.get('id')){
    ...
    style = Object.assign(style, properties?.metadata?.highlighStyle || {})
    _xyz.utils.merge(style, highlightStyle)
  }else{
    if(layer.name === 'Custom'){
      style = Object.assign(style, properties?.metadata?.style || {})
    }
  ...
  // if(layer.name === 'Custom'){
  //   console.log({feature, layer, properties, style});
  //   // style.fillColor = "#935df0"
  //   // style.strokeColor = "#e600ff"
  // }
