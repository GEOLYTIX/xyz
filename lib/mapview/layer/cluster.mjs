  ...
  layer.L = new _xyz.mapview.lib.layer.Vector({
    ...
    style: feature => {
      ...
      const size = layer.cluster_logscale ?
        properties.count === 1 ?
          (marker.markerMin || layer.style.markerMin) :
          (marker.markerMin || layer.style.markerMin) + (marker.markerMax || layer.style.markerMax) / Math.log(layer.max_size) * Math.log(properties.size) :
        properties.count === 1 ?
          (marker.markerMin || layer.style.markerMin) :
          (marker.markerMax || layer.style.markerMin) + (marker.markerMax || layer.style.markerMax) / layer.max_size * properties.size;
