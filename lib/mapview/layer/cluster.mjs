  ...
  layer.L = new ol.layer.Vector({
    ...
    style: feature => {
      ...
      const marker = properties.size > 1 ?
        Object.assign({}, layer.style.cluster) :
      ...
      const size = layer.cluster_logscale ?
        properties.count === 1 ?
          ...
          (marker.size || layer.style.size) + (1.2*marker.size || 1.2*layer.style.size) / Math.log(layer.max_size) * Math.log(properties.size) :
        properties.count === 1 ?
          (marker.size || layer.style.size) :
          (1.2*marker.size || layer.style.size) + (1.2*marker.size || 1.2*layer.style.size) / layer.max_size * properties.size;
