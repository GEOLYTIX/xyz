  ...
  const source = new ol.source.Vector({
    loader: function (extent, resolution, projection) {
      ...
      layer.xhr.open(
        'GET',
        _xyz.host +
          `/api/layer/cluster/${_xyz.mapview.getZoom()}?` +
          _xyz.utils.paramString({
            ...
            theme:
              ...
              layer.style.themes &&
              encodeURIComponent(
                Object.keys(layer.style.themes).find(
                  (k) => layer.style.themes[k] === layer.style.theme
                )
              ),
      ...
      layer.xhr.onload = (e) => {
        ...
        layer.max_size = cluster.reduce(
          (max_size, f) => Math.max(max_size, f.properties.size),
          0
        );

        let id = 1;

        const features = cluster.map(
          (f) =>
            new ol.Feature({
              id: id++,
  ...
  layer.L = new ol.layer.Vector({
    ...
    style: (feature) => {
      ...
      if (theme && theme.type === 'categorized') {
        if (marker.type === 'clusterSummary') {
          Object.assign(marker, { properties, theme: layer.style.theme });
        } else {
          Object.assign(
            marker,
        ...
        Object.entries(properties.cat)
          ...
          .forEach((comp) => {
            ...
            if (theme.cat[comp[0]]) {
              ...
              cat_style.layers[size / properties.size] =
                theme.cat[comp[0]].style.fillColor;
            }
      ...
      let size = layer.cluster_logscale
        ? properties.count === 1
          ...
          : (marker.size || layer.style.size) +
            ((marker.size || layer.style.size) / Math.log(layer.max_size)) *
              Math.log(properties.size)
        : properties.count === 1
        ? marker.size || layer.style.size
        : (marker.size || layer.style.size) +
          ((marker.size || layer.style.size) / layer.max_size) *
            properties.size;
        ...
        return marker.layers.map((l) => {
          return new ol.style.Style({
            ...
            image: _xyz.mapview.icon(
              Object.assign({}, l, {
                scale:
                  (size / (l.size || layer.style.size)) *
                  (l.scale || 1) *
                  (l.relative
                    ? _xyz.mapview.getZoom() /
                      (_xyz.locale.maxZoom - _xyz.locale.minZoom)
                    : 1),
              })
            ),
          });
      ...
      return new ol.style.Style({
        ...
        image: _xyz.mapview.icon(
          Object.assign({}, marker, {
            scale:
              (size / (marker.size || layer.style.size)) *
              (marker.scale || 1) *
              (marker.relative
                ? _xyz.mapview.getZoom() /
                  (_xyz.locale.maxZoom - _xyz.locale.minZoom)
