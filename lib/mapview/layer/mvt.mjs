...
export default _xyz => layer => {
  ...
  layer.reload = () => {
    ...
    if (layer.style.theme && layer.style.theme.type === 'dynamic') {
  ...
  source.on('tileloadstart', function () {
    ...
    if (layer.style.theme && layer.style.theme.type === 'dynamic' && layer.moveStart) {
  ...
  function styleFn(feature) {
    ...
    // Dynamic theme.
    if (theme && theme.type === 'dynamic' && theme.cat_arr) {
      ...
      if (value || value === 0) {
        ...
        // Iterate through cat array.
        for (let i = 1; i < theme.series.length - 1; i++) {

          // Break iteration is cat value is below current cat array value.
          if (value < theme.series[i]) break;
