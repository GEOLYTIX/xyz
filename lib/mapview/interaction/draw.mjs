export default _xyz => {
  ...
  function begin(params) {
    ...
    draw.style = params.drawStyle || draw.style
    ...
    if (params.snapSource) {
      ...
      _xyz.map.addInteraction(draw.snapInteraction)
    }
  ...
  function finish() {
    ...
    if (draw.snapInteraction) {
      _xyz.map.removeInteraction(draw.snapInteraction)
  ...
  function contextmenu(e) {
    ...
    if (features.length) menu.appendChild(_xyz.utils.html.node`
      <li style="padding: 5px;" class="off-white-hover" onclick=${draw.update}>${_xyz.language.layer_draw_save}</li>`);
    ...
    if (!features.length) menu.appendChild(_xyz.utils.html.node`
      <li style="padding: 5px;" class="off-white-hover" onclick=${e=>{
        ...
        draw.vertices.pop();
        _xyz.mapview.popup.node && _xyz.mapview.popup.node.remove();
      }}>${_xyz.language.layer_draw_last}</li>`);
