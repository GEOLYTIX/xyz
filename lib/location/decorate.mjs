...
async function update() {
  ...
  const newValues = Object.fromEntries(this.infoj
    .filter(entry => typeof entry.newValue !== 'undefined' && entry.type !== 'json_field')
    .map(entry => [entry.field,entry.newValue]))

  // Map new JSON values int0 newJSONValues object
  const newJsonValues = Object.fromEntries(this.infoj
    .filter(entry => typeof entry.newValue !== 'undefined' && entry.type === 'json_field')
    .map(entry => [entry.field,entry.newValue]))
  ...
  Object.keys(newJsonValues).length > 0 && promises.push(location_update(this,'location_update_json',newValues))
