...
function remove() {
  ...
  this.Layers?.forEach((L) => this.layer.mapview.Map.removeLayer(L));
...
The features from the layers that been added to the Layers property are added to a Vector,
The extent of that Vector is used in {@link module:/mapview/fitView}. 
...
async function flyTo(maxZoom) {
  ...
  let feature_extent = [0, 0, 0, 0];

  for (const layer of location.Layers) {
    const source = layer.getSource();

    //Features for wkt layers
    if (typeof source.getFeatures === 'function') {
      sourceVector.addFeatures(source.getFeatures());
    } else {
      //Get the layer from the mapview
      const map_layer = location.layer.mapview.layers[layer.values_.key];
      if (Array.isArray(map_layer.featureSet || map_layer.featureLookup)) {
        const ol_extent = await layer.getFeatureExtent();

        //Compare the extent to the current one, use the bigger one
        if (ol.extent.getArea(ol_extent) > ol.extent.getArea(feature_extent)) {
          feature_extent = ol_extent;
        }
      }
    }
  }

  //Use the feature extent if its bigger
  if (
    ol.extent.getArea(sourceVector.getExtent()) <
    ol.extent.getArea(feature_extent)
  ) {
    location.layer.mapview.fitView(feature_extent, {
      maxZoom,
    });
    return;
  }

  location.layer.mapview.fitView(sourceVector.getExtent(), {
    maxZoom,
  });
}
