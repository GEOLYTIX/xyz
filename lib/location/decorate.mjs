...
async function getExtent(location) {
  ...
  for (const layer of location.Layers) {
    ...
    if (typeof source.getFeatures === 'function') {
      ...
      if (
        ol.extent.getArea(ol_extent.box2d_arr) >
        ol.extent.getArea(feature_extent)
      ) {
        feature_extent = ol_extent.box2d_arr;
      }
    }
  }

  //Return the largest extent
  return ol.extent.getArea(sourceVector.getExtent()) >
    ol.extent.getArea(feature_extent)
    ? sourceVector.getExtent()
    : feature_extent;
}
...
async function flyTo(maxZoom) {
  ...
  const flyToExtent = getExtent(location);
