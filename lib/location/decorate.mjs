...
async function update() {
  ...
  for (const entry of this.infoj.filter(entry => typeof entry.newValue !== 'undefined')) {

    if (entry.type === 'integer') {
      entry.newValue = isNaN(parseInt(entry.newValue)) ? null : parseInt(entry.newValue);
    }

    if (entry.type === 'numeric') {
      entry.newValue = isNaN(parseFloat(entry.newValue)) ? null : parseInt(entry.newValue);
    }    

    newValues[entry.field] = entry.newValue
  }
