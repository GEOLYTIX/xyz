...
export default async function ping(params) {
  return new Promise((resolve, reject) => {
    ...
    // Default field to model_status if not provided
    params.field ??= 'model_status';
    ...
    // Default query to ping_status if not provided
    params.query ??= 'ping_status';
    ...
    const checkStatus = async () => {
      ...
      // If the ping stage is error, reject the promise
      if (pingStage === params.stages.error) {
        // Handle error case
      ...
      // You can also resolve with the ping stage if needed
      return pingStage;
    };
