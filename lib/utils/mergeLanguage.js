...
export async function mergeLanguage(languageCode) {
  try {
    ...
    // If the languageCode already exists, return 
    if (Object.hasOwn(mapp.dictionaries, languageCode)) return;

    // Dynamically import the JSON language file
    const languageData = await import(`../../public/dictionaries/${languageCode}.json`, {
      assert: { type: "json" }
    });
