...
export function xhr(params) {
  return new Promise((resolve, reject) => {
    ...
    params.xhr.open(params.method, params.url, true);
    ...
    if (params.requestHeader !== null) {
      ...
      Object.entries(headers).forEach(([k, v]) =>
        params.xhr.setRequestHeader(k, v),
      );
    }

    params.xhr.responseType = params.responseType;
    ...
    params.xhr.onload = (e) => {
      ...
      if (params.xhr.status >= 400) {
        return reject(new Error(`HTTP ${params.xhr.status}`));
      }

      const result = params.resolveTarget ? params.xhr : params.xhr.response;

      if (params.cache) {
        requestMap.set(
          params.url,
          params.resolveTarget ? params.xhr : { response: result },
        );
      }
...
function getDebounceConfig(params) {
  ...
  if (params.debounce === true) {
    return { key: 'global', delay: params.delay };
  }

  return params.debounce;
}
...
function handleDebounce(params, config, resolve, reject) {
  ...
  const timeoutId = setTimeout(() => {
    ...
    params.debounce = false;

    xhr(params).then(resolve).catch(reject);
  }, delay);
