...
export function numericFormatter(params) {
    ...
    // to retire: allow to format other numbers than integers
    //if(params?.type !== 'integer')  return params.value;

    // If formatter is money use en-GB locale
    // to retire. toLocaleString is more flexible
    /*if (params.formatter === 'money') {
        params.formatterParams ??= {}
        params.formatterParams.locale = 'en-GB'
    };*/

    /*if(!params.formatterParams?.locale) { // retire: assigned earlier
        return params.value;
    }*/
...
function getFormatterParams(params) {
    ...
    // Assign if nullish.
    //params.formatterParams ??= {}; // done earlier by default
  
    // Assign user language as locale if nullish but not null.
    /*if (params.formatterParams.locale !== null || params.formatter === 'toLocaleString') {
