...
export function numericFormatter(params) {
    ...
    //do nothing if lcoale is present but null
...
function getFormatterParams(params) {
    ...
    if (params.formatterParams?.decimal || params.formatterParams?.thousand) {
      ...
      rawValue = params.formatterParams.locale ? parseFloat(value).toLocaleString(params.formatterParams.locale, params.formatterParams.options) : parseFloat(value).toLocaleString(params.formatterParams.options)
    }
    ...
    let localeSeparators = Array.from(new Set(`${rawValue}`.match(/\D/g)))
...
export function getSeparators(params) {
    ...
    if (Object.keys(params?.formatterParams).includes('locale') && !params.formatterParams.locale) return { decimals: '.'}
