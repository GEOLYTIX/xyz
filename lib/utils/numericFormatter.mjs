...
export function numericFormatter(params) {
    ...
    if(!params.formatterParams?.locale) return params.value;

    //Do the opposite of formatting
    if (params.undo) {
        return undoFormatting(params)
    }
...
function getFormatterParams(params) {
    ...
    if (params.formatterParams?.decimal || params.formatterParams?.thousand) {
      ...
      let rawList = rawValue.toLocaleString('en-GB', params.formatterParams.options).split('.')
