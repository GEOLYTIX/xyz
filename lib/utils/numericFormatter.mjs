...
@param {Object} params The config object argument.
@property {String} params.suffix Suffix to be sliced from stringValue.
@property {String} params.prefix Prefix to be substringed from stringValue.
...
export function unformatStringValue(params) {
  ...
  if (stringValue.length && params.prefix.length) {

    stringValue = stringValue.substring(params.prefix.length)
  }

  if (stringValue.length && params.suffix.length) {

    stringValue = stringValue.slice(0, params.suffix.length)
  }

  if (stringValue.length && params.formatterParams?.locale) {

    const parts = (1234.5).toLocaleString(params.formatterParams?.locale).match(/(\D+)/g);

    stringValue = stringValue.split(parts[0]).join("");
    stringValue = stringValue.split(parts[1]).join(".");
  }
