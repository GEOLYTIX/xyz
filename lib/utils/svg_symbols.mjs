export default style => {
  ...
  const markers = {
    ...
    circle,
    diamond,
    clusterSummary: () => clusterSummary(style),
    commonplacePin,
  };
  ...
  function legend(style) {
    ...
    function make(m) {
      ...
      return node.appendChild(
        html.node`<img style="${css}" src="${markers[m.type](marker)}"/>`
      );
    }
  ...
  return markers[marker.type](marker);
};
...
function clusterSummary(style) {
  console.log(style);
  if (!style.properties) return null;
  const { cat } = style.properties;
  if (!cat) return null
  ...
  <svg width=24 height=24 viewBox='0 0 30 30' xmlns='http://www.w3.org/2000/svg'>
    <circle cx=15 cy=15 r=10
      stroke="${style.theme.cat[avg].style.strokeColor || '#333'}"
      stroke-width="${style.strokeWidth || 1}"
      stroke-opacity="${style.strokeOpacity || 1}"
      fill="${style.theme.cat[avg].style.strokeColor || 'none'}"></circle>`;
    ...
    const getStrokeColor = (value) => {
      return style.theme.cat[value].style.strokeColor;
    };
    ...
    const getDashArray = (value) => {
      const precedingSegments = Object.keys(style.theme.cat).reduce(
