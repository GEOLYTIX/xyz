...
function target(style) {
  ...
  style.layers && Object.entries(style.layers).forEach(layer => {
    ...
    icon.appendChild(svg.node`<circle cx=10 cy=10 r=${parseFloat(layer[0]) * 10} fill=${layer[1]}>`)
...
function triangle(style) {
  ...
  let shade = svg.node`<path fill='#333' opacity=0.4>`;

  shade.setAttribute('d', t);
  shade.setAttribute('transform', `translate(${r + 40} ${r + 20}) scale(${scale}, ${scale})`);

  _svg.appendChild(shade);

  let path = svg.node`<path fill=${style.fillColor || '#FFF'}>`;

  path.setAttribute('d', t);
  path.setAttribute('transform', `translate(${r} ${r}) scale(${scale}, ${scale})`);
  _svg.appendChild(path);
  
  if(!style.layers) return 'data:image/svg+xml,' + encodeURIComponent(xmlSerializer.serializeToString(_svg));

  Object.entries(style.layers).map(layer => {

    const _layer = svg.node`<path fill=${layer[1]}>`;
  ...
  return 'data:image/svg+xml,' + encodeURIComponent(xmlSerializer.serializeToString(_svg));
...
function square(style) {
  ...
  <svg width=24 height=24 viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'>
    ...
    <rect fill=${style.fillColor || '#FFF'} width=20 height=20 x=0 y=0></rect>`
  ...
  style.layers && Object.entries(style.layers).forEach(layer => {
    ...
    icon.appendChild(svg.node`<rect fill=${layer[1]}
      ...
      y=${10*(1-parseFloat(layer[0]))}></rect>`)
  })
