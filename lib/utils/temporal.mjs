 ...
 */
function dateString({ value, newValue, locale = DEFAULT_LOCALE, options }) {
  if (!value && !newValue) {
    throw new Error('Either value or newValue must be provided');
  }
  const timestamp = newValue || value;
 ...
 */
function datetime({ value, newValue }) {
  const timestamp = newValue || value;
  const date = toJSDate(timestamp);
