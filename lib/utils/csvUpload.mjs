...
export default async function csvUpload(file, params = {}) {
  ...
  return new Promise((resolve) => {
    ...
    reader.onload = async (e) => {
      ...
      console.log(`Field count: ${Object.keys(params.fields).length}`);
      ...
      if (columnCount !== Object.keys(params.fields).length) {
        ...
        resolve([
          new Error(
            `The number of columns in the CSV file (${columnCount}) does not match the number of fields supplied (${Object.keys(params.fields).length}). Please check your CSV file and try again.`,
          ),
        ]);
        return;
      }
