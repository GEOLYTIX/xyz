...
export default async function csvUpload(file, params = {}) {
  return new Promise((resolve) => {
    // Query is required to define how to insert the data.
    if (!params.query) {
      // Should return an error if query is not supplied.
      resolve([new Error(`A query must be supplied for the csv upload.`)]);
      return;
    }

    // Fields is required to split the CSV file into arrays of values for each field using the splitRowIntoFields method.
    // This is used to ensure the CSV is the right length and that the data is formatted correctly.
    if (!params.fields) {
      // Should return an error if fields are not supplied.
      resolve([
        new Error(
    ...
    // A layer must be supplied for the insert query. This is only required if you are using the sql_table_insert query.
    // This is as the sql_table_insert query requires a layer to determine the permissions for the insert query.
    // If using a custom query, this is not a requirement.
    if (params.query === 'sql_table_insert' && !params.layer) {
      resolve([new Error(`A layer must be supplied for the insert query.`)]);
      return;
    }

    // A table must be supplied for the insert query. This is only required if you are using the sql_table_insert query.
    // This is as the the sql_table_insert query requires a table to insert the data into.
    // If using a custom query, this is not a requirement.
    if (params.query === 'sql_table_insert' && !params.queryparams.table) {
      resolve([
        new Error(
    ...
    reader.onload = async (e) => {
      ...
      // Check the number of columns in the csv vs the number of fields supplied.
      const columnCount = splitRowIntoFields(rows[0]).length;
