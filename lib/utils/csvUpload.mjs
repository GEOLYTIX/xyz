 ...
 */ // Uploads can be async chunks or sync chunks.
...
export default async function csvUpload(file, params = {}) {
  ...
  return new Promise((resolve) => {
    ...
    reader.onload = async (e) => {
      ...
      if (params.headerCheck && headers.length != params.schema.length) {
        ...
        outcome.message = `${
          mapp.dictionary.csv_upload_number_of_columns_imported
        } (${headers.length})
             ${mapp.dictionary.csv_upload_number_of_columns_required} 
                (${params.schema.length}). ${
          params.headerCheckErrorMessage || ''
        }`;
      ...
      rows.forEach((_row) => {
        ...
        const row = splitCSVIntoRow(_row, params);

        // Determine blob size of the row.
        const rowSize = new Blob([row]).size;

        // Check whether the chunk would exceed lambda payload limit.
        if (chunkSize + rowSize >= (params.chunkSize || 1024 * 1024 * 4)) {
          // Call the processChunk to upload the chunk.
          processChunk(promises, chunk, params);
      ...
      try {
        ...
        if (params.async) {
          await Promise.all(
            chunks.map((chunk) => processChunk(promises, chunk, params))
          );
        }
...
*/
function splitCSVIntoRow(row, params) {
  ...
  return `(${fields.map((v, i) => schemaMethods[params.schema[i]](v)).join()})`;
}
 ...
 * @param {Object} params - The parameters object.
 */
function processChunk(promises, chunk, params) {
  ...
  const paramString = mapp.utils.paramString(
    mapp.utils.queryParams(queryObject)
  );
