...
export function versionCheck(value) {
  const majorVersion = Number.parseInt(mapp.version.split('.')[0]);

  const minorVersion = Number.parseInt(mapp.version.split('.')[1]);

  const patchVersion = Number.parseInt(mapp.version.split('.')[2]);

  const majorValue = Number.parseInt(value.toString().split('.')[0]);

  const minorValue = Number.parseInt(value.toString().split('.')[1]);

  const patchValue = Number.parseInt(value.toString().split('.')[2]) || 0;
