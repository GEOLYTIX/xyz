...
@param {Object} locale The locale object of the mapview.
*/
export function keyvalue_dictionary(locale) {
  if (!locale.keyvalue_dictionary) return;

  // Create a Map from dictionary object.
  const keyvalue_dictionaryMap = new Map();

  locale.keyvalue_dictionary.forEach((entry) => {
    const compositeKey = `${entry.key}:${entry.value}`;
    keyvalue_dictionaryMap.set(compositeKey, entry);
  });

  parseObject(locale, keyvalue_dictionaryMap);
}
