export default _xyz => {
  ...
  function table(dataview) {
    ...
    dataview.setData = data => {
      ...
      nodataMask && nodataMask.remove()

      if (!dataview.noDataMask && !data) {
        dataview.dataview.style.display = 'none'
        nodataMask = dataview.dataview.parentElement.appendChild(_xyz.utils.html.node`<div
          style="width: 100%; height: 100%; text-align: center; line-height: 2">No Data`)
      } else {
        dataview.dataview.style.display = 'block'
      }

      // Tabulator data must be an array.
      data = (!dataview.noDataMask && !data) && [] || data.length && data || [data]

      // Process data in a response function
      data = dataview.responseFunction && dataview.responseFunction(data) || data
      
      // Set data to the tabulator object
      dataview.Tabulator.setData(data)
