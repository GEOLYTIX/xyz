export default layer => feature => {
  ...
  if (layer.style.filter && Object.keys(layer.style.filter)) {

    if (Object.keys(layer.style.filter).some(key => {

      let set = new Set(layer.style.filter[key].ni)

      return set.has(feature.properties[key])
