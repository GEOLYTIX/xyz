...
async function getExtent() {
  let ids = [];
  if (Array.isArray(this.featureLookup)) {
    // Get array of ids from lookup features.
    ids = this.featureLookup.map(
      (feature) => feature[this.featureLookupId || 'id'],
    );
  } else if (Array.isArray(this.featureSet) || this.featureSet instanceof Set) {
    ids = [...this.featureSet];
  }
  ...
  const response = await mapp.utils.xhr({
    url:
      ...
      mapp.utils.paramString({
        ...
        geom: this.geomCurrent(),
        layer: this.source_key || this.key,
  ...
  // Check if response is invalid: Not an array, empty array, or contains null
  if (
...
/**
@function promoteDisplay

@description
Called from the layer.show() method the promoteDisplay method will shortcircuit if the layer does not have the promoteDisplay flag.

Otherwise the method will iterate through the mapview.layers{} to capture the highest zIndex of any displayed layer.

The captured zIndex will be increased by 1 and set on the Openlayers layer.L to ensure that this layer is rendered on top.

@param {layer} layer
@property {boolean} layer.promoteDisplay The layer should be drawn on top.
*/
function promoteDisplay(layer) {
  if (!layer.promoteDisplay) return;
  let zIndex = 0;
  for (const otherLayer of Object.values(layer.mapview.layers)) {
    if (!otherLayer.display) continue;

    if (otherLayer.L.getZIndex() < zIndex) continue;

    zIndex = otherLayer.L.getZIndex() + 1;
  }
  layer.L.setZIndex(zIndex);
}
...
function tableCurrent() {
  ...
  const zoom = Number.parseInt(this.mapview.Map.getView().getZoom());

  // Get zoom level keys from layer.tables object.
  const zoomKeys = Object.keys(this.tables);

  // Get first zoom level key from array.
  const minZoomKey = Number.parseInt(zoomKeys[0]);
  const maxZoomKey = Number.parseInt(zoomKeys.at(-1));
...
async function zoomToExtent(params) {
  const response = await this.getExtent();

  // getExtent() will return false if no extent is available for the layer.
  if (response === false) {
    return false;
  }
...
async function update(jsonLayer) {
  jsonLayer.mapview = this.mapview;

  const newLayer = await mapp.this.decorate(jsonLayer);
  ...
  if (layersnode) {
    ...
    layersnode?.insertBefore(newLayer.view, this.view);
  }

  this.remove();

  newLayer.mapview.layers[this.key] = newLayer;
...
function remove() {
  ...
  delete this.mapview.layers[this.key];
}
