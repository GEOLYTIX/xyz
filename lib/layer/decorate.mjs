...
export default async function decorate(layer) {
  ...
  layer.mapview.changeEndCallbacks.push(layer);
...
@description
The layer.changeEnd method will be called from the mapview.changeEndCallbacks. The method will check whether a layer should be displayed on the current zoom level.
...
function remove() {
  ...
  const idx = layer.mapview.changeEndCallbacks.findIndex(fn => fn.key === layer.key)

  if (idx > -1) {
    layer.mapview.changeEndCallbacks.splice(idx, 1);
  }
