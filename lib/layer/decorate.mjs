...
export default async function decorate(layer) {
  ...
  if (layer.featureSet || layer.featureLookup) {
    //Assign the feature extent function
    layer.getExtent = () => getExtent(layer);
  }
...
*/
async function getExtent(layer) {
  if (!Array.isArray(layer.featureLookup || layer.featureSet)) return;

  const ids =
    layer.featureLookup?.map?.((feature) => {
      return feature[layer.featureLookupId || 'id'];
    }) || layer.featureSet;
...
function show() {
  ...
  try {
    ...
    //Assign the extent function to the location
...
async function zoomToExtent(params) {
  ...
  if (Array.isArray(layer.featureSet || layer.featureLookup)) {
    response = await layer.getExtent();
  } else {
