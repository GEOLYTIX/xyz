...
@description
A function for asigning a set of IDs to a layer which has `featureSet: true` set. 

Calls a query which returns all the ids currently present on the layer.

@param {object} layer JSON layer.
@property {object} layer.mapview The mapview associated with the layer.
@property {object} layer.filter The filter on the layer.
@property {string} layer.key The layers identifier.
@property {string} layer.qID The name of the layers id field.
@property {string} layer.table The table the layer uses.

@returns {layer} the layer with its new featureSet.
*/
async function setFeatureIDs(layer) {
  const layerFeatures = await mapp.utils.xhr(
    `${layer.mapview.host}/api/query/feature_ids?` +
  ...
  layer.featureSet = new Set(layerFeatures.map((feature) => feature.id));
...
export default async function decorate(layer) {
  ...
  if (layer.featureSet) {
    if (Array.isArray(layer.featureSet)) {
      ...
      //Get featureSet ids from a query if the featureSet was supplied as true.
      setFeatureIDs(layer);

      //Compose the setFeatureIds with the filter reload to change the list as filters are applied.
      mapp.ui.layers.filters.reloadCallback = mapp.utils.compose(
        mapp.ui.layers.filters.reloadCallback,
        setFeatureIDs,
      );
    }
  }
...
async function zoomToExtent(params) {
  ...
  let response;
  if (layer.featureSet) {
    const featureIDs = Array.from(layer.featureSet);

    response = await mapp.utils.xhr({
      url:
        `${layer.mapview.host}/api/query/feature_extent?` +
        mapp.utils.paramString({
          filter: layer.filter.current,
  ...
  } else if (Array.isArray(layer.featureLookup)) {
    const featureIDs = layer.featureLookup.map(
      (f) => f[layer.featureLookupId || 'id'],
    );
