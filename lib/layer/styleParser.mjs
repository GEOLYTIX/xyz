...
export default layer => {
  ...
  function clusterChecks(layer) {
    ...
    // We need to check if the layer.cluster.label is using ID - as this will cause issues with the cluster layer.
    if (layer.cluster.label === 'id') {

      console.warn(`Cluster Layer: ${layer.key}; cluster.label cannot be use 'id'. cluster.label will be removed.`)

      delete layer.cluster.label
    }
  }
