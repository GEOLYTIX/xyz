...
function clusterChecks(layer) {
  ...
  if (Array.isArray(layer.style.cluster.icon)) {
    if (layer.style.cluster.icon.length === 0) {
      layer.style.cluster.icon = layer.style.cluster.icon[0];
    } else {
      return;
    }
  }

  layer.style.cluster.icon.url ??= layer.style.cluster.icon.svg;

  delete layer.style.cluster.icon.svg;
}
...
function catStyle(cat, layer) {
  ...
  if (cat.style.icon) {
    ...
    cat.style.icon.url ??= cat.style.icon.svg;

    delete cat.style.icon.svg;
  ...
  if (!cat.style.icon && defaultStyle.icon) {
    console.warn(
      `Layer:${layer.key}, Failed to evaluate icon: ${JSON.stringify(cat)}. Default icon will be assigned.`,
    );
...
function iconObject(style) {
  ...
  if (style.icon.svg) {
    style.icon.url = style.icon.svg;
  }

  if (!style.icon.url && !style.icon.type) {
    console.warn(
      `${JSON.stringify(style)}: Icon style is missing url or type definition.`,
    );
