...
export default layer => {
  ...
  function parseTheme(theme, layer) {
    ...
    if (typeof theme.cat === 'object') {
      ...
      Object.keys(theme.cat).map(key => {
        key = theme.cat[key]
        if (Array.isArray(key.style.icon)){
          key.style.icon.forEach(icon => {
            if(icon.svg){
              if(svgUrls[icon.svg] && !checkedUrls[icon.svg]){
                let name = icon.svg.substring(icon.svg.lastIndexOf('/')+1,icon.svg.indexOf('.svg'))
                console.warn(`${icon.svg} is used multiple times. This will cause unexpected behaviour, it should be added as an svg template e.g "${name}":"${icon.svg}" `)
                checkedUrls[icon.svg] = 1
              }
              else{
                svgUrls[icon.svg] = 1
              }
        ...
        else if (key.style.icon.svg){
          if(svgUrls[key.style.icon.svg] && !checkedUrls[icon.svg]){
            let name = key.style.icon.svg.substring(key.style.icon.svg.lastIndexOf('/'),key.style.icon.svg.indexOf('.svg'))
            console.warn(`${key.style.icon.svg} is used multiple times. This will cause unexpected behaviour, it should be added as an svg template e.g "${name}":"${key.style.icon.svg}" `)
            checkedUrls[key.style.icon.svg] = 1
          }
          else{
            svgUrls[key.style.icon.svg] = 1
          }  
        }
        return
      })
