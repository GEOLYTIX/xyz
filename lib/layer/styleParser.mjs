...
export default layer => {
  ...
  function warnings(layer) {
    ...
    if (layer.cluster) {
      ...
      if (layer.cluster.distance < 1 && layer.format === 'wkt') {

        console.warn(`Layer: ${layer.key}, cluster.distance is less than 1 and format is wkt. Clustering is in pixels. Either remove the cluster object to have no clustering, or set the cluster value to an integer of more than 1.`)
      }
  ...
  function parseTheme(theme, layer) {
    ...
    if (Array.isArray(theme.categories)) {
      ...
      if (theme.type === 'graduated' && (!theme.graduated_breaks || !theme.graduated_breaks==='less_than' || !theme.graduated_breaks==='greater_than')) {

        console.warn(`You must provide a graduated_breaks value for graduated theme on layer: ${layer.key}; field: ${theme.field}. less_than is assumed.`)
  ...
  */
  function iconObject(style) {
    ...
    // The style can not be an icon.
    if (!style.url || !style.svg || !style.type) {
      return;
    }
