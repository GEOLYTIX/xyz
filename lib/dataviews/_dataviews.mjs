  ...
  function create(dataview) {
    ...
    dataview.update = () => {
      ...
      if (dataview.active || dataview.tab && dataview.tab.classList.contains('active')) {
        _xyz.query(dataview).then(response => dataview.setData(response));
      }
    ...
    if (dataview.chart) {
      ...
      dataview.chart.setData = response => {
        ...
        dataview.chart.ChartJS.data = {
          labels: typeof dataview.chart.labels == 'function' && dataview.chart.labels(response) || dataview.chart.labels,
          datasets: dataview.chart.datasets.map((dataset, i) => Object.assign(dataset, {
            data: getdata(dataset, i)
          }))
        }

        function getdata(dataset, i) {
  
          const responseDataset = response[i] || response
          
          if (dataset.fields) {
            return dataset.fields.map((field) => responseDataset[field] && responseDataset[field])
          }
          
          if (dataset.field) {
            return responseDataset[dataset.field]
          }
    ...
    if (dataview.toolbar && dataview.toolbar.viewport) toolbar.appendChild(_xyz.utils.wire()`
    <button
    ...
    if (dataview.toolbar && dataview.toolbar.download_csv) toolbar.appendChild(_xyz.utils.wire()`
    <button
      class="off-white-hover primary-colour"
      onclick=${() => {
    ...
    if (dataview.toolbar && dataview.toolbar.download_json) toolbar.appendChild(_xyz.utils.wire()`
    <button
      class="off-white-hover primary-colour"
      onclick=${() => {
