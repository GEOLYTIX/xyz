  ...
  function init(params) {
    ...
    tabview.add = dataview => {
      ...
      dataview.showTab = () => {
        ...
        if (_xyz.mapview.attribution.node) _xyz.mapview.attribution.node.style.bottom = `${tabview.node.clientHeight}px`
      ...
      if(dataview.script) {
        ...
        const script_tag = _xyz.utils.html.node`<script src="${dataview.src}">`;
      ...
      if(dataview.query) _xyz.dataviews.create(dataview);
      ...
      dataview.remove = () => {
        ...
        if (_xyz.mapview.attribution.node) _xyz.mapview.attribution.node.style.bottom = '0';
