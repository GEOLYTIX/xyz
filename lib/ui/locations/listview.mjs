...
export default params => {
  ...
  params.mapview.locations = new Proxy(params.mapview.locations, {
    set: function(target, key, location){
      ...
      listview.node.insertBefore(location.view, listview.node.firstChild.nextSibling.nextSibling)
    ...
    deleteProperty: function (target, hook) {
      ...
      setTimeout(() => {
        ...
        // Return if locations present in list.
        if (list.filter(r => r.hook).length) return

        // Otherwise activate layers tab.
