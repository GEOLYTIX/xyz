...
export default function listview(params) {
  ...
  locale.locations.noLocations = params.target.appendChild(
    mapp.utils.html
      .node`<p style="margin: 5% 30%;color: var(--color-font-mid);font-style: italic;font-weight: bold;">
...
function gazCheck(locale) {
  ...
  if (!document.querySelectorAll('#locations > .location-view').length) {
    const display =
      locale.controls?.find((control) => control.key === 'locations')?.style
        ?.display || 'none';
    // Activate the layers panel
    display === 'none' && document.querySelector('[data-id=layers]').click();
    ...
    if (gazInput) {
      ...
      document.querySelector('[data-id=locations]').style.display = display;
    }
  }
}
