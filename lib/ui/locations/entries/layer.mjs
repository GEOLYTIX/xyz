...
export default function layer(entry) {
  ...
  if (entry.layer) {
    ...
    // If layer is not found, warn and return
    if (!layer) {
  ...
  // Check zoom restriction immediately
  if (entry.tables) {
    const isCurrentZoom = entry.tableCurrent()
    entry.display_toggle?.classList.toggle('disabled', !isCurrentZoom)
  }
