...
async function decorateLayer(entry) {
  ...
  if (entry.featureSet || entry.featureLookup) {
    ...
    entry.source_layer_key = entry.key;

    //Assign the feature extent function
    entry.L.getFeatureExtent = () => getFeatureExtent(entry);
  }
...
@description
Gets an extent of a layer which has feateruSet or featureLookup

@param {infoj-entry} entry type:layer entry.

@returns {array} an array of floats, representing the extent
*/
async function getFeatureExtent(entry) {
  //Get the layer the entry is based on.
  const layer = entry.mapview.layers[entry.source_layer_key];

  if (!Array.isArray(entry.featureLookup || entry.featureSet)) return;

  const ids =
    entry.featureLookup?.map?.((feature) => {
      return feature[entry.featureLookupId || 'id'];
    }) || entry.featureSet;
  ...
  console.log(response.box2d);
  const bounds = /\((.*?)\)+/.exec(response.box2d)[1].replace(/ /g, ',');
  const ol_extent = bounds.split(',').map((value) => parseFloat(value));
...
function hideLayer() {
  ...
  this.location.Layers = this.location.Layers.filter(
    (layer) => layer.values_.key !== this.L.values_.key,
  );
