...
function date(entry) {
  ...
  if (entry.edit) {
    ...
    if (!entry.value && !entry.newValue) {
      ...
      entry.newValue = mapp.utils.dateTime.dateToUnixEpoch();
    ...
    return mapp.utils.html.node`<input
      ...
      value="${mapp.utils.dateTime[entry.type](entry)}"
      onchange=${e => {

        entry.newValue = mapp.utils.dateTime.dateToUnixEpoch(e.target.value);
  ...
  const val = entry.value ? mapp.utils.dateTime.dateString(entry) : 'null'
