...
@property {Number} [entry.width] Optional element width in pixels. Defaults to parent container width.
...
export default function link_entry(entry) {
  ...
  const css_class = `val link-with-img ${entry.css_class}`;

  const inline_style = entry.width ? `width: ${entry.width}px` : ``;
