...
@description
This entry function returns an HTML element with a hyperlink to open in a new window. 
The link is labelled with configurable text and a material symbol for readability.
...
@property {string} [entry.css_class] Optional string for css classes to apply on the element, eg. `outlined`.
@property {string} [entry.icon_class] String for css classes to be included alongside icon class `material-symbols-outlined`. 
...
export default function link_entry(entry) {
  ...
  const node = mapp.utils.html.node`<a 
  target="_blank" 
  class=${css_class}
  href=${href}>
  <span style=${entry.icon_style || ''} class=${icon_class}>${entry.icon_name}</span>
