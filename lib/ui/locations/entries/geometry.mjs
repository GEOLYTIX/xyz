...
*/
async function show() {
  // This check must be on when no data and disabled.
  // The reason we need the !this.value is that an entry that initially had no data
  // may be populated later via a backend trigger and then be refreshed as a dependent.
  // Without this check the entry could never be shown again as it would have been initially disabled.
  ...
  // Else block is only reached if value is now present.
    ...
    this.disabled = false;
    // Remove the disabled state from the checkbox.
