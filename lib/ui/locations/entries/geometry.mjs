...
function draw(entry) {
  ...
  Object.keys(entry.draw).forEach((key) => {
    ...
    if (mapp.ui.elements.drawing[key]) {
      ...
      if (entry.draw.dialog && entry.edit) {
        entry.draw.content ??= [];
        el instanceof HTMLElement && entry.draw.content.push(el);
      } else {
        el instanceof HTMLElement && entry.elements.push(el);
      }
    }
  });
  ...
  mapp.ui.utils.panelDialog(entry, panelType);

  if (entry.draw.dialog && entry.edit)
    entry.elements.push(entry.draw.dialog.btn);
}
