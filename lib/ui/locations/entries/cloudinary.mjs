...
function image(entry) {
  if (entry.value) {
    ...
    const trashBtn =
      ...
      class="material-symbols-outlined no-colour no"
...
function images(entry) {
  ...
  if (entry.edit)
    images.push(mapp.utils.html.node`
    <div class="material-symbols-outlined pos-center">
      <input
        type="file"
        accept="image/*"
        capture="camera"
        onchange=${(e) => upload(e, entry)}>add_a_photo`);
...
function documents(entry) {
  ...
  entry.edit &&
    docs.unshift(mapp.utils.html`<input 
    type=file class="flat bold wide primary-colour"
