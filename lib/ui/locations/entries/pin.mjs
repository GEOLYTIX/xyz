...
export default entry => {
  ...
  if(entry.style){
    entry.style.icon = {
      type: entry.style.icon?.type || 'markerLetter',
      letter: entry.style.icon?.letter || entry.location.record.symbol,
      color: entry.style.icon?.strokeColor || entry.location.record.colour,
      scale: entry.style.icon?.scale || 3,
      template: entry.style.icon?.template,
      substitue: entry.style.icon?.substitute,
      anchor: [0.5, 1]
    }
