...
function assignTargetFromString(entry) {
  ...
  if (entry.target === 'dialog') {
    entry.dataview_dialog = {};
    Object.assign(entry.dataview_dialog, {
      header: mapp.utils.html.node`<h3> ${entry.label}`,
      content: mapp.utils.html.node`<div>`,
      data_id: `${entry.key}-dataviews-dialog`,
    ...
    mapp.ui.elements.dialog(entry.dataview_dialog);

    entry.dataview_dialog.dialog.close();

    entry.show = () => {
      entry.update();
      entry.dataview_dialog.dialog.show();
    };
    ...
    entry.target = entry.dataview_dialog.node.querySelector('.content');
