...
export default function view(location) {
  ...
  // Find entry in location with locationHeader true flag
  const locationHeader = location.infoj.find((entry) => entry.locationHeader)?.value;
  // If it exists, create the label as VALUE - (A). Otherwise just A, where A is the location.record.symbol.
  const locationHeaderText = locationHeader
    ? `${locationHeader} - (${location.record.symbol})`
    : `${location.record.symbol}`;

  // Header with expander icon.
  const header = [
    mapp.utils.html`<h2>${locationHeaderText}`,
  ...
  */
  ...
  location.view = mapp.ui.elements.drawer({
    data_id: `location-${location.layer.key}-${location.id}`,
...
A new location will always be editable.

The location.editToggle button element for the location view header is created.

Location view editing will be disabled if the toggleLocationViewEdits property is false.

@param {location} location Mapp Location typedef
@property {layer} location.layer The mapp layer associated with a location.
@property {boolean} [layer.toggleLocationViewEdits] Whether it should be possible to toggle location view edits.

@return {boolean} Returns true if toggleLocationViewEdits should be possible.
*/
function toggleLocationViewEdits(location) {
  // It should not be possible to toggle editing.
  if (location.layer?.toggleLocationViewEdits === false) {
    location.removeEdits();
    return false;
  }

  // Editing will be enabled by default.
