...
Specifying `layer.location_drawer: false` will turn the location view into a panel that can't be collapsed.
...
export default function view(location) {
  ...
  */
  ...
  if (location.layer.location_drawer === false) {
    header.unshift(mapp.utils.html`<h2>${location.record.symbol}`);

    location.view = mapp.utils.html
      .node`<div class="drawer location-view"><div class="header">${header}`;
  } else {
    header.unshift(
