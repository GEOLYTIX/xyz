...
export default function view(location) {
  ...
  // Find entry in location with pinLabel true flag
  const pinLabel = location.infoj.find((entry) => entry.pinLabel)?.pinLabel;
  // If it exists, create the label as VALUE (A). Otherwise just A, where A is the location.record.symbol.
  const pinLabelText = pinLabel
    ? `${pinLabel} - (${location.record.symbol})`
    : `${location.record.symbol}`;

  // Header with expander icon.
  const header = [
    mapp.utils.html`<h2>${pinLabelText}`,
