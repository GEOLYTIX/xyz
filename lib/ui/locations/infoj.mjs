...
Specifying `entry.group_drawer: false` in any of the entries of the group will prevent a drawer from being made for the group.
  ...
  if (!groups[entry.group]) {
    const group_drawer =
      entry.location.infoj
        ...
        .find((e) => e.group_drawer === false)?.group_drawer ?? true;

    let drawer;
    if (group_drawer === false) {
      drawer = mapp.utils.html
        .node`<div class="group drawer"><div class="header"><h3>${entry.group}</h3>`;
    } else {
      drawer = mapp.ui.elements.drawer({
        class: `group`,
    ...
    groups[entry.group] = entry.listview.appendChild(drawer);
  }
...
function entryQuery(entry) {
  ...
  if (entry.queryCheck || entry.run === true) {
    ...
    console.warn(
      `field:"${entry.field}" has a query:"${entry.query}" which is not set to run. To resolve this, add queryCheck:true or run:true to the entry.`,
    );
  }
}
