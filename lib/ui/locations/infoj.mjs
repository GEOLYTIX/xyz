...
function entryObject(entry) {
  ...
  if (entry.objectAssignFromField || entry.json_field) {

    if(entry.json_field && !entry.key){
      console.warn('json_field requires entry.key to be specified')
      return;
    }

    let fieldEntry = entry.location.infoj.find(_entry => _entry.field === (entry.objectAssignFromField || entry.json_field))
