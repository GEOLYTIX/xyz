...
export default (layer) => {
  ...
  if (layer.drawer !== null) {
    ...
    layer.displayToggle = mapp.utils.html.node`
      <button
        ...
        class="${`mask-icon toggle ${(layer.zoomDisplay || layer.display)? 'on': 'off'}`}"
        onclick=${() => layer.display ? layer.hide() : layer.show()}>`
  ...
  layer.mapview.Map.getTargetElement().addEventListener('changeEnd', () => {
    ...
    if (!layer.tableCurrent()) {
      ...
      // Display the zoomBtn if layer is out of zoomDisplay range.
      ...
      // Don't display zoomBtn if layer is in zoomDisplay range.
