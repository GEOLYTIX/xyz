...
export default function layerView(layer) {
  ...
  const drawerClass = `layer-view raised ${layer.classList || ''} ${layer.viewConfig.content.length ? '' : 'empty'}`
...
@description
The viewConfig method will add controls to the layer.view as the configuration provided dictates.



@param {layer} layer A decorated mapp layer.
@property {Object} [layer.viewConfig] Control options for elements in the layer.view.
@property {Boolean} [viewConfig.zoomBtn=true] Controls whether the zoom magnifying close is displayed.
...
@property {Boolean} [viewConfig.panelOrder=true] Controls whether the panel order is used.
...
function viewConfig(layer){
  ...
  layer.viewConfig.panelOrder ??= layer.panelOrder || ['draw-drawer', 'dataviews-drawer', 'filter-drawer', 'style-drawer', 'meta']
  ...
  layer.zoomToExtentBtn = layer.viewConfig.zoomToExtentBtn && layer.filter.zoomToExtent
  ...
  layer.displayToggle = layer.viewConfig.displayToggle && mapp.utils.html.node`<button
  ...
  layer.zoomBtn = layer.viewConfig.zoomBtn && layer.zoomBtn !== false && layer.tables
