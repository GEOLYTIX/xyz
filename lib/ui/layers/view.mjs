...
@description
The filterFunction method will show the layer.zoomToFilteredExtentBtn element if the layer.filter.current object is not empty.
@param {layer} layer A decorated mapp layer.
*/
...
function changeEnd(layer) {
  ...
  if (layer.tableCurrent()) {
    ...
    if (layer.zoomBtn instanceof HTMLElement) {
      layer.zoomBtn.style.setProperty('visibility', 'hidden');
      layer.zoomBtn.style.setProperty('width', '0px');
    }
    ...
    if (layer.zoomBtn instanceof HTMLElement) {
      layer.zoomBtn.style.setProperty('visibility', 'visible');
      layer.zoomBtn.style.removeProperty('width');
    }
