...
function viewConfig(layer) {
  ...
  if (layer.viewConfig.keyOrder) {
    ...
    layer.viewConfig.content = Object.keys(layer)
      .sort((a, b) => {
        return (
          layer.viewConfig.keyOrder.indexOf(a) -
          layer.viewConfig.keyOrder.indexOf(b)
        );
      })
    ...
    if (Array.isArray(layer.viewConfig.panelOrder)) {
      ...
      layer.viewConfig.content.sort((a, b) => {
        const aIndex = layer.viewConfig.panelOrder.findIndex(
          (chk) => chk === a.dataset?.id,
        );
        const bIndex = layer.viewConfig.panelOrder.findIndex(
          (chk) => chk === b.dataset?.id,
        );
