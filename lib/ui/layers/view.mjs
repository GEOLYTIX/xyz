...
function zoomToFilteredExtentBtn(layer) {
  if (!layer.viewConfig.zoomToFilteredExtentBtn) return;
...
function zoomBtn(layer) {
  if (!layer.viewConfig.zoomBtn) return;
...
function popoutBtn(layer) {
  if (!layer.viewConfig.popoutBtn) return;
...
function displayToggle(layer) {
  if (!layer.viewConfig.displayToggle) return;

  const btnClass = `notranslate material-symbols-outlined toggle ${layer.zoomDisplay || layer.display ? 'toggle-on' : ''
    }`;
