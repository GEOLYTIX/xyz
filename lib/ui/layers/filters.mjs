...
async function filter_numeric(layer, filter) {
  ...
  let histogram_element = '';

  if (filter.histogram) {
    histogram_element = await histogram(filter.histogram);
  }

  const node = mapp.utils.html.node`
  ${histogram_element?.target}
  ${slider}`;
...
async function histogram(params) {
  params.target = mapp.utils.html.node`<div>`;
  const dataview = await mapp.ui.utils.histogram.create(params);
  return dataview;
  //return mapp.utils.html`<div>Hello I am a histogram`;
}
