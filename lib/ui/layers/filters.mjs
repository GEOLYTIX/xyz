...
@description
The updateMinMax updates numeric filter element with new min/max range based on changed viewport.

@param {Object} layer
@property {Object} filter The filter configuration.
*/
async function updateMinMax(layer, filter) {
  await generateMinMax(layer, filter);
  const slider = filter_numeric(layer, filter);
  mapp.utils.render(filter.card, slider);
}
