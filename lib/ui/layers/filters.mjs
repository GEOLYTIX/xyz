...
async function filter_numeric(layer, filter){
  ...
  const affix = entry.prefix || entry.suffix
  applyFilter(layer);
  return mapp.ui.elements.slider_ab({
    ...
    step: filter.step,
    entry: entry,
    label_a: `${mapp.dictionary.layer_filter_greater_than} ${affix ? `(${affix.trim()})` : ''}`, // Greater than
    ...
    label_b: `${mapp.dictionary.layer_filter_less_than}  ${affix ? `(${affix.trim()})` : ''}`, // Less than
