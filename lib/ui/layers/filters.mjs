...
async function filter_numeric(layer, filter) {
  ...
  if (filter.histogram) {
    ...
    await histogram(filter.histogram);
  }
...
*/
async function histogram(params) {
  // if dataview has been created before add the node back to the target
  if (params.chart) {
    mapp.utils.render(
      params.target,
      mapp.utils.html`${params.node}${params.xlabel}${params.caption}`,
    );
    return;
  }
  // on initial load create dataview object instance
  params.chart = await mapp.ui.Dataview(params);
  params.chart.update();

  // update chart data on map change end
  if (params.viewport)
    params.layer.mapview.Map.getTargetElement().addEventListener(
      'changeEnd',
      () => {
        params.chart?.update();
      },
