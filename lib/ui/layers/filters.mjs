...
function deleteFilter(layer, filter) {
  if (layer.filter.current[filter.field]) {
    ...
    // The min and max must be removed if they exist and were populated dynamically, not configured.
    if (Object.hasOwn(filter, 'min') && !filter.configured) {
      delete filter.min;
    }
    if (Object.hasOwn(filter, 'max') && !filter.configured) {
      delete filter.max;
    }
  }
...
async function filter_numeric(layer, filter) {
  ...
  // If min amd nax already exist, they have been set by the developer so they must be retained.
  if (!isNaN(filter.min) && !isNaN(filter.max)) {
    // Add a configured true flag to indicate that the filter has been configured.
