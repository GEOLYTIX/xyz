...
async function histogram(filter, layer) {
  ...
  filter.histogram.options.tooltip ??= true;
  ...
  // if dataview has been created before add the node back to the target
  if (filter.histogram.chart) {
    mapp.utils.render(
      filter.histogram.target,
      mapp.utils
        .html`${filter.histogram.node}${filter.histogram.xlabel}${filter.histogram.caption}`,
    );
    return;
  }
  // on initial load create dataview object instance
  filter.histogram.chart = await mapp.ui.Dataview(filter.histogram);
  filter.histogram.chart.update();

  // update chart data on map change end
  if (filter.histogram.viewport) {
    filter.histogram.layer.mapview.Map.getTargetElement().addEventListener(
      'changeEnd',
      () => filter.histogram.chart?.update(),
    );
  }
