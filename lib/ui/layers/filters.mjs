...
async function filter_numeric(layer, entry){
  ...
  if (!entry.filter.max) {
    ...
    entry.filter.max = entry.filter.type === 'integer' ? parseInt(response.max) : parseFloat(response.max);
  }
  ...
  if (!entry.filter.min) {
    ...
    entry.filter.min = entry.filter.type === 'integer' ? parseInt(response.min) : parseFloat(response.min);
  }
