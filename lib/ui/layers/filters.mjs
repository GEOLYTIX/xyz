...
async function updateMinMax(layer) {
  ...
  const filters_to_update = layer.filter.list.filter(
    (item) =>
      item.slider &&
      item.viewport &&
      (item.type == 'numeric' || item.type == 'integer'),
  );
  if (!filters_to_update.length) return;
  for (const filter of filters_to_update) {
    await generateMinMax(layer, filter);
    filter.slider.remove();
    await filter_numeric(layer, filter);
    const header = filter.card.querySelector('header');
    header.after(filter.slider);
  }
}
