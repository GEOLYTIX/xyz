...
async function histogram(filter, layer) {
  ...
  // set slider step based on bin width
  filter.slider.querySelectorAll('input').forEach((input) => {
    input.step = filter.histogram.bin_width;
  });
  ...
  if (filter.histogram.viewport) {
    ...
    function changeEnd() {
      ...
      // update slider step based on current bin width
      filter.slider.querySelectorAll('input').forEach((input) => {
        input.step = filter.histogram.bin_width;
      });
    }
