  ...
  if(filter.searchbox) {
    ...
    const searchbox = mapp.ui.elements.searchbox({
      ...
      searchFunction: (e) => {
        ...
        if(!pattern || !pattern.length) return; // nothing here to match
        ...
        filtered.map((i, j) => {
          ...
          searchbox.list.appendChild(mapp.utils.html.node`<li onclick=${e => {
            ...
            selected_tags.add(i);
            let tag = mapp.utils.html.node`<div 
            onclick=${ev => {
              selected_tags.delete(ev.target.dataset.tag);
              ev.target.remove();
            }}
            data-tag="${i}" 
            class="primary-background" 
            title=${i + ' :click to remove selection'}
