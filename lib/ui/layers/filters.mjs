...
async function filter_in(layer, filter) {
  ...
  if(filter.searchbox) {
    // experimental for gathering further requirements

    const searchbox_filter = mapp.utils.html.node`<div class="filter">`

    const searchbox_filter_tags = mapp.utils.html.node`<div 
    style="display: flex; gap: 4px; flex-flow: row wrap; padding-top: 10px;">`

    const selected_tags = new Set();

    function matchResults(pattern) {
      // to clarify how results are matched
      // this is current search logic case sensitive and js-esque
    ...
    // creates an interactive search tag element
    function createTag(val) {
      return mapp.utils.html.node`<div 
      class="primary-background" 
      title=${val}
      style="cursor: pointer; padding: 2px; border-radius: 2px; font-size: 0.9em; font-weight: bold"
      >${val}
      <button 
      data-tag="${val}" 
      onclick=${e => {
        selected_tags.delete(e.target.dataset.tag);
      ...
      title="Remove filter"
      class="primary-background" 
      style="font-size: 0.8em; padding: 0 2px">&#10005;`
    }
    ...
    const searchbox = mapp.ui.elements.searchbox({
      ...
      searchFunction: (e) => {
        ...
        filtered
        .filter(i => !selected_tags.has(i)) // only those not selected already
        .map((i, j) => {
          if(j > 9) return; // show only 10 top matches
          searchbox.list.appendChild(mapp.utils.html.node`<li onclick=${e => {
            if(selected_tags.has(i)) return;
            selected_tags.add(i);
            let tag = createTag(i);
            searchbox_filter_tags.append(tag);
            //searchbox.input.value = null; // when to (should?) clear input?
            // update in-filter
            Object.assign(layer.filter.current, {
              [filter.field]: {
                [filter.type]: [...selected_tags]
    ...
    return mapp.utils.html.node`${searchbox_filter}${searchbox_filter_tags}`;
  }
