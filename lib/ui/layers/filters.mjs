...
async function filter_numeric(layer, filter) {
  ...
  if (filter.histogram) {
    histogram_element = await histogram(layer, filter.histogram);
  }
...
*/
async function histogram(layer, params) {
  params.target = layer.filter.view;
  params.layer = layer;
  const chart = await mapp.ui.Dataview(params);
  chart.show();
}
