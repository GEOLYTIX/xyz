...
async function filter_in(layer, filter) {
  ...
  if (filter.searchbox) {
    ...
    const pillComponent = mapp.ui.elements.pills({
      // current filter values added as pills on creation.
      pills: layer.filter?.current[filter.field]?.[filter.type].length
        ? layer.filter?.current[filter.field]?.in
        : null,
