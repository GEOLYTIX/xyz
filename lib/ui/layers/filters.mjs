...
async function filter_in(layer, filter) {
  ...
  if(filter.searchbox) {
    ...
    style="display: flex; gap: 10px; flex-flow: row wrap; padding: 10px;">`
    ...
    function createTag(val) {
      ...
      title=${val + ' :click to remove selection'}
    ...
    const searchbox = mapp.ui.elements.searchbox({
      ...
      searchFunction: (e) => {
        ...
        let filtered = filter[filter.type].filter((str) => { 
        ...
        .map((i, j) => {
          ...
          searchbox.list.appendChild(mapp.utils.html.node`<li onclick=${e => {
            ...
            //searchbox.input.value = null; // when to clear input?
            // apply this filter
          }}>${i}`);
