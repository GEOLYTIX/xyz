...
export default function filterPanel(layer) {
  ...
  layer.filter.dropdown = mapp.ui.elements.dropdown({
    ...
    callback: async (e, filter) => {
      ...
      if(layer.tableCurrent && layer.tableCurrent() && !layer.filter.location_count.hidden){
        updateCount(layer) && layer.filter.feature_count.style.setProperty('display','block');
      }
