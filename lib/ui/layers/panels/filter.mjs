...
mapp.utils.merge(mapp.dictionaries, {
  en: {
    ...
    layer_filter_location_match: 'Location(s) match your criteria'
  },
  de: {
    layer_filter_header: 'Filter',
...
export default function filterPanel(layer) {
  ...
  layer.filter.dropdown = mapp.ui.elements.dropdown({
    ...
    callback: async (e, filter) => {
      ...
      layer.filter.feature_count instanceof HTMLElement && updateCount(layer) && layer.filter.feature_count.style.setProperty('display','block');
  ...
  if(layer.filter.location_count !== false){

      mapp.ui.layers.filters.reloadCallback = mapp.utils.compose(mapp.ui.layers.filters.reloadCallback.bind(), updateCount)
  
      layer.filter.count = mapp.utils.html.node`<span class="bold">`
  
      layer.tableCurrent() && updateCount(layer)
  
      layer.filter.feature_count = mapp.utils.html.node`<p class="flat" style="display:none">
          ${layer.filter.count} ${mapp.dictionary.layer_filter_location_match}
        </p>`
      
      !layer.tableCurrent() && layer.filter.feature_count.style.setProperty('display', 'none')
