  ...
  const dropdown = mapp.ui.elements.dropdown({
    ...
    callback: async (e,filter) => {
      ...
      filter.remove = () => {
        ...
        // Reset filter
        const dropDown = layer.view.querySelector(`[data-id=${layer.key}-filter-dropdown]`)
        var filterActive = true;
      
        //Check if any filters current hold any values
        for(let item of Object.keys(layer.filter.current)){
          if(Object.keys(layer.filter.current[item]).length === 0){
            filterActive = false
            break;
          }
        }

        //If no active filter is found do not reset.
        layer.filter.current = filterActive ? JSON.parse(dropDown.style.getPropertyValue('--filter')) : layer.filter.current 
