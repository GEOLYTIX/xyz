...
export default function filterPanel(layer) {
  ...
  if (layer.filter.dialog) {
    ...
    layer.filter.filter_list = mapp.utils.html.node`
    ...
    if (layer.filter.dialog) {
      layer.filter.dialog = {
        btn_label: 'Open Filter Dialog',
        title: 'Filter',
        icon: 'filter_alt',
        view: layer.filter.filter_list,
        ...layer.filter.dialog,
      };
    }
