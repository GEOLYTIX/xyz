...
function listFilter(layer) {
  ...
  for (const entry of layer.infoj) {
    if (entry.skipEntry === true) continue;

    if (entry.field === undefined) continue;

    if (layer.filter.exclude.includes(entry.field)) continue;
    ...
    if (!entry.filter?.type) continue;

    if (!Object.hasOwn(mapp.ui.layers.filters, entry.filter.type)) continue;
