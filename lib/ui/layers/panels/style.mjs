...
export default layer => {
      ...
      <div>${mapp.dictionary.layer_style_select_theme}</div>
        ${mapp.ui.elements.dropdown({
          ...
          callback: (e, entry) => {
            ...
            mapp.ui.layers.styles[layer.style.theme.type] && mapp.utils.render(
              layer.style.drawer.querySelector('.drawer > .legend'),
              mapp.ui.layers.styles[layer.style.theme.type](layer))
  ...
  layer.style.theme && mapp.ui.layers.styles[layer.style.theme.type] && content.push(mapp.utils.html`
    <div class="legend">
      ${mapp.ui.layers.styles[layer.style.theme.type](layer)}`)
  ...
  if (layer.hideStylePanel) return;
