...
export default function reports(layer) {
  ...
  for (const [key, report] of Object.entries(layer.reports)) {
    ...
    // Provide the report as params property for the reportLink method if href is not explicit.
    const params = report.href ? report : { report };

    const el = mapp.ui.elements.reportLink(params);
