...
export default function dataviews(layer) {
  ...
  for (const [key, dataview] of Object.entries(layer.dataviews)) {
    ...
    if (dataview.target === 'dialog') mapp.ui.elements.dataviewDialog(dataview);

    // Assign target html element for dataview.
    assignTarget(dataview);
    ...
    if (!dataview.tabview && !dataview.dataview_dialog) {
...
@description
Assigns an HTMLElement target to the dataview.

@param {Object} dataview The dataview to assign the target to.
@property {Object} dataview.target The current target associated to the dataview.
*/
function assignTarget(dataview) {
  dataview.target =
