export default (layer) => {
  ...
  Object.entries(theme.cat).forEach(cat => {
    ...
    cat[1].disabled = layer.filter?.current[theme.field]?.ni?.indexOf(cat[0]) >= 0

    if (layer.featureFields && theme.distribution === 'count') {

      cat[1].count = layer.featureFields[theme.field]?.[cat[0]]

      if (!cat[1].disabled && !cat[1].count) return;
    }
    ...
    let icon = mapp.utils.html`
      <div
        ...
        ${mapp.ui.elements.legendIcon({ width: 24, height: 24, ...cat_style})}`;

    let classList = `label ${layer.filter && 'switch' ||''} ${cat[1].disabled &&  'disabled' ||''}`;

    let cat_label = (cat[1].label || cat[0]) + (cat[1].count? ` [${cat[1].count}]`:'')
    ...
    cat[1].node = mapp.utils.html.node`<div 
      data-id=${cat[0]}
  ...
  if (layer.style.cluster) {
    console.log(`${layer.key} - ${JSON.stringify(layer.style.cluster)}`);
    // Create cluster icon.
    let icon = mapp.utils.html`
      <div
