...
The categorizedTheme method also creates the legend for type: distributed themes.
...
export default function categorizedTheme(layer) {
  ...
  // A distributed theme without showLegend should not create a legend.
  if (theme.type === 'distributed' && !theme.showLegend) return;
  ...
  // Build the legend items for each category in a categorized theme.
  if (theme.type === 'categorized') {
    theme.categories.forEach((cat) => {
      createLegend(cat, theme, layer);
    });

    // Distributed themes use the lookup to build the legend.
    // But are otherwise built the same way as categorized themes.
  } else if (theme.type === 'distributed') {
 ...
 * @description Create legend entry for category within a theme.
 * @param {Object} cat The category object.
 * @param {Object} theme The theme configuration object.
 * @param {Object} layer The layer object.
 **/
function createLegend(cat, theme, layer) {
