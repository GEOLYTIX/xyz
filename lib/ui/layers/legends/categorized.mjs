export default (layer) => {
  ...
  */
  ...
  //legend.type - Container display type, "grid" | "flex"
  ...
  contentsClass += (legendOptions?.alignContents == "center" && legendOptions?.type == "flex") ? " centered" : "";
  contentsClass += (legendOptions?.alignContents == "row" && legendOptions?.type == "flex") ? " row" : "";
  ...
  layer.style.legend = mapp.utils.html.node`<div class=${`contents-wrapper ${legendOptions?.type == "flex" && 'flex'}`}>${grid}`
