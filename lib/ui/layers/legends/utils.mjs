...
export function createLegend(cat, theme, layer) {
  ...
  // TODO: the label element should not require inline styles.
  const label = mapp.utils.html.node`<div
    class=${classList}
    style="grid-column: 2; overflow: hidden; text-overflow: ellipsis; text-wrap: nowrap;">
    ${cat_label}`;
...
*/
export async function filterRemove(layer, cat) {
...
@param {object} cat The cat object from the theme.
*/
export async function filterAdd(layer, cat) {
  if (layer.style.theme.filterOnly) return;
