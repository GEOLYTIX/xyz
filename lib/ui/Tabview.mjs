...
function addTab(entry) {
  ...
  const layerDataview = entry?.layer?.dataviews[entry.key];
  if (layerDataview) entry.disableTabClose = entry.layer.dataviews.hide;
  ...
  entry.tab_btn = mapp.utils.html.node`
     <button
  ...
  const closeBtn = mapp.utils
    .html`<button class="notranslate material-symbols-outlined close_tab"
              onclick=${(e) => {
