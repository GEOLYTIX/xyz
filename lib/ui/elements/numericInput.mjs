...
export default function numericInput(params) {
  ...
  <input
    ...
    onkeyup=${e => {
      ...
      let decLength = e.target.value.substring(e.target.value.indexOf(separators.decimals)+1).length
      if(e.key === '0' && e.target.value.lastIndexOf('0') === e.target.value.length -1  && e.target.value.includes(separators.decimals) && decLength <= (entry.round || entry.formatterParams.precision)){
