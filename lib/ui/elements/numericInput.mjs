...
export default function numericInput(params) {
  ...
  <input
    ...
    onkeyup=${e => {
      ...
      //Check that numbersi within the min and max
      let value = numericChecks(e,params)

      //Nullify newValue if the value is invalid
      entry.newValue = e.target.classList.contains('invalid') ? null : value
      if(entry.newValue && entry.newValue !== parseFloat(oldValue)){
