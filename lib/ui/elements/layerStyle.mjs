...
function opacitySlider(layer) {
  return mapp.ui.elements.slider({
    callback: (e) => {
      layer.L.setOpacity(Number.parseFloat(e / 100));
    },
    data_id: 'opacitySlider',
    label: `${mapp.dictionary.layer_style_opacity}`,
    max: 100,
    min: 0,
    val: Number.parseInt(layer.L.getOpacity() * 100),
  });
}
...
function icon_scaling(layer) {
  ...
  if (layer.style.icon_scaling.icon) {
    ...
    content.push(
      mapp.ui.elements.slider({
        callback: (val) => {
          layer.style.default.scale = Number.parseFloat(val);
  ...
  if (
    ...
    content.push(
      mapp.ui.elements.slider({
        callback: (val) => {
          layer.style.cluster.clusterScale = Number.parseFloat(val);
          clearTimeout(layer.style.timeout);
          layer.style.timeout = setTimeout(() => layer.L.changed(), 400);
        },
  ...
  if (layer.style.icon_scaling.zoomInScale) {
    ...
    content.push(
      mapp.ui.elements.slider({
        callback: (val) => {
          layer.style.zoomInScale = Number.parseFloat(val);
          clearTimeout(layer.style.timeout);
          layer.style.timeout = setTimeout(() => layer.L.changed(), 400);
        },
  ...
  if (layer.style.icon_scaling.zoomOutScale) {
    ...
    content.push(
      mapp.ui.elements.slider({
        callback: (val) => {
          layer.style.zoomOutScale = Number.parseFloat(val);
          clearTimeout(layer.style.timeout);
          layer.style.timeout = setTimeout(() => layer.L.changed(), 400);
        },
