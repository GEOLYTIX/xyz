...
function icon_scaling(layer) {
  ...
  if (layer.style.icon_scaling.fields) {
    ...
    const placeholder = Object.keys(layer.style.icon_scaling.fields).find(
      (key) =>
    ...
    content.push(
      ...
      mapp.ui.elements.dropdown({
        ...
        placeholder: placeholder || layer.style.icon_scaling.placeholder,
      }),
    );

    //Set the first field in the list as the default
    const first_field =
      Object.keys(layer.style.icon_scaling.fields)[0] ||
      layer.style.icon_scaling.field;

    Object.assign(layer.style.icon_scaling, {
      ...layer.style.icon_scaling.fields[first_field],
    });
