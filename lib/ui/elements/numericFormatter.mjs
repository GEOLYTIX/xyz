...
export function getSeparators(entry){
    ...
    if(!entry) return;
    if(!entry.formatterParams) return { decimals : '.'};
    entry.prefix = '' 
    entry.suffix = ''
...
function getFormatterParams(entry, inValue) {
    ...
    if(entry.formatterParams === 'toLocaleString'){
    ...
    if(entry.formatterParams?.decimal || entry.formatterParams?.thousand){
        ...
        rawValue = entry.formatterParams ? parseFloat(value).toLocaleString(entry.formatterParams.locale, entry.formatterParams.options) : parseFloat(value)
    }
