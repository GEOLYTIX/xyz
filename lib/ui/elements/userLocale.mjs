...
async function saveLocale(params, mapview) {
  ...
  // Add a timestamp to the locale
  mapview.locale.timestamp = mapp.utils.temporal.dateToUnixEpoch();
...
async function listLocales(params, mapview) {
  ...
  const list = locales.map((locale) => {
    ...
    // if no timestamp, don't include it in the display
    if (!locale.timestamp) {
      return mapp.utils.html`<li>${removeBtn}<a href=${href}>${locale.name}`;
    }

    const params = {
      value: locale.timestamp,
    };

    const datetime = mapp.utils.temporal.dateString(params);

    return mapp.utils
      .html`<li>${removeBtn}<a href=${href}>${locale.name} [${datetime}]`;
  });
