export default params => {
  ...
  function onInput(e) {
    ...
    e.target.value = params.value > params.max ? params.max : value
    let idValue = value < params.min ? params.min : value

    element.style.setProperty(`--${e.target.dataset.id}`, idValue > params.max ? params.max : value)
    ...
    element.querySelectorAll('input')
      .forEach(el => {
        ...
        if(e.target.type === 'text' && el.type === 'range'){
          el.value = value
          e.target.value = mapp.ui.elements.numericFormatter(params.entry,value)
        }
        else{
