...
export default function dialog(dialog) {
  ...
  dialog.header = mapp.utils.html.node`<header data-id='dialog-header'>${dialog.header}`

  dialog.headerDrag && dialog.header.classList.add('headerDrag')
  ...
  function minimizeDialog(){
    ...
    dialog.node.querySelector('[data-id=dialog-content]').hidden = !dialog.node.querySelector('[data-id=dialog-content]').hidden
  
    const minimizeBtn = dialog.node.querySelector('[data-id=minimize]')
    const state = minimizeBtn.classList.contains('remove')
  
    minimizeBtn.classList.remove(state ? 'remove' : 'fullscreen')
    minimizeBtn.classList.add(state ? 'fullscreen' : 'remove')
  }

  dialog.close = closeDialog

  dialog.minimize = minimizeDialog
  ...
  const buttons = mapp.utils.html.node`<div data-id='dialog-controls'>${dialog.minimizeBtn}${dialog.closeBtn}`
  dialog.header.append(buttons)
