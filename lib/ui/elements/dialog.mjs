/**
### mapp.ui.element.dialog()

Creates a dialog and handles its positioning, dragging and closing
 ...
 * @param {HTMLElement} dialog.target - The target element where the dialog will be appended.
 * @param {string} [dialog.css_style] - The CSS styles to apply to the dialog dialog.
 * @param {string} [dialog.data_id='dialog'] - The data-id attribute value for the dialog dialog.
 * @param {string} [dialog.header] - The HTML content for the dialog header.
 * @param {string} [dialog.content] - The HTML content for the dialog body.
 * @param {boolean} [dialog.close] - A flag indicating whether to include a close button.
 * @param {boolean} [dialog.dialog] - A flag to determine if its a Dialog or dialog.
 * @param {Function} [dialog.close] - A function to be called when the dialog is closed.
 * @param {number} [dialog.top] - The top position of the dialog dialog.
 * @param {number} [dialog.left] - The left position of the dialog dialog.
 * @param {number} [dialog.right] - The right position of the dialog dialog (used to calculate left position).
 * @param {boolean} [dialog.contained] - A flag indicating whether to keep the dialog dialog within the target element's boundaries.
 * @param {boolean} [dialog.containedCentre] - A flag indicating whether to keep the dialog dialog centered within the target element's boundaries.
 */
export default function dialog (dialog) {
  ...
  const closeBtn = dialog.close && mapp.utils.html.node`
    <button
      data-id=close
  ...
  dialog.node = dialog.target.appendChild(mapp.utils.html.node`
    <dialog 
      style=${dialog.css_style}
      data-id=${dialog.data_id}
      class="${dialog.modal ? 'modal' : 'dialog'}">
      ${closeBtn}
      ${dialog.header}
      ${dialog.content}`);


  dialog.modal ? dialog.node.showModal() : dialog.node.show();
  ...
  function closeDialog(e) {
    dialog.node.close();
    // Run the dialog close function only if it exists
