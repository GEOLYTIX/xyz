...
function circle(layer) {
  ...
  if (circle.dialog) {
    circle.dialog = {
      btn_label: mapp.dictionary.draw_circle_dialog,
      title: mapp.dictionary.draw_settings,
      ...circle.dialog,
    };
  }

  circle.drawer = mapp.ui.elements.drawer({
    data_id: `draw-circle-${layer.key}`,
    drawer: circle.drawer,
    layer,
    header: mapp.utils.html`
    <h3>${mapp.dictionary.circle_config}</h3>`,
    content,
    popout: circle.popout,
    dialog: circle.dialog,
  });
...
function setUnits(circle, option) {
  ...
  mapp.utils.render(
    ...
    mapp.ui.elements.slider({
      callback: (e) => {
        circle.radius = Number.parseFloat(e);
      },
      max: entry.max,
      min: entry.min,
      val: circle.radius,
      ...
      onclick=${(e) => {
        mapp.utils.getCurrentPosition(async (pos) => {
          ...
          const coords = ol.proj.transform(
            [
              Number.parseFloat(pos.coords.longitude),
              Number.parseFloat(pos.coords.latitude),
            ],
            'EPSG:4326',
            `EPSG:${layer.srid}`,
          );
