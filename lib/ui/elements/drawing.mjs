...
*/
  ...
  // lowered z-index to float under dropdown lists
  layer.draw.circle.panel = mapp.utils.html.node`
    <div class="panel flex-col">
      ${unitsDropDown}
      ${rangeSlider}<div class="buttons" style="z-index: 996">
      <button class="raised primary-colour bold" onclick=${e => {
        layer.draw.circle.settings.close();
        drawOnclick(e, layer, layer.draw.circle)
      }}>${mapp.dictionary.draw}
      </button>
      <button class="raised primary-colour bold" onclick=${e => {
        layer.draw.circle.settings.close();
    ...
    <button class="flat wide bold primary-colour"
    onclick=${e => {

      // if no settings panel needed
      if (layer.draw.circle.hidePanel) {
        drawOnclick(e, layer, layer.draw.circle)
        return
      }

      // create settings dialog
      if(layer.draw.circle.settings) layer.draw.circle.settings.show();
        // Create the settings dialog
        layer.draw.circle.settings = mapp.ui.elements.dialog({
        css_style: 'overflow: visible',
        class: 'alert-confirm',
        header: mapp.utils.html`<h4>${mapp.dictionary.circle_config}`,
