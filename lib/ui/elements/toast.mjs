...
@property {string} [params.data_id_mask='ui-elements-toast-mask'] The data-id attribute value for the mask element.
...
@property {string} [params.mask] Flag to indicate if a mask should be applied behind the toast. This prevents any interaction with the underlying content.
...
export default function toast(params = {}) {
  ...
  params.data_id_mask ??= params.data_id + '-mask';
  ...
  return new Promise((resolve) => {
    ...
    // If a mask is required, add it to the body.
    if (params.mask) {
      // Add the mask element to the body.
      const mask = mapp.utils.html.node`
        <div data-id=${params.data_id_mask} class="mask"></div>`;
    ...
    function hide_toast(e, action) {
      ...
      // If mask is present remove it.
      const mask = document.querySelector(`[data-id="${params.data_id_mask}"]`);
      if (mask) mask.remove();
