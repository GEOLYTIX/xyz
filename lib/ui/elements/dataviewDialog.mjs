...
export default function dataviewDialog(dataview) {
  ...
  const dialog_style = `width: ${dataview.dialog_css?.width || '300'}px !important; 
  ...
  // TODO I don't really understand the content_panel
  const content_panel = dataview.dataview_dialog.node.querySelector('.content');

  content_panel.classList.remove('content');
  ...
  //Charts need to be recreated when the dialog os resized.
  if (dataview.chart) {
    const chartObserver = new ResizeObserver(() => dataview.create());

    chartObserver.observe(dataview.dataview_dialog.node);
  }
}
