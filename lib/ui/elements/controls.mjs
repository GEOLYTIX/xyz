function controls(params) {
  const tabs = mapp.utils.html.node`<div id="ctrl-tabs" class="hover">`;
  const panels = mapp.utils.html.node`<div id="ctrl-panel">`;

  for (let tab of params.tabs) {
    const elements = controlAndPanel(tab, tabs, panels);

    tabs.appendChild(elements.tab);
    panels.appendChild(elements.panel);
  }

  panels.children[0].classList.toggle('active');
  tabs.children[0].classList.toggle('active');

  params.target.appendChild(tabs);
  params.target.appendChild(panels);

  return params.target;
}

function controlAndPanel(tab, tabs, panels) {
  if (typeof tab === 'string') tab = { key: tab };

  tab.minWidth ??= 0;
  tab.title = mapp.dictionary[tab.key || tab];
