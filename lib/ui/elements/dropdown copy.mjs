...
export default function dropdown(params) {
  ...
  // Fallback assignment for the placeholder property.
  ...
  if (params.dropdown_search) {
    params.node = mapp.utils.html.node`
      <div class="dropdown" 
        onclick=${(e) => params.headerOnClick(e)}>
        <input
          type="text"
  ...
  // The IntersectionObserver ensures that dropdown node is collapsed when the elements shift in its viewport.
  const observer = new IntersectionObserver(
    (e) => {
      params.node?.classList?.remove('active');
    },
    {
      root: null,
    },
  );
  observer.observe(params.node);
...
The active class will be toggled on the dropdown element. The ul element will be displayed if the parent dropdown has the active class.

The difference between the bottom of the dropdown and the viewport will be calculated to determine whether the ul should be displayed to the top of the dropdown.
...
function headerOnClick(e) {
  ...
  if (!dropdown.classList.toggle('active')) return;
  ...
  ul.style.width = `${dropdown.offsetWidth}px`;
  ...
  const space = ul.offsetHeight < 150 ? ul.offsetHeight + 10 : 150;

  if (mapp.utils.mobile()) {
    if (viewDiff < space) {
      ul.style.top =
        dropdown.getBoundingClientRect().top - ul.offsetHeight + 'px';
    } else {
      ul.style.top =
        dropdown.getBoundingClientRect().top + dropdown.offsetHeight + 'px';
  ...
  if (viewDiff < space) {
    ul.style.top = dropdown.offsetTop - ul.offsetHeight + 'px';
  } else {
    ul.style.maxHeight = viewDiff + 'px';
  }
}
