...
@property {boolean} [params.search_filter] Specify wether a searchbox is supplied in a dropdown filter.
...
export default (params) => {
  ...
  if (params.search_filter)
    ...
    ${params.pills?.container}
      <div class="dropdown" onclick=${(e) => params.headerOnClick(e, params)}>
        <input
          type="text"
          data-id=${params.data_id || 'dropdown'}
          class="head"
          onfocus=${() => (params.ul.style.display = params.ul.style === '' ? 'none' : '')}
  ...
  */
  function liOnClick(e, entry) {
    ...
    if (params.multi) {
      ...
      if (!params.pills && !params.search_filter) {
