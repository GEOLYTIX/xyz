...
function selectOnChange(e, params) {
  ...
  const entry = params.entries[selectedIndex];
...
function pillsElement(params) {
  ...
  params.pills = mapp.ui.elements.pills({
    ...
    removeCallback: (val, pills) => {
      ...
      const index = params.entries.findIndex((entry) => entry.option === val);
...
function searchInput(params) {
  ...
  const datalist = mapp.utils.html.node`<datalist id=${listId}>${params.searchOptions}`;
...
function onSearchInput(e, params) {
  params.entries.forEach((entry) => {
    if (entry.title === e.target.value) {
      ...
      const selectedOption = params.searchOptions.find(
