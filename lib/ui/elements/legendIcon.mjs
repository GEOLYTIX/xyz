...
export default (style) => {
  ...
  // Create icon
  if (style.icon) return icon(style)
...
function iconArray(style) {
  ...
  function onImgLoad() {
    ...
    // Grab the urls or svgs from the icon array.
    const urls = style.icon.map(icon => icon.svg || icon.url)
    
     // All icon images must be loaded prior to drawing the image with the OL API
    if (--style.toLoad === 0) {
      console.log(`${urls}: Image loaded (${style.toLoad})`);
    } else {
      console.log(`${urls}: Image not loaded (${style.toLoad})`);
      return;
    }
