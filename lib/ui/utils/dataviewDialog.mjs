...
export default function dataviewDialog(dataview) {
  ...
  //Charts need to be recreated when the dialog is resized.
  if (dataview.chart) {
    const chartObserver = new ResizeObserver(() => {
      dataview.target.style.width = `calc(${dataview.dataview_dialog.node.style.width} - 5px)`;
      dataview.target.style.height = `calc(${dataview.dataview_dialog.node.style.height} - 50px)`;
    });
