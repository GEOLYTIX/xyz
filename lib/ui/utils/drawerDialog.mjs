...
*/
export default function drawerDialog(params) {
  //identify the panel e.g. drawer or filter, etc.
  const panelParams = params.layer?.[params?.key] || params;
  ...
  panelParams.dialog.btn_label ??= params.name || 'Open Drawer Dialog';

  //Determine icon from content where possible.
  params.icon ??= params.content.querySelector(
    '.material-symbols-outlined',
  ).textContent;

  panelParams.dialog.btn = mapp.utils.html.node`<button
    class="wide flat action multi_hover"
    data-id=${params.data_id || 'drawer_dialog_btn'}
...
function openDialog(params) {
  ...
  const content = params.layer?.[params?.key].content || params.content;

  const data_id = `${params.data_id || 'drawer'}-dialog`;

  params.layer?.show?.();

  if (panelParams.dialog.show) return panelParams.dialog.show();

  panelParams.dialog.title ??= `${params.name || 'Drawer'} Dialog`;
