...
/**
  ...
  @property {String} params.options.xlabel - optional label for vertical axis.
  */
async function create(params) {
  ...
  if (params.options.title)
    params.target.append(mapp.utils.html.node`<h3 style="margin: 0.5em 0">${params.options.title}`);
  ...
  if (params.options.caption)
    params.target.append(
      mapp.utils.html
        .node`<span style="color: var(--color-font-mid)">${params.options.caption}`,
    );
}
...
function createHistogram(params, data) {
  ...
  if (params.options.xlabel) {
    params.node.append(
      mapp.utils.html.node`<div class="xlabel">${params.options.xlabel}`,
    );
  }
