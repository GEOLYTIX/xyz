...
function userLanguage(params) {
  ...
  const languageSave = mapp.utils.html`
    ...
    onclick=${(e) => {
      mapp.utils.xhr(
        `${mapp.host}/api/user/update?email=${mapp.user.email}&field=language&value=${mapp.user.language}`,
      );
...
function userLocale(params) {
  ...
  params.user_locale_desc ??= mapp.dictionary.user_locale_desc;

  const userLocaleButtons = mapp.utils.html`
    <h3>${params.user_locale_desc}</h3>
<div style="display:flex;">        
