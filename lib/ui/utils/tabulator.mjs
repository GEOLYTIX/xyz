...
function numeric(_this) {
  ...
  <input
    ...
    function NumericEvent(e, type) {
      ...
      entry.formatterParams = tableField.formatterParams ? tableField.formatterParams : tableField.formatter
      ...
      if(!e.target.value && type === 'min') return;
      if(stringValue[0] === '-' && stringValue.length === 1) return; 
      if(tableField.headerFilter === 'numeric'){
      ...
      e.target.value = mapp.ui.elements.numericFormatter(entry) || null
    }
