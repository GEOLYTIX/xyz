...
export default function resizeHandler(params) {
  params.resizeEvent ??= (e) => resizeEventHandler(e, params);
  params.stopResize = () => stopResize(params);
  params.axis ??= 'X';
  ...
  params.target.addEventListener('mousedown', (e) => {
    ...
    window.addEventListener('mouseup', params.stopResize);
  });
  ...
  params.target.addEventListener(
    ...
    (e) => {
      ...
      window.addEventListener('touchend', params.stopResize);
    },
    {
      passive: true,
    },
...
function resizeEventHandler(e, params) {
  page.X.size = (e.touches && e.touches[0].pageX) || e.pageX;
  page.Y.size =
    globalThis.innerHeight - ((e.touches && e.touches[0].pageY) || e.pageY);
...
function stopResize(params) {
  document.body.style.cursor = 'auto';
  window.removeEventListener('mousemove', params.resizeEvent);
  window.removeEventListener('touchmove', params.resizeEvent);
  window.removeEventListener('mouseup', params.stopResize);
  window.removeEventListener('touchend', params.stopResize);
}
