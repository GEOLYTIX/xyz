/**
Exports the default userLocale utility method as mapp.ui.utils.userLocale().

@module ui/utils/userLocale
*/
...
export default function userLocale(params, mapview) {
  ...
  params.ulLocales = mapp.utils.html
    .node`<div style="max-height: 150px; overflow: scroll">`;
...
async function listLocales(params, mapview) {
  ...
  const list = locales.map((locale) => {
    ...
    const removeBtn = mapp.utils.html`<button style="height: 24px"
       onclick=${async (e) => await removeLocale(params, mapview, locale.name)}>
        <span class="material-symbols-outlined">delete</span>
      </button>`;

    return mapp.utils
      .html`<li style="display: flex; align-items: center">${removeBtn}<a href=${href}>${locale.name}`;
  });
