...
async function Table(_this) {
  ...
  // The select method should be defined as rowClick event.
  // _this.table.selectable && _this.Tabulator.on("rowClick", mapp.ui.utils.tabulator.select(_this))

  // Assign rowDblClick event on zoomToLocation table dataview.
  // _this.table.zoomToLocation &&
  //   _this.Tabulator.on("rowDblClick", (e, row) => {
  //     // The dataview rowSelect method may be used as callback to alter the zoomToLocation rowClick behaviour.
  ...
  typeof _this.events === "object" &&
    Object.entries(_this.events).forEach((event) => {
      ...
      if (typeof mapp.ui.utils.tabulator[event[1]] === "function") {

        _this.Tabulator.on(event[0], mapp.ui.utils.tabulator[event[1]](_this));
        return;
      }
