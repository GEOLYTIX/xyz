...
export default _xyz => {
  ...
  function decorate(params) {
    ...
    layer.filter = Object.assign(params.filter || {}, {current:{}});

    // Set the first theme from themes array.
    if (layer.style && layer.style.themes) {
      layer.style.theme = layer.style.themes[Object.keys(layer.style.themes)[0]];
    }

    // Initialise Openlayers source and layer.
    _xyz.mapview.layer[params.format](layer);
