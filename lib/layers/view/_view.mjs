...
export default _xyz => {
  ...
  function create(layer) {
    ...
    if (layer.format === 'cluster' && layer.style.default) {
    ...
    header.toggleDisplay = _xyz.utils.html.node`
    <button
    ...
    // the bring to front button should how layer when it is hidden.
    layer.view.addEventListener('toggleDisplay', 
      () => {
        header.toggleDisplay.classList.toggle('on')
        layer.style && layer.style.bringToFront && (layer.style.bringToFront.disabled = !!layer.display)
      })
    ...
    layer.modules && layer.modules.forEach(_module => _xyz.locale.modules[_module](layer))

    layer.scripts && Object.entries(layer.scripts).forEach(script => {

      const tag = _xyz.utils.wire()`<script src="${script[1]}">`

      const eF = e => {
        e.detail(_xyz, layer)
        document.removeEventListener(script[0], eF, true)
        tag.remove()
      }

      document.addEventListener(script[0], eF, true)
      layer.view.appendChild(tag)
    })
