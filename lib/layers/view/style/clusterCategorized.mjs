export default _xyz => layer => {
  ...
  const legend = _xyz.utils.html.node`
    <div style="margin-top: 5px; display: grid; grid-template-columns: 30px auto;">`
  ...
  Object.entries(layer.style.theme.cat).forEach(cat => {
    ...
    image.setAttribute('src', _xyz.utils.svg_symbols(Object.assign({}, layer.style.default, cat[1].style || cat[1])));
  ...
  if (layer.style.theme.other) {
    ...
    image.setAttribute('href', _xyz.utils.svg_symbols(layer.style.default));
  ...
  imageMulti.setAttribute('src', _xyz.utils.svg_symbols(layer.style.cluster));
