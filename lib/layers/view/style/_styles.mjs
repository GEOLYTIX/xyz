...
export default _xyz => {
  ...
  const categorized = _categorized(_xyz)

  const graduated = _graduated(_xyz)
   
  const grid = _grid(_xyz)

  const basic = _basic(_xyz)
  ...
  function panel(layer) {
    ...
    <input type="checkbox"
      checked=${!!layer.style.label.display && true || null}
      onchange=${e => {
  ...
  function legend(layer) {
    ...
    if (layer.style.theme.type === 'grid') return grid(layer)

    if (layer.style.theme.type === 'basic') return basic(layer)

    layer.filter = layer.filter || {}

    if (layer.style.theme.type === 'categorized') return categorized(layer)
  
    if (layer.style.theme.type === 'graduated') return graduated(layer)
