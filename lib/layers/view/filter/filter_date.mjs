export default _xyz => (layer, filter_entry) => {
  ...
  <input type="${filter_entry.type === 'datetime' ? 'datetime-local' : 'date'}" 
  ...
  function applyFilter() {
    ...
    timeout = setTimeout(() => {
      ...
      layer.filter.current[filter_entry.field].gt = input_min.value ? _xyz.utils.meltDateStr(input_min.value) : 0                                                                                
      layer.filter.current[filter_entry.field].lt = input_max.value ? _xyz.utils.meltDateStr(input_max.value) : 9999999999
       
      console.log(layer.filter.current)
