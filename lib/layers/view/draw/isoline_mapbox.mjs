export default _xyz => layer => {
  ...
  container.appendChild(_xyz.utils.html.node`
    <button
      ...
      onclick=${draw}>${_xyz.language.layer_draw_isoline_mapbox}`)
  ...
  function draw(e) {
    ...
    _xyz.mapview.interaction.draw.begin({
      ...
      geometryFunction: function (coordinates, geometry) {
        ...
        xhr.open('GET', `https://api.mapbox.com/isochrone/v1/mapbox/${layer.edit.isoline_mapbox.profile || 'driving'}/${origin.join(',')}?` +
