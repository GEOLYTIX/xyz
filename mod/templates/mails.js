module.exports = {
  ...
  verify_account: {
    ...
    ko: {
      ...
      계정 소유자임을 확인해주십시오. \${protocol}\${link}
  ...
  approved_account: {
  	...
  	fr: {
  		...
  		subject: `Maintenant vous pouvez vous connecter à \${protocol}\${host}`
  	},
    pl: {
      text: `Konto na \${host} zostało zatwierdzone.`,
      subject: `Teraz możesz się zalogować na \${protocol}\${host}`
    },
    ja: {
    	subject: `アカウントは承認されました \${host}`,
  		text: `これで、\${protocol}\${host}にログオンできます。`
    },
    ko: {
  		subject: `계정이 승인되었습니다. \${host}`,
  		text: `로그인이 가능합니다. \${protocol}\${host}`
  	},
  	zh: {
  		subject: `该帐户已得到批准 \${host}`,
  		text: `您现在已可登录 \${protocol}\${host}`
  	}
  },
  deleted_account: {
  	en: {
    ...
    de: {
  		...
  		text: `Einloggen ist nicht länger möglich \${protocol}\${host}`
  	},
  	fr: {
  		subject: `Ce compte sur \${host} a été supprimé.`,
  		text: `Vous ne pouvez plus vous connecter à \${protocol}\${host}`
  	},
    pl: {
      subject: `Konto na \${host} zostało usunięte.`,
      text: `Nie możesz się już logować na \${protocol}\${host}`
    },
    ja: {
    	subject: `\${host}のこのアカウントは削除されました`,
    	text: `\${protocol}\${host}にログインできなくなります削除されました`
    },
    ko: {
  		subject: `계정이 삭제되었습니다. \${host}`,
  		text: `더 이상 로그인이 불가합니다. \${protocol}\${host}`
  	},
  	zh: {
  		subject: `此帐户已被删除 \${host}`,
  		text: `您将不再能登录 \${protocol}\${host}`
  	}
  },
  failed_login: {
  	en: {
  ...
  locked_account: {
    en: {
  	...
  	fr: {
  		...
  		Vérifiez que vous disposez des droits d'accès du compte \${protocol}\${host}/api/user/verify/\${verificationtoken}
    ...
    pl: {
      ...
      Potwierdź swoje prawa dostępu do \${protocol}\${host}/api/user/verify/\${verificationtoken}
      ...
      アカウントホールダーであることを検証してください \${protocol}\${host}/api/user/verify/\${verificationtoken}
      ...
      계정 소유자임을 확인해주십시오. \${protocol}\${host}/api/user/verify/\${verificationtoken}
      ...
      此帐户现已锁定，等待通过验证。
      请确认您是帐户持有人：\${protocol}\${host}/api/user/verify/\${verificationtoken}
      验证帐户将重置失败的登录尝试。
  ...
  admin_email: {
    ...
    de: {
  	...
  	fr: {
  		...
  		Vous pouvez également l'approuver en suivant ce lien \${protocol}\${host}/api/user/approve/\${approvaltoken}`
  	},
    pl: {
      ...
      Możesz też zatwierdzić nowego konto za pomocą tego linku \${protocol}\${host}/api/user/approve/\${approvaltoken}`
    },
    ja: {
    	subject: `\${host}についてアカウントを検証されました`,
    	text: `\${email} を承認するには、管理パネル \${protocol}\${host}/api/user/admin にログインしてください
    	このリンクからもアカウントを承認することができます \${protocol}\${host}/api/user/approve/\${approvaltoken}`
    },
    ko: {
  		subject: `새로운 계정이 확인되었습니다. \${host}`,
  		text: `\${email} 을 승인하려면 관리자 패널 \${protocol}\${host}/api/user/admin 에 로그인하세요.
      다음의 링크로 또한 계정 승인을 할 수 있습니다. \${protocol}\${host}/api/user/approve/\${approvaltoken}`
  	},
  	zh: {
