...
export function layer(mapview) {
  codi.describe(
    ...
    async () => {
      ...
      await codi.it(
        {
          name: 'We should get a basic layer entry',
          parentId: 'ui_locations_entries_layer',
        },
        async () => {
          ...
          codi.assertTrue(
            Object.keys(mapview.layers[entry.key]).length > 0,
            'We should see the unique layer being added to the mapview',
          );
          entry.location.removeCallbacks?.forEach(
            (fn) => fn instanceof Function && fn(this),
          );
          //Need to delay for async functions
          await delayFunction(1000);
          codi.assertTrue(
            typeof mapview.layers[entry.key] === 'undefined',
            'We should see the unique layer being removed from the mapview',
          );
        },
      );
      ...
      await codi.it(
        {
          name: 'Layer entry should fail with warning',
          parentId: 'ui_locations_entries_layer',
        },
        async () => {
          ...
          //Need to delay for async functions
          codi.assertTrue(
            !mapview.layers[entry.key],
            'Should warn about an undefined layer',
          );
      ...
      await codi.it(
        {
          name: 'Layer entry should adhere to zoom restrictions',
          parentId: 'ui_locations_entries_layer',
        },
        async () => {
          const newEntry = {
          ...
          codi.assertTrue(
            mapview.layers[newEntry.key].display_toggle.classList.contains(
              'disabled',
            ),
