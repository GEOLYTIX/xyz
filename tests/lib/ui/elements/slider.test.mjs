...
export async function slider(mapview) {
    codi.describe({ name: 'Slider test:', id: 'ui_elements_slider', parentId: 'ui_elements' }, () => {
        codi.it({ name: 'Should return the slider', parentId: 'ui_elements_slider' }, async () => {
            ...
            const inputSliderParams = layer.infoj.filter(entry => entry.field === 'integer_field');
            ...
            const inputSliderElement = inputSlider.getElementsByTagName('input')[1];
            const assertInput = `<input data-id="a" name="rangeInput" type="range" min="-100000" max="10000" step="1">`;
            codi.assertEqual(inputSliderElement.outerHTML, assertInput, 'We expect to see the input defined in the test');
