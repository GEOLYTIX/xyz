...
export async function addLayer(mapview) {
  ...
  await codi.describe({ name: 'addLayerTest', id: 'mapview_add_layer', parentId: 'mapview' }, () => {

    /**
     * This test is used to check if we can add a single layer to the mapview.
     * @function it
     */
    codi.it({ name: 'Add single layer to mapview.', parentId: 'mapview_add_layer' }, async () => {
   ...
   * @function it
   */
    codi.it({ name: 'Add multiple layer to mapview.', parentId: 'mapview_add_layer' }, async () => {
