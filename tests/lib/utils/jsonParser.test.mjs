...
export function jsonParser() {
    codi.describe({ name: 'jsonParser Test:', id: 'utils_jsonParser', parentId: 'utils' }, () => {
        ...
        codi.it({ name: 'Check plugins', parentId: 'utils_jsonParser' }, () => {
            ...
            codi.assertTrue(Array.isArray(plugins), 'We expect the plugins to be returned as an array');
            codi.assertTrue(plugins.length === 3, 'We expect the plugins to have a length of 3');
        });
        ...
        codi.it({ name: 'Check keys', parentId: 'utils_jsonParser' }, () => {
            ...
            Object.keys(expectedKeys).forEach(key => {
                codi.assertEqual(jsonObject.locale[key], expectedKeys[key], 'We expect the keys parsed to remain the same');
            });
        });

        codi.it({ name: 'Check for nested arrays', parentId: 'utils_jsonParser' }, () => {

            const lordArrArr = jsonObject.locale.lordArrArr;

            codi.assertTrue(Array.isArray(lordArrArr), 'Ensure that we have an array');
            codi.assertTrue(Array.isArray(lordArrArr[0][1]), 'Ensure that we have a nested array');
