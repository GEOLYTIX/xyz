...
export async function getTest(mapview) {
  await codi.describe('Location: getTest', async () => {
    ...
    const location_layer_no_infoj =
      mapview.layers['location_get_test_no_infoj'];
     ...
     */
    await codi.it(
      'Location get should return undefined if location.layer.info is undefined.',
      async () => {
        //Get the location with the id returned from the query above
        const location = await mapp.location.get({
     ...
     */
    await codi.it(
      'The getInfoj method should return an infoj if none is provided on the layer present.',
      async () => {
        //Get the location with the id returned from the query above
