...
describe('login', async () => {
    ...
    const view = (await import('../../../mod/view.js')).default;
    const { setRedirect } = await import('../../../mod/utils/redirect.js');
    ...
    it('renders loginView and clears user cookie if no POST body is provided', () => {
        ...
        expect(setCookieHeader).toContain('TEST_APP=null;HttpOnly;Max-Age=0;Path=/app');
    ...
    it('loginBody: successfully authenticates, generates tokens, and sets cookies', async () => {
        ...
        expect(cookies[0]).toContain('HttpOnly;Max-Age=3600;Path=/app;SameSite=Strict;Secure');

        // Assert redirect clearing cookie
        expect(cookies[1]).toEqual('TEST_APP_redirect=null;HttpOnly;Max-Age=0;Path=/app');
    });
});