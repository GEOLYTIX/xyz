  ...
  codi.describe(
    ...
    () => {
      codi.it(
        ...
        async () => {
          ...
          codi.assertEqual(
            locales[0].key,
            'locale',
            'Ensure that we are getting a locale back from the API',
          );
        },
      );
      ...
      codi.it(
        ...
        async () => {
          const locale = await mapp.utils.xhr(
            `/test/api/workspace/locale?locale=locale`,
          );

          codi.assertTrue(
            !!locale.key,
            'The locale should have a key property',
          );
          ...
          codi.assertTrue(
            !!locale.syncPlugins,
            'The locale should have syncPlugins',
          );
        },
      );
      ...
      codi.it(
        ...
        async () => {
          const locale = await mapp.utils.xhr(
            `/test/api/workspace/locale?locale=locale&layers=true`,
          );
          codi.assertFalse(
            hasRoles(locale),
            'The locale object should have no roles object returned',
          );
        },
      );
    },
