...
async function workspaceTest(mapview) {
    await codi.describe({ name: 'Workspace: Testing Workspace API', id: 'api_workspace' }, async () => {

        await codi.it({ name: 'test endpoint', parentId: 'api_workspace' }, async () => {
        ...
        await codi.describe({ name: 'Roles', parentId: 'api_workspace', id: 'api_workspace_roles' }, async () => {
            ...
            await codi.describe({ name: 'Should return an array of roles as defined in the workspace', parentId: 'api_workspace_roles', id: 'api_workspace_roles_array' }, async () => {

                // Check the response is an array
                codi.it({ name: 'The roles should be an array', parentId: 'api_workspace_roles_array' }, async () => {
                    codi.assertTrue(Array.isArray(roles));
                });

                // Check the response contains 'A', 'B', 'C'
                codi.it({ name: 'Roles should contain A', parentId: 'api_workspace_roles_array' }, async () => {
                    codi.assertTrue(roles.includes('A'));
                });

                codi.it({ name: 'Roles should contain B', parentId: 'api_workspace_roles_array' }, async () => {
                    codi.assertTrue(roles.includes('B'));
                });

                codi.it({ name: 'Roles should contain C', parentId: 'api_workspace_roles_array' }, async () => {
                    codi.assertTrue(roles.includes('C'));
                });

                // Check the response does not contain the reserved role '*'
                codi.it({ name: 'Roles should contain *', parentId: 'api_workspace_roles_array' }, async () => {
                    codi.assertTrue(!roles.includes('*'));
                });

            });

            await codi.describe({ name: 'Should return an object of workspace roles with definitions', id: 'api_workspace_roles_object', parentId: 'api_workspace_roles' }, async () => {
                ...
                codi.it({ name: 'Roles object should contain A with value = Text about A', parentId: 'api_workspace_roles_object' }, async () => {
                    // Check the object contains 'A' role with text' Text about A'
                    codi.assertTrue(roles.A === 'Text about A');
                });
                codi.it({ name: 'Roles object should contain B with value = Text about A', parentId: 'api_workspace_roles_object' }, async () => {
                    // Check the object contains 'B' role with text' Text about B'
                    codi.assertTrue(roles.B === 'Text about B');
                });
                codi.it({ name: 'Roles object should contain C with value = {}', parentId: 'api_workspace_roles_object' }, async () => {
                ...
                codi.it({ name: 'Roles object should not contain *', parentId: 'api_workspace_roles_object' }, async () => {
                    // Check the reserved role '*' is not included in the object
                    codi.assertTrue(!roles['*']);
                });
            });

            codi.it({ name: 'Should not return a layer with roles defined', parentId: 'api_workspace_roles' }, async () => {
