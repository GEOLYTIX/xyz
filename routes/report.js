...
async function view(req, res, token = { access: 'public' }) {
  ...
  try {
    ...
    local_html = await require('fs').readFileSync(`${global.appRoot}/public/views/report/${req.query.template}.html`, 'utf8');
  } catch (err) {
    // apply fallback default template
    local_html = await require('fs').readFileSync(`${global.appRoot}/public/views/report/map_location.html`, 'utf8');
  }
